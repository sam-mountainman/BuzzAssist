...
import { createMangaFinalQualityDecision } from "./mangaQualityHarness.mjs";
...
export async function auditKoyaMangaFinal(options = {}) {
  ...
  if (options.dryRun) {
    ...
    await Promise.all([
      writeJsonAtomic(qualityDecisionPath, qualityDecision),
      writeJsonAtomic(join(qualityHarnessDir, "quality-loop-state.json"), qualityDecision),
    ]);
    record(
