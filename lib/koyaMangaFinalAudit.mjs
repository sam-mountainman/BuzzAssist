...
import { summarizePaidMediaCost } from "./qualityLoop.mjs";
import {
 ...
 */
export async function prepareKoyaQualityRound({ state, signoff, outputDir, revisionDelta = "" } = {}) {
...
export function createKoyaQualityRoundInput({
  ...
  return {
    ...
    reviews: [{
      ...
      notes: reviewNotes.summary || reviewNotes.evidence?.fullVideoReviewed?.note,
...
export async function auditKoyaMangaFinal(options = {}) {
  ...
  if (options.dryRun) {
    ...
    const qualityRoundReady = signoffGate.pass && qualityLoopState.status === "active"
      ? await prepareKoyaQualityRound({ state: qualityLoopState, signoff, outputDir, revisionDelta: options.revisionDelta })
      : { ready: false };
