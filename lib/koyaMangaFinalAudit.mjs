...
export async function auditKoyaMangaFinal(options = {}) {
  ...
  try {
    ...
    const speechReportPath = manifest.production?.audioPipeline?.reportPath
      ? resolve(manifest.production.audioPipeline.reportPath)
      : "";
    const speechReport = speechReportPath ? await readJson(speechReportPath, null) : null;
    const paidDialogueJobs = Array.isArray(speechReport?.mediaJobs) ? speechReport.mediaJobs : [];
