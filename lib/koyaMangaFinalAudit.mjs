...
import { findStaleQualityFeedback, summarizePaidMediaCost } from "./qualityLoop.mjs";
import {
...
import { KOYA_ASSET_QUALITY_FINAL_AUDIT_ID } from "./koyaAssetQualityGatePolicy.mjs";
import { KOYA_SCRIPT_QUALITY_FINAL_AUDIT_ID, auditKoyaScriptQualityFinal } from "./koyaScriptQualityGatePolicy.mjs";
import { auditKoyaEpisodeAssetQuality, koyaAssetQualityFinalAuditStep } from "./koyaAssetQualityFinalAudit.mjs";
import {
...
export async function resolveKoyaFinalAuditPythonRuntime(options = {}, projectDir = process.cwd()) {
  // 動画差し替えの監査（audit-manga-video-substitution.py）が Haar cascade を使う。
  const requiredModules = ["cv2", "cv2:CascadeClassifier", "numpy", ...(options.quick ? [] : ["faster_whisper"])];
  if (options.dryRun) {
    return normalizePythonRuntime(options.pythonRuntime, configuredPythonRuntime({
      purposeEnv: ["KOYA_GATE_PYTHON", "VOICE_QA_PYTHON"],
      projectDir,
    }));
  }
  return requirePythonRuntime({
...
export function validateKoyaVisualSignoff(signoff, {
  ...
  let expectedOuterJobBinding = null;
  let signedOuterJobBinding = null;
  try {
    expectedOuterJobBinding = assertKoyaOuterJobBinding(outerJobBinding, { required: true });
  } catch {
    failures.push("outer-job-binding-expected-invalid");
  }
  try {
    signedOuterJobBinding = assertKoyaOuterJobBinding(signoff?.outerJobBinding, { required: true });
  } catch {
    failures.push("outer-job-binding-signed-invalid");
  }
  if (expectedOuterJobBinding
    && (!signedOuterJobBinding || !sameKoyaOuterJobBinding(expectedOuterJobBinding, signedOuterJobBinding))) {
    failures.push("outer-job-binding-mismatch");
  }
  ...
  // 信頼設定済み reviewer 鍵の署名。期待 subject は signoff の写しではなく、
  // manifest の binding・契約 digest・disk から計算した SHA から組む。
  let reviewerAttestation = { pass: false, failures: [], signerKeyId: "", reviewerLabel: "", trustSha256: "" };
  let expectedSubject = null;
  try {
    expectedSubject = createKoyaReviewAttestationSubject({
      episodeId,
      outerJobBinding: expectedOuterJobBinding,
      contractDigest,
      videoSha256,
      contactSheetSha256,
      reviewNotesContentSha256,
      reviewer: evaluatorProvenance,
    });
  } catch (error) {
    reviewerAttestation.failures.push(...String(error?.message || "reviewer-attestation-expected-subject-invalid").split(", "));
  }
  if (!reviewerTrust) {
    reviewerAttestation.failures.push(reviewerTrustFailure || "reviewer-trust-unconfigured");
  }
  if (expectedSubject) {
    const verified = verifyKoyaReviewAttestation(signoff?.reviewerAttestation, { expectedSubject, trust: reviewerTrust });
    reviewerAttestation = { ...verified, failures: [...new Set([...reviewerAttestation.failures, ...verified.failures])] };
    reviewerAttestation.pass = reviewerAttestation.failures.length === 0;
  }
  failures.push(...reviewerAttestation.failures);
  return {
    pass: failures.length === 0,
    failures: [...new Set(failures)],
...
export async function writeKoyaVisualSignoff(options = {}) {
  ...
  const manifestOuterJobBinding = assertKoyaOuterJobBinding(manifest?.production?.outerJobBinding, { required: true });
  const requestedOuterJobBinding = assertKoyaOuterJobBinding(options.outerJobBinding);
  if (requestedOuterJobBinding && (!manifestOuterJobBinding
    || !sameKoyaOuterJobBinding(requestedOuterJobBinding, manifestOuterJobBinding))) {
    throw new Error("Koya signoff outer Job binding does not match the episode manifest.");
  }
  const outerJobBinding = manifestOuterJobBinding;
  ...
  const substitutedCutCoverage = auditSubstitutedCutReviewCoverage({ manifest, reviewNotes });
  if (!substitutedCutCoverage.pass) {
    throw new Error(`Perceptual review notes do not cover the video-substituted cuts: ${substitutedCutCoverage.failures.join(", ")}`);
  }
  ...
  // reviewer 鍵は信頼リストに active で載っているものだけ。リスト自体は
  // 生成プロセスと別経路（環境変数 / 明示 path）で受け取る。
  const reviewerTrust = await loadKoyaReviewerTrust({
    trust: options.reviewerTrust,
    trustPath: options.reviewerTrustPath,
    env: options.env || process.env,
  });
  const attestationSubject = createKoyaReviewAttestationSubject({
    episodeId: manifest.id,
    outerJobBinding,
    contractDigest: resolvedContract.digest,
    videoSha256,
  ...
  const reviewerAttestation = await createKoyaReviewAttestation({
    ...
    privateKeyPem: options.reviewerPrivateKeyPem,
    privateKeyPath: options.reviewerPrivateKeyPath,
    trust: reviewerTrust,
  });
  const attestationCheck = verifyKoyaReviewAttestation(reviewerAttestation, {
    expectedSubject: attestationSubject,
    trust: reviewerTrust,
  });
  if (!attestationCheck.pass) {
    throw new Error(`Reviewer attestation did not verify against the trust list: ${attestationCheck.failures.join(", ")}`);
  }
  const signoff = {
    version: resolvedContract.contract.qualityReview.version,
    episodeId: manifest.id,
    ...(outerJobBinding ? { outerJobBinding } : {}),
...
/**
 * 品質ループの状態を開く。無ければ作り、契約が変わっていれば作り直し、
 * 合格済みでも証拠が変わっていれば作り直す。
 *
 * 時間の予算（既定 6 時間）はレビューの回数を回すためのもので、画像生成の時間では
 * ない。計画時刻（generator の capturedAt）を起点にしていたので、本番尺の生成
 * （実測 13〜16 時間）を終えて最初の監査に来た時点で time-limit になり、人手待ちへ
 * 落ちていた。ループが実際に始まる時刻を起点にする。
 */
export function openKoyaQualityLoopState({ existing, contract, manifest, evidenceMerkleRoot, now = () => new Date().toISOString() }) {
  const generatorProvenance = manifest.production?.provenance?.generator;
  const fresh = () => createMangaQualityLoopState({
    episodeId: manifest.id,
    contract,
  ...
  if (!existing || existing.contractDigest !== contract.digest) return fresh();
  if (existing.status === "passed" && existing.rounds?.at(-1)?.evidenceMerkleRoot !== evidenceMerkleRoot) return fresh();
  return existing;
}

/**
 * 品質ループの次の回を記録できるかを決める。
 *
 * 2回目以降の回は「前回のどの失敗を、どう直したか」（修正内容）が要る。最終監査は以前、
 * それを渡さずに回を記録しようとして例外で落ち、公式経路では1回目で合格しない限り
 * ループが回らなかった（2026-09-24）。修正内容は --revision-delta か、監査の出力先の
 * revision-delta.json（{ previousFailureFingerprint, revisionDelta }）で受け取り、
 * 無ければ例外ではなく人待ちで止める。同じ署名（同じ評価の文脈）で監査をやり直した
 * ときも、新しいレビューが要るとして止める。
 */
export async function prepareKoyaQualityRound({ state, signoff, outputDir, revisionDelta = "", reviewNotes = null } = {}) {
  const rounds = Array.isArray(state?.rounds) ? state.rounds : [];
  const contextId = String(signoff?.reviewerProvenance?.contextId || "");
  if (rounds.some((round) => (round.reviews || []).some((review) => review.evaluatorContextId === contextId))) {
    return {
      ready: false,
      step: {
        id: "quality-loop-fresh-review",
        detail: "この署名の評価文脈は前の回で使われている。直した版を、新しい文脈のレビューで signoff し直してから audit すること",
      },
    };
  }
  // 文脈が新しくても、所見が前の回の写しなら評価は更新されていない。例外で監査を落とさず、
  // 理由コードつきで新しい評価を求める（品質ループの中核も同じ判定で回を拒否する）。
  const stale = reviewNotes
    ? findStaleQualityFeedback({ state, reviews: [{ notes: koyaReviewFeedbackText(reviewNotes), evaluatorContextId: contextId }] })
    : null;
  if (stale) {
    return {
      ready: false,
      step: {
        id: "quality-loop-feedback-not-updated",
        detail: `${stale.reasonCode}: ${stale.detail}。review notes の summary を、直した版を見た所見に書き直して signoff し直すこと`,
      },
    };
  }
  if (rounds.length === 0) return { ready: true, retry: false };
  const previousFailureFingerprint = String(rounds.at(-1)?.failureFingerprint || "");
  const deltaPath = join(outputDir, "revision-delta.json");
  let text = typeof revisionDelta === "string" ? revisionDelta.trim() : "";
  if (!text) {
    const saved = await readJson(deltaPath, null);
    if (saved && String(saved.previousFailureFingerprint || "") === previousFailureFingerprint) {
      text = String(saved.revisionDelta || "").trim();
    }
  }
  if (text.length < 4) {
    return {
      ready: false,
      step: {
        id: "quality-loop-revision-delta",
        detail: `品質ループの ${rounds.length + 1} 回目には、前回の失敗（${previousFailureFingerprint}）をどう直したかが要る。`
          + "audit に --revision-delta \"直した内容\" を付けるか、"
          + `${deltaPath} に { "previousFailureFingerprint": "${previousFailureFingerprint}", "revisionDelta": "直した内容" } を書いてから audit し直すこと`,
        evidencePath: deltaPath,
      },
    };
  }
  return { ready: true, retry: true, previousFailureFingerprint, revisionDelta: text };
}

/**
 * 有料 take の台帳。音声工程の report（manifest.production.audioPipeline.reportPath）の
 * mediaJobs で、RunReceipt へ recordPaidMediaJob で載せるのと同じ記録。
 */
async function readKoyaPaidDialogueLedger(manifest) {
  const path = manifest.production?.audioPipeline?.reportPath
    ? resolve(manifest.production.audioPipeline.reportPath)
    : "";
  const report = path ? await readJson(path, null) : null;
  return {
    path,
    sha256: report && await pathExists(path) ? await sha256File(path) : "",
    mediaJobs: Array.isArray(report?.mediaJobs) ? report.mediaJobs : [],
  };
}

/** 品質ループの回に載せる評価者の所見（review notes の summary、無ければ全尺視聴の所見）。 */
export function koyaReviewFeedbackText(reviewNotes) {
  return reviewNotes?.summary || reviewNotes?.evidence?.fullVideoReviewed?.note || "";
}

/**
 * 最終監査が品質ループへ渡す1回分の入力を作る（監査の実行とは切り離して試験できるように）。
 */
export function createKoyaQualityRoundInput({
  contract,
  state,
  signoff,
  reviewNotes,
  ...
  // 費用は有料 take の台帳（speech report の mediaJobs。RunReceipt の mediaJobs と同じ記録）
  // から回ごとに数える。前の回までに数えた take（state.costedKeys）は数えない。
  const costAccounting = Array.isArray(paidMediaJobs)
    ? summarizePaidMediaCost(paidMediaJobs, {
      countedKeys: state?.costedKeys,
      source: { kind: "koya-speech-report.mediaJobs", ...(paidCostSource || {}) },
    })
    : null;
  ...
  return {
    ...
    reviews: [{
      ...
      scores: reviewNotes.rubricScores,
      notes: koyaReviewFeedbackText(reviewNotes),
      evidence: roundEvidence,
    }],
    evidence: roundEvidence,
    reviewDigest: signoff.reviewNotesContentSha256,
    evidenceMerkleRoot,
    failureFingerprint: failedGateIds.length > 0 ? sha256Text(failedGateIds.join("\n")) : "",
    ...(qualityRoundReady.retry ? {
      previousFailureFingerprint: qualityRoundReady.previousFailureFingerprint,
      revisionDelta: qualityRoundReady.revisionDelta,
    } : {}),
...
export async function auditKoyaMangaFinal(options = {}) {
  ...
  if (requestedOuterJobBinding && (!manifestOuterJobBinding
    || !sameKoyaOuterJobBinding(requestedOuterJobBinding, manifestOuterJobBinding))) {
    throw new Error("Koya final audit outer Job binding does not match the episode manifest.");
  }
  const outerJobBinding = manifestOuterJobBinding;
  // 信頼リストが読めなくても監査は走らせ、signoff gate を理由コード付きで落とす。
  let reviewerTrust = null;
  let reviewerTrustFailure = "";
  try {
    reviewerTrust = await loadKoyaReviewerTrust({
      trust: options.reviewerTrust,
      trustPath: options.reviewerTrustPath,
      env: options.env || process.env,
    });
  } catch (error) {
    reviewerTrustFailure = reviewerTrustFailureCode(error);
  }
  ...
  const pythonRuntime = await resolveKoyaFinalAuditPythonRuntime(options, projectDir);
  const runPythonReport = (id, args, reportPath, evaluator) => runReport(
    id,
    pythonRuntime.command,
    [...pythonRuntime.args, ...args],
    reportPath,
    evaluator,
  );
  ...
  const videoSubstitutionDir = join(outputDir, "video-substitution");
  let videoSubstitutionReport = null;
  if (options.dryRun) {
    record("video-substitution", false, "dry-run: not executed", join(videoSubstitutionDir, "audit.json"), { planned: true });
  } else {
    try {
      const substitution = await auditCutVideoSubstitutions({
        projectDir,
        manifestPath,
        videoPath,
        contract: resolvedContract.contract,
        outputDir: videoSubstitutionDir,
        pythonPath: pythonRuntime.command,
        pythonArgs: pythonRuntime.args,
      });
      videoSubstitutionReport = substitution.report;
      record(
        "video-substitution",
        substitution.report.pass,
        substitution.report.applicable
          ? (substitution.report.pass ? "passed" : JSON.stringify(substitution.report.failures).slice(0, 1200))
          : substitution.report.detail,
        substitution.reportPath,
        { applicable: substitution.report.applicable },
      );
    } catch (error) {
      record("video-substitution", false, error.message, join(videoSubstitutionDir, "audit.json"));
    }
  }

  // wardrobe-readiness: the gate itself runs before the paid images, so the
  // final audit checks that this episode's images were made under a passing
  // report for the current script — or under a recorded operator override,
  // which it always surfaces.
  const wardrobeReportPath = join(projectDir, "canvas", "assets", manifest.id, KOYA_WARDROBE_READINESS_FILE_NAME);
  const wardrobeStatePath = join(episodeDir, "koya-production-state.json");
  const wardrobeReport = await readJson(wardrobeReportPath, null);
  const wardrobeAudit = auditKoyaWardrobeReadinessFinal({
    state: await readJson(wardrobeStatePath, null),
    report: wardrobeReport,
    reportPath: wardrobeReport ? wardrobeReportPath : "",
    reportSha256: wardrobeReport && await pathExists(wardrobeReportPath) ? await sha256File(wardrobeReportPath) : "",
    episodeId: manifest.id,
    scriptDigest: currentScriptSha256,
  });
  const wardrobeEvidencePath = join(outputDir, "wardrobe-readiness.json");
  await writeJsonAtomic(wardrobeEvidencePath, wardrobeAudit);
  record("wardrobe-readiness", wardrobeAudit.pass, wardrobeAudit.detail, wardrobeEvidencePath, {
    applicable: wardrobeAudit.applicable,
    wardrobeReadiness: wardrobeAudit,
  });

  // asset-quality-loops（契約 v54 から）: 途中の成果物のゲートは登録・完了・採用の瞬間に効くだけなので、
  // 完了済みの行の再利用・汎用の経路の画・登録後に差し替わった参照画は通らずに入りうる。完成の前に、
  // この回が manifest で実際に使っている画・採用テイク・人物と場所の登録・回に含めたサムネを、使った
  // ファイルそのものの SHA で品質ループの合格と照らし直す。v53 以前の契約では対象外（合格・applicable=false）。
  const assetQualityEvidencePath = join(outputDir, `${KOYA_ASSET_QUALITY_FINAL_AUDIT_ID}.json`);
  let assetQualityAudit;
  try {
    assetQualityAudit = await auditKoyaEpisodeAssetQuality({
      contract: resolvedContract.contract,
      manifest,
      manifestPath,
      projectDir,
    ...
    assetQualityAudit = {
      ...
      applicable: true,
      pass: false,
      detail: `asset quality loops could not be verified: ${String(error?.message || error).slice(0, 600)}`,
      failures: [],
      outOfForce: [],
    };
  }
  await writeJsonAtomic(assetQualityEvidencePath, assetQualityAudit);
  const assetQualityStep = koyaAssetQualityFinalAuditStep(assetQualityAudit, assetQualityEvidencePath);
  record(assetQualityStep.id, assetQualityStep.pass, assetQualityStep.detail, assetQualityStep.evidencePath, assetQualityStep.extra);

  // script-quality-accepted（契約 v56 から）: 台本の関門は有料の処理の前に効くだけなので、完成の前に、状態ファイルに
  // 残した答えが合格（人の受け入れか品質ループの合格）で、回の台本（manifest の台本の生テキスト）と同じ台本に対する
  // ものであることを確かめる。v55 以前の契約では対象外（合格・applicable=false）。
  const scriptQualityAudit = auditKoyaScriptQualityFinal({
    contract: resolvedContract.contract,
    state: await readJson(wardrobeStatePath, null),
    scriptTextSha256: currentScriptSha256,
  });
  const scriptQualityEvidencePath = join(outputDir, `${KOYA_SCRIPT_QUALITY_FINAL_AUDIT_ID}.json`);
  await writeJsonAtomic(scriptQualityEvidencePath, scriptQualityAudit);
  record(KOYA_SCRIPT_QUALITY_FINAL_AUDIT_ID, scriptQualityAudit.pass, scriptQualityAudit.detail, scriptQualityEvidencePath, {
    applicable: scriptQualityAudit.applicable,
  });
  ...
  const signoffSha256 = signoff && await pathExists(signoffPath) ? await sha256File(signoffPath) : "";
  ...
  // Machine metrics cannot tell whether a generated clip still shows the same
  // people; the signed review must have looked inside every substituted cut.
  const substitutedCutCoverage = auditSubstitutedCutReviewCoverage({ manifest, reviewNotes: reviewNotesSource });
  const reviewFailures = [...signoffGate.failures, ...substitutedCutCoverage.failures];
  const reviewPass = contactSheet.pass && signoffGate.pass && substitutedCutCoverage.pass;
  record(
    "agent-contact-sheet-review",
    reviewPass,
    reviewPass ? "passed" : `contact sheet/signoff incomplete: ${reviewFailures.join(", ")}`,
    signoffPath,
    {
      evidenceSha256: signoffSha256,
      contactSheetPath,
      contactSheetSha256,
      contactSheetPass: contactSheet.pass,
      evidenceFilesGate,
      signoffGate,
      substitutedCutCoverage,
    },
  );
  ...
  if (options.dryRun) {
    ...
    const evidenceManifest = await createMangaEvidenceManifest({
      ...
      artifacts: [
        ...
        ...(videoSubstitutionReport?.evidence || []).flatMap((row) => [row.clipPath, row.startFramePath]),
        videoSubstitutionReport?.analysisReportPath,
    ...
    let qualityLoopState = openKoyaQualityLoopState({
      existing: await readJson(qualityLoopStatePath, null),
      contract: qualityContract,
      manifest,
      evidenceMerkleRoot: evidenceManifest.merkleRoot,
    });
    const qualityRoundReady = signoffGate.pass && qualityLoopState.status === "active"
      ? await prepareKoyaQualityRound({
        state: qualityLoopState,
        signoff,
        outputDir,
        revisionDelta: options.revisionDelta,
        reviewNotes: reviewNotesSource,
      })
      : { ready: false };
    if (!qualityRoundReady.ready && qualityRoundReady.step) {
      record(qualityRoundReady.step.id, false, qualityRoundReady.step.detail, qualityRoundReady.step.evidencePath || "");
    }
    if (qualityRoundReady.ready) {
      const paidLedger = await readKoyaPaidDialogueLedger(manifest);
      const roundEvidence = [
      ...
      qualityLoopState = await recordVerifiedMangaQualityRound(createKoyaQualityRoundInput({
        contract: qualityContract,
        state: qualityLoopState,
        signoff,
        reviewNotes: reviewNotesSource,
        ...
        evidenceMerkleRoot: evidenceManifest.merkleRoot,
        qualityRoundReady,
        paidMediaJobs: paidLedger.mediaJobs,
        paidCostSource: { path: paidLedger.path, sha256: paidLedger.sha256 },
        artifactSha256: videoSha256,
      }));
    ...
    // 版の識別子: MP4 の SHA と、この監査に渡された修正内容。再発はこれが違うときだけ数える。
    const incidentRevisionDelta = String(qualityRoundReady.revisionDelta || options.revisionDelta || "").trim();
      ...
      for (const step of auditSteps.filter((entry) => (
        ...
        incidentLedger = recordMangaQualityIncident({
          ...
          incident: {
            ...
            // 同じ MP4・同じ修正内容の監査し直しは再発に数えない。
            revision: { artifactSha256: videoSha256, revisionDelta: incidentRevisionDelta },
          },
        });
      }
  ...
  const report = {
    ...
    steps,
    commands,
    // 人が外したゲートは、レポートの上の方に出す。ステップの中だけに書くと
    // 読む人が見落とす。
    operatorOverrides: wardrobeAudit.overrides.map((entry) => ({ gate: "wardrobe-readiness", ...entry })),
    // 品質ループの合格の記録が無い旧登録（契約 v54 より前に登録した人物・場所）。落とさないが、
    // この回の画がそれを参照にしていることを見える所に出す。
    ...(Array.isArray(assetQualityAudit.outOfForce) && assetQualityAudit.outOfForce.length > 0
      ? { assetQualityOutOfForce: assetQualityAudit.outOfForce }
      : {}),
    ...evaluation,
  ...
  // RunReceipt。監査レポートは「このエピソードがどうだったか」で、
  // こちらは「どの版のハーネスが、宣言したどの保証を、何を測って満たしたか」。
  // 版をまたいで並べられる形が無いと、ハーネスを直して良くなったのかが
  // 誰にも分からない——自己改善のループが閉じない理由がそこにあった。
  // 監査レポートを書いた後に作る。ここで落ちても監査結果は失われない。
  let runReceiptPath = "";
  try {
    const declaration = JSON.parse(await readFile(join(repositoryRoot, "config/harnesses/koya-manga-video.harness.json"), "utf8"));
    ...
    const paidDialogueJobs = (await readKoyaPaidDialogueLedger(manifest)).mediaJobs;
    if (report.pass && paidDialogueJobs.length === 0) {
      throw new Error("Koya final audit has no paid dialogue Media Job roster from the speech report.");
    }
    for (const mediaJob of paidDialogueJobs) recordPaidMediaJob(receipt, mediaJob);
    finalizeRunReceipt(receipt, {
      outcome: report.pass ? "pass" : "fail",
      knownRemainingIssues: (evaluation.knownRemainingIssues || []).map((issue) => `${issue.id}: ${issue.detail}`),
      timestamp: report.generatedAt,
    });
    runReceiptPath = await writeRunReceipt(receipt, join(outputDir, "run-receipt.json"));
  } catch (error) {
    // 記録が作れなかったことを黙って飲まない。以前はレポートに書くだけで
    // 監査は pass のまま通していたが、それだと「記録の作成に失敗する状態」
    // ——たとえば対応表と契約の食い違い——を抱えたまま完成扱いになる。
    // 記録は完成の条件なので、作れなければ落とす。
    runReceiptPath = "";
    report.runReceiptError = String(error?.message || error).slice(0, 300);
    const receiptIssue = { id: "run-receipt", detail: `RunReceipt を作成できなかった: ${report.runReceiptError}` };
    report.knownRemainingIssues = [...(report.knownRemainingIssues || []), receiptIssue];
    // 状態ファイルは evaluation.knownRemainingIssues を書く。report 側にだけ
    // 足すと、「audit-failed だが理由は空」という状態が残る——落ちた理由を
    // 読めない記録は、落ちていないのと区別がつかない。
    evaluation.knownRemainingIssues = [...(evaluation.knownRemainingIssues || []), receiptIssue];
  ...
  if (!report.pass && !options.dryRun && manifest.production?.finalKoyaAudit?.pass === true) {
    // 一度合格したエピソードを再監査して落ちたとき、manifest だけが完成の
    // ままだと3つの正本（report / state / manifest）が互いに矛盾する。
    // 「完成済み」と書かれた manifest は下流で承認済み出力として扱われるので、
    // 失効させないと、落ちた回の後も古い合格を根拠に配信できてしまう。
    // MP4 自体は消さない——消すべきなのは「承認済み」という記述の方。
    manifest.status = "audit-failed";
    manifest.knownRemainingIssues = (evaluation.knownRemainingIssues || []).map((issue) => `${issue.id}: ${issue.detail}`);
    manifest.production = {
      ...(manifest.production || {}),
      finalKoyaAudit: {
        ...(manifest.production.finalKoyaAudit || {}),
        pass: false,
        supersededAt: report.generatedAt,
        supersededBy: reportPath,
      },
    };
    if (manifest.outputs?.finalVideo) {
      // 承認済み出力としての参照を外す。ファイルは残すが、
      // 「監査を通った最終動画」という位置づけは取り消す。
      manifest.outputs = {
        ...manifest.outputs,
        revokedFinalVideo: { ...manifest.outputs.finalVideo, revokedAt: report.generatedAt },
        finalVideo: undefined,
      };
      delete manifest.outputs.finalVideo;
    }
  ...
  return { reportPath, report, contactSheetPath, signoffPath, signoffSha256, runReceiptPath };
}
