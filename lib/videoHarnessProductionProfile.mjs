...
// 人の承認を本番の条件にする。承認は在庫 manifest の approval 欄で、版と内容 SHA に
// 束縛されている（skill-inventory --approve、承認者本人の端末から）。運営者の決定
// （2026-09-24）は「未承認なら止める」。6 本の承認が記録された時点で既定を「止める」にした。
// 開発中に一時的に外すときだけ BUZZASSIST_REQUIRE_SKILL_APPROVAL=0（"0" 以外は止める）。
...
function skillApprovalRequired(env = process.env) {
  return String(env[SKILL_APPROVAL_REQUIREMENT_ENV] || "").trim() !== "0";
}
