 ...
 * 4 = skillApproval（正本スキルの人の承認の状態。開発用チェックアウトで承認前の正本スキルのまま作ったか、
 *     写しの種類、承認の付いていないスキルの id・版・sha256。digest つき）を持つ。
 * 5 = codeIdentity（計画したコードと確定したコードの同一性の要約。BuzzAssist の更新をまたいで確定したか、
 *     付け替えの履歴、固定した制作契約の版、入力の同一性の digest、再利用だけの実行で止めた有料の呼び出しの数。
 *     digest つき）を持つ。
 * 版を上げたら、上げる前の版の実例を test/fixtures/run-receipt-past-schema-revisions.json に足す。
 */
export const RUN_RECEIPT_SCHEMA_REVISION = 5;
/**
 * codeIdentity がこの版から載る。これより前の記録に無いのは「当時は記録しなかった」であって改変ではない
 * （当時はコードが変わった Job は確定できなかった）。
 */
export const RUN_RECEIPT_CODE_IDENTITY_IN_FORCE_SINCE = 5;
export const RUN_RECEIPT_CODE_IDENTITY_VERSION = "buzzassist-run-receipt-code-identity-v1";
/**
 * skillApproval がこの版から載る。これより前の記録に無いのは「当時は記録しなかった」であって改変ではない
 * （当時は未承認の正本スキルでは制作そのものが止まった）。
 */
export const RUN_RECEIPT_SKILL_APPROVAL_IN_FORCE_SINCE = 4;
export const RUN_RECEIPT_SKILL_APPROVAL_VERSION = "buzzassist-run-receipt-skill-approval-v1";
...
const SKILL_APPROVAL_CHECKOUTS = new Set(["development", "distributed"]);
const UNAPPROVED_STATES = new Set(["none", "stale"]);
const SKILL_ID = /^[A-Za-z0-9][A-Za-z0-9:._-]{0,127}$/u;
const SEMVER = /^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/u;

function skillApprovalDigest({ version, recorded, checkout, builtWithUnapprovedSkills, unapprovedSkills }) {
  return sha256(canonicalJson({ version, recorded, checkout, builtWithUnapprovedSkills, unapprovedSkills }));
}

/**
 * Receipt の skillApproval 欄。Job の productionProfile.skillApproval（lib/videoHarnessProductionProfile.mjs が
 * 作る）を正規化して写し、digest を付ける。渡されなければ「記録を渡されなかった」（recorded: false）として
 * 作り、推測で埋めない（Koya の最終監査の子 Receipt など）。形の崩れた記録は受け取らない。
 * builtWithUnapprovedSkills は申告を信じず、未承認のスキルの列から導く。
 */
function runReceiptSkillApproval(record) {
  if (record === null || record === undefined) {
    const section = { version: RUN_RECEIPT_SKILL_APPROVAL_VERSION, recorded: false, checkout: "", builtWithUnapprovedSkills: null, unapprovedSkills: [] };
    return { ...section, digest: skillApprovalDigest(section) };
  }
  if (typeof record !== "object" || Array.isArray(record)) throw new Error("RunReceipt の skillApproval の形が不正。");
  const checkout = String(record.checkout || "");
  if (!SKILL_APPROVAL_CHECKOUTS.has(checkout)) throw new Error(`RunReceipt の skillApproval.checkout が不正: ${checkout || "(空)"}`);
  if (!Array.isArray(record.unapprovedSkills) || record.unapprovedSkills.length > 64) throw new Error("RunReceipt の skillApproval.unapprovedSkills が不正。");
  const unapprovedSkills = record.unapprovedSkills.map((row) => {
    const id = String(row?.id || "");
    const version = String(row?.version || "");
    const digest = String(row?.sha256 || "").replace(/^sha256:/u, "").toLowerCase();
    const approvalState = String(row?.approvalState || "");
    if (!SKILL_ID.test(id) || !SEMVER.test(version) || !/^[a-f0-9]{64}$/u.test(digest) || !UNAPPROVED_STATES.has(approvalState)) {
      throw new Error("RunReceipt の skillApproval.unapprovedSkills の行が不正（id・版・sha256・承認の状態が要る）。");
    }
    return { id, version, sha256: digest, approvalState };
  }).sort((left, right) => left.id.localeCompare(right.id));
  const section = {
    version: RUN_RECEIPT_SKILL_APPROVAL_VERSION,
    recorded: true,
    checkout,
...
/**
 * skillApproval 欄を検証する（invocation と同じ考え方）。
 *
 * - 版が RUN_RECEIPT_SKILL_APPROVAL_IN_FORCE_SINCE より前: 欄は当時存在しなかったので "not-in-force"
 *   で通す。その版に欄があるのは形が合わないので拒否する
 * - それ以外: 欄の版・形・digest が合えば、記録を渡されたなら "recorded"、渡されなかったなら "unrecorded"
 * - 知らない版（未来の版・数でない）は検証できないので拒否する
 */
export function verifyRunReceiptSkillApproval(receipt) {
  ...
  if (revision < RUN_RECEIPT_SKILL_APPROVAL_IN_FORCE_SINCE) {
    return receipt?.skillApproval !== undefined
      ? { ok: false, status: "invalid", revision, failures: ["skill-approval-in-legacy-revision"] }
      : { ok: true, status: "not-in-force", revision, failures: [] };
  }
  const section = receipt?.skillApproval;
  if (!section || typeof section !== "object" || Array.isArray(section)) {
    return { ok: false, status: "invalid", revision, failures: ["skill-approval-missing"] };
  }
  const failures = [];
  if (section.version !== RUN_RECEIPT_SKILL_APPROVAL_VERSION) failures.push("skill-approval-version-unknown");
  let normalized = null;
  try {
    normalized = section.recorded === true
      ? runReceiptSkillApproval({ checkout: section.checkout, unapprovedSkills: section.unapprovedSkills })
      : runReceiptSkillApproval(null);
  } catch {
    failures.push("skill-approval-entry-invalid");
  }
  if (normalized) {
    const { digest: expected, ...shape } = normalized;
    const { digest: actual, ...given } = section;
    if (canonicalJson(shape) !== canonicalJson(given)) failures.push("skill-approval-not-normal");
    if (actual !== skillApprovalDigest(given)) failures.push("skill-approval-digest-mismatch");
    else if (actual !== expected) failures.push("skill-approval-not-normal");
  }
  const unique = [...new Set(failures)];
  if (unique.length) return { ok: false, status: "invalid", revision, failures: unique };
  return { ok: true, status: section.recorded === true ? "recorded" : "unrecorded", revision, failures: [] };
}

/** 集計・投影用の要約。版 4 より前の記録と、記録を渡されなかった記録は recorded: false。 */
export function runReceiptSkillApprovalSummary(receipt) {
  const section = runReceiptSchemaRevision(receipt) >= RUN_RECEIPT_SKILL_APPROVAL_IN_FORCE_SINCE ? receipt?.skillApproval : null;
  if (!section || section.recorded !== true) {
    return { recorded: false, checkout: "", builtWithUnapprovedSkills: null, unapprovedSkills: [] };
  }
  return {
    recorded: true,
    checkout: String(section.checkout || ""),
    builtWithUnapprovedSkills: section.builtWithUnapprovedSkills === true,
    unapprovedSkills: (Array.isArray(section.unapprovedSkills) ? section.unapprovedSkills : [])
      .map(({ id, version, sha256: digest, approvalState }) => ({ id, version, sha256: digest, approvalState })),
  };
}

const SHA256_HEX = /^[a-f0-9]{64}$/u;

function codeIdentityDigestOf(section) {
  return sha256(canonicalJson(section));
}

function shaOrEmpty(value, label) {
  const digest = String(value ?? "").replace(/^sha256:/u, "").toLowerCase();
  if (digest && !SHA256_HEX.test(digest)) throw new Error(`RunReceipt の codeIdentity.${label} が SHA-256 ではない。`);
  return digest;
}
...
/** コードの同一性の要約を正規化する（版と SHA だけ。パスは持たない）。 */
function codeIdentitySummaryRow(summary, label) {
  if (summary === null || summary === undefined) return null;
  if (typeof summary !== "object" || Array.isArray(summary)) throw new Error(`RunReceipt の codeIdentity.${label} の形が不正。`);
  const contract = summary.productionContract;
  const row = {
    digest: shaOrEmpty(summary.digest, `${label}.digest`),
    coreVersion: shortText(summary.coreVersion, 100),
    coreSha256: shaOrEmpty(summary.coreSha256, `${label}.coreSha256`),
    runtimeDependenciesDigest: shaOrEmpty(summary.runtimeDependenciesDigest, `${label}.runtimeDependenciesDigest`),
    deploymentDependenciesDigest: shaOrEmpty(summary.deploymentDependenciesDigest, `${label}.deploymentDependenciesDigest`),
    harnessDeclarationSha256: shaOrEmpty(summary.harnessDeclarationSha256, `${label}.harnessDeclarationSha256`),
    deploymentEntrypointSha256: shaOrEmpty(summary.deploymentEntrypointSha256, `${label}.deploymentEntrypointSha256`),
    productionContract: contract && typeof contract === "object"
      ? { contractVersion: shortText(contract.contractVersion, 200), contractDigest: shaOrEmpty(contract.contractDigest, `${label}.productionContract.contractDigest`) }
      : null,
    skills: (Array.isArray(summary.skills) ? summary.skills : []).slice(0, 64).map((skill) => {
      const id = shortText(skill?.id, 128);
      if (!SKILL_ID.test(id)) throw new Error(`RunReceipt の codeIdentity.${label}.skills の id が不正。`);
      return { id, sha256: shaOrEmpty(skill?.sha256, `${label}.skills.sha256`) };
    }).sort((left, right) => left.id.localeCompare(right.id)),
  };
  if (!row.digest) throw new Error(`RunReceipt の codeIdentity.${label}.digest が無い。`);
  return row;
}
...
function reuseOnlyRun(run) {
  if (!run || typeof run !== "object") return null;
  return {
    refusedPaidCalls: finiteCount(run.refusedPaidCalls),
    mediaJobs: Object.fromEntries(["reused", "recovered", "reissued", "issued", "carried"]
      .map((key) => [key, finiteCount(run.mediaJobs?.[key])])),
  };
}

/**
 * 再利用だけの実行の結果（Job の finalizeRuns から数える。Receipt を検証し直すときは書かれた要約を正規化する）。
 * 更新をまたいで Receipt の確定だけをやり直した Job は、子を走らせていないので runs: 0。
 */
function reuseOnlySection(record) {
  if (record.reuseOnly && typeof record.reuseOnly === "object" && !Array.isArray(record.reuseOnly)) {
    return {
      runs: finiteCount(record.reuseOnly.runs),
      refusedPaidCalls: finiteCount(record.reuseOnly.refusedPaidCalls),
      lastRun: reuseOnlyRun(record.reuseOnly.lastRun),
    };
  }
  const runs = Array.isArray(record.finalizeRuns) ? record.finalizeRuns : [];
  return {
    runs: runs.length,
    refusedPaidCalls: runs.reduce((sum, run) => sum + finiteCount(run?.refusedPaidCalls), 0),
    lastRun: reuseOnlyRun(runs.at(-1)),
  };
}

/**
 * Receipt の codeIdentity 欄。計画したコード（planned）と確定したコード（finalized）の同一性の要約、
 * 更新をまたいだ付け替えの履歴（rebinds）、固定した制作契約の版、入力の同一性の digest、再利用だけの実行の
 * 結果（止めた有料の呼び出しの数と Media Job の再利用の内訳）。渡されなければ「記録を渡されなかった」
 * （recorded: false）として作り、推測で埋めない（ジャンルの子 Receipt など）。
 * crossedUpdate は申告を信じず、付け替えの履歴から導く。計画と確定の digest が履歴と食い違う記録は受け取らない。
 */
function runReceiptCodeIdentity(record) {
  if (record === null || record === undefined) {
    const section = {
      version: RUN_RECEIPT_CODE_IDENTITY_VERSION,
      recorded: false,
      crossedUpdate: null,
      planned: null,
      finalized: null,
    ...
    return { ...section, digest: codeIdentityDigestOf(section) };
  }
  if (typeof record !== "object" || Array.isArray(record)) throw new Error("RunReceipt の codeIdentity の形が不正。");
  const planned = codeIdentitySummaryRow(record.planned, "planned");
  const finalized = codeIdentitySummaryRow(record.finalized, "finalized");
  if (!planned || !finalized) throw new Error("RunReceipt の codeIdentity には planned と finalized が要る。");
  const rebinds = (Array.isArray(record.rebinds) ? record.rebinds : []).slice(-50).map((row) => ({
    at: timeOrEmpty(row?.at),
    fromDigest: shaOrEmpty(row?.fromDigest ?? row?.from?.digest, "rebinds.fromDigest"),
    toDigest: shaOrEmpty(row?.toDigest ?? row?.to?.digest, "rebinds.toDigest"),
    fromCoreVersion: shortText(row?.fromCoreVersion ?? row?.from?.coreVersion, 100),
    toCoreVersion: shortText(row?.toCoreVersion ?? row?.to?.coreVersion, 100),
    inputIdentityDigest: shaOrEmpty(row?.inputIdentityDigest, "rebinds.inputIdentityDigest"),
  }));
  if (rebinds.length === 0 && planned.digest !== finalized.digest) {
    throw new Error("RunReceipt の codeIdentity: 付け替えの履歴が無いのに計画と確定のコードの同一性が違う。");
  }
  if (rebinds.length > 0 && (rebinds[0].fromDigest !== planned.digest || rebinds.at(-1).toDigest !== finalized.digest)) {
    throw new Error("RunReceipt の codeIdentity: 付け替えの履歴が計画と確定のコードの同一性とつながらない。");
  }
  const contract = record.pinnedProductionContract;
  const crossedUpdate = rebinds.length > 0;
  const section = {
    version: RUN_RECEIPT_CODE_IDENTITY_VERSION,
    recorded: true,
    crossedUpdate,
    ...
    finalized,
    rebinds,
    pinnedProductionContract: contract && typeof contract === "object"
      ? { contractVersion: shortText(contract.contractVersion, 200), contractDigest: shaOrEmpty(contract.contractDigest, "pinnedProductionContract.contractDigest") }
      : null,
    inputIdentityDigest: shaOrEmpty(record.inputIdentityDigest, "inputIdentityDigest"),
    reuseOnly: crossedUpdate ? reuseOnlySection(record) : null,
  };
  return { ...section, digest: codeIdentityDigestOf(section) };
}

/**
 * codeIdentity 欄を検証する（skillApproval と同じ考え方）。
 *
 * - 版が RUN_RECEIPT_CODE_IDENTITY_IN_FORCE_SINCE より前: 欄は当時存在しなかったので "not-in-force"。
 *   その版に欄があるのは形が合わないので拒否する
 * - それ以外: 欄の版・形・digest が合えば、記録を渡されたなら "recorded"、渡されなかったなら "unrecorded"。
 *   更新をまたいだ事実（crossedUpdate）を消したり書き換えたりすると digest か形が合わなくなる
 * - 知らない版は検証できないので拒否する
 */
export function verifyRunReceiptCodeIdentity(receipt) {
  const revision = runReceiptSchemaRevision(receipt);
  if (!Number.isSafeInteger(revision) || revision > RUN_RECEIPT_SCHEMA_REVISION) {
    return { ok: false, status: "invalid", revision: receipt?.schemaRevision ?? null, failures: ["schema-revision-unsupported"] };
  }
  if (revision < RUN_RECEIPT_CODE_IDENTITY_IN_FORCE_SINCE) {
    return receipt?.codeIdentity !== undefined
      ? { ok: false, status: "invalid", revision, failures: ["code-identity-in-legacy-revision"] }
      : { ok: true, status: "not-in-force", revision, failures: [] };
  }
  const section = receipt?.codeIdentity;
  if (!section || typeof section !== "object" || Array.isArray(section)) {
    return { ok: false, status: "invalid", revision, failures: ["code-identity-missing"] };
  }
  const failures = [];
  if (section.version !== RUN_RECEIPT_CODE_IDENTITY_VERSION) failures.push("code-identity-version-unknown");
  let normalized = null;
  try {
    normalized = section.recorded === true
      ? runReceiptCodeIdentity({
        planned: section.planned,
        finalized: section.finalized,
        rebinds: section.rebinds,
        pinnedProductionContract: section.pinnedProductionContract,
        inputIdentityDigest: section.inputIdentityDigest,
        reuseOnly: section.reuseOnly,
      })
      : runReceiptCodeIdentity(null);
  } catch {
    failures.push("code-identity-entry-invalid");
  }
  const { digest: actual, ...given } = section;
  if (actual !== codeIdentityDigestOf(given)) failures.push("code-identity-digest-mismatch");
  if (normalized) {
    const { digest: _expected, ...shape } = normalized;
    if (canonicalJson(shape) !== canonicalJson(given)) failures.push("code-identity-not-normal");
  }
  const unique = [...new Set(failures)];
  if (unique.length) return { ok: false, status: "invalid", revision, failures: unique };
  return { ok: true, status: section.recorded === true ? "recorded" : "unrecorded", revision, failures: [] };
...
/** 集計・投影用の要約。版 5 より前の記録と、記録を渡されなかった記録は recorded: false。 */
export function runReceiptCodeIdentitySummary(receipt) {
  const section = runReceiptSchemaRevision(receipt) >= RUN_RECEIPT_CODE_IDENTITY_IN_FORCE_SINCE ? receipt?.codeIdentity : null;
  if (!section || section.recorded !== true) {
    return { recorded: false, crossedUpdate: null, plannedCoreVersion: "", finalizedCoreVersion: "", rebindCount: 0, refusedPaidCalls: 0, pinnedContractVersion: "" };
  }
  return {
    recorded: true,
    crossedUpdate: section.crossedUpdate === true,
    plannedCoreVersion: String(section.planned?.coreVersion || ""),
    finalizedCoreVersion: String(section.finalized?.coreVersion || ""),
    rebindCount: Array.isArray(section.rebinds) ? section.rebinds.length : 0,
    refusedPaidCalls: finiteCount(section.reuseOnly?.refusedPaidCalls),
    pinnedContractVersion: String(section.pinnedProductionContract?.contractVersion || ""),
  };
}
...
export function openRunReceipt({
  ...
  // 計画したコードと確定したコード（更新をまたいで確定したか）。形の崩れた記録は、記録を開く前に止める。
  const codeIdentitySection = runReceiptCodeIdentity(codeIdentity);
  // 正本スキルの承認の状態（開発用チェックアウトで承認前の正本のまま作ったか）。止めずに記録するので、
  // 読める形で残す。形の崩れた記録は、記録を開く前に止める。
  const skillApprovalSection = runReceiptSkillApproval(skillApproval);
  ...
  return {
    ...
    skillApproval: skillApprovalSection,
    codeIdentity: codeIdentitySection,
...
// issue の先頭語として受け付ける形。Job 層の blocker コード（kebab-case）と、ハーネス宣言にある監査 id
// （camelCase を含む）だけを通す。
 ...
 * 決着時の学習の捕捉（lib/harnessReceiptLearning.mjs）と Receipt の集計（scripts/harness-receipts.mjs）が同じものを使う。
 */
...
export function redactForPlatform(receipt) {
  ...
  return {
    ...
    // 承認前の正本スキルで作ったか（スキルの id・版・sha256 だけ。パスは元から持たない）。版 4 より前は recorded: false。
    skillApproval: runReceiptSkillApprovalSummary(receipt),
    // BuzzAssist の更新をまたいで確定したか（版と数だけ。SHA もパスも返さない）。版 5 より前は recorded: false。
    codeIdentity: runReceiptCodeIdentitySummary(receipt),
