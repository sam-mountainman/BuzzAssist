...
export function redactSecrets(text, secrets = []) {
  ...
  // Provider credentials must stay server-side, but redact common credential
  // shapes too so a malformed upstream error cannot echo one into a journal.
  out = out
    .replace(
    ...
    .replace(/\b(?:sk|xi)[_-][A-Za-z0-9._-]{8,}\b/giu, "[redacted]")
    .replace(
      /(\b(?:api[_-]?key|auth[_-]?token|access[_-]?token|refresh[_-]?token|client[_-]?secret|private[_-]?key|signing[_-]?key|secret|password|credentials?)\b\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^\s,;}]+)/giu,
      "$1[redacted]",
    )
    .replace(/([?&](?:api[_-]?key|auth[_-]?token|access[_-]?token|refresh[_-]?token|token|secret)=)[^&#\s]+/giu, "$1[redacted]");
  return out;
}
...
export async function paidApiResponseError(response, { label = "request", secrets = [], bodyLimit = 240 } = {}) {
  ...
  let body = bodyLimit > 0 ? "(本文を読めませんでした)" : "(provider body omitted)";
  if (bodyLimit > 0) {
    try {
    ...
    if (attempt < maxAttempts) {
      ...
      if (typeof onRetry === "function") {
        onRetry({
          ...
          error: redactSecrets(lastError?.message, secrets),
        });
      }
