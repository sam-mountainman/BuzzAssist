...
export async function downloadVerified({
  ...
  // 配布元が一時的に応答しないことがある（2026-09-25、CI の macOS で2回続けて fetch failed）。
  // 1つの URL に 4 回、間隔を 1s・3s・9s と広げて取り直す。
  attemptsPerUrl = 4,
  ...
  for (const url of sources) {
    for (let attempt = 1; attempt <= attemptsPerUrl; attempt += 1) {
      try {
        ...
        if (attempt < attemptsPerUrl) await new Promise((resolveWait) => setTimeout(resolveWait, Math.min(10_000, 1_000 * 3 ** (attempt - 1))));
      }
    }
  }
