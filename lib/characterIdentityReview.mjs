...
async function inspectIdentityPackReview(args = {}, options = {}) {
  ...
  for (const expected of expectedExtras) {
    ...
    for (const id of REQUIRED_EYE_OPEN_CELLS) {
      ...
      for (const featureId of requireAsymmetricFeatureChecks(cell, `extraSheets.${expected.role}.cells.${id}`, sideLockedFeatures, "extra", failures, { requirePass })) {
        extraSideVerified.add(featureId);
      }
    }
    requireSideLockedCoverage(`extraSheets.${expected.role}`, sideLockedFeatures, "extra", extraSideVerified, cells.length, failures, { requirePass });
  }
