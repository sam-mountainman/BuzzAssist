...
import { pngPixelSha256 } from "./pngPixelDigest.mjs";
...
/**
 * Python の round() と同じ「偶数丸め」（ちょうど .5 は偶数側へ）。
 *
 * 切り抜きは scripts/audit-koya-character-identity.py が Python の round() で
 * 決める。JS 側で Math.round（.5 は常に切り上げ）を使っていたので、
 * 1672x941 の2段シートでは Python が 470、JS が 471 を境界にし、機械が
 * 記録した正しい sourceBounds を JS 側の検査が不一致として落としていた。
 * この格子の丸めは JS ではここだけで行う。
 */
export function roundHalfToEven(value) {
  const number = Number(value);
  if (!Number.isFinite(number)) return Number.NaN;
  const floor = Math.floor(number);
  // 格子で使う非負の値では、この引き算は誤差なしで求まる（.5 の判定がぶれない）。
...
/**
 * 親シートを columns x rows に割ったときの index 番目（行優先）のセル矩形
 * [x, y, width, height]。Python の split_grid と同じ式・同じ丸めで求める。
 */
export function identitySheetCellBounds({ width, height, columns, rows, index }) {
  const column = index % columns;
  const row = Math.floor(index / columns);
  const left = roundHalfToEven(column * width / columns);
  const right = roundHalfToEven((column + 1) * width / columns);
  const top = roundHalfToEven(row * height / rows);
  const bottom = roundHalfToEven((row + 1) * height / rows);
  return [left, top, right - left, bottom - top];
}

export const IDENTITY_SHEET_BOUNDS_TOLERANCE_PX = 1;

/**
 * 機械レビューが記録した sourceBounds を、そのセルの正として使ってよいかを決める。
 *
 * 記録値は切り抜きを実際に作った側の値なので、格子と矛盾しない限りそれを使う
 * （丸め方の違いで正しい記録を落とさないため）。矛盾しないとは、親シートの
 * 内側に収まる整数矩形で、4辺それぞれが丸める前の格子線から 1px 以内に
 * あること。記録が無い・壊れている・格子から外れている場合は failure を返し、
 * bounds には偶数丸めの格子値を返す。
 */
export function resolveRecordedIdentitySheetCellBounds({ recorded, width, height, columns, rows, index }) {
  const grid = identitySheetCellBounds({ width, height, columns, rows, index });
  const gridValid = [width, height, columns, rows].every((value) => Number.isInteger(value) && value > 0)
    && Number.isInteger(index) && index >= 0 && index < columns * rows;
  // 親シートの寸法が不正なら、どの比較も NaN で素通りしてしまうので先に落とす。
  if (!gridValid) return { bounds: grid, source: "grid", failure: "invalid-grid" };
  if (recorded === undefined || recorded === null) return { bounds: grid, source: "grid", failure: "missing" };
  if (!Array.isArray(recorded) || recorded.length !== 4 || !recorded.every((value) => Number.isInteger(value))) {
    return { bounds: grid, source: "grid", failure: "malformed" };
  }
  const [x, y, boxWidth, boxHeight] = recorded;
  if (x < 0 || y < 0 || boxWidth <= 0 || boxHeight <= 0 || x + boxWidth > width || y + boxHeight > height) {
    return { bounds: grid, source: "grid", failure: "outside-parent-sheet" };
  }
  const column = index % columns;
  const row = Math.floor(index / columns);
  const exactEdges = [
  ...
  const recordedEdges = [x, y, x + boxWidth, y + boxHeight];
  if (recordedEdges.some((edge, edgeIndex) => Math.abs(edge - exactEdges[edgeIndex]) > IDENTITY_SHEET_BOUNDS_TOLERANCE_PX)) {
    return { bounds: grid, source: "grid", failure: "off-grid" };
  }
  return { bounds: [x, y, boxWidth, boxHeight], source: "recorded", failure: "" };
}
...
/**
 * 記録済みの切り抜きが、いま親シートから切り直した切り抜きと同じかを決める。
 *
 * 同じかどうかは画素で決める。切り抜き PNG は PIL が書くので、PIL や zlib の
 * 版が変わるだけで同じ画素のファイルでもバイト列が変わる。ファイル SHA-256 の
 * 一致だけを見ていたので、画素が1つも違わない登録済みキャラが軒並み落ちた。
 *
 * ファイル SHA-256 は出所の証跡として残す。比べる前に、読んだバイト列が
 * それぞれの記録の SHA-256 と一致することを確かめる（別のファイルの画素で
 * 一致を名乗らせない）。ファイルが同一なら従来どおりそれだけで一致とする
 * ので、この変更の前に書かれた記録もそのまま通る。画素へ戻せない PNG は
 * 一致扱いにしない。
 */
export async function identityCropMatchesFreshCrop(check, machineCheck) {
  const recordedSha256 = nonEmptyString(check?.sha256);
  const freshSha256 = nonEmptyString(machineCheck?.sha256);
  if (!recordedSha256 || !freshSha256) return false;
  if (recordedSha256 === freshSha256) return true;
  const recordedPath = nonEmptyString(check?.path);
  const freshPath = nonEmptyString(machineCheck?.path);
  if (!recordedPath || !freshPath) return false;
  try {
    const [recordedBytes, freshBytes] = await Promise.all([
      readFile(resolve(recordedPath)),
      readFile(resolve(freshPath)),
    ]);
    if (sha256Bytes(recordedBytes) !== recordedSha256 || sha256Bytes(freshBytes) !== freshSha256) return false;
    return pngPixelSha256(recordedBytes) === pngPixelSha256(freshBytes);
  } catch {
    return false;
  }
}
...
async function requireMachineCell(check, machineCheck, label, failures, options = {}) {
  ...
  for (const key of ["width", "height", "machineFaceCropLumaDistanceToSelected"]) {
  ...
  if (!(await identityCropMatchesFreshCrop(check, machineCheck))) {
    failures.push(`${label}.sha256 does not match a fresh crop from the current parent sheet (the decoded pixels differ).`);
  }
