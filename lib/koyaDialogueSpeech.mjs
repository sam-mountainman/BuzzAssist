    ...
    try {
      ...
      if (gateWanted) {
        ...
        const runGate = async () => {
            ...
            checks: candidates.map((candidate) => ({
              ...
              segments: cutPlan.inputs.map((entry, inputIndex) => {
                ...
                return {
                  ...
                  start: bounds.startSeconds,
                  end: bounds.endSeconds,
          ...
          await writeJsonAtomic(join(sourceDir, `${cut.id}-voice-quality.json`), gateReport);
          return Object.fromEntries(gateReport.checks.map((check) => [
