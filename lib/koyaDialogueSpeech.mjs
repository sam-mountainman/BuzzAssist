...
import { RetryablePaidApiError, redactSecrets, withPaidApiRetry } from "./paidApiRetry.mjs";
 ...
 */
export async function requestElevenLabsDialogue({ url, apiKey, body, fetchImpl = fetch, label = "ElevenLabs dialogue", sleepFn, maxAttempts = 3 } = {}) {
...
async function generateTake(cutPlan, takeIndex, context) {
  ...
  for (const candidateFormat of ["wav_44100", "wav_24000"]) {
    ...
    // 5xx とネットワーク断だけ再送する。ここには再送が1つも無く、
    // 一過性の503でテイクが死んでいた。429 は下の usageLimitResponse で
    // park する既存の扱いを変えない——残枠を焼かないための判断なので、
    // ここで再送に変えるとその意図を壊す。
    ({ response, payload } = await requestElevenLabsDialogue({
      url,
      apiKey,
      body,
      fetchImpl,
      label: `ElevenLabs ${cutPlan.cutId} take ${takeIndex + 1}`,
  ...
  if (!response?.ok) {
    // payload はレスポンス本文そのもの。秘密が混ざったまま例外へ載せない
    // ——例外の文字列はログにもレポートにも残る。
    throw new Error(redactSecrets(
      `ElevenLabs dialogue generation failed for ${cutPlan.cutId} take ${takeIndex + 1}: ${JSON.stringify(payload)}`,
      [apiKey],
    ));
  }
...
const SPEECH_TEXT_PREFLIGHT = new URL("../scripts/prepare-speech-text.py", import.meta.url).pathname;
...
async function runSpeechTextPreflight(texts, dictionaryPath) {
  ...
  try {
    ...
    const { stdout } = await execFile("python3", [SPEECH_TEXT_PREFLIGHT, configPath], {
...
export async function generateKoyaDialogueSpeech(options = {}) {
  ...
  const gateWantedGlobally = options.voiceQualityGate === true
    || (options.voiceQualityGate !== false && process.env.KOYA_VOICE_QUALITY_GATE === "1");
  if (gateWantedGlobally && (manifest.cuts || []).length > 0) {
  ...
  for (const cut of manifest.cuts || []) {
    ...
    try {
      ...
      if (gateWantedForCut(options) && cutPlan.inputs.some((entry) => !entry.expectedReading)) {
        try {
          const preflight = await runSpeechTextPreflight(
            cutPlan.inputs.map((entry) => ({ id: entry.utteranceId, text: entry.speechText })),
            readingDictionaryPath,
          );
          const byId = new Map((preflight.rows || []).map((row) => [row.id, row]));
          for (const entry of cutPlan.inputs) {
      ...
      if (gateWanted) {
        ...
        const runGate = async () => {
          ...
          const gateReport = pending.length === 0 ? { checks: [] } : await auditVoiceQuality({
            checks: pending.map((candidate) => ({
              ...
              segments: cutPlan.inputs.map((entry, inputIndex) => {
                ...
                const nextStart = inputIndex === cutPlan.inputs.length - 1
        ...
        while (allFailed() && candidates.length < maxAdaptiveTakes && !Number.isInteger(forcedTakeIndex)) {
          candidates.push(await generateTake(cutPlan, candidates.length, { ...options, sourceDir, pronunciationDictionaryLocators }));
          voiceQualityByTake = await runGate();
        }
