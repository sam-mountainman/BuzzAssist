...
import { createKoyaVoiceTakeAssetQualityGate } from "./koyaAssetQualityGate.mjs";
...
/**
 * <cut>-voice-quality.json の形。byTake（選定の減点）に加えて、音声品質ゲートの生の結果 checks を
 * テイクの SHA つきで持つので、そのまま声のテイク（voice-take）工程の品質ループの --measurement になる。
 */
export const KOYA_VOICE_TAKE_MEASUREMENT_VERSION = "koya-voice-take-measurement-v1";

/**
 * 声のテイクのゲート。呼び出し側が関数を渡せばそれ、無ければ契約の版で決める（v54 から効く）。
 * null を渡して外す道は作らない（契約が v54 以降なら必ず効く）。
 */
function koyaVoiceTakeGateFor(options, { contract, canvasDir, episodeId }) {
  if (typeof options.assetQualityGate === "function") return options.assetQualityGate;
  return createKoyaVoiceTakeAssetQualityGate({ contract, canvasDir, episodeId });
}
...
export function normalizeKoyaDialogueMediaJobReceipt(summary = {}) {
  ...
  const adapter = findKoyaDialogueAdapter(summary);
...
export function prepareKoyaDialogueCut(manifest, cut, options = {}) {
  ...
  const adapter = options.dialogueAdapter
    ? findKoyaDialogueAdapter(options.dialogueAdapter)
    : DEFAULT_KOYA_DIALOGUE_ADAPTER;
  if (!adapter) throw new Error("Koya dialogue cut requires an allowed voice.dialogue adapter.");
  // 演技タグ（[angry] 等）は ElevenLabs v3 固有。オトシゴは受けず 400 unsupported_audio_tag を返すので、
  // ElevenLabs 以外には付けない。
  const supportsAudioTags = adapter === DEFAULT_KOYA_DIALOGUE_ADAPTER;
  const inputs = utterances.map((utterance) => {
    ...
    const performancePrompt = !supportsAudioTags
      ? ""
      : utterance.performancePrompt === undefined
    ...
    if (!utterance.voiceId) throw new Error(`An approved ${adapter.provider} voice is required for ${utterance.id}.`);
    return {
  ...
  return {
    ...
    takeCount,
    model: adapter.model,
    // 既定（ElevenLabs）のときは載せない。載せると ElevenLabs の cutPlan の形が変わり、
    // それを写した記録と照合する試験や承認済みの checkpoint を不必要に揺らす。
...
export function buildKoyaDialogueRequest(cutPlan, takeIndex, pronunciationDictionaryLocators = null) {
  ...
  const adapter = cutPlan.adapter || DEFAULT_KOYA_DIALOGUE_ADAPTER;
  if (adapter.provider !== DEFAULT_KOYA_DIALOGUE_ADAPTER.provider) {
    // ElevenLabs 固有の設定（stability、文字正規化、発音辞書の locator）は送らない。
    // 読みの補正は prepareKoyaDialogueCut の文字置換（読み辞書）が既に済ませている。
    // 台詞ごとの時刻（voiceSegments）は仲介側が台詞ごとに合成して実測して返す
    // （docs/otoshigo-media-job-api-spec-ja.md §7）。
    return {
...
export async function requestKoyaDialogueMediaJob(cutPlan, takeIndex, context = {}) {
  ...
  try {
    ...
    job = await mediaJobBroker.start({
      ...
      input: body,
      output: {
        // オトシゴは 48kHz の WAV を返す。ElevenLabs の指定は変えない。
        acceptedFormats: adapter === DEFAULT_KOYA_DIALOGUE_ADAPTER
          ? ["wav_44100", "wav_24000"]
          : ["wav_48000", "wav_44100", "wav_24000"],
        enableLogging: true,
      },
      reservation: {
        unit: "characters",
...
async function generateTake(cutPlan, takeIndex, context) {
  ...
  const adapter = cutPlan.adapter || DEFAULT_KOYA_DIALOGUE_ADAPTER;
  // ElevenLabs の名前は変えない（既存の取得済みテイクをそのまま再利用するため）。
  const takeLabel = adapter === DEFAULT_KOYA_DIALOGUE_ADAPTER
    ? "eleven-v3"
    : `${adapter.provider}-${adapter.model}`.replace(/[^a-z0-9.-]+/giu, "-");
  const sourcePath = join(context.sourceDir, `${cutPlan.cutId}-take-${takeIndex + 1}-${inputDigest}-${takeLabel}-dialogue.wav`);
...
async function splitSelectedTake(manifest, cutPlan, selected, context) {
  ...
  for (let index = 0; index < cutPlan.inputs.length; index += 1) {
    ...
    const sidecar = {
      ...
      pipeline: "koya-dialogue-v44",
      utteranceId: utterance.id,
      provider: (cutPlan.adapter || DEFAULT_KOYA_DIALOGUE_ADAPTER).provider,
...
async function generateKoyaDialogueSpeechSerial(options = {}) {
  ...
  const voiceTakeGate = koyaVoiceTakeGateFor(options, { contract, canvasDir, episodeId: manifest.id });
  ...
  for (const cut of manifest.cuts || []) {
    ...
    const cutPlan = prepareKoyaDialogueCut(manifest, cut, {
      takeCount: options.takeCount || contract.audio.takeCount,
    ...
    try {
      ...
      if (gateWanted) {
        ...
        // 生の結果（テイクの SHA つき）。声のテイクの品質ループの測定として同じファイルに残す。
        ...
        const runGate = async () => {
          ...
          for (const check of gateReport.checks) {
            ...
            if (candidate) {
              ...
              rawCheckCache.set(candidate.sourcePath, check);
            }
          }
          ...
          voiceQualityChecks = candidates.map((candidate) => rawCheckCache.get(candidate.sourcePath)).filter(Boolean);
          await writeJsonAtomic(join(sourceDir, `${cut.id}-voice-quality.json`), {
            cutId: cut.id,
            selectedTakeIndex: null,
            selectedSourcePath: null,
            byTake: merged,
            ...(voiceTakeGate ? { measurementVersion: KOYA_VOICE_TAKE_MEASUREMENT_VERSION, checks: voiceQualityChecks } : {}),
          });
          return merged;
        };
      ...
      const voiceQualityPath = join(sourceDir, `${cut.id}-voice-quality.json`);
      if (voiceQualityByTake) {
        await writeJsonAtomic(voiceQualityPath, {
          cutId: cut.id,
          selectedTakeIndex: selected.takeIndex,
      ...
      // 契約 v54 から: 採用するテイクは、声のテイク（voice-take）工程の品質ループの合格（別文脈の評価者が
      // 直前の地の文と続けて聞いた採点・上の測定・合格した版と同じバイト列）が無いと台帳へ書かない。
      // 未合格のカットは止め、テイクを撮り直さない（再実行は取得済みのテイクをそのまま使う）。
      if (voiceTakeGate) {
        const verdict = await voiceTakeGate({ cutId: cut.id, takePath: selected.sourcePath, measurementPath: voiceQualityPath });
        if (verdict && verdict.required !== false && verdict.pass !== true) {
          report.cuts.push({
            cutId: cut.id,
            ...
            measurementPath: voiceQualityByTake ? voiceQualityPath : "",
            assetQuality: {
              stage: verdict.stage || "voice-take",
  ...
  const heldCuts = report.cuts.filter((row) => row.status === "awaiting-human-review");
  report.status = options.dryRun ? "planned" : heldCuts.length > 0 ? "awaiting-human-review" : "complete";
  report.knownRemainingIssues = heldCuts.map((row) => ({ id: `voice-take-asset-quality:${row.cutId}`, detail: row.assetQuality?.code || "voice-take asset quality loop not passed" }));
  report.updatedAt = new Date().toISOString();
  if (!options.dryRun && heldCuts.length > 0) {
    manifest.status = "speech-awaiting-review";
...
/**
 * 画像と重ねて先に走らせた音声（台本由来の対話の定義に対する同じ generateKoyaDialogueSpeech）の結果を、
 * 正本の manifest（prepare が画像から作ったもの）へ取り込む。collector だけが呼ぶ。
 *
 * 取り込むのは、先の回で complete になった cut だけ。cut の発話の並びと、音声の入力になる欄
 * （話者・本文・読みの上書き・声）が正本と1つでも違えば取り込まない。取り込んだ後の cut の計画に
 * 承認済みの checkpoint（approvedCheckpointImpl。再開のときと同じ判定）が付かなければ、その cut は
 * 取り込まずに通常どおり生成へ回す（有料のテイクは Media Job の requestKey とテイクの記録で再利用される）。
 * 写す欄は、直列の collector が worker から写す欄（speechText・performancePrompt・model・audio）と同じ。
 * 取り込んだ cut の報告の行は、先の回の行（直列のときに worker が返す行と同じ形）をそのまま使う。
 */
const KOYA_OVERLAP_INPUT_FIELDS = Object.freeze([
  "cutId", "speakerId", "text", "speechOverride", "voiceId", "voiceProfileId", "voiceSettings", "expectedReading", "speechReading",
]);
const KOYA_OVERLAP_SPEECH_FIELDS = Object.freeze(["speechText", "performancePrompt", "model", "audio"]);

async function adoptKoyaOverlappedSpeech(manifest, adoptFrom, { takeCount, dialogueAdapter, approvedCheckpointImpl }) {
  const empty = { adoptedCutIds: [], rows: new Map(), mediaJobs: [], skipped: [] };
  const definitionPath = typeof adoptFrom?.manifestPath === "string" ? adoptFrom.manifestPath.trim() : "";
  const reportPath = typeof adoptFrom?.reportPath === "string" ? adoptFrom.reportPath.trim() : "";
  if (!definitionPath || !reportPath) return empty;
  let definition;
  let earlyReport;
  try {
    definition = JSON.parse(await readFile(resolve(definitionPath), "utf8"));
    earlyReport = JSON.parse(await readFile(resolve(reportPath), "utf8"));
  } catch {
    return { ...empty, skipped: [{ cutId: "*", reason: "overlap-evidence-unreadable" }] };
  }
  if (definition?.id !== manifest.id || resolve(earlyReport?.manifestPath || "") !== resolve(definitionPath)) {
    return { ...empty, skipped: [{ cutId: "*", reason: "overlap-evidence-mismatch" }] };
  }
  const definitionCuts = new Map((definition.cuts || []).map((cut) => [cut.id, cut]));
  const definitionUtterances = new Map((definition.utterances || []).map((entry) => [entry.id, entry]));
  const earlyRows = new Map((earlyReport.cuts || []).filter((row) => row?.status === "complete").map((row) => [row.cutId, row]));
  const canonicalUtterances = new Map((manifest.utterances || []).map((entry) => [entry.id, entry]));
  const adoption = { adoptedCutIds: [], rows: new Map(), mediaJobs: [], skipped: [] };
  for (const cut of manifest.cuts || []) {
    const row = earlyRows.get(cut.id);
    if (!row) continue;
    const early = definitionCuts.get(cut.id);
    if (JSON.stringify(early?.utteranceIds || null) !== JSON.stringify(cut.utteranceIds || [])) {
      adoption.skipped.push({ cutId: cut.id, reason: "utterance-order-differs" });
      continue;
    }
    const tentative = [];
    let reason = "";
    for (const utteranceId of cut.utteranceIds || []) {
      const canonical = canonicalUtterances.get(utteranceId);
      const earlyUtterance = definitionUtterances.get(utteranceId);
      if (!canonical || !earlyUtterance?.audio) { reason = "early-audio-missing"; break; }
      const differs = KOYA_OVERLAP_INPUT_FIELDS.find((field) => (
        JSON.stringify(canonical[field] ?? null) !== JSON.stringify(earlyUtterance[field] ?? null)
      ));
      if (differs) { reason = `input-differs:${differs}`; break; }
      tentative.push([canonical, Object.fromEntries(KOYA_OVERLAP_SPEECH_FIELDS.map((field) => [field, structuredClone(earlyUtterance[field])]))]);
    }
    if (reason) {
      adoption.skipped.push({ cutId: cut.id, reason });
      continue;
    }
    // 取り込んだ状態で cut の計画を作り直し、再開のときと同じ checkpoint の判定を通るかを見る。
    const probe = structuredClone(manifest);
    const probeById = new Map(probe.utterances.map((entry) => [entry.id, entry]));
    for (const [canonical, fields] of tentative) Object.assign(probeById.get(canonical.id), fields);
    const plan = prepareKoyaDialogueCut(probe, cut, { takeCount, dialogueAdapter });
    if (!await approvedCheckpointImpl(plan)) {
      adoption.skipped.push({ cutId: cut.id, reason: "checkpoint-not-approved" });
      continue;
    }
    for (const [canonical, fields] of tentative) Object.assign(canonical, fields);
    adoption.adoptedCutIds.push(cut.id);
    adoption.rows.set(cut.id, { ...row, cutId: cut.id, status: "complete" });
  }
  // 先の回の有料テイクは、取り込まなかった cut のぶんも含めてこの Job の支出なので、全部報告に写す。
  adoption.mediaJobs = Array.isArray(earlyReport.mediaJobs) ? earlyReport.mediaJobs : [];
  return adoption;
}
...
export async function generateKoyaDialogueSpeech(options = {}) {
  ...
  // A line without a voice can never have an approved take, so the per-cut
  // preparation below would stop with a bare "voice is required" error before
  // the caller's gate could say who needs a voice. Give the gate that chance
  // first; if it lets the run through, the preparation still refuses.
  if (!options.dryRun && typeof options.beforePaidSpeech === "function") {
    const utteranceById = new Map((manifest.utterances || []).map((entry) => [entry.id, entry]));
    const unvoicedCutIds = cuts
      .filter((cut) => (cut.utteranceIds || []).some((id) => {
        const utterance = utteranceById.get(id);
        return Boolean(utterance) && !utterance.voiceId;
      }))
      .map((cut) => cut.id);
    if (unvoicedCutIds.length > 0) {
      await options.beforePaidSpeech({
        manifest: structuredClone(manifest),
        pendingCutIds: unvoicedCutIds,
      });
    }
  }
  const approvedCheckpointImpl = options.approvedCheckpointImpl || approvedKoyaDialogueCutCheckpoint;
  // 画像と重ねて先に作った音声があれば、承認済みの cut だけを正本へ取り込む（有料の呼び出しはしない）。
  const overlapAdoption = options.adoptFrom
    ? await adoptKoyaOverlappedSpeech(manifest, options.adoptFrom, {
      takeCount,
      dialogueAdapter: resolveKoyaDialogueAdapter(contract),
  ...
  const cutPlans = cuts.map((cut) => ({
    cut,
    plan: prepareKoyaDialogueCut(manifest, cut, { takeCount, dialogueAdapter: resolveKoyaDialogueAdapter(contract) }),
  }));
  ...
  if (overlapAdoption) collectedMediaJobs = mergeKoyaDialogueMediaJobReceipts(collectedMediaJobs, overlapAdoption.mediaJobs);
  // 取り込みの結果は戻り値だけに載せる（報告の形は直列のときと同じに保つ）。
  const overlapResult = overlapAdoption
    ? { overlapAdoption: { adoptedCutIds: [...overlapAdoption.adoptedCutIds], skipped: overlapAdoption.skipped } }
    : {};
  ...
  const rowByCut = new Map(checkpointRows.map(({ cut, plan, checkpoint }) => [
    cut.id,
    checkpoint && overlapAdoption?.rows.has(cut.id)
      ? overlapAdoption.rows.get(cut.id)
      : checkpoint
  ...
  const pool = await runKoyaSpeechCutPool(pendingSelected, {
    ...
    onSettled: async (outcome) => {
      ...
      if (outcome.status === "fulfilled" && outcome.value.reportRow?.status === "awaiting-human-review") {
        // 採用テイクの品質ループ待ち。台帳（manifest）の発話には何も書かず、完了にも数えない。
        rowByCut.set(cutId, { ...outcome.value.reportRow, cutId });
  ...
  if (pool.cancelled || options.signal?.aborted === true) {
    ...
    await persistCanonical();
    return { manifest, manifestPath, report, reportPath, waiting: false, cancelled: true, partial: true, ...overlapResult };
  }
  if (pool.firstError instanceof KoyaUsageLimitError) {
    report.status = "waiting-usage-limit";
    manifest.status = "waiting-usage-limit";
    await persistCanonical();
    return { manifest, manifestPath, report, reportPath, waiting: true, partial: true, ...overlapResult };
  }
  if (pool.firstError) {
    report.status = "failed";
    await persistCanonical();
  ...
  const heldRows = orderedIds.map((id) => rowByCut.get(id)).filter((row) => row?.status === "awaiting-human-review");
  if (heldRows.length > 0) {
    const assetQualityAwaiting = heldRows.map((row) => ({
      cutId: row.cutId,
      stage: row.assetQuality?.stage || "voice-take",
      subjectId: row.assetQuality?.subjectId || "",
      code: row.assetQuality?.code || "",
      reason: row.assetQuality?.reason || "",
      takePath: row.sourcePath || "",
      measurementPath: row.measurementPath || "",
    }));
    report.status = "awaiting-human-review";
    report.knownRemainingIssues = [{
      id: "voice-take-asset-quality",
      detail: `Pass the voice-take asset quality loop for the selected take of each held cut before it is adopted (no take is regenerated): ${assetQualityAwaiting.map((row) => row.code || row.cutId).join("; ")}.`,
      subjects: assetQualityAwaiting,
    }];
    manifest.status = "speech-awaiting-review";
    await persistCanonical();
    return { manifest, manifestPath, report, reportPath, waiting: false, partial: true, awaitingAssetQuality: true, assetQualityAwaiting, ...overlapResult };
  }
  ...
  await persistCanonical();
  return { manifest, manifestPath, report, reportPath, waiting: false, partial, ...overlapResult };
}
