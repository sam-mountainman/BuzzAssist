...
/**
 * チャンネル専用スキルの在庫 manifest（Channel Pack か私有プロジェクトの中。公開しない）。
 * 公開の manifest に私有スキルを載せると、チャンネル名や依頼者の運用が公開リポジトリへ出る。
 * かといって在庫の外に置くと、版・SHA・人の承認を同じ規則で検査できない。そこで在庫の
 * 規則（validateApproval・skillApprovalState・recordSkillApproval の二手）は共有し、manifest の
 * 置き場だけを私有側に分ける。場所は --channel-skills <dir> か環境変数で渡す。
 */
export const CHANNEL_SKILL_INVENTORY_MANIFEST_PATH = "skills/inventory.manifest.json";
export const CHANNEL_SKILLS_ENV = "BUZZASSIST_CHANNEL_SKILLS";
export const CHANNEL_SKILL_ORIGIN = "channel-private";
const CHANNEL_NAMESPACE_PATTERN = /^[a-z0-9][a-z0-9-]{0,40}$/u;
...
// ---- スキルの束（SKILL.md と、一緒に配る references・付属物）の digest ----------------------------
//
// contentSha256 は SKILL.md のバイト列だけを覆う。references/ は配布物に入り AI の動きを決める
// （品質ループの決まり、署名の運用など）のに、そこだけを変えると承認は「今の版」のままに見え、人が
// 見ないまま運営者の端末へ届いた（2026-09-26 に references だけを変えたコミットが2つあった）。
// bundleSha256 はスキルのディレクトリ全体を覆い、人の承認はこれにも束縛する。
//
// 覆うもの: SKILL.md、references/、scripts/、agents/、assets/ など、スキルのディレクトリの中で
//   配布されるファイル全部（setup-agents と npm の files はディレクトリを丸ごと写す）。
// 外すもの:
//   - evals/（直下）: skill-evals が採点に使う入力で、ホストは制作中に読まない。結果は版と
//     contentSha256 に別に束縛される（--require-evals）。入れると、eval を1件足すたびに運営者へ
//     届く指示が変わらないまま承認が外れる
//   - references/learned-auto.md・references/learned-archive.md: 自己改善の sync・setup・自動更新が
//     機械で書き直す overlay と退避。SKILL.md に「監査・承認・合否の証跡には使えない」と書いてあり、
//     運営者の写しでは端末ごとに中身が違う（配った後に届け直す）。入れると sync のたびに承認が外れ、
//     配布された写しの制作が止まる。RunReceipt の tree 指紋も同じ2つを外している
//   - OS が勝手に置く .DS_Store・Thumbs.db・desktop.ini・._*: 運営者の端末で Finder を開いただけで
//     承認が外れないようにする
// OS の違いで値が変わらないように、パスは "/" 区切り・NFC、テキスト（NUL を含まない）は CRLF を LF に
// そろえてから数える。並べ順はコード単位の順（localeCompare はロケールで変わる）。
export const SKILL_BUNDLE_DIGEST_VERSION = "buzzassist-skill-bundle-v1";
export const SKILL_BUNDLE_EXCLUDED_DIRECTORIES = Object.freeze(["evals"]);
export const SKILL_MACHINE_OWNED_FILES = Object.freeze([
  "references/learned-auto.md",
  "references/learned-archive.md",
]);
const OS_METADATA_FILE = /^(?:\.DS_Store|Thumbs\.db|desktop\.ini|\._.*)$/iu;

/** テキストなら CRLF を LF にそろえる（UTF-8 の多バイト列に 0x0D/0x0A は現れないのでバイト単位で安全）。 */
function bundleBytes(buffer) {
  if (buffer.includes(0) || !buffer.includes(0x0d)) return buffer;
  const output = Buffer.allocUnsafe(buffer.length);
  let length = 0;
  for (let index = 0; index < buffer.length; index += 1) {
    if (buffer[index] === 0x0d && buffer[index + 1] === 0x0a) continue;
    output[length] = buffer[index];
    length += 1;
  }
...
/**
 * 束に入るファイルの正規の一覧（相対パスと、そろえた内容の sha256）。symlink などの普通でない
 * ファイルは配布物に入れない（setup-agents の staging 検査が拒む）ので、ここでも数えずに止める。
 */
export async function skillBundleEntries(skillDir) {
  const root = resolve(skillDir);
  const entries = [];
  const visit = async (dir, parts) => {
    for (const child of await readdir(dir, { withFileTypes: true })) {
      if (OS_METADATA_FILE.test(child.name)) continue;
      const childParts = [...parts, child.name.normalize("NFC")];
      const relativePath = childParts.join("/");
      const full = join(dir, child.name);
      if (child.isDirectory()) {
        if (parts.length === 0 && SKILL_BUNDLE_EXCLUDED_DIRECTORIES.includes(child.name)) continue;
        await visit(full, childParts);
        continue;
      }
      if (SKILL_MACHINE_OWNED_FILES.includes(relativePath)) continue;
      if (!child.isFile()) {
        throw new Error(`スキルの束に普通のファイルでないものがある（symlink などは配らない）: ${relativePath}`);
      }
      entries.push({ path: relativePath, sha256: sha256Hex(bundleBytes(await readFile(full))) });
    }
  };
  await visit(root, []);
...
/** 正規の一覧から digest を作る。版の文字列も数に入れるので、数え方を変えたら値も変わる。 */
export function skillBundleDigestOf(entries) {
  const lines = entries.map(({ path, sha256: digest }) => `${digest} ${JSON.stringify(path)}\n`).join("");
  return sha256(`${SKILL_BUNDLE_DIGEST_VERSION}\n${lines}`);
}

/** スキルのディレクトリ（canonicalPath の親）の束の digest と、数えたファイルの一覧。 */
export async function computeSkillBundle(skillDir) {
  const files = await skillBundleEntries(skillDir);
  return { bundleSha256: skillBundleDigestOf(files), fileCount: files.length, files };
}

async function bundleForCanonical(canonicalPath) {
  try {
    return { ...(await computeSkillBundle(dirname(canonicalPath))), error: null };
  } catch (error) {
    return { bundleSha256: null, fileCount: 0, files: [], error: error.code || error.message };
  }
}

/** 在庫の bundleSha256 と実物の食い違い。無ければ null。 */
function bundleIssue(skill, bundle, label = "manifest") {
  if (bundle.error) return `${skill.id}: skill bundle could not be read (${bundle.error})`;
  const hint = "（SKILL.md 以外の references・付属物が在庫と違う。bundleSha256 を更新し、配った版・承認済みの版なら semver を上げる）";
  if (!skill.bundleSha256) return `${skill.id}: ${label} bundleSha256 is missing (actual ${bundle.bundleSha256})${hint}`;
  if (skill.bundleSha256 !== bundle.bundleSha256) return `${skill.id}: ${label} bundle hash ${skill.bundleSha256} != ${bundle.bundleSha256}${hint}`;
  return null;
}
...
// 置く場所がこれまで無く、承認をどこにも束縛できなかった。承認は版・SKILL.md の内容 SHA・束の
// digest（bundleSha256）を持ち、どれかが今の正本と違えば stale（その承認は今の版を保証しない）。
// bundleSha256 の無い承認（2026-09-26 までの記録）は references・付属物を覆っていないので stale。
...
function validateApproval(approval, field, issues) {
  ...
  // 古い承認には無い（読めるが stale）。あるなら形は SHA-256。
  if (approval.bundleSha256 !== undefined && !SHA256_PATTERN.test(String(approval.bundleSha256))) issues.push(`${field}.bundleSha256 must be SHA-256`);
...
/** 在庫の skill の束の digest の欄（任意。あるなら形は SHA-256。無ければ在庫の検査が落とす）。 */
function validateBundleField(skill, field, issues) {
  if (skill?.bundleSha256 !== undefined && !SHA256_PATTERN.test(String(skill.bundleSha256))) issues.push(`${field}.bundleSha256 must be SHA-256`);
}

/** stale の理由。人に見せる文は describeSkillApprovalStaleReason。 */
export const SKILL_APPROVAL_STALE_REASONS = Object.freeze({
  notHumanVerified: "not-human-verified",
  versionChanged: "version-changed",
  contentChanged: "content-changed",
  bundleNotCovered: "bundle-not-covered",
  bundleChanged: "bundle-changed",
});

const STALE_REASON_TEXT = Object.freeze({
  "not-human-verified": "人の承認ではない記録",
  "version-changed": "承認した版から版が変わった",
  "content-changed": "承認した SKILL.md から中身が変わった",
  "bundle-not-covered": "承認が references・付属物を覆っていない（bundleSha256 の無い古い承認）",
  "bundle-changed": "承認した後に references・付属物が変わった",
});

export function describeSkillApprovalStaleReason(reason) {
  return STALE_REASON_TEXT[reason] || "";
}

/**
 * 承認の状態と、stale ならその理由。actualSha256 は SKILL.md の、actualBundleSha256 は束の、
 * 今の実物の値（渡されなければ在庫に書いてある値で見る）。
 */
export function skillApprovalStatus(skill, actualSha256 = skill?.contentSha256, actualBundleSha256 = skill?.bundleSha256) {
  const approval = skill?.approval;
  if (!plainObject(approval)) return { state: "none", reason: null };
  const stale = (reason) => ({ state: "stale", reason });
  if (approval.attestedBy !== SKILL_APPROVAL_ATTESTATION) return stale(SKILL_APPROVAL_STALE_REASONS.notHumanVerified);
  if (approval.version !== skill.version) return stale(SKILL_APPROVAL_STALE_REASONS.versionChanged);
  if (approval.contentSha256 !== actualSha256) return stale(SKILL_APPROVAL_STALE_REASONS.contentChanged);
  if (typeof approval.bundleSha256 !== "string" || !approval.bundleSha256) return stale(SKILL_APPROVAL_STALE_REASONS.bundleNotCovered);
  if (!actualBundleSha256 || approval.bundleSha256 !== actualBundleSha256) return stale(SKILL_APPROVAL_STALE_REASONS.bundleChanged);
  return { state: "current", reason: null };
}

/** none: 承認が無い / current: 今の版・SHA・束に束縛されている / stale: 別の版・別の内容への承認か、束を覆っていない承認 */
export function skillApprovalState(skill, actualSha256 = skill?.contentSha256, actualBundleSha256 = skill?.bundleSha256) {
  return skillApprovalStatus(skill, actualSha256, actualBundleSha256).state;
}

/**
 * 承認に書く値を、正本の実物から作る。在庫の contentSha256・bundleSha256 が実物と違えば書かない
 * （人が読んだものと在庫が違うまま承認を付けない）。
 */
async function approvalFor({ skill, canonicalPath, reviewer, now, scopeLabel }) {
  const source = await readFile(canonicalPath, "utf8");
  const actualSha256 = sha256(source);
  if (actualSha256 !== skill.contentSha256) {
    throw new Error(`${skill.id}: ${scopeLabel}の contentSha256 が正本と一致しない。先に在庫の版と SHA を更新すること。`);
  }
  const bundle = await computeSkillBundle(dirname(canonicalPath));
  if (skill.bundleSha256 !== bundle.bundleSha256) {
    throw new Error(`${skill.id}: ${scopeLabel}の bundleSha256 が正本の references・付属物と一致しない（在庫 ${skill.bundleSha256 || "なし"}、実物 ${bundle.bundleSha256}）。先に在庫の digest と版を更新すること。`);
  }
  return {
    reviewer,
    approvedAt: now(),
...
/**
 * 人の承認を、今の版・SKILL.md の内容 SHA・束の digest に束縛して manifest へ書く。
...
export async function recordSkillApproval({
  ...
  // チャンネル専用スキル（buzzassist: 以外の名前空間）は、私有側の manifest に承認を書く。
  // 二手の要求と、版・内容 SHA への束縛は公開の在庫と同じ。
  if (!String(skillId).startsWith("buzzassist:")) {
    if (!String(channelSkillsDir || "").trim()) {
      throw new Error(`${skillId} は公開の在庫に無い名前空間です。チャンネル専用スキルなら --channel-skills <dir>（か ${CHANNEL_SKILLS_ENV}）で私有の在庫を指定してください。`);
    }
    const channel = await loadChannelSkillManifest(channelSkillsDir);
    if (channel.issues.length > 0) {
      throw new Error(`チャンネル専用スキルの在庫 manifest が不正:\n${channel.issues.map((issue) => `  - ${issue}`).join("\n")}`);
    }
    const skill = channel.manifest.skills.find((entry) => entry.id === skillId);
    if (!skill) throw new Error(`チャンネル専用スキルの在庫に無い skill: ${skillId}`);
    skill.approval = await approvalFor({
      skill,
      canonicalPath: channelInside(channel.root, skill.canonicalPath),
      reviewer: name,
      now,
      scopeLabel: "私有の manifest ",
    });
    const issues = validateChannelSkillInventoryManifest(channel.manifest, { root: channel.root });
    if (issues.length > 0) throw new Error(`承認を書いた私有の manifest が不正:\n${issues.map((issue) => `  - ${issue}`).join("\n")}`);
    await writeFile(channel.manifestPath, `${JSON.stringify(channel.manifest, null, 2)}\n`, "utf8");
    return { skillId, approval: { ...skill.approval }, inventoryPath: channel.manifestPath, scope: "channel" };
  }
  ...
  skill.approval = await approvalFor({
    skill,
    canonicalPath: resolve(projectDir, skill.canonicalPath),
    reviewer: name,
    now,
    scopeLabel: "manifest ",
  });
...
export function validateSkillInventoryManifest(manifest) {
  ...
  for (const [index, skill] of (manifest.skills ?? []).entries()) {
    ...
    validateBundleField(skill, field, issues);
...
/** 私有の在庫の中のパスだけを受ける（絶対パス・外へ出る .. は拒否）。 */
function channelInside(root, relativePath) {
  const text = String(relativePath || "");
  if (!text || isAbsolute(text) || /^[A-Za-z]:/u.test(text)) throw new Error(`私有の在庫のパスは在庫からの相対にしてください: ${text}`);
  const full = resolve(root, text);
  const rel = relative(root, full);
  if (!rel || rel.startsWith("..") || isAbsolute(rel)) throw new Error(`私有の在庫の外を指しています: ${text}`);
  return full;
}

function channelPathIssue(root, relativePath, field) {
  try {
    channelInside(root, relativePath);
    return null;
  } catch {
    return `${field} must be a relative path inside the channel skill directory`;
  }
}

/**
 * チャンネル専用スキルの在庫 manifest を検査する。skill の欄と承認の規則は公開の在庫と同じ。
 * 違うのは、scope が "channel"、名前空間が buzzassist 以外、origin が channel-private、
 * plugin を持たないこと、パスが私有の在庫の中に限られること。
 */
export function validateChannelSkillInventoryManifest(manifest, { root = "." } = {}) {
  const issues = [];
  if (!plainObject(manifest)) return ["channel manifest must be an object"];
  if (manifest.schemaVersion !== SKILL_INVENTORY_SCHEMA_VERSION) issues.push(`schemaVersion must be ${SKILL_INVENTORY_SCHEMA_VERSION}`);
  if (manifest.scope !== "channel") issues.push("scope must be channel");
  if (!SEMVER_PATTERN.test(manifest.manifestVersion || "")) issues.push("manifestVersion must be semver");
  const namespace = String(manifest.namespace || "");
  if (!CHANNEL_NAMESPACE_PATTERN.test(namespace) || namespace === "buzzassist") {
    issues.push("namespace must be a lowercase id other than buzzassist");
  }
  if (manifest.plugins !== undefined) issues.push("channel manifest must not declare plugins");
  ...
  for (const [index, skill] of manifest.skills.entries()) {
    ...
    if (ids.has(skill?.id)) issues.push(`${field}.id is duplicated: ${skill?.id}`);
    ids.add(skill?.id);
    if (typeof skill?.id === "string" && !skill.id.startsWith(`${namespace}:`)) issues.push(`${field}.id must start with ${namespace}:`);
    if (skill?.origin !== CHANNEL_SKILL_ORIGIN) issues.push(`${field}.origin must be ${CHANNEL_SKILL_ORIGIN}`);
    if (!SEMVER_PATTERN.test(skill?.version || "")) issues.push(`${field}.version must be semver`);
    if (!SHA256_PATTERN.test(skill?.contentSha256 || "")) issues.push(`${field}.contentSha256 must be SHA-256`);
    validateBundleField(skill, field, issues);
    if (typeof skill?.canonicalPath === "string") {
      const pathIssue = channelPathIssue(base, skill.canonicalPath, `${field}.canonicalPath`);
      if (pathIssue) issues.push(pathIssue);
    }
    if (!Array.isArray(skill?.hosts) || skill.hosts.length === 0) issues.push(`${field}.hosts must be non-empty`);
    if (!Array.isArray(skill?.adapters)) issues.push(`${field}.adapters must be an array`);
    for (const [adapterIndex, adapter] of (Array.isArray(skill?.adapters) ? skill.adapters : []).entries()) {
      const pathIssue = channelPathIssue(base, adapter?.path, `${field}.adapters[${adapterIndex}].path`);
      if (pathIssue) issues.push(pathIssue);
    }
    validateClassification(skill?.classification, `${field}.classification`, issues);
    validateApproval(skill?.approval, `${field}.approval`, issues);
...
/** 私有の在庫を読む。読めない・不正でも投げずに issues で返す（公開の在庫の報告は続ける）。 */
export async function loadChannelSkillManifest(channelSkillsDir) {
  const root = resolve(String(channelSkillsDir || ""));
  const manifestPath = join(root, CHANNEL_SKILL_INVENTORY_MANIFEST_PATH);
  let manifest = null;
  try {
    manifest = JSON.parse(await readFile(manifestPath, "utf8"));
  } catch (error) {
    return { root, manifestPath, manifest: null, issues: [`channel skill manifest could not be read: ${error.code || error.message}`] };
  }
  return { root, manifestPath, manifest, issues: validateChannelSkillInventoryManifest(manifest, { root }) };
}

async function channelSkillRecords(channel) {
  const records = [];
  const issues = channel.issues.map((issue) => `channel: ${issue}`);
  if (!channel.manifest || channel.issues.length > 0) return { records, issues };
  const scope = `channel:${channel.manifest.namespace}`;
  for (const skill of channel.manifest.skills) {
    const canonicalPath = channelInside(channel.root, skill.canonicalPath);
    let source;
    try {
      source = await readFile(canonicalPath, "utf8");
    } catch (error) {
      issues.push(`${skill.id}: channel canonical skill is missing: ${skill.canonicalPath} (${error.code || error.message})`);
      continue;
    }
    const metadata = parseSkillFrontmatter(source);
    const actualSha256 = sha256(source);
    const bundle = await bundleForCanonical(canonicalPath);
    if (metadata.name !== skill.name) issues.push(`${skill.id}: frontmatter name is ${metadata.name || "missing"}, expected ${skill.name}`);
    if (actualSha256 !== skill.contentSha256) issues.push(`${skill.id}: channel manifest hash ${skill.contentSha256} != ${actualSha256}`);
    const bundleProblem = bundleIssue(skill, bundle, "channel manifest");
    if (bundleProblem) issues.push(bundleProblem);
    const approval = skillApprovalStatus(skill, actualSha256, bundle.bundleSha256);
    records.push({
      kind: "skill",
      name: skill.name,
      qualifiedId: skill.id,
      scope,
      sourceRole: "channel-canonical",
      path: canonicalPath,
      ...
      bundleSha256: bundle.bundleSha256,
      expectedBundleSha256: skill.bundleSha256 ?? null,
      bundleFiles: bundle.files,
      classification: { ...skill.classification },
      approval: skill.approval ? { ...skill.approval } : null,
      approvalState: approval.state,
      approvalStaleReason: approval.reason,
    });
    for (const adapter of skill.adapters) {
      const adapterPath = channelInside(channel.root, adapter.path);
      let adapterSource;
      try {
        adapterSource = await readFile(adapterPath, "utf8");
      } catch (error) {
        issues.push(`${skill.id}: ${adapter.host} channel adapter is missing: ${adapter.path} (${error.code || error.message})`);
        continue;
      }
      if (parseSkillFrontmatter(adapterSource).name !== adapter.name) {
        issues.push(`${skill.id}: ${adapter.host} channel adapter name does not match ${adapter.name}`);
      }
      if (!adapterSource.includes(skill.canonicalPath.replace(/^\.\//u, ""))) {
        issues.push(`${skill.id}: ${adapter.host} channel adapter does not route to ${skill.canonicalPath}`);
      }
    }
  }
...
async function projectSkillRecords(projectDir, manifest) {
  ...
  for (const skill of manifest.skills) {
    ...
    // references・付属物だけを変えて在庫を更新していないものもここで落とす（SKILL.md の SHA は一致していても）。
    const bundleProblem = bundleIssue(skill, bundle);
    if (bundleProblem) issues.push(bundleProblem);
    const approval = skillApprovalStatus(skill, actualSha256, bundle.bundleSha256);
    records.push({
      kind: "skill",
...
export function findMissingInstalledCopies(records) {
  ...
  for (const record of records) {
    ...
    if (!match) continue; // .claude/skills の host adapter は配布物に入らない
...
export function analyzeSkillInventory(records, manifestIssues = []) {
  ...
  return {
    ...
    // 人の承認が今の版・今の SHA・今の束（bundleSha256）に束縛されていない本番スキル。skill-creator の合否
    ...
    unapprovedProductionSkills: records
      .filter((record) => record.kind === "skill" && ["project-canonical", "channel-canonical"].includes(record.sourceRole)
        && record.classification?.productionAllowed === true && record.approvalState !== "current")
      .map((record) => ({
        id: record.qualifiedId,
        version: record.version,
        approvalState: record.approvalState,
        ...(record.approvalStaleReason ? { reason: record.approvalStaleReason } : {}),
        ...(record.sourceRole === "channel-canonical" ? { scope: record.scope } : {}),
      }))
...
export async function buildSkillInventory(options = {}) {
  ...
  const channel = String(options.channelSkillsDir || "").trim() ? await loadChannelSkillManifest(options.channelSkillsDir) : null;
  const channelSkills = channel ? await channelSkillRecords(channel) : { records: [], issues: [] };
  ...
  const installedPlugins = options.includePluginCache
    ...
    : [];
  const skills = [...projectSkills.records, ...channelSkills.records, ...installedSkills];
  const plugins = [...projectPlugins.records, ...installedPlugins];
  const manifestIssues = [...projectSkills.issues, ...projectPlugins.issues, ...channelSkills.issues];
  ...
  return {
    ...
    generatedFrom: {
      ...
      ...(channel ? { channelSkillsManifest: channel.manifestPath } : {}),
    },
    ...(channel ? {
      channel: {
        namespace: channel.manifest?.namespace || null,
        manifestVersion: channel.manifest?.manifestVersion || null,
        skills: channelSkills.records.length,
      },
    } : {}),
