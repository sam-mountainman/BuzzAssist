...
export async function recordSkillApproval({
  ...
  if (!String(skillId).startsWith("buzzassist:")) {
    ...
    const source = await readFile(channelInside(channel.root, skill.canonicalPath), "utf8");
    const actualSha256 = sha256(source);
    if (actualSha256 !== skill.contentSha256) {
      throw new Error(`${skillId}: 私有の manifest の contentSha256 が正本と一致しない。先に在庫の版と SHA を更新すること。`);
    }
    skill.approval = {
      reviewer: name,
      approvedAt: now(),
