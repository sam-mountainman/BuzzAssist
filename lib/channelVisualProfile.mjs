...
export function selectChannelVisualReferences(profile, scene = {}, options = {}) {
  ...
  const characterCount = Array.isArray(scene.characterIds ?? scene.character_ids ?? scene.characters)
    ...
    : 0;
  const maximum = characterCount >= 3 ? Math.min(1, requestedMaximum) : requestedMaximum;
...
export function buildChannelVisualStylePrompt(profile, scene = {}, referenceCount = 0) {
  ...
  const lines = [
    ...
    referenceCount > 0
      ? `Reference images 1-${referenceCount} are STYLE-ONLY references. Match their linework, cel shading, palette, lighting, background finish, lens language, and visual density. Never copy their people, faces, hair, clothing, text, speech balloons, or exact composition.`
