...
export function buildChannelVisualStylePrompt(profile, scene = {}, referenceCount = 0) {
  if (!profile) return "";
  const tags = inferChannelVisualTags(scene);
  const referenceSheet = tags.some((tag) => ["turnaround", "expression-sheet", "outfit-sheet"].includes(tag));
  const lines = [
    ...
    profile.stylePrompt,
    referenceSheet ? "" : profile.compositionPrompt,
    referenceSheet ? "" : profile.shotRhythmPrompt,
    profile.continuityPrompt,
    `Scene style tags: ${tags.join(", ")}.`,
    referenceSheet
      ? "REFERENCE SHEET PRIORITY: use the exact grid, views and pure-white cell clearance specified by this sheet job. Channel prose controls drawing style only; scene framing, backgrounds, full-bleed composition and shot rhythm do not apply. Approved identity and the character-specific eye state override generic iris and expression advice."
