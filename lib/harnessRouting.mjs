...
import { channelPackPresent } from "./channelPackResolver.mjs";
...
export function checkCanonicalRouting({ genre = "manga-video", toolName, projectDir, acknowledgedBenchmarkMigration = false } = {}) {
  ...
  if (!channelPackPresent(dir)) {
    return { allowed: true, reason: "Channel Pack 未設置。ジャンル共通ハーネスの範囲内" };
  }
  ...
  return {
    allowed: false,
    reason: "Channel Pack 設置済みの環境で旧入口が本番として使われようとしている",
    canonical,
    message:
      `${toolName} は移行ベンチマーク専用の旧入口で、この環境には Channel Pack が置かれている。`
