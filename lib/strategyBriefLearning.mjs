...
//   - **台帳のチャンネルが分かればその保存先へ**。ブリーフの channel.id が台帳のチャンネルの id か、作業フォルダが
//     台帳のチャンネルの strategy.workDir（・projectDir）に当たるときは、そのチャンネルの保存先へ積む
//     （lib/learningChannelResolver.mjs）。両方が別のチャンネルを指せば決めずに積まない。決まったのに保存先を
//     決められなければ channel-learning-store-unresolved で積まない。チャンネルが無ければ従来どおり
...
export async function captureStrategyBriefLearning({
  ...
  captureOptions = {},
  // 積むチャンネルの手がかり（lib/learningChannelResolver.mjs）。ループは workDir とブリーフの channel.id を渡す。
  workDir = "",
  briefChannelId = "",
  channelId = "",
  ...
  const channel = await learningChannelCaptureOptions({
    captureOptions, env, workDir, briefChannelId, channelId, ...(resolveChannel ? { resolveChannel } : {}),
  });
