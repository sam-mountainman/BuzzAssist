...
import { briefFixStep, checkJobStrategyBrief, isResearchReasonCode } from "./channelStrategyBrief.mjs";
...
/** 使える前のブリーフ（下書きの --previous / next の --from に渡す）。別のチャンネルのブリーフは渡さない。 */
function previousBrief(ctx) {
  return ctx.brief?.provided && ctx.brief.state !== "unusable" ? ctx.brief.path : "";
}

/** 企画の品質ループ（形の確認 → 始める／始め直す → 別の文脈で採点 → 記録 → 制作へ渡す前の判定）。 */
...
/**
 * 戦略スキルの工程が終わったあと、作業フォルダからブリーフを組み立てる（docs/strategy-handoff-spec-ja.md の 7）。
 * draft --from-hyp（公開後は next）→ 機械で埋められない欄を成果物から埋める → 形の確認 → 前提を変えたら
 * 根拠ごとの当てはまりの判定 → 企画の品質ループ。
 */
function briefAssemblyCommands(ctx, { via = "draft" } = {}) {
  const workDir = workDirOf(ctx);
  const out = ctx.nextDraftPath || path.join(workDir, "strategy-brief-r1.json");
  const previous = previousBrief(ctx);
  const skill = ctx.skill?.status === "readable" ? ` --strategy-skill-dir ${q(ctx.skill.dir)}` : "";
  const first = via === "next" && previous
    ? {
        what: "公開後の数字と前のブリーフを照らし、次のブリーフの下書きを作る（数字のファイルは作業フォルダの中に置く）",
        cli: `node scripts/strategy-brief.mjs next --from ${q(previous)} --metrics <指標の集計 JSON> --work-dir ${q(workDir)} --out ${q(out)}`,
      }
    : {
        what: "戦略スキルの成果物からブリーフの下書きを組み立てる（機械で集められるものだけ。戦略スキルのスクリプトは実行しない）",
        cli: `node scripts/strategy-brief.mjs draft --from-hyp ${q(workDir)} --channel ${ctx.channel.id}${previous ? ` --previous ${q(previous)}` : ""}${skill} --out ${q(out)}`,
      };
  return [
    first,
    { what: "下書きの needsAuthoring の欄を、戦略スキルの成果物を読んで埋める（推測で埋めない。確かめられないことは openQuestions へ、制作の前に要るものは blocksProduction: true）" },
    ...(previous ? [{ what: "前提（問い・見る人・制作条件）を変えたなら、当てはまりの確認待ちの根拠を一覧にして根拠ごとに判定を記録する", cli: `node scripts/strategy-brief.mjs applicability --brief ${q(out)} --work-dir ${q(workDir)}` }] : []),
    ...briefLoopCommands(ctx, { briefFile: out }),
  ];
}
...
function hypStep(id, ctx, { reason, reasonCodes = [], then = null, extra = {} }) {
  ...
  return {
    ...
    ...extra,
    then: then ?? (id === "hyp-research" ? [planAgainCommand(ctx)] : briefAssemblyCommands(ctx)),
  };
}

/**
 * 台本の品質ループのジャンル。台帳の scriptQuality.genre、無ければ制作のハーネスが有料の処理の前に問うジャンル
 * （lib/scriptQualityUseGate.mjs。チャンネル自身のハーネスから決まる）。どちらも無ければ空。
 */
function scriptGenre(ctx) {
  return nonEmpty(ctx.channel.scriptQuality?.genre) || nonEmpty(ctx.scriptGateGenre);
}

/**
 * 台本の品質ループの作業フォルダ。start が Job の options.scriptQualityWorkDir に入れるのと同じ規則
 * （明示の --script-quality-work-dir、無ければ台本のあるフォルダ。lib/scriptQualityUseGate.mjs の scriptQualityWorkDirFor）。
 */
function scriptWorkDirArgument(ctx) {
  return nonEmpty(ctx.scriptQualityWorkDir) ? q(ctx.scriptQualityWorkDir) : "<台本の作業フォルダ（台本のあるフォルダ。start の --script-quality-work-dir と同じ）>";
}

function scriptQualityCommands(ctx) {
  const genre = scriptGenre(ctx);
  const workDir = scriptWorkDirArgument(ctx);
...
function produceStep(ctx, { reason, reasonCodes = [], blocked = null }) {
  ...
  return {
    ...
    ...(nonEmpty(ctx.scriptGateGenre)
      ? { scriptQuality: { genre: ctx.scriptGateGenre, workDir: nonEmpty(ctx.scriptQualityWorkDir) ? ctx.scriptQualityWorkDir : null } }
      : {}),
    note: "--channel で作業フォルダ・署名済み Channel Pack・ハーネスが台帳から決まる。--confirmed を付けない start は計画だけを保存し、有料 API を呼ばない。"
      ...
      + (nonEmpty(ctx.scriptGateGenre)
        ? ` 使う台本は、台本の品質ループ（ジャンル ${ctx.scriptGateGenre}、作業フォルダは Job の options.scriptQualityWorkDir。省けば台本のあるフォルダ）で合格した版か、人がそのまま使うと認めた版（accept-human）でないと、有料の処理の前に止まる。`
        : ""),
  };
}
...
function reviewStep(ctx, { reason, reasonCodes }) {
  return buzzassistStep("strategy-brief-review", ctx, {
    ...
    extra: {
      ...
      workDir: workDirOf(ctx),
      then: [
        // 下書きのまま（形の検査を通らない）なら、先に埋める。
        ...((ctx.brief.reasonCodes || []).includes("strategy-brief-invalid")
          ? [{ what: "下書きの needsAuthoring の欄を、戦略スキルの成果物を読んで埋める（推測で埋めない。確かめられないことは openQuestions へ）" }]
          : []),
        ...briefLoopCommands(ctx, { briefFile: ctx.brief.path }),
      ],
    },
  });
}
...
function gapLabel(gap) {
  const id = gap.evidenceId ?? gap.openQuestionId ?? gap.claimId;
  const kind = gap.openQuestionId !== undefined ? "未確認事項 " : gap.claimId !== undefined ? "主張 " : "";
  return `${kind}${id}（${gap.reasonCode}）`;
}
...
function researchStep(ctx, { reason }) {
  ...
  const detail = [
    gaps.length > 0 ? `確かめ直す・取り直す対象: ${gaps.map(gapLabel).join(", ")}` : "",
    fields.length > 0 ? `変わった前提: ${fields.join("・")}` : "",
  ].filter(Boolean).join(" / ");
  const workDir = workDirOf(ctx);
  const brief = ctx.brief?.provided ? ctx.brief.path : "";
  const applicability = gaps.filter((gap) => String(gap.reasonCode).startsWith("strategy-evidence-applicability-review-required"));
  const then = [];
  if (brief && applicability.length > 0) {
    // 前提を変えた版の根拠は、古いのではなく当てはまりの確認待ち。先に判定し、当てはまらない根拠だけを取り直す。
    then.push({ what: "当てはまりの確認待ちの根拠を一覧にする", cli: `node scripts/strategy-brief.mjs applicability --brief ${q(brief)} --work-dir ${q(workDir)}` });
    then.push({
      what: `根拠（${applicability.map((gap) => gap.evidenceId).join(", ")}）を開いて、今の前提に当てはまるかを1件ずつ記録する（yes は再利用、no はその根拠だけ取り直し）`,
      cli: `node scripts/strategy-brief.mjs applicability --brief ${q(brief)} --evidence <根拠の id> --applies yes|no --reason "<当てはまる／当てはまらない理由>" --context <判定した会話・タスクの ID> --work-dir ${q(workDir)}`,
    });
  }
  const needsNewEvidence = gaps.some((gap) => !String(gap.reasonCode).startsWith("strategy-evidence-applicability-review-required"));
  if (!brief || needsNewEvidence) {
    then.push({ what: "戦略スキルの工程で、名指しの根拠・未確認事項だけを確かめ直して作業フォルダに置く（当てはまる根拠は再利用する）" });
    then.push(...briefAssemblyCommands(ctx));
  } else {
    then.push(...briefLoopCommands(ctx, { briefFile: brief }));
  }
  return hypStep("hyp-additional-research", ctx, {
    reason: detail ? `${reason}（${detail}）` : reason,
    reasonCodes: (ctx.brief?.reasonCodes || []).filter(isResearchReasonCode),
    extra: {
      brief: ctx.brief?.provided ? { path: ctx.brief.path, label: ctx.brief.label || null } : null,
      gaps,
      premiseChangedFields: fields,
      reuse: "当てはまる根拠は再利用し、名指しの根拠と前提だけを取り直す。古いという理由だけで全部を取り直さない",
    },
    then,
  });
}
...
export function recommendNextSteps(ctx) {
    ...
    case "script-review": {
      recommended = hypStep("hyp-script-review", ctx, {
        ...
        extra: {
          ...
          scriptQuality: scriptGenre(ctx)
            ? {
                genre: scriptGenre(ctx),
                genreSource: nonEmpty(ctx.channel.scriptQuality?.genre) ? "channel-registry" : "production-harness",
                ...(nonEmpty(ctx.channel.scriptQuality?.channelConfig) ? { channelConfig: ctx.channel.scriptQuality.channelConfig } : {}),
                workDir: nonEmpty(ctx.scriptQualityWorkDir) ? ctx.scriptQualityWorkDir : null,
                note: "制作の start はこの作業フォルダを Job の options.scriptQualityWorkDir に入れ、有料の処理の前に、使う台本がこのループで合格した版か人がそのまま使うと認めた版（accept-human）かを問う",
              }
            : { status: "not-configured", note: "台帳に scriptQuality.genre が無く、制作のハーネスにも台本のジャンルが無い（台本の品質ループのジャンルが決まらない）" },
          produces: { kind: "script-revision", where: nonEmpty(ctx.scriptQualityWorkDir) ? ctx.scriptQualityWorkDir : "<台本の作業フォルダ>" },
        },
  ...
  // 学習の保存先はチャンネルごと（lib/channelRegistry.mjs が重なりを読み込みで拒む）なので、同じハーネスの
  // チャンネルがあっても警告しない。
  return { recommended, alternatives, warnings };
}

function postPublishCommands(ctx) {
  // 前のブリーフがあれば next（公開後の数字との照合）、無ければ新規設計と同じく draft --from-hyp から。
  return briefAssemblyCommands(ctx, { via: "next" });
}
