  ...
  return {
    ...
    boards: plan.jobs.map((job) => {
      ...
      return {
        ...
        checks: Object.fromEntries(checkKeys.map((key) => [key, reviewedById.get(job.boardId)?.checks?.[key] === true])),
      };
    }),
...
function portableLocationPrivacyFailures(value, expectedCheckKeys) {
  ...
  for (const [index, board] of (Array.isArray(value.boards) ? value.boards : []).entries()) {
    if (exactPortableKeys(board, PORTABLE_LOCATION_BOARD_KEYS, `location snapshot.boards[${index}]`, failures)) {
      exactPortableKeys(board.checks, expectedCheckKeys, `location snapshot.boards[${index}].checks`, failures);
    }
  }
  return failures;
}
...
async function verifyPortableLocationEntry({ bundleRoot, entry, authority }) {
  ...
  for (const [index, board] of boards.entries()) {
    ...
    provenanceContexts.add(board.generatorContextId);
