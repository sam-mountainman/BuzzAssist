...
import {
  ...
  KOYA_LOCATION_PROVENANCE_GAP_CHECK,
  KOYA_LOCATION_PROVENANCE_GAP_FLAGS,
  KOYA_LOCATION_REVIEW_VERSION,
  koyaLocationBoardCheckKeys,
  koyaLocationEntryProvenanceGap,
  koyaLocationGapDeclares,
  koyaLocationTextPolicy,
...
export const KOYA_HANDOFF_LOCATION_REVIEW_ATTESTATION_VERSION = "koya-handoff-location-review-attestation-v1";
...
const PORTABLE_CHARACTER_APPROVAL_ROUTES = Object.freeze(["anonymous-candidate-selection", "human-best-of-n", "legacy-migration"]);

function stripPrivateApproval(approval, publicRoutes = PORTABLE_CHARACTER_APPROVAL_ROUTES) {
  ...
  return {
    ...
    // 出所が欠けたまま登録したボードの申告は、移送先でも消さない。理由は
    // 送り先の機械に生の文を残さないよう一方向に畳む。
    ...portableProvenanceGaps(approval.provenanceGaps),
...
function portableProvenanceGaps(value) {
  const gaps = (Array.isArray(value) ? value : []).map((gap) => ({
    boardId: nonEmptyString(gap?.boardId),
    reason: portableIdentifier("source-reason-sha256", gap?.reason),
    specificationSha256: nonEmptyString(gap?.specificationSha256),
    // どの記録が無かったのかも運ぶ。畳むと、受け取り側で attestation と
    // 突き合わせても「会話は残っていた」という書き換えが通ってしまう。
    ...Object.fromEntries(KOYA_LOCATION_PROVENANCE_GAP_FLAGS
      .filter((flag) => gap?.[flag] === false)
      .map((flag) => [flag, false])),
  }));
  return gaps.length > 0 ? { provenanceGaps: gaps } : {};
}
...
async function copyRegistryAsset({ source, bundleRoot, bundleId, characterId, role, copied, files, collection = "characters", kind = "approved-character-evidence" }) {
  ...
  const relativePath = `project/canvas/assets/${collection}/${safeSegment(characterId)}/${safeSegment(role)}/${actualSha.slice(0, 12)}-${extensionName}`;
  files.push(await copyEvidence(source, bundleRoot, relativePath, kind));
  const tokenPath = `${CANVAS_TOKEN}assets/${collection}/${safeSegment(characterId)}/${safeSegment(role)}/${actualSha.slice(0, 12)}-${extensionName}`;
...
/*
 * 承認済みロケーション（背景ボード）の移送。
 *
 * 公式 Job は束から作業場を作り直すので、束に入らない登録は作業場で消え、
 * 登録済みの場所でも本編の画像生成へ渡らない。人物と同じく、
 * 送り手で正本の監査をやり直し、ボードのバイト列と、審査の判断だけを
 * 残した移送用 attestation（生のパス・agent/session 識別子なし）を運ぶ。
 */
const PORTABLE_LOCATION_SNAPSHOT_KEYS = Object.freeze([
  "version",
  "locationId",
  "textPolicy",
  "reviewer",
...
const PORTABLE_LOCATION_BOARD_KEYS = Object.freeze([
  "boardId",
  "sha256",
  "width",
  "height",
...
const PORTABLE_LOCATION_ASSET_ROLE = "boards";

function locationEntryIsImported(entry) {
  return Boolean(entry && typeof entry === "object" && Object.hasOwn(entry, "import"));
}

async function auditSourceLocationForHandoff({ entry, authority, canvasDir, projectDir }) {
  const failures = [];
  const reviewPath = sourceCanvasPath(canvasDir, entry?.approval?.identityReviewPath);
  let reviewBytes = null;
  let review = null;
  if (!reviewPath) failures.push("the registered location has no review path.");
  else {
    try {
      reviewBytes = await readFile(reviewPath);
      review = JSON.parse(reviewBytes.toString("utf8"));
    } catch (error) { failures.push(`location review is unreadable (${error.message}).`); }
  }
  if (reviewBytes && createHash("sha256").update(reviewBytes).digest("hex") !== nonEmptyString(entry?.approval?.identityReviewSha256)) {
    failures.push("location review SHA-256 differs from the approved registry binding.");
  }
  let audit = null;
  if (review) {
    audit = await auditKoyaLocationReview({
      projectDir,
    ...
    if (!audit.pass) failures.push(...audit.failures.map((failure) => `current location review audit: ${failure}`));
  }
  const assets = Array.isArray(entry?.referenceAssets) ? entry.referenceAssets : [];
  const rows = audit?.rows || [];
  if (assets.length !== rows.length || rows.length === 0) failures.push("registered boards must be exactly the reviewed boards.");
  for (const [index, row] of rows.entries()) {
    const asset = assets[index];
    if (!asset || asset.id !== row.boardId || sourceCanvasPath(canvasDir, asset.path) !== row.path || asset.sha256 !== row.sha256) {
      failures.push(`registered board ${row.boardId} differs from the reviewed board.`);
    }
    if (asset && sourceCanvasPath(canvasDir, asset.sourceReviewPath) !== reviewPath) {
      failures.push(`registered board ${row.boardId} is not bound to the approved location review.`);
    }
  }
  if (failures.length > 0) {
    throw new Error(`${entry.id} source location review is not exportable:\n- ${failures.join("\n- ")}`);
  }
  return { audit, review, reviewBytes };
}

function portableLocationSnapshot({ audit, review, plan }) {
  const entryById = new Map(audit.generationEntries.map((entry) => [entry?.boardId, entry]));
  const reviewedById = new Map((review.boards || []).map((board) => [board?.boardId, board]));
  const checkKeys = koyaLocationBoardCheckKeys({ textPolicy: audit.textPolicy });
  return {
    version: nonEmptyString(review.version),
    locationId: nonEmptyString(review.locationId),
    textPolicy: audit.textPolicy,
    reviewer: {
      host: portableIdentifier("source-host-sha256", review?.reviewer?.host),
      id: portableIdentifier("source-agent-sha256", review?.reviewer?.id),
      contextId: portableIdentifier("source-context-sha256", review?.reviewer?.contextId),
      reviewedAt: nonEmptyString(review.reviewedAt),
    },
    anchorReview: {
      source: nonEmptyString(audit.anchorApproval?.source),
      sha256: nonEmptyString(audit.anchorApproval?.sha256),
      reviewerContextId: portableIdentifier("source-context-sha256", audit.anchorApproval?.reviewer?.contextId),
      reviewedAt: nonEmptyString(audit.anchorApproval?.reviewedAt),
    },
    generationManifestSha256: nonEmptyString(audit.generationManifestSha256),
    checks: {
      crossViewArchitectureContinuity: review?.checks?.crossViewArchitectureContinuity === true,
      originalScaleReview: review?.checks?.originalScaleReview === true,
    },
    boards: plan.jobs.map((job) => {
      const row = audit.rows.find((entry) => entry.boardId === job.boardId);
      const generated = entryById.get(job.boardId);
      const imported = locationEntryIsImported(generated);
      const gap = koyaLocationEntryProvenanceGap(generated);
      return {
        boardId: job.boardId,
        sha256: row.sha256,
        width: Number(row.dimensions?.width),
        height: Number(row.dimensions?.height),
        planPromptSha256: imported ? nonEmptyString(generated.import.planPromptSha256) : nonEmptyString(generated?.promptSha256),
        provenance: imported ? "external-import" : "official-generation",
        generatorContextId: portableIdentifier("source-context-sha256", generated?.generator?.contextId),
        importerContextId: imported ? portableIdentifier("source-context-sha256", generated.import?.importedBy?.contextId) : "",
        // 出所の欠落は移送先でも見える。プロンプトと会話は記録が無い、
        // 代わりに満たすべき文書がこの SHA だ、という事実だけを運ぶ。
        ...(gap ? {
          provenanceGap: {
            reason: portableIdentifier("source-reason-sha256", gap.reason),
            specificationSha256: nonEmptyString(gap.specificationSha256),
            // どの記録が無かったのかを、そのまま運ぶ。この旗が attestation に
            // 無いと、受け取り側は空の会話 id と本物の token を見分けられない。
            ...Object.fromEntries(KOYA_LOCATION_PROVENANCE_GAP_FLAGS
        ...
        checks: Object.fromEntries(
          (gap ? [...checkKeys, KOYA_LOCATION_PROVENANCE_GAP_CHECK] : checkKeys)
            .map((key) => [key, reviewedById.get(job.boardId)?.checks?.[key] === true]),
        ),
      };
    }),
...
async function writeLocationReviewAttestation({ locationId, audit, review, reviewBytes, plan, bundleRoot, files }) {
  const attestation = {
    version: KOYA_HANDOFF_LOCATION_REVIEW_ATTESTATION_VERSION,
    subjectId: locationId,
    sourceReview: {
      version: nonEmptyString(review.version),
      sha256: createHash("sha256").update(reviewBytes).digest("hex"),
    },
    snapshot: portableLocationSnapshot({ audit, review, plan }),
    note: "Portable location approval evidence. Source-machine paths, prompts and raw reviewer, generator and importer identifiers are redacted; every approved board byte stays SHA-bound by the portable registry and bundle manifest.",
  };
  const bytes = `${JSON.stringify(attestation, null, 2)}\n`;
  const actualSha = createHash("sha256").update(bytes).digest("hex");
  const relative = `assets/reviews/locations/${safeSegment(locationId)}/${actualSha.slice(0, 12)}-location-review-attestation.json`;
  const destination = resolveInside(bundleRoot, `project/canvas/${relative}`);
  await writeJsonAtomic(destination, attestation);
  files.push(await fileEvidence(bundleRoot, destination, "approved-location-review-attestation"));
  return { tokenPath: `${CANVAS_TOKEN}${relative}`, sha256: actualSha };
}

async function buildPortableLocations({ projectDir, canvasDir, bundleRoot, bundleId, authority, source, files }) {
  const declaredIds = new Set((authority.locationBible?.locations || []).map((location) => location.id));
  const copied = new Map();
  const locations = [];
  for (const entry of source.characters.filter((character) => character.kind === "location" && character.status === "approved" && declaredIds.has(character.id))) {
    const { audit, review, reviewBytes } = await auditSourceLocationForHandoff({ entry, authority, canvasDir, projectDir });
    ...
    const attestation = await writeLocationReviewAttestation({ locationId: entry.id, audit, review, reviewBytes, plan, bundleRoot, files });
    const referenceAssets = [];
    for (const asset of entry.referenceAssets) {
      const copiedAsset = await copyRegistryAsset({
        source: sourceCanvasPath(canvasDir, asset.path),
        bundleRoot,
        bundleId,
        characterId: entry.id,
        role: PORTABLE_LOCATION_ASSET_ROLE,
        copied,
        files,
        collection: "locations",
        kind: "approved-location-evidence",
      });
      referenceAssets.push({
        id: asset.id,
        role: asset.role,
        path: copiedAsset.tokenPath,
        sha256: copiedAsset.sha256,
        storyStage: "",
        sourceReviewPath: attestation.tokenPath,
      });
    }
    const approval = stripPrivateApproval(entry.approval, [KOYA_LOCATION_REVIEW_VERSION]);
    approval.identityReviewPath = attestation.tokenPath;
    approval.identityReviewSha256 = attestation.sha256;
    locations.push({
      id: entry.id,
      name: entry.name,
      kind: "location",
      ...
      referenceImagePaths: referenceAssets.map((asset) => asset.path),
      referenceAssets,
      stylePrompt: "",
      voiceId: "",
      approval,
...
function portableLocationPrivacyFailures(value, expectedCheckKeys) {
  const failures = [];
  if (!exactPortableKeys(value, PORTABLE_LOCATION_SNAPSHOT_KEYS, "location snapshot", failures)) return failures;
  exactPortableKeys(value.reviewer, ["host", "id", "contextId", "reviewedAt"], "location snapshot.reviewer", failures);
  exactPortableKeys(value.anchorReview, ["source", "sha256", "reviewerContextId", "reviewedAt"], "location snapshot.anchorReview", failures);
  exactPortableKeys(value.checks, ["crossViewArchitectureContinuity", "originalScaleReview"], "location snapshot.checks", failures);
  for (const [index, board] of (Array.isArray(value.boards) ? value.boards : []).entries()) {
    // 出所の欠落を申告したボードだけが provenanceGap と確認チェックを持つ。
    const declaresGap = Boolean(board) && typeof board === "object" && Object.hasOwn(board, "provenanceGap");
    const boardKeys = declaresGap ? [...PORTABLE_LOCATION_BOARD_KEYS, "provenanceGap"] : PORTABLE_LOCATION_BOARD_KEYS;
    if (exactPortableKeys(board, boardKeys, `location snapshot.boards[${index}]`, failures)) {
      if (declaresGap) {
        // 旗は、宣言したボードにだけ現れ、false 以外は受けない。
        // 1つも宣言していない欠落は、何も言っていないのと同じなので落とす。
        const declaredFlags = KOYA_LOCATION_PROVENANCE_GAP_FLAGS.filter((flag) => Object.hasOwn(board.provenanceGap || {}, flag));
        exactPortableKeys(
          board.provenanceGap,
          ["reason", "specificationSha256", ...declaredFlags],
          `location snapshot.boards[${index}].provenanceGap`,
          failures,
        );
        if (declaredFlags.length === 0) {
          failures.push(`location snapshot.boards[${index}].provenanceGap must say which records were never kept.`);
        }
        for (const flag of declaredFlags) {
          if (board.provenanceGap[flag] !== false) {
            failures.push(`location snapshot.boards[${index}].provenanceGap.${flag} may only be false.`);
          }
        }
      }
      exactPortableKeys(
        board.checks,
        declaresGap ? [...expectedCheckKeys, KOYA_LOCATION_PROVENANCE_GAP_CHECK] : expectedCheckKeys,
        `location snapshot.boards[${index}].checks`,
        failures,
      );
    }
  }
...
/**
 * 台帳の承認が記録した出所の欠落と、attestation の申告が一致することを見る。
 * 片方から消せるなら、欠落は運ぶうちに消えてしまう。
 */
function portableLocationGapAgreementFailures({ entry, boards }) {
  const failures = [];
  const declared = entry?.approval?.provenanceGaps;
  if (declared !== undefined && !Array.isArray(declared)) {
    return [`${entry.id} approval provenanceGaps must be a list of declared gaps.`];
  }
  // 旗は「宣言されたか」で畳んで突き合わせる。false 以外に書き換えても、
  // 宣言されていない側と形が変わるので一致しない。
  const foldedFlags = (gap) => Object.fromEntries(KOYA_LOCATION_PROVENANCE_GAP_FLAGS.map((flag) => [flag, gap?.[flag] === false]));
  const fromApproval = (Array.isArray(declared) ? declared : []).map((gap) => ({
    boardId: nonEmptyString(gap?.boardId),
    reason: nonEmptyString(gap?.reason),
    specificationSha256: nonEmptyString(gap?.specificationSha256),
    ...foldedFlags(gap),
  }));
  for (const gap of fromApproval) {
    if (!gap.boardId || !/^source-reason-sha256:[a-f0-9]{64}$/u.test(gap.reason) || !validSha256(gap.specificationSha256)) {
      failures.push(`${entry.id} approval provenance gap must name a board, a one-way reason token and the specification SHA-256.`);
    }
  }
  const fromSnapshot = boards
    .filter((board) => Object.hasOwn(board, "provenanceGap"))
    .map((board) => ({
      boardId: nonEmptyString(board.boardId),
      reason: nonEmptyString(board.provenanceGap?.reason),
      specificationSha256: nonEmptyString(board.provenanceGap?.specificationSha256),
      ...foldedFlags(board.provenanceGap),
    }));
  const byBoardId = (rows) => canonicalJson([...rows].sort((left, right) => left.boardId.localeCompare(right.boardId)));
  if (byBoardId(fromApproval) !== byBoardId(fromSnapshot)) {
    failures.push(`${entry.id} location snapshot must declare exactly the provenance gaps the approved review recorded.`);
  }
  return failures;
}

async function verifyPortableLocationEntry({ bundleRoot, entry, authority }) {
  const failures = [];
  const location = (authority.locationBible?.locations || []).find((candidate) => candidate.id === entry.id);
  if (!location) return [`${entry.id} is a location entry that the bundled location bible does not declare.`];
  if (entry.name !== location.name || entry.role !== "fixed" || entry.status !== "approved"
    || nonEmptyString(entry.episodeId) || nonEmptyString(entry.voiceId) || nonEmptyString(entry.stylePrompt)) {
    failures.push(`${entry.id} location entry must be the fixed, approved, voice-less entry named in the bundled location bible.`);
  }
  let plan;
  try {
    plan = buildKoyaLocationBoardPlan({
    ...
    return [...failures, `${entry.id} board plan cannot be rebuilt from the bundled authority (${error.message}).`];
  }
  const boardIds = plan.jobs.map((job) => job.boardId);
  const assets = entry.referenceAssets || [];
  if (canonicalJson(assets.map((asset) => asset.id)) !== canonicalJson(boardIds)
    || assets.some((asset) => asset.role !== "supplemental" || nonEmptyString(asset.storyStage))) {
    failures.push(`${entry.id} must carry exactly the planned boards (${boardIds.join(", ")}) as supplemental references.`);
  }
  if (canonicalJson(entry.referenceImagePaths) !== canonicalJson(assets.map((asset) => asset.path))) {
    failures.push(`${entry.id} referenceImagePaths must list exactly its bundled boards.`);
  }
  const approval = entry.approval || {};
  if (approval.route !== KOYA_LOCATION_REVIEW_VERSION
    || !/^source-approver-sha256:[a-f0-9]{64}$/u.test(nonEmptyString(approval.approvedBy))
    ...
    failures.push(`${entry.id} location approval metadata is missing or exposes raw approver or reason data.`);
  }
  const reviewToken = nonEmptyString(approval.identityReviewPath);
  if (!reviewToken) return [...failures, `${entry.id} is approved without a portable location review attestation.`];
  for (const asset of assets) {
    if (asset.sourceReviewPath !== reviewToken) failures.push(`${entry.id}/${asset.id} points at a different review attestation.`);
  }
  let attestation;
  try {
    const attestationPath = bundleCanvasTokenPath(bundleRoot, reviewToken);
    if (await sha256File(attestationPath) !== nonEmptyString(approval.identityReviewSha256)) {
      failures.push(`${entry.id} location review attestation SHA-256 mismatch.`);
    }
    attestation = await readJsonStrict(attestationPath);
  } catch (error) {
    return [...failures, `${entry.id} location review attestation: ${error.message}`];
  }
  if (attestation?.version !== KOYA_HANDOFF_LOCATION_REVIEW_ATTESTATION_VERSION || attestation?.subjectId !== entry.id) {
    failures.push(`${entry.id} location review attestation version or subject is invalid.`);
  }
  if (attestation?.sourceReview?.version !== KOYA_LOCATION_REVIEW_VERSION || !validSha256(attestation?.sourceReview?.sha256)) {
    failures.push(`${entry.id} location review attestation is not bound to a ${KOYA_LOCATION_REVIEW_VERSION} source review SHA-256.`);
  }
  if (containsAbsolutePath(attestation)) failures.push(`${entry.id} location review attestation leaks a source-machine absolute path.`);
  const textPolicy = koyaLocationTextPolicy(location);
  const checkKeys = koyaLocationBoardCheckKeys(location);
  const snapshot = attestation?.snapshot;
  const privacyFailures = portableLocationPrivacyFailures(snapshot, checkKeys);
  failures.push(...privacyFailures.map((failure) => `${entry.id} ${failure}`));
  if (privacyFailures.length > 0) return failures;
  if (snapshot.version !== KOYA_LOCATION_REVIEW_VERSION || snapshot.locationId !== entry.id || snapshot.textPolicy !== textPolicy) {
    failures.push(`${entry.id} location snapshot version, subject or text policy differs from the bundled location bible.`);
  }
  const contextToken = /^source-context-sha256:[a-f0-9]{64}$/u;
  if (!/^source-host-sha256:[a-f0-9]{64}$/u.test(nonEmptyString(snapshot.reviewer.host))
    || !/^source-agent-sha256:[a-f0-9]{64}$/u.test(nonEmptyString(snapshot.reviewer.id))
    || !contextToken.test(nonEmptyString(snapshot.reviewer.contextId))
    || !validIsoDate(snapshot.reviewer.reviewedAt)) {
    failures.push(`${entry.id} location reviewer provenance must use one-way tokens and an ISO review time.`);
  }
  if (!["manifest", "review"].includes(snapshot.anchorReview.source)
    || !validSha256(snapshot.anchorReview.sha256)
    || !contextToken.test(nonEmptyString(snapshot.anchorReview.reviewerContextId))
    || !validIsoDate(snapshot.anchorReview.reviewedAt)) {
    failures.push(`${entry.id} location snapshot must bind a passed, SHA-bound anchor review.`);
  }
  if (!validSha256(snapshot.generationManifestSha256)) failures.push(`${entry.id} location snapshot must bind the reviewed generation manifest SHA-256.`);
  if (snapshot.checks.crossViewArchitectureContinuity !== true || snapshot.checks.originalScaleReview !== true || snapshot.pass !== true) {
    failures.push(`${entry.id} location snapshot requires passing continuity, original-scale and overall judgments.`);
  }
  const boards = Array.isArray(snapshot.boards) ? snapshot.boards : [];
  if (canonicalJson(boards.map((board) => board.boardId)) !== canonicalJson(boardIds)) {
    failures.push(`${entry.id} location snapshot must cover the planned boards exactly once and in order.`);
  }
  if (new Set(boards.map((board) => board.sha256)).size !== boards.length) {
    failures.push(`${entry.id} location snapshot boards must be distinct images.`);
  }
  ...
  const provenanceContexts = new Set();
  for (const [index, board] of boards.entries()) {
    const job = plan.jobs[index];
    const asset = assets[index];
    const label = `${entry.id}/${board.boardId}`;
    if (!asset || board.sha256 !== asset.sha256 || !validSha256(board.sha256)) failures.push(`${label} reviewed SHA-256 differs from the bundled board.`);
    if (!job || board.planPromptSha256 !== createHash("sha256").update(job.prompt).digest("hex")) {
      failures.push(`${label} was reviewed against a different location bible or channel art style than the bundled one.`);
    }
    if (!Number.isInteger(board.width) || !Number.isInteger(board.height) || board.width < minimumWidth || board.height < minimumHeight) {
      failures.push(`${label} reviewed dimensions are below ${minimumWidth}x${minimumHeight}.`);
    }
    if (asset) {
      try {
        const dimensions = getImageDimensionsFromBuffer(await readFile(bundleCanvasTokenPath(bundleRoot, asset.path)), label);
        if (dimensions.width !== board.width || dimensions.height !== board.height) failures.push(`${label} bundled board dimensions differ from the review.`);
      } catch (error) { failures.push(`${label} bundled board is unreadable (${error.message}).`); }
    }
    const imported = board.provenance === "external-import";
    const gap = Object.hasOwn(board, "provenanceGap") ? board.provenanceGap : null;
    if (!["official-generation", "external-import"].includes(board.provenance)
      // 会話 id は旗そのもので決まる。「会話は残っていない」と申告した
      // ボードは空でなければならず、token を持っていたら矛盾。それ以外は
      // 欠落の申告があっても従来どおり token でなければならない。
      // 「欠落あり」でまとめて緩めると、記録の無い会話を後から本物らしい
      // token で騙る attestation が通ってしまう。
      || (koyaLocationGapDeclares(gap, "generatorContextRecorded")
        ? nonEmptyString(board.generatorContextId) !== ""
        : !contextToken.test(nonEmptyString(board.generatorContextId)))
      || (imported ? !contextToken.test(nonEmptyString(board.importerContextId)) : board.importerContextId !== "")) {
      failures.push(`${label} generation provenance must use one-way context tokens.`);
    }
    if (gap) {
      if (!imported) failures.push(`${label} declares a provenance gap but is not an imported board.`);
      if (!/^source-reason-sha256:[a-f0-9]{64}$/u.test(nonEmptyString(gap.reason)) || !validSha256(gap.specificationSha256)) {
        failures.push(`${label} provenance gap must carry a one-way reason token and the specification SHA-256.`);
      }
      if (board.checks[KOYA_LOCATION_PROVENANCE_GAP_CHECK] !== true) {
        failures.push(`${label} check '${KOYA_LOCATION_PROVENANCE_GAP_CHECK}' must be true.`);
      }
    }
    if (nonEmptyString(board.generatorContextId)) provenanceContexts.add(board.generatorContextId);
    if (imported) provenanceContexts.add(board.importerContextId);
    for (const key of checkKeys) {
      if (board.checks[key] !== true) failures.push(`${label} check '${key}' must be true.`);
    }
  }
  // 申告された欠落を attestation から落として運べないようにする。
  // 台帳の承認と attestation が、同じ board・同じ理由・同じ仕様 SHA を言うこと。
  failures.push(...portableLocationGapAgreementFailures({ entry, boards }));
  if (provenanceContexts.has(snapshot.reviewer.contextId)) {
    failures.push(`${entry.id} location reviewer context must differ from every generator and importer context.`);
  }
  if (provenanceContexts.has(snapshot.anchorReview.reviewerContextId)) {
    failures.push(`${entry.id} anchor reviewer context must differ from every generator and importer context.`);
  }
  return failures;
}

async function buildPortableRegistry({ projectDir, canvasDir, bundleRoot, bundleId, showBible, characterIds, files, workflowStore, identityReviewRecheckRoot, authority }) {
  ...
  // 承認済みロケーションは登録簿の同じファイルに載るので、束でも同じファイルで運ぶ。
  // 受領側の restore はこの登録簿をそのまま作業場へ書き戻す。
  const locations = await buildPortableLocations({ projectDir, canvasDir, bundleRoot, bundleId, authority, source, files });
...
export async function exportKoyaHandoffBundle(args = {}) {
  ...
  await writeFile(readmePath, [
    ...
    "承認済みロケーションの背景ボードも、location bibleが宣言する場所だけを、実ファイルと移送用review attestation付きで運びます。未承認・未宣言の場所は含みません。",
  ...
  const manifestBody = {
    ...
    createdAt: new Date().toISOString(),
    includes: {
      approvedCharacters: registry.characters.filter((entry) => entry.kind === "character").length,
      approvedLocations: registry.characters.filter((entry) => entry.kind === "location").length,
...
export async function verifyKoyaHandoffBundle(args = {}) {
  ...
  const registryEvidence = await verifyPortableRegistryEvidence(bundleRoot, authority);
  // 旧版の束には approvedLocations が無い（ロケーションを運ばなかった）。
  // 欠けている＝0件として扱い、そのまま検証できるようにする。
  if (registryEvidence.locationCount !== Number(manifest?.includes?.approvedLocations || 0)) {
    throw new Error("Koya handoff manifest does not count the approved locations carried by the portable registry.");
  }
...
async function verifyPortableRosterReviewEvidence({ bundleRoot, manifest, showBible, registry }) {
  ...
  if ((registry?.characters || []).filter((entry) => entry?.kind === "character").length !== expectedMemberCount) {
    failures.push("Portable registry must contain exactly the complete fixed-cast roster.");
  }
...
async function verifyPortableRegistryEvidence(bundleRoot, authority) {
  ...
  const locationEntries = registry.characters.filter((entry) => entry.kind === "location");
  for (const entry of registry.characters) {
    if (!["character", "location"].includes(entry.kind)) {
      failures.push(`${entry.id} has kind '${entry.kind}'; a handoff bundle carries only fixed-cast characters and approved locations.`);
    }
    for (const asset of entry.referenceAssets || []) {
      try {
        const path = bundleCanvasTokenPath(bundleRoot, asset.path);
        if (await sha256File(path) !== nonEmptyString(asset.sha256)) failures.push(`${entry.id}/${asset.id || asset.role} asset SHA-256 mismatch.`);
      } catch (error) { failures.push(`${entry.id}/${asset.id || asset.role}: ${error.message}`); }
    }
  }
  for (const entry of locationEntries) {
    failures.push(...await verifyPortableLocationEntry({ bundleRoot, entry, authority }));
  }
  for (const character of registry.characters.filter((entry) => entry.kind === "character")) {
  ...
  return {
    pass: true,
    characterCount: registry.characters.filter((entry) => entry.kind === "character").length,
...
export async function restoreKoyaHandoffBundle(args = {}) {
  ...
  try {
    ...
    return {
      ...
      restoredCharacters: incomingRegistry.characters.filter((entry) => entry.kind === "character").length,
      restoredLocations: incomingRegistry.characters.filter((entry) => entry.kind === "location").length,
