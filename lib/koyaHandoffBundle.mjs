...
async function stylingSpecPaths(projectDir, showBible) {
  ...
  const directory = join(projectDir, "config", "koya-character-styling");
...
export async function restoreKoyaHandoffBundle(args = {}) {
  ...
  for (const relativePath of [...PROJECT_CONFIG_PATHS, ...await stylingSpecPaths(join(bundleRoot, "project"), await readJsonStrict(join(bundleRoot, "project", "config", "koya-show-bible.json")))]) {
