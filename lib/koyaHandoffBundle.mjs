...
function portableProvenanceGaps(value) {
  const gaps = (Array.isArray(value) ? value : []).map((gap) => ({
    ...
    ...(gap?.referenceImagesRecorded === false ? { referenceImagesRecorded: false } : {}),
  }));
      ...
      return {
        ...
        ...(gap ? {
          provenanceGap: {
            reason: portableIdentifier("source-reason-sha256", gap.reason),
            specificationSha256: nonEmptyString(gap.specificationSha256),
            // 参照画像の記録が無かったという事実も、そのまま運ぶ。
            ...(koyaLocationGapDeclares(gap, "referenceImagesRecorded") ? { referenceImagesRecorded: false } : {}),
          },
        } : {}),
...
function portableLocationPrivacyFailures(value, expectedCheckKeys) {
  ...
  for (const [index, board] of (Array.isArray(value.boards) ? value.boards : []).entries()) {
    ...
    if (exactPortableKeys(board, boardKeys, `location snapshot.boards[${index}]`, failures)) {
      if (declaresGap) {
        // 参照画像の旗は、宣言したボードにだけ現れ、false 以外は受けない。
        const declaresReferenceGap = Object.hasOwn(board.provenanceGap || {}, "referenceImagesRecorded");
        exactPortableKeys(
          board.provenanceGap,
          declaresReferenceGap ? ["reason", "specificationSha256", "referenceImagesRecorded"] : ["reason", "specificationSha256"],
          `location snapshot.boards[${index}].provenanceGap`,
          failures,
        );
        if (declaresReferenceGap && board.provenanceGap.referenceImagesRecorded !== false) {
          failures.push(`location snapshot.boards[${index}].provenanceGap.referenceImagesRecorded may only be false.`);
        }
      }
      exactPortableKeys(
        board.checks,
...
function portableLocationGapAgreementFailures({ entry, boards }) {
  ...
  const fromApproval = (Array.isArray(declared) ? declared : []).map((gap) => ({
    ...
    referencesUnrecorded: gap?.referenceImagesRecorded === false,
  }));
  ...
  const fromSnapshot = boards
    ...
    .map((board) => ({
      ...
      referencesUnrecorded: board.provenanceGap?.referenceImagesRecorded === false,
    }));
...
async function verifyPortableLocationEntry({ bundleRoot, entry, authority }) {
  ...
  for (const [index, board] of boards.entries()) {
    ...
    if (!["official-generation", "external-import"].includes(board.provenance)
      // 旗はボードごとに独立なので、欠落を申告したボードの会話 id は、
      // 会話まで残っていなければ空、プロンプトや参照だけが欠けているなら
      // token になる。申告の無いボードは従来どおり token でなければならない。
      || (gap
        ? (nonEmptyString(board.generatorContextId) !== "" && !contextToken.test(nonEmptyString(board.generatorContextId)))
