...
async function auditSourceReviewGrid({ section, label, columns, rows, ids, booleanKeys, failures }) {
  ...
  for (const [index, id] of ids.entries()) {
    ...
    if (canonicalJson(cell?.sourceBounds) !== canonicalJson([left, top, right - left, bottom - top])) {
      failures.push(`${label}.${id}.sourceBounds does not match its required parent-sheet cell.`);
    }
    if (Number(cell?.width) !== right - left || Number(cell?.height) !== bottom - top) {
...
async function auditSourceIdentityReviewForHandoff({ character, showMember, canvasDir, workflowStore, machineRecheckPath }) {
  ...
  if (eyeAssets.length > 1 || extraReviews.length !== eyeAssets.length || extraReviews.some((row) => row?.role !== "eye-open")) {
    failures.push("extraSheets must exactly cover the optional single registered eye-open asset.");
  }
  if (eyeAssets[0]) {
    const row = extraReviews[0];
    await auditSourceReviewAsset(row, eyeAssets[0], "extraSheets.eye-open", failures);
    if (row?.sameIdentity !== true || row?.pass !== true || nonEmptyString(row?.note).length < 4) {
      failures.push("extraSheets.eye-open requires identity/pass judgments and a visual note.");
    }
    await auditSourceReviewGrid({
      section: row,
      label: "extraSheets.eye-open",
      columns: 2,
      rows: 2,
...
async function verifyPortableRegistryEvidence(bundleRoot) {
  ...
  for (const voice of registry.voices) {
    if (nonEmptyString(voice.previewUrl) || (voice.casting && Object.keys(voice.casting).length > 0)
