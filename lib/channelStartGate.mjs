...
export async function loadRuntimeChannelRegistry({ env = process.env, deploymentPath = "", harnessIds = null } = {}) {
  ...
  // 台帳の台本のジャンルは、制作の Job が有料の処理の前に問うジャンル（lib/scriptQualityUseGate.mjs）とそろえる。
  const { scriptQualityGenreForHarness } = await import("./scriptQualityUseGate.mjs");
  return loadChannelRegistry({
    env,
    deploymentPath,
    harnessIds: ids,
    genres: Object.keys(SCRIPT_QUALITY_GENRES),
    genreForHarness: scriptQualityGenreForHarness,
  });
}
