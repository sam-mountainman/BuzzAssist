 ...
 * 人物の映像の音は使わない（感想の語りは声のテイク。Pack が bookends.review.voice: none を宣言した感想パートは
 * 語りも無く、字幕と BGM だけ）。配置の区間の長さは文の長さから決まり、声の有無に依らない（字幕だけの文は
 * 字幕の秒数の長さ）。
 */
...
/**
 * 固定の重ね物（lib/narratedStoryOverlays.mjs）など、配置と関係なく出る範囲（描く寸法の px）を、測る画の上の
 * mask（1 = 測らない）にする。
 */
function layoutExclusionMask(rects, render, width, height) {
  const list = (rects || []).filter((rect) => rect && rect.width > 0 && rect.height > 0);
  if (list.length === 0) return null;
  const mask = new Uint8Array(width * height);
  for (const rect of list) {
    const left = Math.max(0, Math.floor((rect.x * width) / render.width) - 1);
    const top = Math.max(0, Math.floor((rect.y * height) / render.height) - 1);
    const right = Math.min(width - 1, Math.ceil(((rect.x + rect.width) * width) / render.width) + 1);
    const bottom = Math.min(height - 1, Math.ceil(((rect.y + rect.height) * height) / render.height) + 1);
    for (let y = top; y <= bottom; y += 1) for (let x = left; x <= right; x += 1) mask[y * width + x] = 1;
  }
  return mask;
}

/** 2枚の輝度の画の、矩形の中の平均絶対差（expectedRect は期待の画の中の同じ大きさの矩形。mask の画素は測らない）。 */
function regionDiff(frame, frameWidth, rect, expected, expectedWidth, expectedRect = rect, mask = null) {
  let sum = 0;
  let count = 0;
  for (let y = 0; y < rect.height; y += 1) {
    for (let x = 0; x < rect.width; x += 1) {
      if (mask?.[(rect.y + y) * frameWidth + rect.x + x]) continue;
...
function flatDiff(frame, frameWidth, rect, luma, mask = null) {
  let sum = 0;
  let count = 0;
  for (let y = 0; y < rect.height; y += 1) {
    for (let x = 0; x < rect.width; x += 1) {
 ...
 */
export async function measureNarratedReviewLayout({ ffmpeg, videoPath, sections, layout, render, reviewStartFrame, presenterPath = "", presenterBackground = "", tvVideoPath = "", exclude = [] }) {
  ...
  // 固定の重ね物の範囲は測らない（重ね物は枠と重ならない置き場所しか宣言できないが、背景と全面の人物の映像には重なりうる）。
  const mask = layoutExclusionMask(exclude, render, width, height);
  ...
  for (const section of sections) {
    ...
    for (const local of samples) {
      ...
      if (section.layout === "tv-left-presenter-right") {
        ...
        if (background) entry.backgroundDiff = flatDiff(frame, width, background, lumaOfHex(layout.backgroundColor), mask);
        ...
        entry.presenterDiff = expected ? regionDiff(frame, width, { x: 0, y: 0, width, height }, expected, width, undefined, mask) : 255;
