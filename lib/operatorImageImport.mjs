 ...
 *       "cameraFocus": { "x": 0.32, "y": 0.41 }          // この画の中の焦点（顔の位置など。任意、下）
 *     }]
 *   }
 *
 * パスは manifest のあるフォルダからの相対（区切りは "/"）。フォルダの外・絶対パス・"\\"・".." は受けない。
 *
 * cameraFocus は、その場面の画の中で寄り引きの焦点にする位置（画の幅・高さに対する割合、左上が 0・右下が 1）。
 * 台本パッケージの本編の場面の cameraFocus と同じ形・同じ検査（lib/narratedStoryCamera.mjs の
 * normalizeNarratedCameraFocus）で、形の誤りは operator-image-manifest-invalid:scenes[i].cameraFocus.* で止める。
 * 焦点は画に属する値で、画ができるまで決められない。台本パッケージに後から書き足すと台本のバイト列が変わり、
 * 台本の品質ループの合格が外れるので、画の後で決める焦点はここに書く。使い方はジャンルが決める（ナレーション
 * 物語は「取り込みの記録 → 台本パッケージ → Pack の型ごとの焦点」の順。両方に書いて値が違えば止める）。
 * 焦点は書いた値だけを使い、画から顔を推測しない。記録の本文の sha256（manifestSha256・digest）に入るので、
 * 焦点を直せば入力の指紋が変わり、同じ Job の resume で描き直す（画の取り込み直しの鍵 importKey は変わらない）。
 *
 * 守ること:
...
// 焦点の形の検査は台本パッケージと同じ1か所（依存は node の組み込みだけの純粋な検査）。
import { normalizeNarratedCameraFocus } from "./narratedStoryCamera.mjs";
...
export const OPERATOR_IMAGE_MANIFEST_FIELDS = Object.freeze({
  ...
  scene: Object.freeze([
    ...
    "generatedAt", "conversationUrl", "reuseReason", "assetLoop", "cameraFocus",
  ]),
...
export async function readOperatorImageManifest({ manifestPath, now = new Date() } = {}) {
  ...
  for (const [index, entry] of scenes.entries()) {
    ...
    // この画の中の焦点（任意。台本パッケージの cameraFocus と同じ形・同じ検査）。
    const focus = normalizeNarratedCameraFocus(entry.cameraFocus);
    row.cameraFocus = focus.focus;
    if (focus.problem) problems.push(`operator-image-manifest-invalid:${at}.${focus.problem}`);
    rows.push(row);
  }
...
function publicSceneRow(row, { fit, referenceApprovals, reuseOf }) {
  return {
    ...
    // 画の中の焦点（数だけ。書かれていない行には欄を足さない）。
    ...(row.cameraFocus ? { cameraFocus: { x: row.cameraFocus.x, y: row.cameraFocus.y } } : {}),
    fit: {
      method: fit.method,
      source: fit.source,
      target: fit.target,
