...
/** 画の寸法（PNG / JPEG / WebP の見出しだけを読む。寸法の読み方は lib/canvasScene.mjs の1か所）。 */
...
function normalizedTimestamp(value, now) {
  const text = nonEmpty(value);
...
function conversationUrlOf(value) {
 ...
 * 状態ファイルの置き場は本体の規則（<作業フォルダ>/quality/assets/<工程>--<対象 id>.json）で、そこから
 * 作業フォルダと対象 id を戻し、本体の assetQualityPaths で同じ置き場になることを確かめてから問い合わせる
 * （置き場を推測で読まない）。合格は「評価者の採点と要る人の確認が揃い、取り込む画のファイルが合格した版
 * そのもの」のときだけ（assetPath に取り込む画を渡して、本体に照合させる）。
 */
function assetQualityStatusVerifier(module) {
  return async ({ statePath, stage, imagePath, imageSha256 }) => {
    ...
    const prefix = `${stage}--`;
    if (!base.startsWith(prefix)) return { pass: false, detail: "asset-loop-state-not-this-stage" };
    ...
    if (!expected || path.resolve(expected) !== absolute) return { pass: false, detail: "asset-loop-state-path-layout" };
