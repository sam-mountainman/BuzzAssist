...
/** 品質ループ本体（別の担当が作る）に要る検査の関数名。無ければ requireAssetLoopPass は止める。 */
 ...
 * 本体は `verifyAssetLoopPassForImport({ statePath, stage, subjectId, imageSha256, passedSha256 })` →
 * `{ pass, passedSha256, detail }` を export すれば、ここから使われる。
 */
export async function loadDefaultAssetLoopVerifier({ importModule = (specifier) => import(specifier) } = {}) {
  ...
  if (typeof module?.[OPERATOR_IMAGE_ASSET_LOOP_VERIFIER] !== "function") return { available: false, reason: "verifier-missing" };
  return { available: true, verify: (args) => module[OPERATOR_IMAGE_ASSET_LOOP_VERIFIER](args) };
}
