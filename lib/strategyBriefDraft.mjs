...
function makeIdFactory(taken) {
  return (prefix, source) => {
    ...
    let base = (body === prefix || body.startsWith(`${prefix}-`) ? body : `${prefix}-${body}`).slice(0, 44).replace(/-+$/u, "");
    if (!ITEM_ID.test(base)) base = `${prefix}-${sha256Hex(Buffer.from(String(source ?? ""), "utf8")).slice(0, 8)}`;
    let id = base;
    let index = 2;
    while (taken.has(id)) {
      id = `${base}-${index}`;
...
export async function draftStrategyBriefFromTool({
  ...
  if (previous) {
    for (const row of previous.brief.evidence) {
  ...
  // 根拠の表（evidence.csv）。行ごとに根拠の行を作り、観測の列は observations[] にも入れる。
  ...
  const observations = [];
  let evidenceCsv = { provided: false };
  ...
  if (csvFile?.isFile) {
    ...
    evidenceCsv = { provided: true, path: csvFile.rel, sha256: csvSha256, rows: 0, used: 0, skipped: [] };
    if (bytes.length > MAX_CSV_BYTES) issues.push("strategy-draft-csv-too-large");
    else {
      let rows = [];
      try {
      ...
      if (rows.length > 0 && column("evidence_id") < 0) issues.push("strategy-draft-csv-evidence-id-column-missing");
      else {
        const body = rows.slice(1);
        evidenceCsv.rows = body.length;
        ...
        for (const [index, cells] of body.slice(0, MAX_CSV_ROWS).entries()) {
          ...
          if (!sourceId) {
            evidenceCsv.skipped.push({ line: index + 2, reason: "evidence-id-missing" });
            continue;
          }
          ...
          const id = newId("csv", sourceId);
          const limitations = cell("limitations");
          evidence.push({
            id,
            kind: "other",
            ...
            note: clip(`evidence.csv の ${sourceId}${cell("source_type") ? `（${cell("source_type")}）` : ""}${limitations ? `。限界: ${limitations}` : ""}`, 300),
          });
          evidenceCsv.used += 1;
          const statement = cell("observation");
          if (Array.from(statement).length >= 4 && observations.length < MAX_OBSERVATIONS) {
            observations.push({ id: newId("obs", sourceId), statement: clip(statement, 300), evidenceIds: [id] });
          }
        }
      }
    }
