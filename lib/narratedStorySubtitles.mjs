 ...
 * 実測（MP4 の輝度で）: 基準版 1.00（縁・帯とも。原色の帯が並ぶ画の上でも 1.00）、焼き込みを抜いた版
 * 0.02〜0.18、3px ずらした版 0.17〜0.40、12px ずらした版 0.11〜0.25。
 */
 ...
 * 実測: 基準版で頁の始まりの1つ前のフレーム 0.02〜0.42（前の頁の字の上で高め。字面が近い頁が続くときは
 * 違う画素だけで測る）、終わりの次のフレーム 0.04〜0.19、1フレーム早く出した版 1.00。
 */
export const SUBTITLE_ABSENT_MATCH_MAX = 0.5;
/**
 * 読める明るさの差: MP4 で測った字の芯の平均輝度と、縁（または帯）の平均輝度の差の下限。
 * 実測: 白い字に黒い縁 252、帯に濃い字 161〜166、字を縁と同じ暗さにした版 14.5。
 */
...
export function videoConcatAssembled(graph, { count, label }) {
  ...
  // 固定の重ね物（lib/narratedStoryOverlays.mjs の fixedOverlayGraph）は、concat の出力 [label] の上に宣言の数だけ
  // 1段ずつ重ね、最後の段が [<label>fixed] を出す。その上に字幕の層を重ねる（字幕が無ければ yuv420p にして [video]）。
  const fixedStages = (text.match(/\[fixedoverlay\d+\]overlay=/gu) || []).length;
  if (fixedStages === 0 && text.includes(`concat=n=${count}:v=1:a=0[video]`)) return !/subtitlelayer/u.test(text);
  if (!text.includes(`concat=n=${count}:v=1:a=0[${label}]`)) return false;
  let current = label;
  if (fixedStages > 0) {
    if (!text.includes(`[${label}][fixedoverlay1]overlay=`)) return false;
    for (let stage = 1; stage <= fixedStages; stage += 1) {
      const input = stage === 1 ? label : `${label}fixed${stage - 1}`;
      const output = stage === fixedStages ? `${label}fixed` : `${label}fixed${stage}`;
      if (!new RegExp(`\\[${input}\\]\\[fixedoverlay${stage}\\]overlay=[^;\\[]*\\[${output}\\]`, "u").test(text)) return false;
    }
    current = `${label}fixed`;
  }
  const overlays = text.match(/\[subtitlelayer\]overlay=/gu) || [];
  if (overlays.length === 0) return fixedStages > 0 && text.endsWith(`[${current}]format=yuv420p[video]`);
  return overlays.length === 1
    && text.includes(`[${current}][subtitlelayer]overlay=`)
...
/**
 * MP4 のフレームの輝度（Y）を切り取って読む。RGB へ戻してから輝度を計算すると、4:2:0 の色の間引きで
 * 鮮やかな背景の色が白い字の画素へにじみ、RGB が飽和して輝度が下がる（2026-09-25 実測: 原色の帯の上の
 * 白い字で一致率が 0.58 まで落ちた）。字幕の判定は輝度だけで行う。
 */
async function extractLumaCrop(ffmpeg, videoPath, frame, fps, box, count = 1) {
  const { stdout } = await runRuntime(ffmpeg, [
    "-hide_banner", "-loglevel", "error", "-ss", Math.max(0, (frame - 0.25) / fps).toFixed(6), "-i", videoPath,
    "-frames:v", String(count), "-vf", `crop=${box.width}:${box.height}:${box.x}:${box.y},format=gray`, "-f", "rawvideo", "-",
  ], { encoding: "buffer" });
...
/**
 * MP4 の1フレーム（crop 済みの輝度）に、頁の PNG の分け方を当てて測る（純粋関数）。
...
export function measureCueFrame(frame, box, classified, frameSize, config) {
  ...
  for (let y = 0; y < box.height; y += 1) {
    for (let x = 0; x < box.width; x += 1) {
      ...
      const luma = frame[y * box.width + x];
...
export async function measureNarratedBurnedSubtitles({ ffmpeg, videoPath, overlay, config, fps, concurrency = 4 }) {
  ...
  const rows = await mapLimit(cues, concurrency, async (cue, index) => {
    ...
    const [previousImage, startImage] = cue.startFrame > 0
      ? await extractLumaCrop(ffmpeg, videoPath, cue.startFrame - 1, fps, box, 2)
      : [null, ...await extractLumaCrop(ffmpeg, videoPath, 0, fps, box, 1)];
    ...
    if (!covered.has(cue.endFrame) && cue.endFrame < overlay.totalFrames) {
      ...
      const [frame] = await extractLumaCrop(ffmpeg, videoPath, cue.endFrame, fps, endBox, 1);
