 ...
 *
 *   - 動画クリップ（video-clip。監査契約 v7 から）: 回ごとの運営者の動画（OP 映像・感想パートの人物の映像。
 *     lib/operatorVideoImport.mjs）が、取り込みの記録の assetLoop.statePath が指す作業フォルダ（記録のフォルダ）の
 *     工程 video-clip のループで合格していること、描く写しの sha256 が合格した版と同じであること。形式・全フレームの
 *     デコード・尺・fps・解像度・音声の有無はループの機械ゲート（測定は lib/videoClipMeasurement.mjs）が見る
 *
 ...
 *   動画クリップは、取り込んだ動画が無い回（Pack が回ごとの動画の枠を宣言していない・任意の枠に渡していない）は
 *   見る物が無いのが正しいので合格にする（本編の画・声と違い、動画は任意の成果物）。
 */
...
/**
 * 回ごとの運営者の動画（OP 映像・感想パートの人物の映像）の品質ループ（工程 video-clip）の監査。監査契約 v7 から
 * （ハーネス宣言の保証 operator-video-asset-loop の inForceSince と同じ）。v6 以前の state は、当時無かったこの監査を
 * 求めずに確定する（NARRATED_ASSET_LOOP_AUDIT_IDS には入れない）。
 */
export const NARRATED_VIDEO_CLIP_LOOP_AUDIT_ID = "operatorVideoAssetLoopPassed";
export const NARRATED_VIDEO_CLIP_LOOP_SINCE = "buzzassist-narrated-story-audit-v7";
/**
...
export function narratedAssetLoopsRequired(contractVersion) {
  return requiredSince(NARRATED_ASSET_LOOP_SINCE, contractVersion);
}

/** 動画クリップの関門（監査契約 v7 から）が要るか。読めない・系列が違うときは「要る」に倒す。 */
export function narratedVideoClipLoopsRequired(contractVersion) {
  return requiredSince(NARRATED_VIDEO_CLIP_LOOP_SINCE, contractVersion);
}

function requiredSince(sinceVersion, contractVersion) {
  const since = CONTRACT.exec(sinceVersion).groups;
...
/**
 * 回ごとの運営者の動画（取り込みの記録の1行）の、動画クリップの工程（video-clip）の対象。
 *   assetPath: 照合するファイル（Job の作業フォルダへ写した描く写し。plan-only では記録のフォルダの元の動画）
 *   sourcePath: 記録のフォルダの元の動画（評価者と運営者が --asset に渡す値の元）
 *   assetSha256: 取り込みの記録が実測した sha256
 *   assetLoopStatePath: 記録の assetLoop.statePath（無ければ record-required）
 *   declaration: 枠の決まりから作った測定の宣言（運営者が measure-video に渡す見本。関門の判定には使わない）
 */
export function operatorVideoLoopSubject(slot, { assetPath = "", sourcePath = "", assetSha256 = "", assetLoopStatePath = "", declaration = null } = {}) {
  const base = {
    slot,
    source: "operator-video",
    assetPath,
    sourcePath: sourcePath || assetPath,
    assetSha256: lowerSha(assetSha256),
    ...(declaration ? { declaration } : {}),
  };
  if (!assetLoopStatePath) return { ...base, recordProblem: "record-required" };
  return { ...base, assetLoopStatePath };
}

async function videoVerdict(video) {
  let location = { workDir: "", subjectId: "", problem: "" };
  if (video.assetLoopStatePath) {
    const { ASSET_QUALITY_DIR, assetQualityPaths } = await loadAssetLoopCore();
    const found = assetLoopStateLocation(video.assetLoopStatePath, "video-clip", { ASSET_QUALITY_DIR, assetQualityPaths });
    location = found.ok ? { workDir: found.workDir, subjectId: found.subjectId, problem: "" } : { workDir: "", subjectId: "", problem: found.detail };
  }
  const base = {
    slot: video.slot,
    source: video.source || "operator-video",
    workDir: location.workDir,
    subjectId: location.subjectId,
    assetPath: video.assetPath || "",
    sourcePath: video.sourcePath || video.assetPath || "",
    assetSha256: lowerSha(video.assetSha256),
    assetLoopStatePath: video.assetLoopStatePath || "",
    ...(video.declaration ? { declaration: video.declaration } : {}),
    pass: false,
    reason: "",
  };
  if (video.recordProblem) return { ...base, reason: video.recordProblem };
  ...
  // 取り込みの記録が実測した sha256 と今の写しが違えば、合格の照合より先に止める。
  if (base.assetSha256 && base.assetSha256 !== currentSha) return { ...base, reason: "sha256-mismatch" };
  const status = await loopStatus({ workDir: base.workDir, stage: "video-clip", subjectId: base.subjectId, assetPath: base.assetPath });
  const passedSha = lowerSha(status.check?.assetSha256);
  if (status.pass === true && passedSha === currentSha) return { ...base, assetSha256: currentSha, pass: true, rounds: Number(status.check?.rounds) || 0 };
 ...
 *   videos: [operatorVideoLoopSubject の戻り値]（監査契約 v7 から。null なら見ない、[] は取り込んだ動画が無い回で合格）
 */
export async function gateNarratedAssetLoops({
  ...
  if (Array.isArray(videos)) {
    const videoRows = [];
    for (const video of videos) videoRows.push(await videoVerdict(video));
    plan.videos = videoRows.map((row) => ({
      slot: row.slot,
      source: row.source,
      assetPath: row.assetPath,
      sourcePath: row.sourcePath,
      assetSha256: row.assetSha256,
      assetLoopStatePath: row.assetLoopStatePath,
      ...(row.declaration ? { declaration: row.declaration } : {}),
    }));
    for (const row of videoRows) {
      if (row.pass) continue;
      issues.push(`video-clip-asset-loop-not-passed:${row.slot}:${row.reason}`);
      pending.push({
        stage: "video-clip",
        subjectId: row.subjectId || row.slot,
        slot: row.slot,
        source: row.source,
        workDir: row.workDir,
        // 評価者と運営者が --asset に渡すのは、記録のフォルダの元の動画（Job の写しと同じバイト列）。
        assetPath: displayPath(row.workDir, row.sourcePath),
        assetSha256: row.assetSha256,
        reason: row.reason,
        ...(row.declaration ? { declaration: row.declaration } : {}),
      });
    }
    const failed = videoRows.filter((row) => !row.pass);
    // 取り込んだ動画が無い回（ほとんどの回）は、数を載せない（生成記録の形を変えない）。
    if (videoRows.length > 0) counts.videoClips = { passed: videoRows.length - failed.length, total: videoRows.length };
    checks[NARRATED_VIDEO_CLIP_LOOP_AUDIT_ID] = check(
      failed.length === 0,
      videoRows.length === 0
        ? "no per-episode operator video was imported (nothing to check)"
        : (failed.length === 0
          ? `${videoRows.length} per-episode operator video clip(s) passed the video-clip asset quality loop (ffprobe/ffmpeg measurement gates, independent evaluator, required human checks, rendered copy SHA-256 = passed version)`
          : `video-clip asset quality loop not passed for ${failed.length}/${videoRows.length}: ${failed.map((row) => `${row.slot}(${row.reason})`).join(", ")}`),
      {
        contractVersion,
        clips: videoRows.map((row) => ({ slot: row.slot, pass: row.pass, reason: row.reason, assetSha256: row.assetSha256 || null })),
      },
    );
  }
    ...
    summary: {
      ...
      detail: pass
        ? "every scene image, character reference and adopted voice take passed its asset quality loop"
          + `${Array.isArray(videos) && videos.length > 0 ? " (and every per-episode operator video clip)" : ""}`
        : `途中の成果物の品質ループの合格が要る（${pending.length} 件）。${NARRATED_ASSET_LOOP_CLI} start / sheet / record / verify で`
          ...
          + " 同じ工程の対象が多いとき（声のテイク・本編の画）は sheet --batch / record --batch で1つの評価文脈がまとめて採点できる"
          + "（1回の上限あり。人の確認は対象ごと）。"
          + `${pending.some((row) => row.stage === "video-clip")
            ? " 運営者の動画は、取り込みの記録のフォルダで measure-video（pending の declaration を宣言にする）→ 工程 video-clip のループを回し、記録の assetLoop に状態の置き場と合格した版の sha256 を書く。"
            : ""}`
...
export async function reverifyNarratedAssetLoops({ runDir, plan, generatorContextId = "" } = {}) {
  // 監査契約 v7 から、plan は描いた運営者の動画（videos。取り込んだ動画が無い回は []）も持つ。v6 以前の plan は
  // 持たないので、動画の関門は当時の契約のまま求めない。
  const videosRequired = narratedVideoClipLoopsRequired(plan?.contractVersion);
  if (!plan || plan.version !== NARRATED_ASSET_LOOP_GATE_VERSION || !Array.isArray(plan.scenes) || !Array.isArray(plan.voices)
    || plan.scenes.length === 0 || plan.voices.length === 0 || (videosRequired && !Array.isArray(plan.videos))) {
    ...
    const ids = [...NARRATED_ASSET_LOOP_AUDIT_IDS, ...(videosRequired ? [NARRATED_VIDEO_CLIP_LOOP_AUDIT_ID] : [])];
    return {
      pass: false,
      issues: ["asset-quality-loop-plan-missing"],
      checks: Object.fromEntries(ids.map((id) => [id, check(false, detail)])),
  ...
  return gateNarratedAssetLoops({
    ...
    scenes: plan.scenes,
    voices: plan.voices,
    videos: videosRequired ? plan.videos : null,
  });
}
