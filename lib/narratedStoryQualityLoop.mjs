...
export function narratedQualityReviewSheet(contract, runDir) {
  ...
  return {
    ...
    revisionDeltaPath: paths.revisionDeltaPath,
  };
}
...
export async function advanceNarratedQualityLoop({
  ...
  const failedGateIds = Object.entries(auditChecks)
    .filter(([id, value]) => id !== NARRATED_QUALITY_AUDIT_ID && value?.pass !== true)
    .map(([id]) => id)
    .sort();
