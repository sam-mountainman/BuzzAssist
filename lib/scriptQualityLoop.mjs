 ...
 *   "acceptance": { "mode": "average", "evaluators": ["<評価者 id>", ...] }      // 任意。無ければ今までどおり
 * }
 *
 * acceptance を書いたチャンネルだけ、1つの版を「宣言した評価者の全員」で採点する（評価の組）。
 * 書かなければ組は1人で、今までの1件ずつの記録と同じに動く。既定をどのチャンネルにも強制しない。
 */
...
function normalizeAcceptanceConfig(value, blockers) {
  ...
  for (const key of Object.keys(value)) {
    if (!["mode", "evaluators"].includes(key)) blockers.push(`script-quality.acceptance.${key}-unknown`);
  }
  const mode = value.mode === undefined ? "average" : value.mode;
  if (!QUALITY_ACCEPTANCE_MODES.includes(mode)) blockers.push("script-quality.acceptance.mode");
  const evaluators = [];
