...
// 解説動画のハーネスはまだ無く、ジャンル固有の正本も無い。基準はリポジトリの一般原則から引いた:
...
// 下限は方針値: 根拠の範囲は誤った主張が致命傷になるので 85、冒頭の約束の回収は 80、ほかは足切りとして 60〜70。
 ...
 * harnessId は、そのジャンルの署名済み Channel Pack を検証するときに照らすハーネス。まだハーネスの無い
 * ジャンル（解説動画）は null で、その Pack の script-quality.json には genre の明記を求める。
 */
...
export async function recordScriptQualityRound({
  ...
  return withCanvasFileLock(paths.statePath, async () => {
    ...
    if ((existing.rounds || []).length > 0) {
      ...
      if (!source && nonEmpty(findingDispositionsPath)) {
        const file = insideWorkDir(paths.workDir, findingDispositionsPath, "--finding-dispositions");
        try {
          source = JSON.parse(await readFile(file.full, "utf8"));
        } catch {
    ...
    const panel = {
      ...
      reviews: [panelReviewEntry({ existing, contract, review, reviewFile, reviewSha256 })],
    };
...
function panelReviewEntry({ existing, contract, review, reviewFile, reviewSha256 }) {
  return {
...
async function joinPanel({
  ...
  if (rejected) return rejected;
  const panel = { ...pending, reviews: [...pending.reviews, panelReviewEntry({ existing, contract, review, reviewFile, reviewSha256 })] };
  return settlePanel({ existing, contract, paths, panel, now, captureLearning });
}
 ...
 */
export async function scriptQualityVerdict({ workDir, scriptPath = "", scriptSha256 = "" } = {}) {
  const paths = scriptQualityPaths(workDir);
