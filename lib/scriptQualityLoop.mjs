 ...
 *   - 続いているループを採点なしで止められるのは、理由と人の確認つきの stop だけ（評価項目の改定を決めたとき
 *     など）。回も費用も時間も数えず、開いている評価の組は閉じずに破棄した記録を残す。止めても合格にはならない
...
/**
 * 評価者へ渡す採点表。評価項目の id・名前・説明・0〜100 の尺度・契約の digest だけを載せ、
 * 合格点・項目ごとの重み・下限は載せない（見ると採点がそれに寄る。合格点のすぐ上に集まる）。
 * 途中の成果物・企画ブリーフ・完成動画の評価シートと同じ決まり（platform-craft の references/quality-loops-ja.md）。
 * 2026-09-27 まで台本のシートだけが合格点・重み・下限を載せていた。
 */
...
/**
 * 運営者が契約を見るための表（`contract` のコマンド）。合格点・重み・下限まで載せる。
 * 評価者には渡さない（評価者へは scriptQualityReviewSheet）。
 */
export function scriptQualityContractSummary(contract) {
  return {
    contractVersion: contract.version,
    contractDigest: contract.digest,
    genre: contract.genre,
    ...
    commonPerspectives: SCRIPT_QUALITY_GENRES[contract.genre]
      ? scriptQualityPerspectiveCoverage(contract.genre).perspectives
      : [],
    note: "運営者向けの表。評価者には sheet の出力（合格点・重み・下限を載せない）だけを渡す",
  };
}
...
function checkFor(state, extra = {}) {
  ...
  return {
    ...
    // 人が採点なしで止めたループ: 止めた人・理由・時刻と、そのとき閉じずに破棄した評価の組。
    ...(state?.humanStop ? { humanStop: { ...state.humanStop } } : {}),
    ...(state?.script?.discardedPanel ? { discardedPanel: { ...state.script.discardedPanel } } : {}),
...
/** 止めたときに開いていた評価の組の記録（閉じずに破棄した。採点のファイルは消さない）。 */
function discardedPanelRecord(panel, discardedAt) {
  return {
    discardedAt,
    reason: HUMAN_STOP_REASON,
    versionLabel: panel.versionLabel,
    stage: panel.stage,
    scriptPath: panel.scriptPath,
    scriptSha256: panel.scriptSha256,
    declaredEvaluators: [...(panel.declaredEvaluators || [])],
    missing: panelMissing(panel),
    // 組に入っていた採点（ファイルと SHA・評価者・文脈だけ。点数と所見は採点ファイルに残っている）。
    reviews: (panel.reviews || []).map((row) => ({
      evaluatorId: row.evaluatorId,
      evaluatorContextId: row.evaluatorContextId,
      reviewPath: row.reviewPath,
    ...
    // 組を開いた record の費用と外部モデルの呼び出し。回として閉じていないので累計には数えない
    // （始め直した後に同じ版を採点し直せば、その回で数える。ここで数えると二重になる）。
    cost: typeof panel.cost === "number" && Number.isFinite(panel.cost) ? panel.cost : null,
    externalCallIds: (panel.externalCalls || []).map((row) => row.id),
  };
}
 ...
 * 開いている評価の組があれば、組を閉じずに破棄した記録（script.discardedPanel）を残す（採点のファイルは消さない）。
 * 止めた後は start --restart --reason で始め直せる（累計は今の決まりどおり持ち越す）。止めても合格にはならず、
 * scriptQualityVerdict は script-quality-stopped を返す（制作の関門は人待ちで止まる）。
 *
 * 数えるのは、止めると決めた人の対話端末から --human-verified を付けた記録だけ（判定は scripts/harness-learn.mjs の
 * attestationFor。accept-human・reset-cumulative と同じ一つの実装）。--agent-attested は数えず何も変えない。
 */
export async function stopScriptQualityLoop({
  workDir,
  reviewer = "",
  reason = "",
  humanVerified = false,
  ...
  return withCanvasFileLock(paths.statePath, async () => {
    const existing = await readJsonIfExists(paths.statePath, null);
    if (!existing?.script) return { stopped: false, issues: ["script-quality-loop-not-started"], detail: "先に start でループを始める", check: checkFor(null) };
    if (existing.status !== "active") {
      return {
        stopped: false,
        state: existing,
        issues: [`script-quality-stop-loop-not-active:${existing.status}`],
        detail: `このループは ${existing.status}（${existing.stopReason || "-"}）で既に止まっている。止め直さない。始め直すなら start --restart --reason "..."`,
        check: checkFor(existing),
      };
    }
    if (attestation.attestedBy !== HUMAN_STOP_ATTESTATION) {
      return {
        ...
        counted: false,
        state: existing,
        issues: [`script-quality-stop-not-counted:${attestation.attestedBy}`],
        detail: "ループを止めるのは、止めると決めた人の確認（その人の対話端末から --human-verified）があるときだけ。何も変えていない",
        check: checkFor(existing),
      };
    }
    const stoppedAt = new Date(now()).toISOString();
    const core = stopQualityLoopByHuman({
      state: existing, reason: text, reviewer: attestation.reviewer, attestedBy: attestation.attestedBy, stoppedAt,
    });
    const { pendingPanel, ...scriptRest } = existing.script;
    const discardedPanel = pendingPanel ? discardedPanelRecord(pendingPanel, stoppedAt) : null;
    const next = { ...core, script: { ...scriptRest, ...(discardedPanel ? { discardedPanel } : {}) } };
    await writeJsonAtomic(paths.statePath, next);
    const cumulative = scriptQualityCumulative(next);
    return {
      stopped: true,
      counted: true,
      state: next,
      ...(discardedPanel ? { discardedPanel } : {}),
      issues: issuesFromState(next),
      detail: `品質ループを採点なしで止めた（${next.rounds.length} 回目まで記録。回・費用・時間は数えていない`
        + `${discardedPanel ? `。版 ${discardedPanel.versionLabel} の評価の組（${discardedPanel.reviews.length}/${discardedPanel.declaredEvaluators.length}）は閉じずに破棄した` : ""}）。`
        + `この作業フォルダの累計は ${cumulative.loops} ループ・${cumulative.rounds} 回・費用 ${cumulative.cost}`
        + `${cumulative.unpricedCount ? `＋不明 ${cumulative.unpricedCount} 件` : ""}。${nextStepDetail(next, paths)}`,
      check: checkFor(next),
    };
  });
}
...
export async function scriptQualityVerdict({ workDir, scriptPath = "", scriptSha256 = "", genre = "" } = {}) {
  ...
  if (state.stopReason === HUMAN_STOP_REASON) {
    return notPass(codes.stopped, `品質ループは人が採点なしで止めた（${state.humanStop?.reason || "理由なし"}）。止めても合格にはならない。`
      + "続けるなら start --restart --reason \"...\" で始め直し、採点を受ける");
  }
