...
const READ_ONLY_ACTIONS = new Set(["contract", "channel-contract", "character-bootstrap-status", "character-roster-audit", "cast-readiness", "story-review-draft", "story-audit", "location-plan", "location-anchor-review-draft", "location-anchor-audit", "location-review-draft", "thumbnail-plan-draft", "thumbnail-audit", "handoff-verify", "status"]);
...
export async function doctorKoyaMcp(args = {}) {
  ...
  const projectAuthorityPaths = [
    join(projectDir, "config", "koya-show-bible.json"),
    join(projectDir, "config", "koya-location-bible.json"),
    join(projectDir, "config", "koya-thumbnail-contract.json"),
  ];
  ...
  const showBiblePath = join(authorityRoot, "config", "koya-show-bible.json");
  const required = [
    officialCli,
    ...
    showBiblePath,
    join(authorityRoot, "config", "koya-location-bible.json"),
    join(authorityRoot, "config", "koya-thumbnail-contract.json"),
  ];
  try {
