...
// canvasScene.mjs は先頭で外部パッケージを読む。送信は Job の決着時（依存を入れる前の
// 配布物でも動く実行系）から呼ばれるので、同じ関数を依存の無いモジュールから読む。
...
/**
 * 送り先 URL の検査（手動 upload と、自動送信の outbox が同じ規則を使う）。
 * HTTPS だけ（loopback の試験だけ HTTP）、URL に credential / query / fragment を入れない、
 * path は API ごとに1つだけ（"/" なら既定の path を補う）。
 */
export function normalizeFeedbackEndpoint(value, { endpointPath = "/v1/feedback/bundles" } = {}) {
  let endpoint;
  ...
  if (endpoint.pathname === "/" || !endpoint.pathname) endpoint.pathname = endpointPath;
  if (endpoint.pathname !== endpointPath) {
    throw new HarnessFeedbackUploadError(`feedback upload endpoint pathは${endpointPath}であること。`);
  }
  return endpoint.toString();
}

function normalizedEndpoint(value) {
  return normalizeFeedbackEndpoint(value);
}
...
/** 応答本文を上限つき・時間制限つきで読む（手動 upload と自動送信で共通）。 */
export async function readBoundedResponse(response, { timeoutMs = RESPONSE_TIMEOUT_MS } = {}) {
