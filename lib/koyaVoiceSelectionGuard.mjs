...
function auditionCommands(byRegistryEpisode, { projectFlag, pathsFor }) {
  const commands = [];
  for (const [registryEpisodeId, ids] of byRegistryEpisode) {
    const { planPath, selectionsPath } = pathsFor(registryEpisodeId);
    commands.push(
...
export function koyaVoiceSelectionCommands(audit, options = {}) {
  ...
  const handoffCommands = auditionCommands(handoff, {
    ...
    pathsFor: (registryEpisodeId) => ({
      ...
      selectionsPath: `${HANDOFF_SOURCE_PROJECT}/canvas/voice-casting/${shellArgument(registryEpisodeId)}-elevenlabs-selections.json`,
    }),
  });
...
export function formatKoyaVoiceSelectionError(audit, options = {}) {
  ...
  const howToSelect = "listen to every anonymous candidate (at least two), fill winnerLabel, selectionReason, approvedBy and previewConfirmed=true in the selections file, then approve. These two casting actions only write the voice record; the rest of that legacy script stays benchmark-only.";
