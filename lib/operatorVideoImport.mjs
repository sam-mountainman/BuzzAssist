 ...
 *       "assetLoop": { "statePath": "…", "passedSha256": "<64桁>" }   // 途中の成果物の品質ループ（工程 video-clip）
 *     }]
 *   }
 *
 * assetLoop は、manifest のフォルダ（＝品質ループの作業フォルダ）で回した工程 video-clip のループの状態ファイルと、
 * 合格した版の sha256。ここでは形・置き場・合格した版と取り込む動画の一致だけを見る。合格そのもの（別文脈の評価者の
 * 採点・要る人の確認）はジャンルの関門（ナレーション物語は lib/narratedStoryAssetLoops.mjs。監査契約 v7 から）が
 * 本体に聞く。
 *
...
export const OPERATOR_VIDEO_MANIFEST_FIELDS = Object.freeze({
  ...
  clip: Object.freeze(["slot", "video", "route", "routeNote", "modelLabel", "prompt", "generatedAt", "conversationUrl", "assetLoop"]),
  video: Object.freeze(["path", "sha256"]),
  prompt: Object.freeze(["path", "sha256"]),
  assetLoop: Object.freeze(["statePath", "passedSha256"]),
});
/** 途中の成果物の品質ループでの工程名（lib/assetQualityLoop.mjs の video-clip）。 */
export const OPERATOR_VIDEO_ASSET_LOOP_STAGE = "video-clip";
...
export async function readOperatorVideoManifest({ manifestPath, ffprobe, now = new Date() } = {}) {
  ...
  for (const [index, entry] of (Array.isArray(body.clips) ? body.clips.slice(0, MAX_CLIPS) : []).entries()) {
    ...
    // 途中の成果物の品質ループ（工程 video-clip）。置き場は manifest のフォルダの中。合格した版と取り込む動画が
    // 違えば止める（別の版を取り込もうとしている）。合格そのものはジャンルの関門が本体に聞く。
    clip.assetLoop = null;
    if (entry.assetLoop !== undefined) {
      const loop = entry.assetLoop;
      if (!plainObject(loop) || Object.keys(loop).some((key) => !OPERATOR_VIDEO_MANIFEST_FIELDS.assetLoop.includes(key))) {
        add("operator-video-asset-loop-record-invalid");
      } else {
        const resolved = resolveManifestRelativePath(manifestDir, loop.statePath);
        const passedSha256 = lowerSha(loop.passedSha256);
        clip.assetLoop = { rel: resolved?.rel || "", full: resolved?.full || "", passedSha256, stateSha256: "" };
        if (!resolved || !passedSha256) add("operator-video-asset-loop-record-invalid");
        else {
          const read = await insideFile(manifestDir, resolved.full, MAX_STATE_BYTES);
          if (!read.ok) add("operator-video-asset-loop-state-missing");
          else {
            clip.assetLoop.full = read.realPath;
            clip.assetLoop.stateSha256 = sha256(await readFile(read.realPath));
          }
        }
        if (passedSha256 && clip.video.sha256 && passedSha256 !== clip.video.sha256) add("operator-video-asset-loop-pass-mismatch");
      }
    }
    clips.push(clip);
  }
  // 品質ループの状態は、書いてあるときだけ指紋に入れる（書いていない従来の manifest の指紋は変えない。変えると、
  // 確定を待っている古い監査契約の Job が「入力が変わった」で作り直しになる）。
  const digest = sha256(canonicalJson({
    version: OPERATOR_VIDEO_IMPORT_VERSION,
    manifestSha256,
    files: clips.map((clip) => ({
      slot: clip.slot,
      video: clip.video.sha256,
      prompt: clip.prompt.sha256,
      ...(clip.assetLoop ? { assetLoopState: clip.assetLoop.stateSha256 } : {}),
    })),
  }));
...
function publicClip(clip, copy) {
  return {
    ...
    ...(clip.assetLoop ? { assetLoop: { stateSha256: clip.assetLoop.stateSha256, passedSha256: clip.assetLoop.passedSha256 } } : {}),
...
export async function materializeOperatorVideos({ manifest, outputDir, privateDir, now = () => new Date().toISOString() }) {
  ...
  for (const clip of manifest.clips) {
    ...
    clips.set(clip.slot, { ...copy, probe: clip.probe, public: publicClip(clip, copy), assetLoopStatePath: clip.assetLoop?.full || "" });
    privateClips.push({
      slot: clip.slot,
