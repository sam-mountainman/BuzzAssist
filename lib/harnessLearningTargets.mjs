...
export const HARNESS_LEARNING_ROUTES = Object.freeze({
  ...
  // 解説動画。ジャンル固有の正本スキルがまだ無いので、ジャンル層の宛先は持たない（Canvas feedback が genre: を
  // 明示しても受けない）。自動の捕捉は全部チャンネルの非公開台帳へ行き、共有層へは人が target を明示したとき
  // （platform: だけ）しか上がらない。
  "explainer-video": Object.freeze({
    channel: "channel-pack:explainer",
    genres: Object.freeze([]),
  }),
});
 ...
 * 自動で拾った候補を積む宛先。docs/learning/targets.json の scriptQualityRoutes と同じ（試験が一致を見る）。
 *   - narrated-story: 動画の Channel Pack 宛（channel-pack:narrated-story）とは宛先を分け、台帳の置き場（非公開）は
 *     同じにする。台本の型を直す人が、台本の話だけを読めるようにするため
 *   - manga: 漫画のハーネス（koya-manga-video）の既存のチャンネルの宛先（HARNESS_LEARNING_ROUTES の channel）。
 *     漫画の台本は依頼者が書くので、台本だけの台帳を新しく作らず、そのチャンネルの要求台帳の側で人が読む
 *   - explainer: 解説動画のハーネス（explainer-video）の既存のチャンネルの宛先（HARNESS_LEARNING_ROUTES の channel）。
 *     解説動画の台本はチャンネルの制作の側で書くので、漫画と同じく台本だけの台帳を新しく作らず、そのチャンネルの台帳の
 *     側で人が読む。保存先はチャンネルごと（lib/channelRegistry.mjs の channelLearning、lib/learningChannelResolver.mjs）
 */
export const SCRIPT_LEARNING_ROUTES = Object.freeze({
  "narrated-story": "channel-pack:narrated-story-script",
  manga: HARNESS_LEARNING_ROUTES["koya-manga-video"].channel,
  explainer: HARNESS_LEARNING_ROUTES["explainer-video"].channel,
});
