...
import { checkResumeStrategyBrief, loadRuntimeChannelRegistry, prepareHarnessStart } from "./channelStartGate.mjs";
import { captureSettledJobLearning } from "./harnessReceiptLearning.mjs";
import { normalizeHostInvocation, unprovidedHostInvocation } from "./harnessHostProvenance.mjs";
...
import { KOYA_SCRIPT_QUALITY_GENRE, koyaScriptQualityGateInForce } from "./koyaScriptQualityGatePolicy.mjs";
import {
...
export function assertVideoHarnessStartOptions({
  ...
  // 解説動画: 実行の型と手元の URL の形（Job の識別子に入るので、Job を作る前に止める）。
  if (selected?.harness?.id === EXPLAINER_HARNESS_ID) {
    assertExplainerStartOptions(options);
    return null;
  }
...
/**
 * 台本の品質ループの作業フォルダを、台本の関門を持つハーネス（漫画・ナレーション物語）の Job の options に入れる
 * （lib/scriptQualityUseGate.mjs。明示が無ければ start に渡した台本のあるフォルダ）。options は Job の識別子に
 * 入るので、どのフォルダのループを見るかは Job を作った時点で決まる。CLI と MCP の start が同じ規則を通る。
 * ハーネスが決まらない（未知の ID・曖昧な依頼文）ときは触らない（Job 作成が同じ選択器で名指しして止める）。
 */
export function videoHarnessStartOptionsWithScriptQuality({
  harnessId = "",
  want = "",
  options = {},
  scriptPath = "",
  ...
  return withScriptQualityWorkDirDefault({ harnessId: selected?.harness?.id || "", options, scriptPath, baseDir: projectDir });
}
...
/**
 * 入口（MCP / CLI）が判定した呼び出し元に、操作と実行の別を足す。入口が何も渡さなかった
 * 呼び出し（ライブラリの直接呼び出し）は "not-provided" として残し、推測で埋めない。
 * 形の誤りは Job を作る前・有料処理の前に止める（normalizeHostInvocation が拒否する）。
 */
function hostInvocationFor(invocation, operation, mode) {
  const caller = invocation && typeof invocation === "object" ? invocation : unprovidedHostInvocation();
  const entry = {
    host: caller.host,
    hostVersion: caller.hostVersion,
    clientName: caller.clientName,
    candidates: caller.candidates,
    model: caller.model,
    modelSource: caller.modelSource,
    buzzassistVersion: caller.buzzassistVersion,
    via: caller.via,
    detectedFrom: caller.detectedFrom,
    operation,
    mode,
  };
  normalizeHostInvocation({ ...entry, at: "1970-01-01T00:00:00.000Z" });
  return entry;
}
...
function serviceResult({
  ...
  return {
    ...
    ...(preflight ? { preflight } : {}),
    // チャンネルの台帳で決めた start（lib/channelStartGate.mjs）と、ブリーフの受け渡し・resume の照合。
    ...(channel ? { channel } : {}),
    ...(strategyBrief ? { strategyBrief } : {}),
    ...(strategyBriefCheck ? { strategyBriefCheck } : {}),
    note: String(note || ""),
...
export async function planOnlyPreflight({ job, scriptPath, channelPackPath, env = process.env } = {}) {
  ...
  if (harnessId === "koya-manga-video") return koyaPlanOnlyScriptQuality({ job, scriptPath });
  if (harnessId === EXPLAINER_HARNESS_ID) return explainerPlanOnlyPreflight({ job, scriptPath, channelPackPath, env });
  ...
  // 運営者の画・動画を取り込む Pack なら、Job の options にある manifest も読むだけで検査する。
  const plan = await inspectNarratedStoryPlan({
    ...
    operatorVideoManifestPath: typeof job?.options?.operatorVideoManifestPath === "string" ? job.options.operatorVideoManifestPath : "",
    scriptQualityWorkDir: typeof job?.options?.[SCRIPT_QUALITY_WORK_DIR_OPTION] === "string" ? job.options[SCRIPT_QUALITY_WORK_DIR_OPTION] : "",
    projectDir: typeof job?.projectDir === "string" ? job.projectDir : "",
    // Windows の作業フォルダの長さ（Job の run のフォルダは projectDir と Job の id で決まる）。
...
/**
 * 解説動画の plan-only の preflight。署名 Channel Pack を運営者の信頼鍵で検証し（展開しない）、何を再利用し
 * 何を実行するか・止まる理由を読むだけで出す（lib/explainerVideo.mjs の planExplainerVideo）。モデルも有料 API も
 * 呼ばず、制作のスクリプトも起動しない。動画の実デコードは resume の取り込みの監査で行う。
 */
async function explainerPlanOnlyPreflight({ job, scriptPath, channelPackPath, env }) {
  const harnessId = EXPLAINER_HARNESS_ID;
  let pack;
  let verified;
  try {
    verified = await verifyChannelPackEnvelope({
    ...
    if (verified.payloadKind !== EXPLAINER_PAYLOAD_KIND) throw new Error(`payloadKind が ${EXPLAINER_PAYLOAD_KIND} ではない`);
    pack = await loadExplainerChannelPack(verified.payloadDir);
  } catch (error) {
    return {
      ok: false,
      harnessId,
      blockers: [error?.code === "explainer-channel-pack-invalid" ? "explainer-channel-pack-invalid" : "channel-pack-unverified"],
      detail: redactSecrets(error?.message || String(error)),
      paidCallsAttempted: false,
    };
  }
  const { planExplainerVideo } = await import("./explainerVideo.mjs");
  const plan = await planExplainerVideo({ pack, scriptPath, options: job?.options || {} });
  return {
    ok: plan.blockers.length === 0,
    harnessId,
    channelPackPayloadSha256: verified.payloadSha256,
...
/**
 * 漫画の plan-only の preflight は、台本の関門（制作契約 v56 から）だけを読むだけで問う。Koya の full が有料の
 * 処理の前に返すのと同じ理由コード（script-quality-required:<理由>）と次のコマンドを返す。契約の版は Job を
 * 計画したときの正本の契約（canonicalIdentity.productionContract）で決める。
 */
async function koyaPlanOnlyScriptQuality({ job, scriptPath }) {
  const harnessId = "koya-manga-video";
  const contractVersion = String(job?.canonicalIdentity?.productionContract?.contractVersion || "");
  if (!koyaScriptQualityGateInForce({ version: contractVersion })) return null;
  const gate = await checkScriptQualityBeforeProduction({
    workDir: scriptQualityWorkDirFor({ options: job?.options || {}, scriptPath }),
    scriptPath,
...
const DEFAULT_RUNTIME = Object.freeze({
  ...
  // チャンネルの台帳（config/harness-deployments.json の channels）。start と resume の関門が読む。
  channelRegistry: ({ env } = {}) => loadRuntimeChannelRegistry({ env: env ?? process.env }),
  prepareStart: prepareHarnessStart,
  checkResumeBrief: checkResumeStrategyBrief,
});
...
export function createVideoHarnessService(overrides = {}) {
  ...
  async function execute(projectDir, jobId, { reviewerTrustPath = "", retryFailedImages = false, invocation = null, finalizeAfterUpdate = false } = {}) {
    const planned = await runtime.readJob({ projectDir, jobId });
    // 更新をまたいで確定させるときは、計画時のスキルの SHA では確かめない（更新で変わっている）。Job 層が今の
    // コードの同一性を作るときに、今のスキルで production profile を確かめる。
    if (finalizeAfterUpdate !== true) await runtime.productionProfile({ job: planned });
    ...
    return runtime.withJobLock({ projectDir, jobId }, () => runtime.runJob({
      ...
      ...(reviewerTrustPath ? { reviewerTrustPath } : {}),
      // 再開したホスト。Job の identity（options）には入れず、Job 層が別の一覧に積む。
      ...(invocation ? { invocation } : {}),
      // 更新をまたいで作り直さずに確定させる（入力が同じでコードだけ変わった Job。lib/videoHarnessUpdateFinalize.mjs）。
      ...(finalizeAfterUpdate === true ? { finalizeAfterUpdate: true } : {}),
    }));
  }
  ...
  async function start({
    ...
    if (!plainObject(requestedOptions)) throw new Error("options はJSON objectであること。");
    assertNoSensitiveFields(requestedOptions);
    // チャンネルの台帳（作業フォルダ・Pack・ハーネスは台帳の値だけ）と企画ブリーフ。requireBrief のチャンネルは
    // 合格したブリーフが無ければここで止まる（Job を作る前・有料の処理の前）。lib/channelStartGate.mjs。
    const prepared = await runtime.prepareStart({
      registry: () => runtime.channelRegistry({ env: runtime.env }),
      channelId: nonEmpty(channelId),
      projectDir,
      channelPackPath,
      harnessId: nonEmpty(harnessId),
      options: requestedOptions,
      strategyBriefPath: nonEmpty(strategyBriefPath),
    });
    const project = resolve(prepared.projectDir ?? projectDir ?? process.cwd());
    const channelPack = nonEmpty(prepared.channelPackPath ?? channelPackPath);
    const selectedHarnessId = prepared.harnessId ?? nonEmpty(harnessId);
    const options = prepared.options;
    if (!channelPack) throw new Error("本番上位Jobには署名済み Channel Pack の channelPackPath が要る。");
    const createdBy = hostInvocationFor(invocation, "start", confirmed === true ? "execute" : "plan-only");
    ...
    // 台本の品質ループの作業フォルダ（台本の関門を持つハーネスだけ。明示が無ければ台本のあるフォルダ）。
    const jobOptions = videoHarnessStartOptionsWithScriptQuality({
      harnessId: selectedHarnessId,
      want: nonEmpty(want),
      options,
    ...
    const created = await runtime.createJob({
      ...
      options: structuredClone(jobOptions),
      invocation: createdBy,
      // チャンネルとブリーフのファイルの場所（Job の識別子に入れない。resume がブリーフの変化を見る）。
      ...(Object.keys(prepared.metadata || {}).length > 0 ? { metadata: prepared.metadata } : {}),
    });
    const channelOut = prepared.channel ? { id: prepared.channel.id, selectedBy: prepared.metadata.channel.selectedBy } : null;
    ...
    // 既存 Job に接続した confirmed start は、その Job を作った呼び出しではなく再開として残す。
    const job = await execute(project, created.job.id, {
      reviewerTrustPath: trustPath,
      invocation: created.attached === true ? { ...createdBy, operation: "start-attached" } : null,
    });
    return serviceResult({
      operation: "start",
      projectDir: project,
  ...
  async function resume({ projectDir = process.cwd(), jobId, confirmed = false, reviewerTrustPath = "", retryFailedImages = false, finalizeAfterUpdate = false, invocation = null } = {}) {
    ...
    if (finalizeAfterUpdate === true && retryFailedImages === true) {
      throw new Error("finalizeAfterUpdate（更新をまたいで作り直さずに確定させる）と retryFailedImages（失敗した画像を作り直す）は一緒に使えない。");
    }
    const resumedBy = hostInvocationFor(invocation, "resume", "execute");
    // start のときのブリーフが変わった・無くなったなら、Job を変えず有料の処理の前に止める（新しい Job として start する）。
    // requireBrief では止めない（再レンダー＝既存の Job の再開は、戦略スキルも採点も回さない）。
    // 読めない Job（無い・journal の破損）はここでは判定せず、Job 層の再開がその理由で止める。
    let recorded = null;
    try {
      recorded = await runtime.readJob({ projectDir: project, jobId: id });
    } catch {
      recorded = null;
    }
    const strategyBriefCheck = recorded
      ? await runtime.checkResumeBrief({ job: recorded, registry: () => runtime.channelRegistry({ env: runtime.env }) })
      : null;
    const trustPath = await verifiedReviewerTrustPath(reviewerTrustPath);
    ...
    const job = await execute(project, id, {
      reviewerTrustPath: trustPath,
      retryFailedImages: retryFailedImages === true,
      finalizeAfterUpdate: finalizeAfterUpdate === true,
      invocation: resumedBy,
    });
    return serviceResult({
      operation: "resume",
      projectDir: project,
      ...
      confirmed: true,
      reviewerTrust,
      ...(strategyBriefCheck?.status && strategyBriefCheck.status !== "no-brief" ? { strategyBriefCheck } : {}),
      learningCapture: await captureSettledLearning(job),
    });
  }
