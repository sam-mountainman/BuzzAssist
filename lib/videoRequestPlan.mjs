...
async function resolvePlanChannel({ channelId, channelRegistry, channelRegistryPath, env, harnessIds, projectDir, channelPackPath }) {
  ...
  let registry;
  try {
    registry = channelRegistry ?? loadChannelRegistry({
      deploymentPath: channelRegistryPath,
      env,
      harnessIds,
      genres: Object.keys(await scriptQualityGenres()),
    });
  } catch (error) {
  ...
  if (channel) {
    ...
    if (kindDecision.status === "selected") {
      ...
      workflow = recommendNextSteps({
        ...
        scriptGenreHarnessId: genres[channel.scriptQuality?.genre]?.harnessId ?? null,
      });
    } else {
