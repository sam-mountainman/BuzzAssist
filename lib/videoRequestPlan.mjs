...
function reasonLines(candidate) {
  const lines = [];
...
function startCommand(view, { scriptPath, channelPackPath, options }) {
  ...
  for (const option of view.inputs.startOptions) {
...
function mcpStartArguments(view, { scriptPath, channelPackPath, options }) {
  const optionKeys = view.inputs.startOptions.map((entry) => entry.key);
  ...
  return {
    ...
    ...(optionKeys.length > 0 ? { options: given } : {}),
  };
}
...
export async function planVideoRequest({
  ...
  if (!nonEmpty(want) && !nonEmpty(harnessId)) {
  ...
  const project = nonEmpty(projectDir) ? path.resolve(projectDir) : null;
  const declared = harnesses ?? loadHarnesses();
  const harnessIds = declared.map((harness) => harness.id);

  const decision = decideVideoHarness({ harnesses: declared, harnessId, want });
  const script = await inspectScriptInput(scriptPath);
  const pack = await inspectChannelPackTarget(channelPackPath, { env, harnessIds });
  const hint = packHint(pack);
  ...
  const ordered = decision.status === "no-match" || decision.status === "unknown-harness"
  ...
  for (const [index, harness] of ordered.entries()) {
    ...
    candidate.reasons = reasonLines(candidate);
  ...
  const nextStep = selected
    ? {
        ...
        cli: startCommand(selected.capability, { scriptPath, channelPackPath, options }),
        mcp: { tool: "run_video_harness", arguments: mcpStartArguments(selected.capability, { scriptPath, channelPackPath, options }) },
  ...
  const summaryLines = [];
  if (status === "selected") {
    summaryLines.push(`判定: ${selectedId} に決まった（${decision.selectedBy === "explicit" ? "明示" : `一致した語: ${selected.matchedTerms.join(", ")}`}）`);
  ...
  if (candidates.length === 0) summaryLines.push("候補なし");
