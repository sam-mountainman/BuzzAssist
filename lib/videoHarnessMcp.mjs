...
//
// reviewer 工程（signoff / reviewer-key-create）の MCP 入口もここに置く。ナレーション物語と解説動画の
// Job から同等に届くこと（CLI = MCP）が目的で、引数名は Koya の MCP 入口（koyaMcpAdapter の
...
const EXPLAINER_HARNESS_ID = "explainer-video";
// signoff_video_harness_job が受けるハーネス（Job の配置表の入口の signoff を起動する）。
const SIGNOFF_HARNESS_IDS = Object.freeze([NARRATED_HARNESS_ID, EXPLAINER_HARNESS_ID]);
...
export function videoHarnessToolDefinitions() {
  return [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          options: {
            type: "object",
            description: "Harness-specific non-secret JSON options (e.g. episodeId, protagonistSpeakerId, characterBiblePath, storyReviewPath). For koya-manga-video those four are required at start, even plan-only: they are part of the Job identity and cannot be added later, so start refuses with koya-start-options-missing (naming the missing ones) and creates no Job. Paths only: API keys, tokens, trusted/signing keys, private key material, credentials, and passwords are rejected, and so is any reviewerTrust* field and any reviewer key field (reviewerKeyPath, reviewerPrivateKeyPem, ...). The trust anchor comes only from the executing host's BUZZASSIST_REVIEWER_TRUST; the top-level reviewerTrustPath argument merely cross-checks it. Reviewer signoff is a separate reviewer-context step and never part of the production Job: run_koya_manga_pipeline action=signoff for koya-manga-video Jobs, signoff_video_harness_job for narrated-story-video and explainer-video Jobs (all take reviewerKeyPath as a file path). explainer-video (import a finished delivery made by the channel's own local production): options.explainerMode (import-delivery default | produce) and options.explainerRendererUrl (produce only, a localhost URL) are part of the Job identity; the Job stays awaiting-human-review until a human reviewer signs the full-length review through signoff_video_harness_job and the machine audits plus the thumbnail asset quality loop pass. Script quality: options.scriptQualityWorkDir (optional, a path) names the script-quality-loop work folder; omitted, start fills in the script's folder. It is part of the Job identity, and from Koya production contract v56 / narrated audit contract v8 paid work waits for a human with script-quality-required:<reason> unless those exact script bytes passed that loop or were accepted with accept-human. Operator-made media for narrated-story-video: options.operatorImageManifestPath (a buzzassist-operator-image-manifest-v1 file, needed when the Channel Pack declares image.source: \"operator-file\") and options.operatorVideoManifestPath (per-episode opening video manifest); both are paths, part of the Job identity, and must be given at start. Scene images then come from the manifest with their provenance instead of paid image Media Jobs; conversation URLs stay in the private Job folder and only SHA-256 values reach the Receipt and Canvas.",
            additionalProperties: true,
          },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          finalizeAfterUpdate: {
            type: "boolean",
            description: "Optional. Finish a Job that stopped with canonical-identity-drift only because BuzzAssist was updated. If the inputs (script, Channel Pack, options including the strategy brief SHA, operator import manifests, Koya episode override) are unchanged, the Job's code identity is rebound to the current code (the planned one is kept; Job ID and identityDigest do not change) and the Job runs reuse-only: every paid call must hit an existing Media Job, and any new paid call is refused before it is sent (finalize-after-update-paid-call-required, nothing billed). Audits and finalization use the contract version fixed for the Job (Koya: the pinned production contract), never the newer version's required audits. Changed inputs stop with canonical-identity-drift plus finalize-after-update-input-changed / -input-unrecorded / -pinned-contract-unavailable: create a new Job. The RunReceipt records codeIdentity (planned and finalized code, rebinds, refused paid calls). Cannot be combined with retryFailedImages. Same as CLI resume --finalize-after-update. Default false.",
          },
          hostModel: HOST_MODEL_PROPERTY,
        },
    ...
    },
    {
      ...
      description: "Independent-reviewer step for a durable narrated-story-video or explainer-video Job: runs the Job's genre CLI signoff (scripts/narrated-story-video.mjs signoff / scripts/explainer-video.mjs signoff, resolved from the Job's deployment) with the same argument names as run_koya_manga_pipeline action=signoff, so Claude Code and Codex reach the identical entry the CLI offers. The reviewer task/session must differ from the production Job's context. Hashes the reviewed MP4 and contact sheet from disk, signs the subject with the Ed25519 private key read from reviewerKeyPath (file path only), requires that key to be active in the operator's trust list, and writes the signed signoff into the Job workspace. For explainer-video Jobs this records the human review: a person watched the whole finished MP4 as a first-time viewer (fullLengthViewed must be true), scored every criterion of the Job's explainer/review/review-sheet.json (the sheet carries no pass mark or floors) and wrote findings the machine cannot measure; reviewerId is that person's name and required, the subject also binds the imported DELIVERY.json SHA, and videoPath / contactSheetPath / signoffPath are refused because the Job's audit report fixes them. Never spends credits. koya-manga-video Jobs sign off through run_koya_manga_pipeline action=signoff instead and are refused here.",
      inputSchema: {
        type: "object",
        properties: {
          projectDir: PROJECT_DIR_PROPERTY,
          ...
          reviewerId: { type: "string", description: "Reviewer identity label. Optional for narrated-story-video; required for explainer-video (the name of the person who watched and scored the video)." },
          ...
          fullLengthViewed: { type: "boolean", description: "explainer-video only, and then required to be exactly true: the reviewer watched (listened to) the finished MP4 from start to end. Refused for other harnesses." },
        },
...
// 解説動画の signoff が受けない path（Job の監査の報告が完成 MP4・contact sheet・signoff の置き場を決める）。
const EXPLAINER_REFUSED_SIGNOFF_OPTIONS = Object.freeze(["videoPath", "contactSheetPath", "signoffPath"]);
...
export function createVideoHarnessReviewerActions({
  ...
  async function signoff(args = {}) {
    ...
    if (harnessId === KOYA_HARNESS_ID) {
      throw new Error(`Job ${jobId} is a ${KOYA_HARNESS_ID} Job: sign it off through run_koya_manga_pipeline action=signoff (episodeId, reviewNotesPath, reviewerKeyPath, reviewerContextId, pass). signoff_video_harness_job serves ${SIGNOFF_HARNESS_IDS.join(" / ")} Jobs.`);
    }
    if (!SIGNOFF_HARNESS_IDS.includes(harnessId)) {
      throw new Error(`Job ${jobId} declares harness '${harnessId || "(empty)"}', which has no reviewer signoff entry here.`);
    }
    const explainer = harnessId === EXPLAINER_HARNESS_ID;
    if (explainer) {
      if (args.fullLengthViewed !== true) {
        throw new Error("full-length-viewing-required: an explainer-video signoff records a human review of the whole finished MP4; pass fullLengthViewed=true only after the reviewer watched (listened to) it from start to end.");
      }
      if (!nonEmpty(args.reviewerId)) throw new Error("explainer-video signoff requires reviewerId (the name of the person who watched and scored the video).");
      const refused = EXPLAINER_REFUSED_SIGNOFF_OPTIONS.filter((name) => args[name] !== undefined && args[name] !== null && args[name] !== "");
      if (refused.length > 0) {
        throw new Error(`explainer-video signoff does not take ${refused.join(", ")}: the Job's audit report fixes the reviewed MP4, contact sheet and signoff location.`);
      }
    } else if (args.fullLengthViewed !== undefined) {
      throw new Error(`fullLengthViewed applies only to ${EXPLAINER_HARNESS_ID} Jobs.`);
    }
    ...
    if (explainer) argv.push("--full-length-viewed");
