...
export function videoHarnessToolDefinitions() {
  return [
    ...
    },
    {
      ...
      description: "Resume a durable Job through the common doctor and adapter path. A Job held in awaiting-human-review with pendingReceiptFinalization (e.g. blocker reviewer-trust-unconfigured after paid generation) retries only the RunReceipt finalization with the stored artifacts and Media Jobs; nothing is regenerated or re-billed. Because other resumes may invoke paid providers, confirmed=true is mandatory.",
      inputSchema: {
        type: "object",
        properties: {
          projectDir: PROJECT_DIR_PROPERTY,
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          force: { type: "boolean", description: "Overwrite an existing signoff file." },
          pass: { type: "boolean", description: "Must be exactly true, and only after the contact sheet was actually inspected." },
        },
        required: ["jobId", "confirmed", "reviewer", "reviewerContextId", "reviewerKeyPath", "pass"],
        additionalProperties: false,
      },
      annotations: {
        readOnlyHint: false,
...
const SIGNOFF_STRING_OPTIONS = Object.freeze([
  ...
  "videoPath", "contactSheetPath", "signoffPath",
]);
const SIGNOFF_BOOLEAN_OPTIONS = Object.freeze(["force", "pass"]);
...
export function createVideoHarnessReviewerActions({
  ...
  async function signoff(args = {}) {
    ...
    if (args.pass !== true) throw new Error("Signoff requires pass=true after the contact sheet has actually been inspected.");
