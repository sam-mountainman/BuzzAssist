...
// ここで決めること:
//
//   - 開発用チェックアウト（.git と .claude/skills と .agents/skills がある。
...
// 提供元（BuzzAssist の開発側）へ学習を返すことへの運営者の同意。既定は「同意なし」で、
// 記録が無い・壊れている・版が違うときも同意なしとして扱う（欠落を許可として扱わない）。
export const FEEDBACK_CONSENT_VERSION = "buzzassist-feedback-consent-v1";
/** 同意の範囲。決着の要約（ゲートの合否の件数）・提案の要約（既知の提案 ID）・未知の提案（一般化した文）。 */
export const FEEDBACK_CONSENT_SCOPES = Object.freeze(["settlements", "proposals", "new-candidates"]);
/** 範囲を指定せずに同意したときの範囲。文を運ぶ new-candidates は明示したときだけ入れる。 */
export const FEEDBACK_DEFAULT_SCOPES = Object.freeze(["settlements", "proposals"]);
export const FEEDBACK_CONSENT_VIAS = Object.freeze(["cli", "setup"]);
...
export function resolveLearningState({
  ...
  if (development) {
    ...
    return {
      ...
      // チャンネルの台帳（config/harness-deployments.json の channels）で作った Job の学習の既定の保存先。
      // 開発用チェックアウトでもリポジトリの外に置く（docs/learning は公開の共有台帳、channel-packs/ に
      // 別の Pack の名前のフォルダを作ると既定の Pack の探索が変わる）。
      channelStoreRoot: pathApi.join(operatorDir, "channels"),
      receiptsDir,
      ...
      // 提供元へ返す同意・送信待ちの置き場。端末ごとの決定なので、開発用チェックアウトでも
      // リポジトリの外（git で追跡しない場所）に置く。
      feedbackDir: pathApi.join(operatorDir, "feedback"),
      syncStatePath: null,
  ...
  return {
    ...
    channelStoreRoot: pathApi.join(stateDir, "channels"),
    receiptsDir,
    receiptIndexPath: pathApi.join(receiptsDir, "index.jsonl"),
    ...
    feedbackDir: pathApi.join(stateDir, "feedback"),
...
/**
 * チャンネル（台帳の channels の id）の学習の既定の保存先の root。台帳の channelLearning に
 * { target, channel, root } の宣言が無いチャンネルが使う。中に docs/learning（提案・反映記録）と、
 * 宛先の正本（docs/<台帳>.md）を置く。チャンネルごとに別の root なので、同じハーネスのチャンネルでも混ざらない。
 */
export function channelStoreDefaultRoot(state, channelId, pathApi = path) {
  const id = String(channelId || "");
  if (!CHANNEL_ID.test(id)) throw new Error(`チャンネルの id が保存先の名前として使えない: ${JSON.stringify(id).slice(0, 80)}`);
  if (!state?.channelStoreRoot) throw new Error("学習の置き場に channelStoreRoot が無い。");
  return pathApi.join(state.channelStoreRoot, id);
}
...
/**
 * 索引の行・Job のチャンネルの id（Job の metadata.channel.id）。形の合わない値は「チャンネルなし」（""）。
 * channel の欄は後から足した（欄の無い古い行は「チャンネルなし」として読む。索引の版は変えない）。
 */
export function receiptIndexChannelId(value) {
...
export function appendReceiptIndexRow(indexPath, row) {
  ...
  const channelId = receiptIndexChannelId(channel);
  const entry = { version: RECEIPT_INDEX_VERSION, ...rest, ...(channelId ? { channel: channelId } : {}) };
...
/** 同意の記録の置き場（学習の置き場の feedback/consent.json）。 */
export function feedbackConsentPath(state, pathApi = path) {
  if (!state?.feedbackDir) throw new Error("学習の置き場に feedbackDir が無い。");
  return pathApi.join(state.feedbackDir, "consent.json");
}

function normalizeConsentScopes(value) {
  if (!Array.isArray(value)) throw new Error("同意の範囲は配列にしてください。");
  const scopes = [...new Set(value.map((scope) => String(scope).trim()))];
  const unknown = scopes.filter((scope) => !FEEDBACK_CONSENT_SCOPES.includes(scope));
  if (unknown.length > 0) {
    throw new Error(`同意の範囲が不明です: ${unknown.join(", ")}（使えるのは ${FEEDBACK_CONSENT_SCOPES.join(" / ")}）`);
  }
  return FEEDBACK_CONSENT_SCOPES.filter((scope) => scopes.includes(scope));
}

/**
 * 同意の記録を読む。無い・壊れている・版が違う・形が合わないときは「同意なし」。
 * recorded は「読める記録があった」か（setup がもう一度聞くかどうかに使う）。
 */
export function readFeedbackConsent(state) {
  const empty = { recorded: false, enabled: false, scopes: [], decidedAt: null, via: null };
  let file;
  try { file = feedbackConsentPath(state); } catch { return empty; }
  const text = readText(file);
  if (text === null) return empty;
  try {
    const parsed = JSON.parse(text);
    if (parsed?.version !== FEEDBACK_CONSENT_VERSION || typeof parsed.enabled !== "boolean") return { ...empty, invalid: true };
    const scopes = normalizeConsentScopes(parsed.scopes);
    if (parsed.enabled && scopes.length === 0) return { ...empty, invalid: true };
    if (!Number.isFinite(Date.parse(String(parsed.decidedAt || "")))) return { ...empty, invalid: true };
    if (!FEEDBACK_CONSENT_VIAS.includes(parsed.via)) return { ...empty, invalid: true };
    return {
      recorded: true,
      enabled: parsed.enabled,
      scopes: parsed.enabled ? scopes : [],
...
/**
 * 同意を記録する（上書き）。取り消し（enabled: false）も同じ形で残す——記録が無いのと
 * 「断った」は、setup がもう一度聞くかどうかで違う。
 */
export function writeFeedbackConsent(state, {
  enabled, scopes = [], via = "cli", interactive = false, now = new Date().toISOString(),
} = {}) {
  if (typeof enabled !== "boolean") throw new Error("enabled は true か false にしてください。");
  if (!FEEDBACK_CONSENT_VIAS.includes(via)) throw new Error(`via は ${FEEDBACK_CONSENT_VIAS.join(" / ")} のどれかにしてください。`);
  const normalized = enabled ? normalizeConsentScopes(scopes.length > 0 ? scopes : [...FEEDBACK_DEFAULT_SCOPES]) : [];
  const record = {
    version: FEEDBACK_CONSENT_VERSION,
    enabled,
    scopes: normalized,
  ...
  writeFileAtomic(feedbackConsentPath(state), `${JSON.stringify(record, null, 2)}\n`);
  return record;
}
