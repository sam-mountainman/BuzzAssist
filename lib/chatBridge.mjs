...
export function startChatBridgeWorker({ canvasDir }) {
  ...
  try {
    ...
    watcher.unref?.()
    // try/catch は同期例外しか捕まえない。EMFILE のような非同期の error は
    // リスナーが無いと unhandled 'error' になり、**MCP のプロセスごと落ちる**。
    // ここは 1500ms のポーリングという代替があるので、watcher を失っても
    // 機能は続く——落とす理由が無い。
    watcher.on("error", (error) => {
      process.stderr.write(`[chat-bridge] ディレクトリ監視を諦めてポーリングに切り替えます: ${error?.message || error}\n`)
  ...
  // watcher を外から観測できるようにする。監視が壊れたときの挙動
  // （プロセスを落とさずポーリングへ落ちる）は、観測できなければ検証できず、
  // 「たぶん大丈夫」のまま残る。呼び出し側は今までどおり関数として使える。
  Object.defineProperty(stop, "watcher", { get: () => watcher })
  return stop
}
