...
async function recordKoyaWardrobeState(paths, episodeId, record, { appendOverride = null, top = {} } = {}) {
...
async function verifyKoyaWardrobeReportForImages({ projectDir, episodeId, paths, scriptText, scriptDigest, options, runtime, env, previousState }) {
  ...
  if (recomputed.inventoryDigest !== report.inventoryDigest || recomputed.outcomeDigest !== report.outcomeDigest) {
    return stale("the character registry, show bible or scene keyword rules changed after the report; rerun wardrobe-readiness");
  }
  if (report.review) {
...
export function formatKoyaWardrobeReadinessError(verdict, { projectDir, episodeId, scriptPath = "", jobId = "", jobProjectDir = "" } = {}) {
  ...
  return [
    ...
    "A pending or undecidable slot is resolved only by a koya-wardrobe-review-v1 file written in a context other than the generator's"
      + " (per slot: decision fits/does-not-fit, the outfit it fits with, and a reason; bound to the inventoryDigest), passed with --wardrobe-review-path;"
...
export async function assertKoyaWardrobeReadinessBeforeImages(options = {}, context = {}) {
  ...
  if (verdict.pass) {
    ...
    await recordKoyaWardrobeState(paths, episodeId, binding);
  ...
  await recordKoyaWardrobeState(paths, episodeId, {
    ...
    top: {
      ...
      currentStage: "wardrobe-readiness",
      knownRemainingIssues: [{
