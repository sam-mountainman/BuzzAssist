...
export async function planKoyaMangaProduction(options = {}) {
  ...
  // sheets reach only the images whose beat needs them, and a beat without an
  // approved sheet stops here, before any paid call.
  ...
  const eyeOpenGovernance = auditKoyaEyeOpenPlan({
    policy: eyeOpenPolicy,
    eyeOpenPlan: eyeOpenPlan.eyeOpen,
    activeCastIds: fixedCastReadiness.activeCastIds || [],
  });
  if (!eyeOpenGovernance.pass && authority.source === "project") {
    throw new Error(`Koya eye-open governance failed before generation: ${eyeOpenGovernance.failures.join("; ")}`);
  }
