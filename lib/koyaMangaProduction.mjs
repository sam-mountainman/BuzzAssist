...
export async function planKoyaMangaProduction(options = {}) {
  ...
  const eyeOpenGovernance = auditKoyaEyeOpenPlan({ policy: eyeOpenPolicy, eyeOpenPlan: eyeOpenPlan.eyeOpen });
...
export async function generateKoyaMangaImages(options = {}) {
  await assertKoyaFullPreflight(options);
...
export async function assertKoyaSpeechVoiceSelections({
  ...
  let validationCanary = { active: false };
  if (policy) {
    const canaryShowBible = showBible || (await readKoyaChannelAuthority({ projectDir: root })).showBible;
 ...
 * when a cut would really be generated; a refusal is recorded in the episode
 * state so `status` shows who still needs a human voice selection.
 */
export function koyaSpeechVoiceSelectionGate({ projectDir, episodeId, resolved, jobId = "" } = {}) {
...
export async function explainKoyaUnvoicedProtagonist(error, {
  ...
  let protagonist;
  try {
    protagonist = resolveKoyaProtagonistSpeaker(manifest, protagonistRequest);
  } catch {
    return error;
  }
  const isNarration = (row) => row.speakerId === "narration" || row.preset === "narration";
  const voiced = (manifest?.utterances || []).some((row) => (
    !isNarration(row) && row.speakerId === protagonist.speakerId && nonEmpty(row.voiceId)
  ));
  if (voiced) return error;
  // Narration is rebound to the protagonist once a voice exists; until then its
  // own placeholder voice would only blur the report.
  const probe = structuredClone(manifest);
  for (const row of probe.utterances || []) {
    if (isNarration(row)) Object.assign(row, { voiceId: "", voiceProfileId: "" });
  }
  ...
  try {
    assertKoyaHumanVoiceSelections({
      ...
      projectDir: paths.projectDir,
      jobId,
      auditionPaths: (registryEpisodeId) => voiceLibraryAuditionPaths({ projectDir: paths.projectDir, episodeId: registryEpisodeId }),
    });
  } catch (guardError) {
    ...
    await updateState(paths, {
      ...
      knownRemainingIssues: [{
        id: "voice-selection-required",
        characterIds: guardError.characterIds,
...
async function runKoyaMangaFullProductionUnlocked(options = {}, runtime = {}) {
  ...
  const imageResult = await runImages(boundOptions);
  ...
  const prepared = await prepareManifest({ ...boundOptions, episodeId });
  if (prepared.waiting) {
    return {
      exitCode: 3,
      preflight,
  ...
  const speech = await runSpeech({ ...boundOptions, episodeId });
