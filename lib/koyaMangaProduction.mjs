...
import {
  ...
  createEpisodeDialogueDefinition,
...
import { koyaVoiceAuditionPaths } from "./koyaVoiceAudition.mjs";
import {
...
// 同じ Job・版・binding を、このプロセスで最初に検証した時刻。verifyKoyaUpstreamPreflight を見よ。
const PREFLIGHT_ANCHORS = new Map();
...
/**
 * doctor が非課金で測った台詞音声のアダプタが、契約の指すアダプタと同じであること。
 * 違えば、doctor は A を ready と確かめたのに、本番は確かめていない B に課金することになる。
 */
export function assertKoyaDoctorMeasuredContractAdapter(preflight, resolved) {
  const expected = resolveKoyaDialogueAdapter(resolved);
  const measured = preflight?.doctor?.ttsAdapter;
  if (!measured) {
    throw new Error("Koya speech preflight carries no measured voice.dialogue adapter.");
  }
  for (const key of ["provider", "model", "adapterVersion"]) {
    if (measured[key] !== expected[key]) {
      throw new Error(
        `Koya doctor measured ${measured.provider}/${measured.model}/${measured.adapterVersion}, `
        + `but the contract names ${expected.provider}/${expected.model}/${expected.adapterVersion}. `
        + "Re-run the common doctor against the contract's adapter before any paid speech.",
      );
    }
  }
  return expected;
...
function assertKoyaDoctorEvidence(evidence, { projectDir, nowMs }) {
  ...
  // どの有料アダプタを測ったかは契約で決まる（KOYA_DIALOGUE_ADAPTERS）。ここでは
  // 「一覧の1件とぴったり一致し、ready で、voice.dialogue である」ことだけを確かめ、
  // それが契約の指すアダプタと同じかは音声工程の入口（有料呼び出しの前）で照合する。
  const ttsAdapter = findKoyaDialogueAdapter(tts);
  if (!ttsAdapter || tts?.kind !== "voice.dialogue" || tts?.status !== "ready") {
    throw new Error("The Koya doctor TTS evidence is not the exact paid adapter (it must be a ready, allowed voice.dialogue adapter).");
  }
  const expectedTts = { ...ttsAdapter, status: "ready" };
...
export async function verifyKoyaUpstreamPreflight(options = {}, runtime = {}) {
  ...
  // 鮮度は「この証跡をこのプロセスで最初に検証した時刻」を基準に測る。
  // 各有料工程が Date.now() で測り直していたので、画像工程（実測 13〜16 時間）を
  // 終えた直後の音声工程で必ず「doctor が古い」と落ち、払った分が Job ごと死んだ。
  // 外側の Job は再開のたびに doctor を走らせ直して新しい binding を渡すので、
  // 「初回は 15 分以内」という保証はそのまま残る。それ以外の再検証（Job の同一性、
  // Channel Pack の指紋、配置バイト）は毎回そのまま行う。
  const anchors = runtime.preflightAnchors instanceof Map ? runtime.preflightAnchors : PREFLIGHT_ANCHORS;
  const anchorKey = `${job.id}#${job.revision}#${expectedBinding}`;
  const anchorMs = anchors.get(anchorKey) ?? nowMs;
  parseFreshTimestamp(doctorStage.finishedAt, anchorMs, "Upstream doctor evidence");
  anchors.set(anchorKey, anchorMs);
...
export function applyKoyaValidationCanaryVoiceProfiles(
  manifestInput,
  validationCanary,
  registry,
  showBible,
  ...
  for (const utterance of manifest.utterances || []) {
    ...
    if (!voice || !providerVoiceId || voice.modelId !== dialogueAdapter.model || !/^approved/u.test(nonEmpty(voice.status))) {
      throw new Error(`Validation canary voice profile ${profileId} for ${member.name} must be an existing approved ${dialogueAdapter.model} Japanese profile.`);
    }
    ...
    utterance.model = dialogueAdapter.model;
...
export async function substituteKoyaMangaCutVideos(options = {}, runtime = {}) {
  ...
  const result = await runCutVideoSubstitutions({
    ...
    // 工程 video-clip の品質ループの作業フォルダ（契約 v55 から。canvas/）。
...
/**
 * 本編の画像と環境アトラスに入れる番組の画風。show bible の artStyle は必須で
 * （validateKoyaChannelArtStyle が無ければ投げる）、visual profile はキャラクター工程
 * （prepareCharacterWorkflow）と同じく、その作業場の既定 profile を使う。
 * プロンプトの文面を決める項目だけを渡す（参照画像の置き場などは文面に出ない）。
 */
export async function resolveKoyaSceneChannelStyle({ projectDir, canvasDir, showBible } = {}) {
  validateKoyaChannelArtStyle(showBible);
  const art = showBible.artStyle;
  const artStyle = {
    id: nonEmpty(art.id),
    medium: nonEmpty(art.medium),
    sharedIdiom: Array.isArray(art.sharedIdiom) ? [...art.sharedIdiom] : [],
    characterIdiom: Array.isArray(art.characterIdiom) ? [...art.characterIdiom] : [],
    environmentIdiom: Array.isArray(art.environmentIdiom) ? [...art.environmentIdiom] : [],
    forbidden: Array.isArray(art.forbidden) ? [...art.forbidden] : [],
  };
  const visualProfile = await resolveChannelVisualProfileSnapshot({ projectDir, canvasDir }, "");
  return { artStyle, visualProfile: visualProfile || null };
}
...
export async function planKoyaMangaProduction(options = {}) {
  ...
  // 番組の画風を本編と環境アトラスのプロンプトへ入れる（宣言が無い show bible は上で落ちる）。
  const channelStyle = await resolveKoyaSceneChannelStyle({ projectDir, canvasDir: paths.canvasDir, showBible: authority.showBible });
  ...
  plan.production = {
    ...
    eyeOpenGovernance,
    channelStyle: {
      artStyleId: channelStyle.artStyle.id,
      visualProfileId: channelStyle.visualProfile?.id || "",
    },
  ...
  return { episodeId, paths, plan, resolved, state, channelStyle };
}
...
export async function generateKoyaMangaImages(options = {}, runtime = {}) {
  ...
  // 契約 v56 から: 使う台本が台本の品質ループに合格した版か、人がそのまま使うと認めた版でなければ、有料の画の
  // 前に止まる（KOYA_SCRIPT_QUALITY_REQUIRED）。本物の有料の画の工程を守る関門なので、画の工程を差し替えた試験
  // （何も払わない）は関門を差し込んだときだけ通す（full の声・衣装の前検査と同じ規則）。
  const checkScriptQuality = runtime.checkScriptQuality
    || (runtime.runImagePipeline ? null : assertKoyaScriptQualityBeforeProduction);
  if (checkScriptQuality) await checkScriptQuality(options, { runtime, stage: "images" });
  ...
  const result = await (runtime.runImagePipeline || runMangaScriptImagePipeline)({
    ...
    // 計画と同じ画風宣言で本編と環境アトラスを作る（入力 hash もこれを含む）。
    channelStyle: planned.channelStyle,
    retryFailed: options.retryFailed === true,
    // 契約 v54 から: 本編の画（1枚の画と分割ページの各コマ）は、機械の QA の後、行を complete に
    // する前に本編の画（scene-image）工程の品質ループの合格が要る。未合格の行は止め、再生成しない。
    assetQualityGateFactory: ({ episodeId, canvasDir }) => createKoyaSceneImageAssetQualityGate({ contract, canvasDir, episodeId }),
    // 画の計画ができ、有料の本編の画を作り始める直前（人物の承認待ちを抜けた後）に知らせる。full はここで
    // 台詞の音声を画と並べて始める。計画は画の工程の後で書き戻す形（production と台本を足したもの）で渡す。
    ...(typeof runtime.onScenePlanReady === "function" ? {
      onScenePlanReady: ({ plan: scenePlan, planPath }) => runtime.onScenePlanReady({
        planPath,
        plan: {
          ...scenePlan,
          production: { ...(scenePlan?.production || {}), ...(planned.plan.production || {}) },
          sourceScript: {
  ...
  const awaitingAssetQuality = result.status === "awaiting-human-review";
  const awaitingRows = Object.values(result.ledger?.jobs || {})
    .filter((entry) => entry?.status === "awaiting-human-review")
    .map((entry) => ({
      jobId: entry.id,
      stage: entry.assetQuality?.stage || "scene-image",
      subjectId: entry.assetQuality?.subjectId || "",
      code: entry.assetQuality?.code || "",
      reason: entry.assetQuality?.reason || "",
      outputPath: entry.outputPath || "",
    }));
  const state = await updateState(planned.paths, {
    status: waiting ? "awaiting-character-approval" : failed ? "failed" : awaitingAssetQuality ? "awaiting-human-review" : "images-ready",
    currentStage: waiting ? "character-approval" : failed || awaitingAssetQuality ? "images" : "source-face-placement",
    ...
    knownRemainingIssues: waiting
      ...
      : failed ? [{ id: "image-generation", detail: "One or more image jobs failed." }]
        : awaitingAssetQuality ? [{
          id: "scene-image-asset-quality",
          detail: `Pass the scene-image asset quality loop for every held image before it is used (no image is regenerated): ${awaitingRows.map((row) => row.code || row.jobId).join("; ")}.`,
          subjects: awaitingRows,
        }] : [],
  });
  ...
  const imageSummary = {
    ...
    paidImages: Number(summary.paidImages || 0),
    attempts: Number(summary.attempts || 0),
    retriedFailed: summary.retriedFailed || { requested: options.retryFailed === true, jobIds: [], count: 0, attempts: 0, completed: 0 },
    ...(awaitingAssetQuality ? { awaitingHumanReview: awaitingRows.length } : {}),
  };
  return {
    ...planned,
    result,
    ...
    failed,
    imageSummary,
    ...(awaitingAssetQuality ? { awaitingAssetQuality: true, assetQualityAwaiting: awaitingRows } : {}),
  };
}
...
export async function approveKoyaCharacterCandidate(options = {}) {
  ...
  // 契約 v54 から: 人が選んだ顔を承認一覧へ載せ（各シートのループの参照の照合に使う）、登録の前に
  // 各シートの品質ループが要ることを残作業として示す。ループを回すのは別文脈の評価者と人。
  const contract = await koyaContractForAssetQuality(options, projectDir);
  let assetQuality = null;
  if (koyaAssetQualityGateInForce(contract)) {
    const approvedReferences = await refreshKoyaApprovedReferences({ projectDir, canvasDir: paths.canvasDir });
    const { subjects, rows } = await checkKoyaIdentityPackAssetQuality({ contract, canvasDir: paths.canvasDir, cast: staged.cast });
    assetQuality = {
      approvedReferencesPath: approvedReferences.path,
      approvedReferencesSha256: approvedReferences.sha256,
      subjects: subjects.map((subject, index) => ({
        role: subject.role,
        storyStage: subject.storyStage,
        subjectId: subject.subjectId,
        assetFile: subject.assetFile,
        pass: rows[index].pass,
        reason: rows[index].reason,
      })),
    };
  }
  const pendingAssetQuality = assetQuality ? assetQuality.subjects.filter((subject) => !subject.pass) : [];
  const state = await updateState(paths, {
    status: "character-identity-review-required",
    currentStage: "character-identity-review",
    knownRemainingIssues: [
      { id: "character-identity-review", detail: `Review the real turnaround and every expression cell at ${staged.identityReviewDraftPath}, then register the character.` },
      ...(pendingAssetQuality.length > 0 ? [{
        id: "character-asset-quality",
        detail: `Before registration, pass the character asset quality loop for every identity-pack sheet (reference: the selected face, approved list ${assetQuality.approvedReferencesPath}): ${pendingAssetQuality.map((subject) => subject.subjectId).join(", ")}.`,
        subjects: pendingAssetQuality.map((subject) => ({ stage: "character", subjectId: subject.subjectId, assetFile: subject.assetFile, reason: subject.reason })),
      }] : []),
    ],
  });
  return {
...
/**
 * 途中の成果物の品質ループを見るときの制作契約。解決済みの契約を渡されればそれ、無ければこの回の
 * 解決済み契約（ランタイムの今の版と、あれば回の例外）で決める。渡し忘れでゲートが外れる道を残さない。
 */
async function koyaContractForAssetQuality(options = {}, projectDir) {
  const given = options.productionContract;
  if (given) return given.contract || given;
  return (await resolveKoyaMangaProductionContract({
    projectDir,
    episodeId: options.episodeId,
    contractPath: options.contractPath,
    overridePath: options.overridePath,
...
/**
 * identity pack の各シート（三面図・表情・開眼・衣装）の品質ループの対象。
 * 選ばれた顔（匿名の候補から人が選んだもの）は各シートの参照（身元の基準）なのでここに入れない。
 */
export function koyaIdentityPackAssetQualitySubjects(cast) {
  const pack = cast?.identityPack || {};
  const rows = [];
  const add = (role, sheet, storyStage = "") => {
    if (!nonEmpty(sheet?.assetFile)) return;
    rows.push({
      role,
      storyStage,
      subjectId: koyaAssetQualitySubjectId(cast.id, role, storyStage),
      assetFile: resolve(sheet.assetFile),
      sha256: nonEmpty(sheet.sha256),
    });
  };
  add("turnaround", pack.turnaround);
  add("expression", pack.expression);
  for (const sheet of identityPackEyeOpenSheets(pack)) add("eye-open", sheet, nonEmpty(sheet?.storyStage));
  for (const sheet of pack.outfitSheets || []) add("outfit", sheet, nonEmpty(sheet?.storyStage));
  return rows;
}

/** identity pack の全シートのループを確かめる。合格した版と、登録に写す staged の SHA も一致させる。 */
async function checkKoyaIdentityPackAssetQuality({ contract, canvasDir, cast }) {
  const subjects = koyaIdentityPackAssetQualitySubjects(cast);
  const rows = [];
  for (const subject of subjects) {
    const checked = await checkKoyaAssetQuality({
      contract,
      workDir: koyaAssetQualityWorkDir({ canvasDir }),
      stage: "character",
      subjectId: subject.subjectId,
      assetPath: subject.assetFile,
    });
    rows.push(checked.pass && checked.assetSha256 && subject.sha256 && checked.assetSha256 !== subject.sha256
      ? {
        ...checked,
        pass: false,
        reason: "asset-sha-mismatch",
        code: koyaAssetQualityFailureCode("character", subject.subjectId, "asset-sha-mismatch"),
        detail: "identity pack に staged したシートと、品質ループで合格した版が違う",
      }
      : checked);
  }
...
export async function registerKoyaCharacterIdentity(options = {}) {
  ...
  const paths = episodePaths(projectDir, options.episodeId);
  // 契約 v54 から: identity pack の各シートは、人物（character）工程の品質ループの合格（別文脈の評価者の
  // 採点・同一性と手指の人の確認・合格した版と同じバイト列）が無いと登録しない。
  const contract = await koyaContractForAssetQuality(options, projectDir);
  let assetQualityApproval = null;
  if (koyaAssetQualityGateInForce(contract)) {
    const store = await readCharacterWorkflowStore({ projectDir, canvasDir: paths.canvasDir });
    const workflow = getCharacterWorkflow(store, options.workflowId);
    const cast = workflow ? findWorkflowCast(workflow, options.castId) : null;
    if (cast?.identityPack) {
      const { subjects, rows } = await checkKoyaIdentityPackAssetQuality({ contract, canvasDir: paths.canvasDir, cast });
      assertKoyaAssetQualityRows(rows, `Koya character ${cast.name || cast.id} registration`);
      assetQualityApproval = {
        version: KOYA_ASSET_QUALITY_GATE_VERSION,
        contractVersion: contract.version,
        rows: subjects.map((subject) => ({ stage: "character", subjectId: subject.subjectId, assetSha256: subject.sha256 })),
      };
    }
    // identity pack が無い（staged されていない）ときは、下の登録が理由つきで止める。
  }
  const finalized = await finalizeApprovedCharacter({
    ...
    ...(assetQualityApproval ? { assetQualityApproval } : {}),
  });
  // 登録した人物の参照画を、以降の本編の画・サムネのループが照合に使う承認一覧へ載せる。
  const approvedReferences = assetQualityApproval
    ? await refreshKoyaApprovedReferences({ projectDir, canvasDir: paths.canvasDir })
    : null;
  ...
  return { episodeId: options.episodeId, finalized, showBibleSync, state, ...(approvedReferences ? { approvedReferences } : {}) };
}
...
export function koyaEpisodeManifestPipelineOptions({
  ...
  return {
    ...
    model: resolveKoyaDialogueAdapter(contract).model,
    motion: "left-only",
...
/**
 * Where the refusal message points: this project, this Job, and its audition
 * files. With the resolved contract the guard also checks that every voice
 * belongs to the contract's dialogue provider, and a non-ElevenLabs contract
 * is pointed at the Koya voice-audition route.
 */
function koyaVoiceSelectionMessageOptions({ projectDir, jobId = "", jobProjectDir = "", resolved = null } = {}) {
  const root = resolve(projectDir || process.cwd());
  let dialogueAdapter = null;
  if (resolved) {
    try {
      dialogueAdapter = resolveKoyaDialogueAdapter(resolved);
    } catch {
      // An invalid adapter is refused by the stage that resolves the contract.
    }
  }
  return {
    projectDir: root,
    ...
    koyaAuditionPaths: (registryEpisodeId) => koyaVoiceAuditionPaths({ projectDir: root, episodeId: registryEpisodeId }),
...
export async function assertKoyaSpeechVoiceSelections({
  ...
  return assertKoyaHumanVoiceSelections({
    ...
    ...koyaVoiceSelectionMessageOptions({ projectDir: root, jobId, jobProjectDir, resolved }),
  });
}
...
export async function explainKoyaUnvoicedProtagonist(error, {
  ...
  try {
    assertKoyaHumanVoiceSelections({
      ...
      validationCanary,
      narrationVoicePolicy: "protagonist-voice",
      ...koyaVoiceSelectionMessageOptions({ projectDir: paths.projectDir, jobId, jobProjectDir, resolved }),
    });
  } catch (guardError) {
...
export async function assertKoyaVoiceSelectionsBeforeImages(options = {}, context = {}) {
  ...
  try {
    ...
    resolved = await resolveKoyaMangaProductionContract({
      projectDir,
      episodeId,
      contractPath: options.contractPath,
      overridePath: options.overridePath,
    ...
    manifest = applyKoyaValidationCanaryVoiceProfiles(manifest, validationCanary, registry, canary.showBible, resolveKoyaDialogueAdapter(resolved));
  ...
  try {
    const audit = assertKoyaHumanVoiceSelections({
      ...
      validationCanary,
      narrationVoicePolicy,
      ...koyaVoiceSelectionMessageOptions({ projectDir, jobId: context.jobId, jobProjectDir: context.jobProjectDir, resolved }),
    });
 ...
 */
/**
 * 台本由来の対話の定義（発話・cut・声）に Koya の声の規則を当てる。prepare（画像から作る正本の manifest）と、
 * 画像と重ねて走らせる音声の入力（buildKoyaDialogueDefinition）が同じ関数を通るので、両者の音声の入力は
 * 食い違わない。声が決まらないときの説明と止め方（状態ファイルに残すか）は呼び出し側が渡す。
 */
async function applyKoyaSpeechDefinition(manifestInput, {
  plan,
  resolved,
  projectDir,
  episodeId,
  protagonistSpeakerId = "",
  explainUnvoicedProtagonist = async (error) => error,
  assertSpeakersVoiced = async () => null,
} = {}) {
  ...
  manifest = applyKoyaValidationCanaryVoiceProfiles(manifest, validationCanary, registry, authority.showBible, resolveKoyaDialogueAdapter(resolved));
  const protagonistRequest = protagonistSpeakerId
  ...
  try {
    ...
    throw await explainUnvoicedProtagonist(error, { manifest, protagonistRequest, registry, validationCanary });
  }
  ...
  await assertSpeakersVoiced(manifest, { registry, validationCanary });
  for (const utterance of manifest.utterances || []) {
...
/** 画像と重ねて走らせる音声の置き場（正本の manifest・状態ファイルとは別のファイル）。 */
export function koyaSpeechOverlapPaths(paths) {
  return {
    definitionPath: join(paths.episodeDir, "koya-dialogue-overlap-definition.json"),
    reportPath: join(paths.episodeDir, "koya-dialogue-overlap-generation.json"),
  };
}

/**
 * 画像を待たずに、台本から音声の入力（対話の定義）だけを作る。prepare と同じ解析・同じ声の規則を通すが、
 * 画像・吹き出しの配置・顔の検出には触れず、ファイルも状態も書かない。声が決まらない話者がいれば、状態に
 * 何も残さずに例外で止める（その場合は prepare が従来どおり止め方を決める）。
 */
export async function buildKoyaDialogueDefinition({
  projectDir,
  episodeId,
  scriptPath = "",
  plan,
  resolved,
  protagonistSpeakerId = "",
  outerJobBinding = null,
} = {}) {
  const root = resolve(projectDir || process.cwd());
  const created = await createEpisodeDialogueDefinition(koyaEpisodeManifestPipelineOptions({
    projectDir: root,
    episodeId,
    scriptPath,
    plan,
  ...
  const manifest = await applyKoyaSpeechDefinition(created.manifest, {
    plan,
    resolved,
    projectDir: root,
    episodeId,
    protagonistSpeakerId,
    assertSpeakersVoiced: async (current) => {
      const unvoiced = (current?.utterances || []).filter((row) => !isKoyaNarrationRow(row) && !nonEmpty(row.voiceId));
      if (unvoiced.length > 0) {
        throw Object.assign(new Error("Speech overlap skipped: a speaker has no voice yet; prepare decides how to stop."), {
          code: "koya-speech-overlap-unvoiced",
        });
      }
      return null;
    },
  ...
  if (outerJobBinding) {
    manifest.production = { ...(manifest.production || {}), outerJobBinding: structuredClone(outerJobBinding) };
  }
  return manifest;
}
...
export async function createKoyaEpisodeManifest(options = {}, runtime = {}) {
  ...
  let manifest = await applyKoyaSpeechDefinition(created.manifest, {
    plan,
    resolved,
    projectDir,
    episodeId: options.episodeId,
    protagonistSpeakerId: options.protagonistSpeakerId,
    explainUnvoicedProtagonist: (error, context) => explainKoyaUnvoicedProtagonist(error, {
      ...context,
      resolved,
      paths,
      episodeId: options.episodeId,
    ...
    assertSpeakersVoiced: (current, context) => assertKoyaPreparedSpeakersVoiced(current, {
      ...context,
      narrationVoicePolicy: resolved.contract.audio.narrationVoicePolicy,
      resolved,
      paths,
...
export const KOYA_SPEECH_OVERLAP_VERSION = "koya-speech-overlap-v1";

/**
 * 画像の生成と重ねて、台詞の音声を先に作る（full の中だけで使う）。
 *
 * 音声の入力は台本由来の対話の定義（buildKoyaDialogueDefinition）だけで決まり、画像には依らない。画像の
 * 計画ができて有料の本編の画の生成が始まる直前（人物の承認待ちを抜けた後）に始め、画像の工程と並べて走らせる。
 * 書くのは重ねた工程の置き場（koyaSpeechOverlapPaths）と、音声の工程がもともと自分で持つテイク・音・
 * alignment だけで、正本の manifest（episode-manifest.json）と状態ファイル（koya-production-state.json）には
 * 触らない。正本へ入れるのは prepare の後の speech の collector（adoptFrom）だけ。
 *
 * 有料の声は端末全体の paid-speech の枠の中で取る（画像は paid-image の枠）。有料の呼び出しの前の関門
 * （doctor が測った adapter、声の人選）は正本の工程と同じものを通すが、止まっても状態に何も残さない
 * （正本の工程が同じ理由で改めて止め、止め方を記録する）。例外は投げずに結果の status で返す。
 */
export async function runKoyaOverlappedSpeech(options = {}, { preflight = null, plan = null, runtime = {} } = {}) {
  const startedAt = new Date().toISOString();
  const projectDir = resolve(options.projectDir || process.cwd());
  const paths = episodePaths(projectDir, options.episodeId);
  const overlapPaths = koyaSpeechOverlapPaths(paths);
  const settle = (status, extra = {}) => ({
    version: KOYA_SPEECH_OVERLAP_VERSION,
    status,
    startedAt,
    finishedAt: new Date().toISOString(),
    definitionPath: overlapPaths.definitionPath,
    reportPath: overlapPaths.reportPath,
    mediaJobStateDir: join(paths.episodeDir, ".koya-dialogue-source", "paid-media-jobs"),
    mediaJobs: [],
    ...extra,
  });
  let resolved;
  ...
  try {
    ...
    assertKoyaDoctorMeasuredContractAdapter(preflight, resolved);
    ...
    // 正本の speech と同じ入力の形にする（契約と人物設定の読み上げの指示を当て直す）。
    const characterBiblePath = options.characterBiblePath || plan?.production?.characterBiblePath || "";
    const characterBible = characterBiblePath ? await readJson(resolve(characterBiblePath)) : null;
    const contracted = applyKoyaContractToManifest(definition, resolved);
    manifest = characterBible ? applyKoyaCharacterBibleSpeechDirectives(contracted, characterBible) : contracted;
  } catch (error) {
    return settle("skipped", { reason: String(error?.code || error?.message || error).slice(0, 300) });
  }
  const runDialogueSpeech = runtime.generateDialogueSpeech || generateKoyaDialogueSpeech;
  try {
    await mkdir(paths.episodeDir, { recursive: true });
    await writeJsonAtomic(overlapPaths.definitionPath, manifest);
    const result = await runDialogueSpeech({
      // 声の人選の関門は正本の工程と同じ判定。止めても状態には残さない（正本の工程が記録する）。
      beforePaidSpeech: async ({ manifest: current } = {}) => assertKoyaSpeechVoiceSelections({
        projectDir,
        episodeId: options.episodeId,
        manifest: current,
        resolved,
      ...
      ...koyaDialogueSpeechArguments(options, { projectDir, paths, resolved }),
      manifest,
      manifestPath: overlapPaths.definitionPath,
      reportPath: overlapPaths.reportPath,
    });
    const mediaJobs = Array.isArray(result.report?.mediaJobs) ? result.report.mediaJobs : [];
    const status = result.waiting
      ? "waiting-usage-limit"
      : result.cancelled
        ? "cancelled"
        : result.awaitingAssetQuality
          ? "awaiting-human-review"
          : result.partial ? "partial" : "complete";
    return settle(status, {
      mediaJobs,
      completedCutIds: result.report?.completedCutIds || [],
      pendingCutIds: result.report?.pendingCutIds || [],
    });
  } catch (error) {
    return settle(error?.code === KOYA_VOICE_SELECTION_REQUIRED_CODE ? "skipped" : "failed", {
      reason: String(error?.code || error?.message || error).slice(0, 300),
      mediaJobs: Array.isArray(error?.mediaJobs) ? error.mediaJobs : [],
    });
  }
}

/**
 * 台詞の音声の依頼に渡す、正本の工程と重ねた工程で共通の引数。重ねた工程だけ別の値にすると、
 * 同じ台本から別のテイクが作られて取り込めなくなる（または取り込みが別の入力の音を拾う）。
 */
function koyaDialogueSpeechArguments(options, { projectDir, paths, resolved }) {
  return {
    projectDir,
    canvasDir: paths.canvasDir,
    contract: resolved,
...
export async function generateKoyaMangaSpeech(options = {}, runtime = {}) {
  ...
  const result = await runDialogueSpeech({
    ...
    // 画像と重ねて先に作った音声（同じ Job のこの回）。承認済みの cut だけを取り込む。
    ...(options.adoptOverlappedSpeech ? { adoptFrom: options.adoptOverlappedSpeech } : {}),
  });
  if (result.waiting) {
    await updateState(paths, {
  ...
  if (result.cancelled || result.partial) {
    const status = result.cancelled
      ? "speech-checkpointed"
      : result.awaitingAssetQuality ? "awaiting-human-review" : "speech-partial";
    const state = await updateState(paths, {
      status,
      currentStage: "speech",
...
export async function renderKoyaMangaVideo(options = {}) {
  ...
  // fallback; anything else stops before ffmpeg starts. From contract v55 the
  // clip must also have passed its video-clip asset quality loop.
...
/**
 * 画像工程で止まったときに、次に打つコマンドの全文を返す。
 * 失敗行が残っている場合、同じ Job の resume に --retry-failed-images を付けると、
 * その行だけを同じ制作のまま作り直せる（jobId の指紋には入らない実行時の引数。
 * 使った事実は台帳の summary.retriedFailed と Job／Receipt に残る）。
 * 今までは短い一行しか出ず、深夜に1人では次の一手が打てなかった。
 */
/** 案内に載せるシェル引数。声ゲート（koyaVoiceSelectionGuard）と同じ最小の引用。 */
function shellArgumentForGuidance(value) {
  const text = String(value ?? "");
  return /^[A-Za-z0-9_./:@%+=,-]+$/u.test(text) ? text : `'${text.replaceAll("'", "'\\''")}'`;
}

export function koyaImagePauseGuidance(imageResult, jobContext = {}) {
  const jobId = nonEmpty(jobContext?.jobId);
  const jobProjectDir = nonEmpty(jobContext?.jobProjectDir);
  const resume = [
    "node scripts/run-video-harness.mjs resume",
    `--job-id ${jobId ? shellArgumentForGuidance(jobId) : "<job id>"}`,
    `--project-dir ${jobProjectDir ? shellArgumentForGuidance(jobProjectDir) : "<project that holds the Job>"}`,
    "--confirmed",
  ];
  const summary = imageResult?.imageSummary || {};
  const lines = [];
  if (imageResult?.failed) {
    lines.push(
      `${Number(summary.failed || 0)} image job(s) failed and stay failed until you ask for a retry; completed images (${Number(summary.complete || 0)}) are reused as-is.`,
      "Retry only the failed images inside the same Job (no re-billing of completed work):",
      `  ${[...resume, "--retry-failed-images"].join(" ")}`,
      `Ledger: ${nonEmpty(imageResult?.state?.imageLedgerPath) || "<image ledger>"}`,
    );
  } else if (imageResult?.waiting) {
    lines.push(
      "Image generation is waiting for a human decision (see knownRemainingIssues). After recording it, resume the same Job:",
      `  ${resume.join(" ")}`,
    );
  } else if (imageResult?.awaitingAssetQuality) {
    const held = Array.isArray(imageResult.assetQualityAwaiting) ? imageResult.assetQualityAwaiting : [];
    lines.push(
      `${held.length} scene image(s) passed the machine QA but are held until their scene-image asset quality loop passes; nothing is regenerated.`,
      "Run each loop from another context (node scripts/asset-quality-loop.mjs start/sheet/record/verify --work-dir <canvas> --harness koya-manga-video --stage scene-image --subject <subject>), then resume the same Job:",
      ...held.map((row) => `  - ${row.subjectId || row.jobId}: ${row.outputPath}`),
      `  ${resume.join(" ")}`,
    );
  }
  return lines;
...
// ---------------------------------------------------------------------------------------------
// 台本の関門（契約 v56 から）。規則の純粋な部分は lib/koyaScriptQualityGatePolicy.mjs、答えは
// lib/scriptQualityLoop.mjs の scriptQualityVerdict の1か所、作業フォルダの決め方・止め方・次のコマンドは
// ナレーション物語と共通の lib/scriptQualityUseGate.mjs。漫画の台本は依頼者が書くので、通常は
// script-quality-loop.mjs accept-human（人がそのまま使うと認めた記録）で通る。
// ---------------------------------------------------------------------------------------------

/**
 * 使う台本（options.scriptPath のバイト列）について台本の品質ループの答えを問う。何も書かない
 * （plan の表示と、下の関門が使う）。効力の外の契約（v55 以前・版の無い合成の契約）では問わない。
 * 作業フォルダは options.scriptQualityWorkDir（上位 Job の options から子へ渡る）、無ければ台本のあるフォルダ。
 */
export async function inspectKoyaScriptQuality(options = {}, context = {}) {
  const projectDir = resolve(options.projectDir || process.cwd());
  const scriptPath = nonEmpty(options.scriptPath);
  if (!scriptPath) throw new Error("The script quality gate needs --script-path (the exact script this episode uses).");
  const resolved = context.resolvedContract || await resolveKoyaMangaProductionContract({
    projectDir,
    episodeId: options.episodeId,
    contractPath: options.contractPath || undefined,
    overridePath: options.overridePath || undefined,
  });
  const contractVersion = nonEmpty(resolved?.contract?.version);
  if (!koyaScriptQualityGateInForce(resolved)) {
    return { applicable: false, pass: true, contractVersion, resolved, reasonCode: "", issues: [], next: [], record: null };
  }
  const gate = await checkScriptQualityBeforeProduction({
    workDir: scriptQualityWorkDirFor({
      options: nonEmpty(options.scriptQualityWorkDir) ? { scriptQualityWorkDir: options.scriptQualityWorkDir } : {},
      scriptPath,
    }),
    scriptPath,
    genre: KOYA_SCRIPT_QUALITY_GENRE,
    // 直接の呼び出しでは台本そのものが作業フォルダの中にある（上位 Job の写しなら外なので、置き場の説明になる）。
    commandScriptPath: scriptPath,
    ...(context.verdict ? { verdict: context.verdict } : {}),
  });
  let scriptTextSha256 = "";
  try {
    scriptTextSha256 = koyaScriptTextSha256(await readFile(resolve(scriptPath), "utf8"));
  } catch {
    scriptTextSha256 = "";
  }
  const { version: useVersion, ...evidence } = gate.evidence;
  const record = {
    version: KOYA_SCRIPT_QUALITY_RECORD_VERSION,
    useVersion,
    contractVersion,
    ...evidence,
    // 最終監査が回の台本（manifest の台本の生テキスト）と照らす SHA。画像計画の scriptSha256 と同じ定義。
    scriptTextSha256,
  };
  return {
    applicable: true,
...
/**
 * 有料の処理（本編の画・台詞の音声・動画クリップ）の前の関門。答えを Koya の状態ファイルの scriptQuality に
 * 残し（最終監査と Canvas が読む。完成した回の状態は書き換えない）、合格でなければ
 * KOYA_SCRIPT_QUALITY_REQUIRED で止める（full は終了コード 3 の人待ちにする）。
 */
export async function assertKoyaScriptQualityBeforeProduction(options = {}, context = {}) {
  if (!options.episodeId) throw new Error("episodeId is required for the script quality gate.");
  // 上位 Job から呼ばれたときは、渡された作業フォルダが Job の options（識別子に入る）と同じであること。
  if (nonEmpty(options.upstreamJobPath)) {
    let job = null;
    try { job = await readJson(resolve(options.upstreamJobPath)); } catch { job = null; }
    assertScriptQualityWorkDirBoundToJob({ job, scriptQualityWorkDir: options.scriptQualityWorkDir });
  }
  const inspected = await inspectKoyaScriptQuality(options, context);
  if (!inspected.applicable) return inspected;
  const paths = episodePaths(resolve(options.projectDir || process.cwd()), options.episodeId);
  let previous = null;
  try { previous = await readJson(paths.statePath); } catch { previous = null; }
  const completed = nonEmpty(previous?.status) && previous.status === inspected.resolved?.contract?.lifecycle?.completionStatus;
  if (!completed && JSON.stringify(previous?.scriptQuality || null) !== JSON.stringify(inspected.record)) {
    await updateState(paths, { episodeId: options.episodeId, scriptQuality: inspected.record });
  }
  if (inspected.pass) return inspected;
  const error = new Error([
    `Koya stopped before any paid work: the script is not accepted by the script quality loop (${inspected.reasonCode}).`,
    ...inspected.next,
  ].join("\n"));
  error.code = KOYA_SCRIPT_QUALITY_REQUIRED_CODE;
  error.reasonCode = inspected.reasonCode;
  error.issues = [...inspected.issues];
  error.next = [...inspected.next];
  error.workDir = inspected.workDir;
  throw error;
}

/**
 * 台本の関門で止まったときの人待ち（終了コード 3）。失敗にすると終端になり、依頼者の台本を accept-human で
 * 受け入れたあとに同じ Job を resume できなくなる。外側の Job へ残るのは knownRemainingIssues と文字列の next。
 */
export function koyaScriptQualityPauseResult(error, { preflight, projectDir, episodeId, stage }) {
  if (error?.code !== KOYA_SCRIPT_QUALITY_REQUIRED_CODE) return null;
  const next = Array.isArray(error.next) ? error.next.map(String) : [];
  return {
    exitCode: 3,
    preflight,
    payload: {
      episodeId,
      status: KOYA_SCRIPT_QUALITY_PAUSE_STATUS,
      waiting: true,
      stage,
      checkpoint: episodePaths(projectDir, episodeId).statePath,
      scriptQuality: {
        reasonCode: nonEmpty(error.reasonCode),
        workDir: nonEmpty(error.workDir),
        next,
      },
      knownRemainingIssues: Array.isArray(error.issues) && error.issues.length > 0
        ? error.issues.map(String)
        : [`script-quality-required:${nonEmpty(error.reasonCode) || "unknown"}`],
      next: next.join("\n"),
    },
  };
}
...
/**
 * 工程ごとの開始・終了を記録する（full の結果の stageTimings。外側の Job と RunReceipt が写す）。
 * 重ねた工程（speech-overlap）は、画の工程と時間が重なって記録される。
 */
export const KOYA_STAGE_TIMINGS_VERSION = "koya-stage-timings-v1";

function createKoyaStageTimer(now = () => Date.now()) {
  const rows = [];
  return {
    rows,
    record(id, startedAtMs, finishedAtMs, extra = {}) {
      rows.push({
        id,
    ...
    async track(id, action) {
      const startedAtMs = now();
      try {
        return await action();
      } finally {
        this.record(id, startedAtMs, now());
      }
    },
  };
}

function koyaSpeechOverlapSummary(overlap) {
  if (!overlap) return null;
  return {
    version: overlap.version,
    status: overlap.status,
    startedAt: overlap.startedAt,
    finishedAt: overlap.finishedAt,
    ...(overlap.reason ? { reason: overlap.reason } : {}),
    completedCutIds: overlap.completedCutIds || [],
    pendingCutIds: overlap.pendingCutIds || [],
    adoptedCutIds: overlap.adoptedCutIds || [],
  };
}
 ...
 *
 * 結果には工程ごとの所要時間（stageTimings）と、画と重ねた台詞の音声の結果（speechOverlap）を載せる。
 * 重ねた音声が有料の Media Job を作っていれば、画や prepare で止まった結果にも mediaJobs を載せる
 * （払ったものを Job が記録できるように）。
 */
async function runKoyaMangaFullProductionUnlocked(options = {}, runtime = {}) {
  const context = { timer: createKoyaStageTimer(runtime.stageClock), overlap: null };
  const result = await runKoyaMangaFullProductionStages(options, runtime, context);
  if (!result?.payload || typeof result.payload !== "object") return result;
  const payload = { ...result.payload, stageTimings: { version: KOYA_STAGE_TIMINGS_VERSION, stages: context.timer.rows } };
  const summary = koyaSpeechOverlapSummary(context.overlap);
  if (summary) {
    payload.speechOverlap = summary;
    const overlapJobs = Array.isArray(context.overlap.mediaJobs) ? context.overlap.mediaJobs : [];
    if (overlapJobs.length > 0) {
      const known = new Set((payload.mediaJobs || []).map((row) => row?.requestKey).filter(Boolean));
      payload.mediaJobs = [...(payload.mediaJobs || []), ...overlapJobs.filter((row) => !known.has(row?.requestKey))];
      if (!nonEmpty(payload.mediaJobStateDir)) payload.mediaJobStateDir = context.overlap.mediaJobStateDir;
    }
  }
  return { ...result, payload };
}

async function runKoyaMangaFullProductionStages(options = {}, runtime = {}, context = {}) {
  if (!options.episodeId) throw new Error("episodeId is required.");
  ...
  // 台本の関門（契約 v56 から）も同じ規則。有料の処理（画・台詞の音声・動画クリップ）の最初より前に問う。
  const checkScriptQuality = runtime.checkScriptQuality
    || (runtime.generateImages ? null : assertKoyaScriptQualityBeforeProduction);
  const projectDir = resolve(options.projectDir || process.cwd());
  ...
  let scriptQualityAnswer = null;
  if (checkScriptQuality) {
    try {
      scriptQualityAnswer = await checkScriptQuality(boundOptions, { ...jobContext, runtime, stage: "before-paid-work" });
    } catch (error) {
      const paused = koyaScriptQualityPauseResult(error, { preflight, projectDir, episodeId: options.episodeId, stage: "before-paid-work" });
  ...
  // 画と台詞の音声を重ねる。音声の入力は台本由来の対話の定義だけで決まるので、画の計画ができて有料の
  // 本編の画を作り始める直前に、音声を並べて始める（options.speechOverlap: false で従来の直列に戻せる）。
  // 重ねた音声は正本の manifest と状態ファイルに触らず、正本へ入れるのは下の speech の collector だけ。
  let overlapPromise = null;
  // 台本の関門は上で問うた。画の工程は同じ答えを使い、二度問わない（間に答えが変わっても、この回は上の答えで進む）。
  const imageScriptQuality = checkScriptQuality ? { checkScriptQuality: async () => scriptQualityAnswer } : {};
  const imageRuntime = options.speechOverlap === false ? { ...imageScriptQuality } : {
    ...imageScriptQuality,
    onScenePlanReady: ({ plan }) => {
      if (overlapPromise) return;
      const startOverlap = runtime.runSpeechOverlap || runKoyaOverlappedSpeech;
      overlapPromise = Promise.resolve()
        .then(() => startOverlap(boundOptions, { preflight, plan, runtime }))
        .catch((error) => ({
          version: KOYA_SPEECH_OVERLAP_VERSION,
          status: "failed",
  ...
  let imageError = null;
  try {
    imageResult = await context.timer.track("images", () => runImages(boundOptions, imageRuntime));
  } catch (error) {
    imageError = error;
  }
  // 画の工程がどう終わっても、重ねた音声が決着するまで待つ（full の錠を持ったまま、音声を宙に浮かせない）。
  if (overlapPromise) {
    context.overlap = await overlapPromise;
    const started = Date.parse(context.overlap.startedAt || "");
    const finished = Date.parse(context.overlap.finishedAt || "");
    if (Number.isFinite(started) && Number.isFinite(finished)) {
      context.timer.record("speech-overlap", started, finished, { overlapsWith: "images", status: context.overlap.status });
    }
  }
  if (imageError) {
    const paused = wardrobePauseFor(imageError, "images", options.episodeId);
    if (paused) return paused;
    throw imageError;
  }
  const episodeId = imageResult.episodeId || options.episodeId;
  if (imageResult.waiting || imageResult.failed || imageResult.awaitingAssetQuality) {
    return {
      exitCode: 3,
      preflight,
      payload: {
        ...
        imageSummary: imageResult.imageSummary || null,
        knownRemainingIssues: imageResult.state?.knownRemainingIssues || [],
        next: koyaImagePauseGuidance(imageResult, jobContext),
      },
    };
  }
  let prepared;
  try {
    prepared = await context.timer.track("prepare", () => prepareManifest({ ...boundOptions, episodeId, jobProjectDir: jobContext.jobProjectDir }));
  } catch (error) {
  ...
  let speech;
  try {
    // 重ねた音声があれば、承認済みの cut だけを collector が正本へ取り込み、残りを従来どおり作る。
    const adoptOverlappedSpeech = context.overlap && context.overlap.status !== "skipped" && nonEmpty(context.overlap.definitionPath)
      ? { manifestPath: context.overlap.definitionPath, reportPath: context.overlap.reportPath }
      : null;
    speech = await context.timer.track("speech", () => runSpeech({
      ...boundOptions,
      episodeId,
      ...(adoptOverlappedSpeech ? { adoptOverlappedSpeech } : {}),
    }));
    if (context.overlap) context.overlap.adoptedCutIds = speech?.overlapAdoption?.adoptedCutIds || [];
  } catch (error) {
  ...
  // 音声の有料 Media Job の journal は子の作業場（<sourceDir>/paid-media-jobs）にある。
  // 外側の Job が failed からの resume で recover を試みるとき、既定の journal
  // （~/.buzzassist/media-jobs）を見ても行が無い。場所を結果に載せて、親が記録できるようにする。
  const mediaJobStateDir = nonEmpty(speech.paths?.sourceDir) ? join(speech.paths.sourceDir, "paid-media-jobs") : "";
  const imageSummary = imageResult.imageSummary || null;
    ...
    return {
      ...
      payload: {
        ...
        status: speech.waiting
          ...
          : speech.cancelled
            ? "speech-checkpointed"
            : speech.awaitingAssetQuality ? "awaiting-human-review" : "speech-partial",
  ...
  const substituted = await context.timer.track("video-substitution", () => runVideoSubstitution({ ...boundOptions, episodeId }, runtime));
  ...
  const rendered = await context.timer.track("render", () => runRender({ ...boundOptions, episodeId }));
  const audited = await context.timer.track("audit", () => auditFinal({
    projectDir: resolve(options.projectDir || process.cwd()),
 ...
 */
export const KOYA_FULL_RUN_LOCK_HELD_STATUS = "full-run-lock-held";

/**
 * フルラン錠が取れないときは、失敗ではなく人待ち（exit 3）で止める。
 * 錠は待ち時間ゼロで、lib/canvasFileLock.mjs は「2分以上古く、かつ持ち主が死んでいる」
 * ときしか外さない。親だけが落ちて子が生き残っている状態で resume すると、ここで
 * 即 throw → 外側の Job が failed になり、払い終えた画像の行が Job に残らない
 * （Koya の throw 経路は Media Job の行を返さない）。人待ちにして、同じ Job の resume で
 * 先へ進めるようにする。
 */
export function koyaFullRunLockPauseResult(error, { lockTarget, episodeId, jobContext = {} }) {
  if (!/Timed out waiting for canvas write lock/u.test(String(error?.message || ""))) return null;
  const resume = [
    "node scripts/run-video-harness.mjs resume",
    `--job-id ${nonEmpty(jobContext?.jobId) ? shellArgumentForGuidance(jobContext.jobId) : "<job id>"}`,
    `--project-dir ${nonEmpty(jobContext?.jobProjectDir) ? shellArgumentForGuidance(jobContext.jobProjectDir) : "<project that holds the Job>"}`,
    "--confirmed",
  ].join(" ");
  return {
    exitCode: 3,
    payload: {
      episodeId,
      status: KOYA_FULL_RUN_LOCK_HELD_STATUS,
      waiting: true,
      checkpoint: "",
      lockPath: lockTarget,
      knownRemainingIssues: [{ id: "full-run-lock", detail: `Another full run of this episode holds the lock: ${lockTarget}` }],
      next: [
        `Another process holds the full-run lock for ${episodeId}: ${lockTarget}`,
        "If a previous run is still generating (check it before touching anything), let it finish and resume this Job afterwards.",
        "If nothing is running, the lock is released automatically once it is older than 2 minutes and its owner process is gone; then resume the same Job (no image is re-billed):",
        `  ${resume}`,
      ],
    },
  };
...
export async function runKoyaMangaFullProduction(options = {}, runtime = {}) {
  ...
  try {
    return await withFullRunLock(
      lockTarget,
      () => runKoyaMangaFullProductionUnlocked({ ...options, projectDir }, runtime),
      { timeoutMs: 0 },
    );
  } catch (error) {
    const paused = koyaFullRunLockPauseResult(error, {
      lockTarget,
      episodeId: options.episodeId,
      jobContext: {
        jobId: nonEmpty(options.upstreamJobId),
        // 外側 Job の job.json は <project>/canvas/harness-runs/<jobId>/job.json に置かれる。
        jobProjectDir: nonEmpty(options.upstreamJobPath) ? resolve(dirname(resolve(options.upstreamJobPath)), "..", "..", "..") : "",
      },
    });
    if (paused) return paused;
    throw error;
