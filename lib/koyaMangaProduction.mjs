...
async function validateSettingSheetFindingRepair({ projectDir, findingsPath, repairPlanPath, cast } = {}) {
  ...
  const roleAssets = [
    ...
    { role: "eye-open", path: cast.identityPack?.eyeOpen?.assetFile },
...
export async function repackKoyaCharacterIdentityPack(options = {}) {
  ...
  const unsupported = failedReview.failedRoles.filter((role) => !["turnaround", "expression", "eye-open"].includes(role));
  ...
  const roleConfig = {
    ...
    ...(cast.identityPack.eyeOpen?.assetFile
      ? { "eye-open": { asset: cast.identityPack.eyeOpen, columns: 2, rows: 2 } }
      : {}),
  };
