...
export async function withCanvasFileLock(filePath, callback, options = {}) {
  ...
  while (!handle) {
    try {
      ...
      // Windows では、別のプロセスが消したばかりの錠ファイル（削除待ち）を開くと EEXIST ではなく
      // EPERM / EACCES が返る（CI の windows-latest で観測、2026-09-25）。一時的なので、同じ待ち時間の
      // 上限の中で待って取り直す（lib/paidMediaJobBroker.mjs・lib/machineSlots.mjs と同じ扱い）。
