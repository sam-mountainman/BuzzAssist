...
//   canvas/quality/assets/*.json（と作業場の直下の quality/assets/）  途中の成果物の品質ループ（lib/assetQualityLoop.mjs）
//
// 守ること:
...
async function readAssetQualityIndex(workspace, notes) {
  ...
  // Koya の品質ループの作業フォルダは作業場の canvas/（lib/koyaAssetQualityGatePolicy.mjs の
  // KOYA_ASSET_QUALITY_WORK_DIR）。作業場の直下に置いた記録も従来どおり読む（先に読んだ方を優先）。
  for (const workDir of [path.join(workspace, "canvas"), workspace]) {
    let listed;
    try {
