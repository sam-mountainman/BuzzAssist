...
//   canvas/manga-videos/<話数>/koya-production-state.json   Koya の工程の状態（scriptQuality: 台本の確認の答え）
 ...
 * 台本の確認（制作契約 v56 から）は有料の処理の最初（本編の画）より前に止める。
 */
export const KOYA_PROGRESS_DAG = Object.freeze([
  Object.freeze({ id: "doctor", title: "事前点検（doctor）", needs: [] }),
  Object.freeze({ id: "script-quality", title: "台本の確認", needs: ["doctor"] }),
  ...
  Object.freeze({ id: "images", title: "本編の画", needs: ["script-quality", "characters", "wardrobe", "voice-selection"] }),
...
export const KOYA_PROGRESS_ORDER = Object.freeze([
  "doctor", "script-quality", "voice-selection", "wardrobe", "characters", "images", "layout",
...
const STATUS_TO_NODE = Object.freeze([
  [/^awaiting-script-quality$/u, "script-quality"],
...
export function deriveKoyaProgressDag(job, evidence = {}) {
  ...
  // 台本の確認: full が有料の処理の前に問うた答え（状態ファイルの scriptQuality）。理由コードと受け入れ方だけを出す。
  const scriptQuality = plainObject(evidence.state?.scriptQuality) ? evidence.state.scriptQuality : null;
  if (scriptQuality) {
    const reasonCode = nonEmpty(scriptQuality.reasonCode);
    if (scriptQuality.pass === true) {
      detail.set("script-quality", scriptQuality.acceptedBy === "human" ? "人がそのまま使うと認めた台本" : "台本の品質ループに合格");
      if (!codes.includes("awaiting-script-quality")) explicit.set("script-quality", "pass");
    } else {
      detail.set("script-quality", `未確認（${reasonCode || "理由不明"}）`);
      explicit.set("script-quality", "awaiting-human-review");
    }
  }
