...
//   quality/assets/*.json                                    途中の成果物の品質ループ（lib/assetQualityLoop.mjs）
//
// 守ること:
...
export function deriveKoyaProgressDag(job, evidence = {}) {
  ...
  else if (ledgerStatus === "waiting") explicit.set("images", "awaiting-human-review");
...
async function readAssetQualityIndex(workspace, notes) {
  ...
  let listed;
  try {
    listed = await listAssetQualityStatus({ workDir: workspace });
  } catch {
  ...
  for (const entry of listed.entries) {
    let status;
    try {
      status = await assetQualityStatus({ workDir: workspace, stage: entry.stage, subjectId: entry.subjectId });
    } catch {
      continue;
    }
    ...
    if (SHA256.test(digest)) index.bySha.set(`${entry.stage}:${digest}`, record);
    index.bySubject.set(`${entry.stage}:${entry.subjectId}`, record);
  }
  return index;
}
...
async function sceneImageItems({ workspace, reader, plan, ledger, quality, running }) {
  ...
  for (const page of pages) {
    ...
    const entry = grouped.get(assetJobId) || { cutIds: [], utteranceIds: [], outputPath: "" };
  ...
  for (const [assetJobId, entry] of grouped) {
    ...
    const { record, subjectMatched } = qualityFor(quality, "scene-image", media, [...entry.utteranceIds, ...entry.cutIds]);
...
async function voiceTakeItems({ workspace, reader, episodeDir, manifest, speechReport, quality, running, speechCurrent }) {
  const rows = new Map();
  ...
  for (const cutId of cutIds) {
    ...
    const { record, subjectMatched } = qualityFor(quality, "voice-take", media, [cutId]);
    const verdict = qualityVerdict(record);
    let status = "complete";
...
export async function readKoyaMangaProgressSnapshot(job) {
  ...
  const images = workspace && episodeId
    ? await sceneImageItems({ workspace, reader, plan: evidence.plan, ledger: evidence.ledger, quality, running })
