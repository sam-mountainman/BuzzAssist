...
/** 完成前チェック（Stop フック）のイベント名。信頼の表の鍵では小文字の snake_case で入る。 */
export const BUZZASSIST_CODEX_STOP_HOOK_EVENT = "stop";

export const CODEX_HOOK_TRUST_FIX =
  "Codex を端末で起動して `/hooks` を開き、BuzzAssist の 2 つのフック（UserPromptSubmit の harness-learn-hook と、"
  + "Stop の harness-stop-hook）を確認して信頼する。信頼されるまで、Codex ではユーザーの訂正に気づいて capture を促す"
  + "フックも、Job が終わっていないのに「完成」と書いた返答を差し戻すフックも動かない（Claude Code は信頼の手順なしで動く）。"
...
function isBuzzAssistHookKey(key, event = BUZZASSIST_CODEX_HOOK_EVENT) {
  const text = String(key);
  return text.startsWith(`${BUZZASSIST_CODEX_PLUGIN}:`) && text.split(":").map((part) => part.toLowerCase()).includes(event);
}
 ...
 */
export function inspectCodexHookTrust({ env = process.env, homeDir = homedir(), configText = undefined, event = BUZZASSIST_CODEX_HOOK_EVENT } = {}) {
  ...
  const entries = [...parsed.hooks.entries()].filter(([key]) => isBuzzAssistHookKey(key, event));
...
function probeCodexHookTrust(options = {}, { event, label }) {
  const trust = inspectCodexHookTrust({ ...options, event });
  const expectedKey = `${BUZZASSIST_CODEX_PLUGIN}:${BUZZASSIST_CODEX_HOOK_FILE}:${event}:0:0`;
  switch (trust.status) {
    case "not-installed":
    ...
    case "trusted":
      return {
        ok: true,
        status: trust.status,
        detail: `Codex が BuzzAssist の${label}を信頼済み（config.toml の hooks.state に trusted_hash がある。`
    ...
    case "disabled":
      return {
        ok: false,
        status: trust.status,
        detail: `Codex で BuzzAssist の${label}が無効にされている（hooks.state の enabled = false）`,
    ...
    default:
      return {
        ok: false,
        status: "untrusted",
        detail: `Codex が BuzzAssist の${label}を信頼していない（config.toml に [hooks.state."${expectedKey}"] が無い）。`
...
export function probeCodexLearningHookTrust(options = {}) {
  return probeCodexHookTrust(options, { event: BUZZASSIST_CODEX_HOOK_EVENT, label: "学習フック" });
}

/** doctor の advisory 検査（completion-hook-trust）の形。完成前チェック（Stop フック）の信頼。止めない。 */
export function probeCodexStopHookTrust(options = {}) {
  return probeCodexHookTrust(options, { event: BUZZASSIST_CODEX_STOP_HOOK_EVENT, label: "完成前チェックのフック（Stop）" });
}
