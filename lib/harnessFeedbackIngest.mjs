...
/**
 * 署名鍵ごとに、取り込み済みの bundle（receipts/）と承認済みの観測（imports/）の件数を数える。
 * 読むだけで、署名 chain は検証しない（警告に添える目安。読めないファイルは数えない）。
 * 受け取り口で止まった鍵がこちらでまだ active なときに、owner が revoke を判断する材料にする。
 */
export async function countHarnessFeedbackRecordsBySigner({ rootDir, signerKeyIds = [] } = {}) {
  const paths = ingestPaths(rootDir);
  const wanted = new Set(signerKeyIds);
  const counts = new Map([...wanted].map((keyId) => [keyId, { importedBundles: 0, approvedImports: 0 }]));
  ...
  const readQuietly = (file) => readJsonIfExists(file, null).catch(() => null);
  for (const name of await jsonNames(paths.receipts)) {
    const keyId = (await readQuietly(join(paths.receipts, name)))?.signerKeyId;
    if (wanted.has(keyId)) counts.get(keyId).importedBundles += 1;
  }
  for (const name of await jsonNames(paths.imports)) {
    const keyId = (await readQuietly(join(paths.imports, name)))?.operator?.signerKeyId;
    if (wanted.has(keyId)) counts.get(keyId).approvedImports += 1;
  }
  return counts;
}
