...
import { SCRIPT_LEARNING_ROUTES } from "./harnessLearningTargets.mjs";
...
export async function captureScriptRoundLearning({
  ...
  const result = { ...base, target, roundDigest: digest.slice(0, 16), candidates: candidates.length, proposalIds: [] };
  const stage = SCRIPT_STAGES.includes(version?.stage) ? version.stage : "revision";
    ...
    try {
      const output = capture({
        ...
        metadata: {
          ...
          harness: { id: genreSpec.harnessId, version: String(contract?.version || "unknown") },
