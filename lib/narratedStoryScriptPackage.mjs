...
import { NARRATED_CAMERA_MOVES } from "./narratedStoryCamera.mjs";
...
export const NARRATED_SCRIPT_PACKAGE_FIELDS = Object.freeze({
  ...
  storySegment: Object.freeze(["id", "text", "speaker", "sceneIntent", "musicSection", "camera"]),
...
export function validateNarratedScriptPackage(pkg) {
  ...
  const checkSegments = (list, label, fields) => {
    ...
    return list.map((segment, index) => {
      ...
      if (segment.captionOnlySeconds !== undefined && !(Number.isFinite(captionOnlySeconds) && captionOnlySeconds >= 0.5 && captionOnlySeconds <= 30)) add(`${at}.captionOnlySeconds`);
      return {
        id,
        text,
        speaker,
        ...
        captionOnlySeconds: Number.isFinite(captionOnlySeconds) && captionOnlySeconds >= 0.5 && captionOnlySeconds <= 30 ? captionOnlySeconds : null,
      };
    }).filter(Boolean);
  };
  ...
  const expand = (list, part, idPrefix) => {
    ...
    for (const source of list) {
      ...
      if (source.captionOnlySeconds) {
        const text = part === "review" && source.operatorReplacementRequired ? `${OPERATOR_REPLACEMENT_MARKER}${source.text}` : source.text;
        output.push({
          id: source.id,
          text,
          ...
          speakerId: source.speaker,
          delivery: "caption-only",
          captionOnly: true,
          captionOnlySeconds: source.captionOnlySeconds,
