...
function resolveMediaJobBroker(input = {}) {
  if (input.mediaJobBroker && typeof input.mediaJobBroker.start === "function") {
    return input.mediaJobBroker;
  }
  return createPaidMediaJobBroker({
    stateDir: input.mediaJobStateDir,
    apiBase: input.mediaJobApiBase,
    apiFetch: typeof input.mediaJobFetch === "function" ? input.mediaJobFetch : input.apiFetch,
...
export async function generateBuzzAssistSpeech(input = {}) {
  ...
  const mediaJobBroker = resolveMediaJobBroker(input);
  const startedAt = Date.now();
  let job = await mediaJobBroker.start({
    kind: "voice.synthesis",
    provider: nonEmptyString(input.providerAdapter) || "buzzassist",
    model: nonEmptyString(input.model) || "default",
    adapterVersion: nonEmptyString(input.adapterVersion) || "buzzassist-voice-server-v1",
    voiceId,
  ...
  if (job.status !== "completed") {
    job = await mediaJobBroker.waitFor({ requestKey: job.requestKey }, {
      signal: input.signal,
      timeoutMs: finiteNumber(input.jobTimeoutMs, JOB_TIMEOUT_MS),
  ...
  if (job.status !== "completed") {
    const detail = nonEmptyString(job.error?.message);
    throw new Error(detail || `音声ジョブは ${job.status} のため完了していません。recoverPaidMediaJob で既存ジョブを復旧してください。`);
  }
  const artifactReader = typeof input.artifactReader === "function"
    ? input.artifactReader
    : readPaidMediaJobArtifact;
  const audioBuffer = await artifactReader(job, {
    fetchImpl: input.artifactFetch,
    signal: input.signal,
  });

  const result = job.result || {};
  const rawServerAlignment = result.alignment;
  const alignment = Array.isArray(rawServerAlignment)
    ? alignmentFromBuzzAssist(rawServerAlignment)
    : (rawServerAlignment && Array.isArray(rawServerAlignment.characters)
      ? rawServerAlignment
      : alignmentFromBuzzAssist([]));
  const durationSeconds = Math.max(0, finiteNumber(result.durationSeconds ?? job.usage?.seconds));
  const receipt = typeof mediaJobBroker.receipt === "function"
  ...
  return {
    ...
    chargedSeconds: finiteNumber(job.usage?.seconds),
    freeRegeneration: Boolean(job.usage?.freeRegeneration),
    remainingMinutes: finiteNumber(result?.data?.quota?.remainingMin, NaN),
