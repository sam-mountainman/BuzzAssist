 ...
 *   コメント）
 * - 感想パートの声の有無（Pack の bookends.review.voice。none なら感想パートは字幕だけで、境目の監査は語りの
 *   代わりに「lead の終わりで最初の字幕」「感想の区間の voice stem が無音」「BGM が途切れない」を測る）
...
import { FFMPEG_COMMAND_LINE_BUDGET, ffmpegCommandLineLength } from "./ffmpegFilterArgs.mjs";
import { audioSequenceConcatenates, concatAudioSequence, sequenceEvidenceGraphs } from "./ffmpegSequenceRender.mjs";
import { normalizeNarratedReviewLayoutConfig } from "./narratedStoryReviewLayout.mjs";
import { fixedOverlayGraph, overlayLayerInputs } from "./narratedStoryOverlays.mjs";
import { inspectNarratedSceneVideoGraph, planNarratedSceneJoins, renderNarratedSceneVideo } from "./narratedStorySceneTransitions.mjs";
...
/**
 * 感想パートの声の有無（Pack の `bookends.review.voice`）。既定の voiced は台本の感想の文を声にする（従来どおり）。
 * none は感想パートの全部の文を字幕だけにする: 声の Media Job を作らず（有料の TTS を呼ばない）、文ごとの字幕の秒数は
 * 台本パッケージが決める（lib/narratedStoryScriptPackage.mjs の narratedVoicelessCaptionSeconds）。
 */
export const NARRATED_REVIEW_VOICE_MODES = Object.freeze(["voiced", "none"]);

/**
 * Channel Pack が感想パートを声なし（bookends.review.voice: none）と宣言しているか。台本の計画（字幕だけにする）・
 * 見た目の計画（声のある文が混ざれば止める）・時間割（感想の部に voice: none）が同じこの判定を使う。
 */
export function narratedReviewVoiceless(config) {
  const bookends = config?.bookends || { enabled: false };
  return bookends.enabled === true && Boolean(bookends.review) && bookends.review.voice === "none";
}
/**
...
/**
 * 声なしの部（感想パート、Pack の bookends.review.voice: none）の実測は、上の BOUNDARY_STEM_SILENCE_DBFS と
 * BOUNDARY_SILENCE_FLOOR_DBFS・BOUNDARY_MAX_SILENCE_RUN_SECONDS を部の全体へ当てる（新しい閾値は足さない）。
 * 当てる前に、test/narratedStoryVoicelessReview.test.mjs と同じ合成の fixture（640x360・24fps、焼き込み字幕、
 * 感想は plain の挨拶と TV 枠・空の人物の枠、感想の曲あり＋film-burn／曲なし＋hard-cut の2通り）を同じ測定に
 * かけた（2026-09-26、ffmpeg 7.1.1 / libx264 / AAC 192k）:
 *   - voice stem の部の最大: 基準版 −120 dBFS（挿入した無音）、部の頭から 0.5 秒に声のテイクを足した版 −35.0 dBFS
 *     （−60 との差 25 dB）。声を 40 dB 下げて足した版は −75.0 で捕まえない（−45 dBFS の BGM の下で聞こえない
 *     大きさ。境目の stem の判定と同じ限界）
 *   - 完成 MP4 の部の最長の無音: 基準版 0.00 秒（部の最小ブロック −45.3〜−45.4 dBFS）、途中の 0.4 秒の音を消した版
 *     0.38〜0.40 秒。0.1 秒を消した版は 0.08〜0.10 秒で、境目と同じく 2 フレーム未満の穴はフェード端の範囲として許す
 * VOICELESS_PART_TAIL_EXCLUDE_SECONDS は、部の終わり（番組の終わり）から MP4 の BGM の途切れの判定に入れない長さ。
 * AAC は最後のフレーム（1024 サンプル＝21.3ms）を詰め物で埋めるので、エンコーダーによって終端の数 ms が下がりうる。
 * 実測の基準版は最後の 10ms まで −44.8〜−45.3 dBFS で下がらなかった。AAC 2 フレームぶんを丸めた 0.05 秒とし、
 * 許す無音の長さ（0.08 秒）より短くして、終端の外し方で穴を隠さないようにした。
 */
export const VOICELESS_PART_TAIL_EXCLUDE_SECONDS = 0.05;
...
export function normalizeNarratedBookendsConfig(source, { render = { width: 1280, height: 720 } } = {}) {
  ...
  if (source.review !== undefined && source.review !== null) {
    ...
    if (!plainObject(review)) blockers.push("bookends.review");
      ...
      // 感想パートの声の有無（無ければ voiced＝従来どおり声にする）。知らない値は推測で voiced に倒さず止める
      // （声なしのつもりの Pack で有料の TTS を呼ばないため）。
      const voice = review.voice === undefined ? "voiced" : nonEmpty(review.voice);
      if (!NARRATED_REVIEW_VOICE_MODES.includes(voice)) blockers.push("bookends.review.voice");
      config.review = {
        scriptMarker,
        ...
        voice: NARRATED_REVIEW_VOICE_MODES.includes(voice) ? voice : "voiced",
      };
    }
  }
...
/**
 * 文の枠の秒。本編の間（Pack の pacing。lib/narratedStoryPacing.mjs）を前後に持つ文は、声の実尺に間を足した長さ。
 * 間の無い文は声の実尺そのもの（今までと同じ値）。
 */
export function narratedSegmentSlotSeconds(segment) {
  return (Number(segment?.pauseBeforeSeconds) || 0) + Number(segment?.durationSeconds) + (Number(segment?.pauseAfterSeconds) || 0);
}

/**
 * 部の中の文の時間割（純粋関数）。枠のフレーム（segmentFramePlan を枠の秒で。場面の画とカメラのショットはこの
 * フレームで描く）と、枠の中の声の区間（部の頭からの秒）、字幕の頁を置くフレーム（声の区間を丸めたもの）。
 * 間の無い文は、字幕のフレームが枠のフレームと同じ（今までと同じ時間割）。間を持つ文は、枠の頭・尾の
 * 間のフレーム数（pauseHeadFrames / pauseTailFrames）も返す（場面の切り替えの重なりを間の中に収めるのに使う）。
 */
export function segmentTimelinePlan(segments, fps) {
  const frames = segmentFramePlan(segments.map(narratedSegmentSlotSeconds), fps);
  let cursor = 0;
  return segments.map((segment, index) => {
    const { startFrame, frames: count } = frames[index];
    const before = Number(segment.pauseBeforeSeconds) || 0;
    const after = Number(segment.pauseAfterSeconds) || 0;
    const voiceStartSeconds = cursor + before;
    const voiceEndSeconds = voiceStartSeconds + segment.durationSeconds;
    cursor += before + segment.durationSeconds + after;
    const end = startFrame + count;
    const captionStart = before > 0 ? Math.min(end - 1, Math.max(startFrame, Math.round(voiceStartSeconds * fps))) : startFrame;
    const captionEnd = after > 0 ? Math.min(end, Math.max(captionStart + 1, Math.round(voiceEndSeconds * fps))) : end;
    return {
      startFrame,
      frames: count,
      voiceStartSeconds,
      voiceEndSeconds,
      captionStartFrame: captionStart,
      captionFrames: captionEnd - captionStart,
      ...(before > 0 ? { pauseHeadFrames: captionStart - startFrame } : {}),
      ...(after > 0 ? { pauseTailFrames: end - captionEnd } : {}),
    };
  });
}
 ...
 * 次の部の語りと最初の字幕は lead の終わりで同時に始まる。感想パートが声なし（reviewVoice: "none"）なら、
 * 感想の部に voice: "none" を付ける（語りは無く、最初の字幕が lead の終わりで始まる。境目の実測がこれを読む）。
 */
export function planNarratedBookendProgram({ fps, openingFrames = 0, story = [], review = [], reviewPresenter = false, reviewVisual = "", reviewSegmentCamera = null, reviewVoice = "voiced", transitions = {} }) {
  ...
  const addSegmentsPart = (id, list, { presenter = false, visual = "", cameraOf = null, voiceless = false } = {}) => {
    const startFrame = frame;
    const startSeconds = startFrame / fps;
    // 文の枠（本編の間を含む）と、その中の声・字幕の区間（lib/narratedStoryPacing.mjs）。間の無い部は今までと同じ。
    const timeline = segmentTimelinePlan(list, fps);
    let cursor = 0;
    list.forEach((segment, index) => {
      ...
      segments.push({
        ...
        partIndex: index,
        frames: slot.frames,
        partStartFrame: slot.startFrame,
        captionPartStartFrame: slot.captionStartFrame,
        captionFrames: slot.captionFrames,
        ...(slot.pauseHeadFrames === undefined ? {} : { pauseHeadFrames: slot.pauseHeadFrames }),
        ...(slot.pauseTailFrames === undefined ? {} : { pauseTailFrames: slot.pauseTailFrames }),
        startSeconds: startSeconds + slot.voiceStartSeconds,
        endSeconds: startSeconds + slot.voiceStartSeconds + segment.durationSeconds,
      });
      cursor += narratedSegmentSlotSeconds(segment);
    });
    const frames = timeline.reduce((sum, entry) => sum + entry.frames, 0);
    parts.push({
      id,
      startFrame,
      frames,
      // 部の語りの秒（本編の間を含む枠の合計）。
      voiceSeconds: cursor,
      visual: visual || (presenter ? "presenter-video" : "segment-images"),
      // 声なしの部（生成記録にも残し、記録から組み直した時間割でも同じ実測をする）。
      ...(voiceless ? { voice: "none" } : {}),
    });
    frame += frames;
  };
  if (openingFrames > 0) {
  ...
  if (review.length > 0) {
    ...
    addSegmentsPart("review", review, { presenter: reviewPresenter, visual: reviewVisual, cameraOf: reviewSegmentCamera, voiceless: reviewVoice === "none" });
  }
...
/**
 * 本編の場面の切り替えの宣言（Pack の sceneTransition。無ければ cut）。本編の部だけに効き、画の感想パートは
 * cut のまま。
 */
export function sceneTransitionForPart(config, part) {
  return part === "story" ? (config?.sceneTransition || null) : null;
}

/**
 * segment 画像の部（本編・画像の感想）を映像だけで描く。同じ画が続く文は1つのショットとして1つの動きで
 * 通し（lib/narratedStoryCamera.mjs）、ショットの間は concat（転換を足さない）。本編は Pack が crossfade を
 * 宣言したときだけ、場面の切り替えを重ねて混ぜる（lib/narratedStorySceneTransitions.mjs。尺は変えない）。
 * ショットの数に比例してコマンド行が伸びるので、上限を越えない塊に分けて描いてつなぐ
 * （lib/ffmpegSequenceRender.mjs）。長い graph はファイルで渡す（lib/ffmpegFilterArgs.mjs）。
 */
export async function renderSegmentImagesPart({ ffmpeg, segments, config, outputPath, budget }) {
  const shots = partCameraShots(segments, config);
  const rendered = await renderNarratedSceneVideo({
    ffmpeg,
    shots,
    transition: sceneTransitionForPart(config, segments[0]?.part || "story"),
    render: config.render,
    outputPath,
    encode: PART_ENCODE,
    ...(budget ? { budget } : {}),
  });
  return { frames: rendered.frames, shots, joins: rendered.joins, graph: rendered.graph };
}
...
/**
 * 部と境目クリップを順に並べた [program] に、固定の重ね物（lib/narratedStoryOverlays.mjs。宣言の区間だけ）と
 * 焼き込み字幕の層を重ねる graph と、その入力（firstInput 番目から。重ね物の画像、字幕の concat の一覧の順）。
 * 番組の組み立て（bookends の有無に依らない）で同じ形を使い、graph の検査は videoConcatAssembled が見る。
 */
export function programLayersGraph({ assembled, firstInput, overlayLayer = null, subtitleLayer = null }) {
  const inputs = [];
  const stages = [];
  let current = "program";
  let next = firstInput;
  if (overlayLayer?.items?.length) {
    inputs.push(...overlayLayerInputs(overlayLayer));
    stages.push(fixedOverlayGraph(overlayLayer, { firstInputIndex: next, input: "program", output: "programfixed" }));
    next += overlayLayer.items.length;
    current = "programfixed";
  }
  if (subtitleLayer) {
    inputs.push("-f", "concat", "-i", subtitleLayer.listPath);
    stages.push(subtitleOverlayGraph({ inputIndex: next, input: current, output: "video" }));
  } else if (stages.length > 0) stages.push(`[${current}]format=yuv420p[video]`);
  const head = `${assembled}[${stages.length > 0 ? "program" : "video"}]`;
  return { inputs, filterGraph: [head, ...stages].join(";") };
}

/**
 * 部と境目クリップを順に並べ、master 音声と字幕を mux して番組 MP4 を作る。固定の重ね物と焼き込み字幕の層
 * （lib/narratedStorySubtitles.mjs の concat の一覧）があれば、番組の時間で1回だけ重ねる
 * （転換と字幕なし lead-in の間には頁も重ね物も無いので、そこには何も重ならない）。
 */
export async function renderBookendProgram({ ffmpeg, clipPaths, masterAudioPath, srtPath, plan, outputPath, subtitleLayer = null, overlayLayer = null }) {
  const args = ["-hide_banner", "-loglevel", "error", "-y"];
  ...
  const layers = programLayersGraph({
    assembled: `${clipPaths.map((_, index) => `[${index}:v]`).join("")}concat=n=${clipPaths.length}:v=1:a=0`,
    firstInput: subtitleIndex + 1,
    overlayLayer,
    subtitleLayer,
  });
  args.push(...layers.inputs);
  const filterGraph = layers.filterGraph;
  args.push(
 ...
 */
export async function makeBookendVoiceStem({ ffmpeg, plan, voicePaths, outputPath, budget = FFMPEG_COMMAND_LINE_BUDGET }) {
  // 部ごとの声の入力（部の声を1本につないだファイルが partInputs にあれば、それを1つの入力として使う）。
  const build = (partInputs = null) => {
    const args = ["-hide_banner", "-loglevel", "error", "-y"];
    const filters = [];
    const blocks = [];
    let input = 0;
    ...
    for (const part of plan.parts) {
      ...
      if (partSegments.length === 0) {
        ...
        const paths = partInputs?.[part.id] ? [partInputs[part.id]] : partSegments.map((segment) => voicePaths[segment.id]);
    ...
    return { args: [...args, "-filter_complex", filterGraph, "-map", "[voice]", "-c:a", "pcm_s16le", outputPath], filterGraph, inputCount: input };
  };
  const whole = build();
  const wholeLength = ffmpegCommandLineLength(ffmpeg, whole.args);
  if (wholeLength <= budget) {
    await runRuntime(ffmpeg, whole.args);
    return { filterGraph: whole.filterGraph, inputCount: whole.inputCount, outputMap: "[voice]", maxCommandLength: wholeLength };
  }
  // 文が多くてコマンド行に入らないときは、部ごとに声を先に1本へつなぎ（塊に分けて。標本は足し引きしない。
  // lib/ffmpegSequenceRender.mjs）、その部の声を入力にして同じ時間割で並べる。
  const partInputs = {};
  const parts = {};
  for (const part of plan.parts) {
    const partSegments = plan.segments.filter((segment) => segment.part === part.id);
    if (partSegments.length === 0) continue;
    const partPath = `${outputPath}.${part.id}.voice.wav`;
    parts[part.id] = await concatAudioSequence({
      ffmpeg,
      paths: partSegments.map((segment) => voicePaths[segment.id]),
      outputPath: partPath,
      workDir: `${outputPath}.${part.id}.chunks`,
      sampleRate: SAMPLE_RATE,
      channelLayout: "mono",
      label: "voice",
      budget,
    });
    partInputs[part.id] = partPath;
  }
  const assembled = build(partInputs);
  const assembledLength = ffmpegCommandLineLength(ffmpeg, assembled.args);
  await runRuntime(ffmpeg, assembled.args);
  return {
    filterGraph: assembled.filterGraph,
    inputCount: plan.segments.length,
    outputMap: "[voice]",
    parts,
    maxCommandLength: Math.max(assembledLength, ...Object.values(parts).map((entry) => entry.maxCommandLength || 0)),
  };
}

/** 声の stem の証拠が、部の count 個の声をつないでいるか（1回の graph か、部ごとに先につないだ塊）。 */
export function voiceStemConcatenatesPart(evidence, partId, count) {
  if (evidence?.parts?.[partId]) {
    return audioSequenceConcatenates(evidence.parts[partId], count)
      && String(evidence.filterGraph || "").includes(`concat=n=1:v=0:a=1,apad=`);
  }
...
export function boundaryExclusionMask(region, render, width = 64, height = 36) {
  // 複数の範囲（字幕と固定の重ね物など）は和をとる。
  if (Array.isArray(region)) {
    const masks = region.map((entry) => boundaryExclusionMask(entry, render, width, height)).filter(Boolean);
    if (masks.length === 0) return null;
    return masks.reduce((union, mask) => union.map((value, index) => value || mask[index]));
  }
...
/** 音（MP4 の音声・voice stem・声のテイク）の [start, start + duration) 秒を 48kHz mono の PCM で読む。 */
export async function extractNarratedPcm(ffmpeg, path, start, duration) {
  return extractPcm(ffmpeg, path, start, duration);
}
...
/** MP4 の字幕のトラック（mov_text）の cue の時刻（秒）。本編の間の監査（lib/narratedStoryPacing.mjs）も同じ読み方を使う。 */
export async function extractNarratedSubtitleCues(ffmpeg, path) {
  return extractSubtitleCues(ffmpeg, path);
}
...
/**
 * 語りの基準音量: 本編の文（最大 8 個、等間隔に選ぶ）の声の区間（startSeconds〜endSeconds。本編の間は含まない）を
 * MP4 から測った RMS の中央値。境目の監査と本編の間の監査（lib/narratedStoryPacing.mjs）が同じ基準を使う。
 */
export async function measureNarrationReferenceDb({ ffmpeg, videoPath, segments = [] }) {
...
/**
 * 声なしの部（感想パート、Pack の bookends.review.voice: none）を実測する。
 * - voice stem: 転換の頭から部の終わりまでの 10ms ブロックの最大が BOUNDARY_STEM_SILENCE_DBFS 以下（語りが無い）
 * - 完成 MP4: lead の終わりから部の終わり（AAC の終端の揺れを避けて VOICELESS_PART_TAIL_EXCLUDE_SECONDS 手前）まで、
 *   BOUNDARY_SILENCE_FLOOR_DBFS を下回る連続が BOUNDARY_MAX_SILENCE_RUN_SECONDS 以下（BGM が鳴り続ける）
 * 閾値は境目の監査と同じ値を使い、声なしの fixture で基準版と壊した版を測って確かめた（VOICELESS_PART_TAIL_EXCLUDE_SECONDS
 * のコメント）。
 */
async function measureVoicelessIncomingPart({ ffmpeg, videoPath, voiceStemPath, part, effectStart, incomingStart }) {
  const problems = [];
  const partEnd = part.endSeconds;
  const stem = await extractPcm(ffmpeg, voiceStemPath, effectStart, partEnd - effectStart);
  // 部は数分になりうる（10ms ブロックが数万）。最大・最小は展開せずに畳む。
  const stemSeries = blockDbSeries(stem, effectStart, effectStart, partEnd - 0.005, 0.01);
  const stemMaxDb = stemSeries.reduce((maximum, entry) => Math.max(maximum, entry.db), -120);
  if (stemSeries.length === 0) problems.push("voiceless-part-stem-unmeasured");
  else if (stemMaxDb > BOUNDARY_STEM_SILENCE_DBFS) problems.push("narration-in-voiceless-part");
  const bedEnd = Math.max(incomingStart, partEnd - VOICELESS_PART_TAIL_EXCLUDE_SECONDS);
  const mp4 = await extractPcm(ffmpeg, videoPath, incomingStart, bedEnd - incomingStart);
  const bedSeries = blockDbSeries(mp4, incomingStart, incomingStart, bedEnd, 0.01);
  const longestSilence = longestRunBelow(bedSeries, BOUNDARY_SILENCE_FLOOR_DBFS);
  const bedMinDb = bedSeries.length ? bedSeries.reduce((minimum, entry) => Math.min(minimum, entry.db), Infinity) : -120;
  if (bedSeries.length === 0) problems.push("voiceless-part-bed-unmeasured");
  else if (longestSilence > BOUNDARY_MAX_SILENCE_RUN_SECONDS) problems.push("dead-air-in-voiceless-part");
  return {
    problems,
    metrics: {
      partId: part.id,
      stemFromSeconds: round(effectStart, 4),
      partEndSeconds: round(partEnd, 4),
      stemMaxDb: round(stemMaxDb),
      bedMeasuredSeconds: round(bedEnd - incomingStart, 3),
      bedMinDb: round(bedMinDb),
      longestSilenceSeconds: round(longestSilence, 3),
    },
  };
}
 ...
 *   入る側が声なしの部（plan の part.voice が none）なら、発話の開始の代わりに「最初の字幕が lead の終わりに
 *   始まる」「転換の頭から部の終わりまで voice stem が無音」「部の間 BGM が途切れない」を測る
...
export async function measureNarratedBookendBoundaries({ ffmpeg, ffprobe, videoPath, voiceStemPath, plan, excludeRegion = null, render = null }) {
  ...
  const narrationReferenceDb = await measureNarrationReferenceDb({ ffmpeg, videoPath, segments: plan.segments });
  ...
  for (const boundary of plan.boundaries) {
    ...
    // 入る側が声なしの感想パート（Pack の bookends.review.voice: none）なら、入る側に語りは無い。
    const incomingPart = plan.parts.find((part) => part.id === boundary.incomingPart) || null;
    const voicelessIncoming = incomingPart?.voice === "none";
    const onsetSearchEnd = Math.min(windowEnd, incomingStart + 0.75);
    const stemOnset = voicelessIncoming ? null : firstOnset(blockDbSeries(stem, stemStart, incomingStart - 0.05, onsetSearchEnd, 0.005), -45);
    const mp4Onset = voicelessIncoming || narrationReferenceDb === null
      ? null
    ...
    let voiceless = null;
    if (voicelessIncoming) {
      // 声なしの感想パートの代わりの決まり（「lead の終わりで次の語りと最初の字幕が同時に始まる」の語りが無い）:
      // - lead の終わりで最初の字幕が始まる（下の incoming-caption-not-aligned-with-narration が同じ判定で見る）
      // - 転換の頭から感想パートの終わりまで、MP4 へ入れた voice stem は挿入した無音のまま（語りが1つも無い）
      // - 感想パートの間、完成 MP4 の音にデジタル無音の穴が無い（語りの代わりに BGM が鳴り続ける）
      voiceless = await measureVoicelessIncomingPart({ ffmpeg, videoPath, voiceStemPath, part: incomingPart, effectStart, incomingStart });
      problems.push(...voiceless.problems);
    } else {
    ...
    audioBoundaries.push({
      ...
      metrics: {
        ...
        ...(voiceless ? { incomingVoice: "none", voicelessIncoming: voiceless.metrics } : {}),
      },
    });
 ...
 */
export function inspectNarratedBookendRenderGraphs(renderGraphs = {}, plan, { sceneTransition = null } = {}) {
  ...
  const shotsOf = (id) => groupNarratedCameraShots(plan.segments
    .filter((segment) => segment.part === id)
    .map((segment) => ({ ...segment, startFrame: segment.partStartFrame })));
  const shotCount = (id) => shotsOf(id).length;
  // 本編の場面の間は concat だけ。Pack が crossfade を宣言したときだけ、計画した切り替えの長さの xfade の fade が
  // あってよい（lib/narratedStorySceneTransitions.mjs）。塊に分けて描いたときは塊ごとの graph を見る。
  const storyShots = shotsOf("story");
  const storyJoins = planNarratedSceneJoins({ shots: storyShots, transition: sceneTransition });
  if (!graph("storyVideo") || !inspectNarratedSceneVideoGraph(renderGraphs.storyVideo, { shotCount: storyShots.length, joins: storyJoins, fps: plan.fps }).ok) {
  ...
  if (reviewPart) {
    ...
    // 配置の型の感想パートは、区間を concat でつなぐ（区間の中で転換を足さない。塊に分けたときは塊ごとに）。
    const ok = reviewPart.visual === "presenter-video"
      ...
      : (reviewPart.visual === "review-layout"
        ? Boolean(reviewGraph) && sequenceEvidenceGraphs(renderGraphs.reviewVideo).every((chunk) => /concat=n=\d+:v=1:a=0\[video\]$/u.test(chunk) && !TRANSITION_FILTER.test(chunk))
        : Boolean(reviewGraph) && inspectNarratedSceneVideoGraph(renderGraphs.reviewVideo, { shotCount: shotCount("review"), joins: [], fps: plan.fps }).ok);
  ...
  const parentAudioMapped = Boolean(voiceGraph)
    ...
    && plan.parts.filter((part) => count(part.id) > 0).every((part) => voiceStemConcatenatesPart(renderGraphs.voiceStem, part.id, count(part.id)))
