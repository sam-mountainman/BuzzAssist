...
export async function renderSegmentImagesPart({ ffmpeg, segments, config, outputPath }) {
  ...
  const graphArgs = await filterComplexArgs(ffmpeg, filterGraph, dirname(outputPath));
  await runRuntime(ffmpeg, ["-hide_banner", "-loglevel", "error", "-y", ...inputs, ...graphArgs, "-map", "[video]", "-frames:v", String(frames), ...PART_ENCODE, outputPath]);
