...
export async function missingBookendFfmpegFilters(ffmpeg, config, { run = runRuntime } = {}) {
  ...
  return needed.filter((name) => !new RegExp(`\\s${name}\\s`, "u").test(listing));
}
...
export function normalizeNarratedBookendsConfig(source, { render = { width: 1280, height: 720 } } = {}) {
  ...
  if (source.opening !== undefined && source.opening !== null) {
    ...
    if (!plainObject(opening)) blockers.push("bookends.opening");
      ...
      if (!["title-card", "video"].includes(kind)) blockers.push("bookends.opening.kind");
  ...
  if (source.review !== undefined && source.review !== null) {
    ...
    if (!plainObject(review)) blockers.push("bookends.review");
      ...
      if (presenterRequired && !presenterVideo) blockers.push("bookends.review.presenter-media-required");
      const presenterBackground = colorOf(presenterSource.backgroundColor);
      if (presenterVideo && !presenterBackground) blockers.push("bookends.review.presenter.backgroundColor");
      ...
      config.review = {
        scriptMarker,
        presenter: { required: presenterRequired, video: presenterVideo, backgroundColor: presenterBackground },
        music,
        musicGain,
 ...
 */
export function planNarratedBookendProgram({ fps, openingFrames = 0, story = [], review = [], reviewPresenter = false, transitions = {} }) {
  ...
  const addSegmentsPart = (id, list, { presenter = false } = {}) => {
    ...
    list.forEach((segment, index) => {
      segments.push({
        ...segment,
        part: id,
        camera: presenter ? "presenter-video" : "monotonic-zoom-with-horizontal-pan",
        partIndex: index,
    ...
    parts.push({ id, startFrame, frames, voiceSeconds: cursor, visual: presenter ? "presenter-video" : "segment-images" });
    frame += frames;
  };
  if (openingFrames > 0) {
  ...
  if (review.length > 0) {
    ...
    addSegmentsPart("review", review, { presenter: reviewPresenter });
  }
...
export async function renderOpeningPart({ ffmpeg, ffprobe, opening, media, config, workDir, outputPath }) {
  ...
  if (opening.kind === "video") {
    ...
    return { frames, graph: { filterGraph, inputCount: 1, outputMap: "[video]" }, embeddedAudio: hasAudio ? media["opening.video"] : "" };
  }
...
/** segment 画像の部（本編・画像の感想）を映像だけで描く。場面間は concat（転換を足さない）。 */
export async function renderSegmentImagesPart({ ffmpeg, segments, config, outputPath }) {
  const { width, height, fps } = config.render;
  const args = ["-hide_banner", "-loglevel", "error", "-y"];
  for (const segment of segments) args.push("-loop", "1", "-framerate", String(fps), "-i", segment.imagePath);
  const filters = segments.map((segment, index) => {
    const denominator = Math.max(1, segment.frames - 1);
    const progress = `on/${denominator}`;
    const horizontal = index % 2 === 0 ? `(iw-iw/zoom)*(${progress})` : `(iw-iw/zoom)*(1-${progress})`;
    return `[${index}:v]scale=${width}:${height}:force_original_aspect_ratio=decrease,`
      + `pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2:black,setsar=1,`
      + `zoompan=z='min(max(pzoom,1.0)+0.002,1.08)':x='${horizontal}':`
      + `y='(ih-ih/zoom)/2':d=1:s=${width}x${height}:fps=${fps},`
      + `trim=end_frame=${segment.frames},setpts=N/(${fps}*TB),fps=${fps},format=yuv420p[v${index}]`;
  });
  ...
  await runRuntime(ffmpeg, [...args, "-filter_complex", filterGraph, "-map", "[video]", "-frames:v", String(frames), ...PART_ENCODE, outputPath]);
  return { frames, graph: { filterGraph, imageInputCount: segments.length, videoMap: "[video]" } };
}
...
/** 部と境目クリップを順に並べ、master 音声と字幕を mux して番組 MP4 を作る。 */
export async function renderBookendProgram({ ffmpeg, clipPaths, masterAudioPath, srtPath, plan, outputPath }) {
  const args = ["-hide_banner", "-loglevel", "error", "-y"];
  ...
  const filterGraph = `${clipPaths.map((_, index) => `[${index}:v]`).join("")}concat=n=${clipPaths.length}:v=1:a=0[video]`;
  args.push(
...
export async function makeBookendBedStem({ ffmpeg, plan, storyMusicPath, storyGain, reviewMusicPath = "", reviewGain = storyGain, openingAudioPath = "", outputPath }) {
  ...
  if (opening) {
    ...
    storyChain.push(`afade=t=in:st=0:d=${fadeIn.toFixed(6)}:curve=tri`);
  }
  if (crossfade) {
...
/** 64x36 RGB フレーム列の平均絶対差・平均輝度。 */
export function frameMeanAbsDiff(left, right) {
...
export function frameLuma(frame) {
  let sum = 0;
 ...
 */
export function judgeBoundaryFrames(boundary, frames) {
  ...
  for (let frame = boundary.effectStartFrame; frame < boundary.effectEndFrame; frame += 1) {
    ...
    const toOutgoing = frameMeanAbsDiff(image, refOutgoing);
    const toIncoming = frameMeanAbsDiff(image, refIncoming);
    effect.push({ frame, luma: frameLuma(image), toOutgoing, toIncoming, effect: Math.min(toOutgoing, toIncoming) });
  }
  const lead = [];
  for (let frame = boundary.effectEndFrame; frame < boundary.incomingStartFrame; frame += 1) {
    lead.push(frameMeanAbsDiff(at(frame), refIncoming));
  }
  const preEffectDiff = frameMeanAbsDiff(at(boundary.effectStartFrame - 2), refOutgoing);
  // 転換が宣言より早く始まると、出る側の最終フレームの明るさが 1 フレーム前から跳ぶ（光の漏れは明るく、
  // 黒への溶暗は暗くする）。本編のカメラ移動は輝度をほとんど変えないので、差分ではなく輝度差で見る。
  const preEffectLumaStep = Math.abs(frameLuma(refOutgoing) - frameLuma(at(boundary.effectStartFrame - 2)));
  const incomingContinuity = frameMeanAbsDiff(at(boundary.incomingStartFrame), refIncoming);
  ...
  const metrics = {
    ...
    cutDiff: round(frameMeanAbsDiff(refOutgoing, at(boundary.effectEndFrame))),
  };
 ...
 */
export async function measureNarratedBookendBoundaries({ ffmpeg, ffprobe, videoPath, voiceStemPath, plan }) {
  const fps = plan.fps;
  const probe = JSON.parse((await runRuntime(ffprobe, [
  ...
  for (const boundary of plan.boundaries) {
    ...
    visualBoundaries.push({ id: boundary.id, type: boundary.type, ...judgeBoundaryFrames(boundary, frames) });
  }
...
export function inspectNarratedBookendRenderGraphs(renderGraphs = {}, plan) {
  ...
  if (!storyGraph.includes(`concat=n=${count("story")}:v=1:a=0[video]`) || TRANSITION_FILTER.test(storyGraph)) {
  ...
  if (reviewPart) {
    ...
    const ok = reviewPart.visual === "presenter-video"
      ...
      : reviewGraph.includes(`concat=n=${count("review")}:v=1:a=0[video]`) && !TRANSITION_FILTER.test(reviewGraph);
  ...
  if (!programGraph.includes(`concat=n=${clipCount}:v=1:a=0[video]`) || TRANSITION_FILTER.test(programGraph)) {
