...
import { loadHarnesses, matchHarnesses } from "../scripts/harness-registry.mjs";
...
async function canonicalIdentityForJob(job, validateProductionProfile = assertVideoHarnessProductionProfile) {
  ...
  if (declaration.id !== job.harness.id || declaration.version !== job.harness.declarationVersion) {
  ...
  if (canonicalJson(canonicalSkills.map((entry) => entry.path)) !== canonicalJson(storedSkillPaths)) {
...
export async function assertVideoHarnessJobIdentity(job, {
  ...
  if (canonicalJson(actual) !== canonicalJson(job.canonicalIdentity)) {
    throw new Error("Video Harnessのcanonical identityが計画後に変わった。Core・宣言・配置・Skill・profileを確認し、新しいJobとして計画し直すこと。");
  }
  if (job.harness?.id === "koya-manga-video" && job.resolvedProductionContract) {
    ...
    if (canonicalJson(currentContract) !== canonicalJson(job.resolvedProductionContract)) {
      throw new Error("Koyaの解決済み制作契約がdoctor前の固定値から変わった。新しいJobとして計画し直すこと。");
    }
...
/**
 * 明示 ID を最優先し、依頼文から推測するときは最高得点が同点なら止める。
...
export function selectVideoHarness({ harnesses = loadHarnesses(), harnessId = "", want = "" } = {}) {
  ...
  if (explicit) {
    ...
    if (!harness) throw new Error(`未知のハーネス: ${explicit}`);
    return { harness, selectedBy: "explicit", evidence: { harnessId: explicit } };
  }
  const candidates = matchHarnesses(harnesses, want);
  if (candidates.length === 0) {
    throw new Error("依頼に一致する動画ハーネスが無い。--harness で明示すること。");
  }
  const topScore = candidates[0].score;
  const top = candidates.filter((entry) => entry.score === topScore);
  if (top.length !== 1) {
    const ids = top.map((entry) => entry.harness.id).join(", ");
    throw new Error(`動画ハーネスを一意に選べない: ${ids}。有料生成前に --harness で1つ指定すること。`);
  }
  return {
    harness: top[0].harness,
    selectedBy: "capability-match",
    evidence: { score: top[0].score, hits: [...top[0].hits] },
  };
}
...
export async function createVideoHarnessJob({
  ...
  try {
    ...
    if (existing.identityDigest !== identity || existing.script?.sha256 !== scriptDigest
      || canonicalJson(existing.canonicalIdentity) !== canonicalJson(canonicalIdentity)) {
...
function mergeMediaJobs(previous, current) {
  return mergeByIdentity(previous, current, (row) => row.requestKey || row.jobId
 ...
 * 確認できないものは通さない。
 */
async function assertFailedJobResumable(job) {
 ...
 */
async function reopenFailedJob(job, { recoverMediaJob, env, now, projectCanvas }) {
  await assertFailedJobResumable(job);
...
export async function runVideoHarnessJob({
  ...
  if (job.status === "failed") {
    ...
    job = await reopenFailedJob(job, { recoverMediaJob, env, now, projectCanvas });
  ...
  try {
    ...
    job = await persistJobTransition(job, (current) => (receiptPending
      ? {
        ...
        status: "awaiting-human-review",
        updatedAt: now(),
        blockers: [RECEIPT_FINALIZATION_BLOCKER, "canonical-identity-drift"],
        knownRemainingIssues: [
      ...
      : {
        ...
        knownRemainingIssues: [detail],
        driftHistory: driftRecord(current, "preflight"),
