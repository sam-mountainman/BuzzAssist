...
/**
 * 時刻を持つ文（{ id, part, imageKey, imagePath, startFrame, frames, cameraMove? }）を、同じ画が続く
 * 範囲ごとのショットにまとめる（部の中で、画の鍵が同じ隣り合う文）。文の順は変えない。
 */
 ...
 */
export function planCameraMove(kind, spec, durationSeconds) {
  const seconds = Math.max(0, Number(durationSeconds) || 0);
  if (kind === "slow-push-in" || kind === "slow-pull-out") {
    const peak = Math.min(spec.maxZoom, (1 + spec.zoomPerSecond) ** seconds);
    const zoomed = { zoom: peak, centerX: spec.focusX ?? 0.5, centerY: spec.focusY ?? 0.5 };
  ...
  if (kind === "pan-left" || kind === "pan-right") {
    ...
    const travel = Math.min(1 - 1 / spec.zoom, (spec.panPerSecond / spec.zoom) * seconds);
    const direction = kind === "pan-right" ? 1 : -1;
    return {
      from: { zoom: spec.zoom, centerX: 0.5 - (direction * travel) / 2, centerY: spec.focusY ?? 0.5 },
      to: { zoom: spec.zoom, centerX: 0.5 + (direction * travel) / 2, centerY: spec.focusY ?? 0.5 },
    };
  }
  const still = { zoom: spec.zoom ?? 1, centerX: 0.5, centerY: 0.5 };
...
export function planNarratedCameraShots({ segments = [], camera, fps }) {
  ...
  const planned = shots.map((shot) => {
    ...
    const move = planCameraMove(kind, camera.moves[kind], durationSeconds);
    return {
 ...
 */
export function cameraPerspectiveFilter(shot) {
  const denominator = Math.max(1, shot.frames - 1);
  const progress = `min(1,on/${denominator})`;
...
export function cameraShotChains({ shots, render, inputOffset = 0 }) {
  ...
  const chains = shots.map((shot, index) => {
    inputs.push("-loop", "1", "-framerate", String(fps), "-i", shot.imagePath);
    const still = shot.move === "static" && shot.from.zoom === 1;
    return `[${inputOffset + index}:v]scale=${width}:${height}:force_original_aspect_ratio=decrease,`
      + `pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2:black,setsar=1,format=yuv444p,`
      + `${still ? "null" : cameraPerspectiveFilter(shot)},`
      + `trim=end_frame=${shot.frames},setpts=N/(${fps}*TB),fps=${fps},format=yuv420p[c${index}]`;
  });
...
export async function measureNarratedCameraMotion({ ffmpeg, videoPath, shots, camera, render, exclude = [], concurrency = 4 }) {
  ...
  const rows = await mapLimit(shots, concurrency, async (shot) => {
    const problems = [];
    if (shot.frames < 3) return { id: shot.id, move: shot.move, pass: true, skipped: "shot-too-short-to-measure", problems };
    ...
    const first = 1;
    const last = shot.frames - 2 >= first + 1 ? shot.frames - 2 : shot.frames - 1;
    ...
    for (const cut of shot.cutFrames || []) {
      ...
      if (local >= 1 && local < shot.frames) cuts.push(measure("cut", local - 1, local));
    }
