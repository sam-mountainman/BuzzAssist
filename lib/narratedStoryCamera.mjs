 ...
 * - 場面ごとの焦点（台本パッケージの本編の場面の `cameraFocus: { x, y }`、画の中の顔の位置など。0〜1）。
 *   焦点を指定した場面は、Pack の型ごとの焦点（focusX / focusY）の代わりにその場面の焦点を使う。
 *   指定の無い場面は今までどおり Pack の型ごとの焦点（後方互換）。焦点は台本か Pack の宣言からだけ取り、
 *   画から顔を推測しない。完成 MP4 で焦点どおりの範囲を見せていることは cameraFocusMeasured が測る
 *
...
/** 場面ごとの焦点の欄（台本パッケージの本編の場面の cameraFocus）。 */
...
/**
 * 場面ごとの焦点（`{ x, y }`、画の中の位置の 0〜1。左上が 0）を正規化する（純粋関数）。台本パッケージの検査と
 * ショットのまとめ方が同じ規則を使う。戻り値の problem は欄の名前（"" なら正しい）。undefined は「指定なし」。
 */
...
/**
 * 時刻を持つ文（{ id, part, imageKey, imagePath, startFrame, frames, cameraMove?, cameraFocus? }）を、同じ画が
 * 続く範囲ごとのショットにまとめる（部の中で、画の鍵が同じ隣り合う文）。文の順は変えない。
 * 場面ごとの焦点はショットの最初の文から取る。同じショットの文が違う焦点を持てば（片方だけ持つ場合も）、
...
export function groupNarratedCameraShots(segments = []) {
  ...
  for (const segment of segments) {
    ...
    shots.push({
      ...
      ...(focus.focus ? { requestedFocus: focus.focus } : {}),
 ...
 * 場面が焦点を指定していれば、その焦点で動きを決め、ショットに focus（{ x, y, source: "script-package" }）を残す。
 */
export function planNarratedCameraShots({ segments = [], camera, fps }) {
  ...
  const planned = shots.map((shot) => {
    ...
    const { requestedFocus, focusConflicts, ...rest } = shot;
    const focus = requestedFocus ? { ...requestedFocus, source: "script-package" } : null;
...
export function narratedCameraManifestEntry(plan, camera) {
  return {
    ...
    shots: plan.shots.map((shot) => ({
      ...
      // 場面ごとの焦点（台本パッケージの cameraFocus）。指定の無いショットには欄を足さない（従来の記録と同じ形）。
