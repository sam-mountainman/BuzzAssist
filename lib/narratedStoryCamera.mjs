...
export function groupNarratedCameraShots(segments = []) {
  ...
  for (const segment of segments) {
    ...
    if (last && last.part === (segment.part || "story") && last.imageKey === key && last.startFrame + last.frames === segment.startFrame) {
      ...
      // ショットの尾の間（本編の間。lib/narratedStoryPacing.mjs）は最後の文のもの。
      if (Number.isInteger(segment.pauseTailFrames)) last.pauseTailFrames = segment.pauseTailFrames;
      else delete last.pauseTailFrames;
      continue;
    }
    shots.push({
      ...
      // ショットの頭・尾の間のフレーム数（本編の間を持つ文だけ。場面の切り替えの重なりを間の中に収める）。
