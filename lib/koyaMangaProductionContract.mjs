...
export function validateKoyaMangaProductionContract(contract) {
  ...
  requireValue(failures, contract?.qualityLoop?.version === "koya-quality-loop-v2", "qualityLoop.version", "must use quality loop v2");
  for (const key of [
  ...
  for (const key of ["requireDistinctVariationAxes", "anonymousComparison", "revealMappingOnlyAfterVerdict", "requireSelectionReason"]) {
