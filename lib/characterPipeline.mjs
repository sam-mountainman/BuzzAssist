...
export function buildApprovedTurnaroundJob(workflow, cast, candidate, options = {}) {
  ...
  const prompt = [
    ...
    styleReferences.length > 0
      ...
      : "",
    "On a pure white 16:9 canvas, show front, left-profile, and back full-body standing views plus front, gentle 3/4, and profile head views. Every view must depict the exact same approved person.",
...
function defaultShotType(tags) {
  if (tags.includes("wide")) return "eye-level establishing wide with a normal lens";
  if (tags.includes("closeup")) return "eye-level reaction close-up with a normal portrait lens";
  if (tags.includes("dialogue")) return "eye-level medium two-shot with a normal lens";
  return "eye-level medium shot with a normal lens";
}
...
export function buildCharacterStoryboardJobs(workflow, scenes, options = {}) {
  ...
  return normalizedScenes.map((scene, index) => {
    ...
    const styleReferences = selectChannelVisualReferences(workflow.visualProfile, sceneWithCharacters, options);
    ...
    const sceneDirectives = [
      `Shot type: ${nonEmptyString(scene.shotType ?? scene.shot_type) || defaultShotType(styleTags)}.`,
      nonEmptyString(scene.camera) ? `Camera: ${nonEmptyString(scene.camera)}.` : "",
