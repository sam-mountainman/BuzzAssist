...
import {
  ...
  eyeOpenReviewKey,
  identityPackEyeOpenSheets,
  prepareCandidateDiversityReviewDraft,
  normalizeSideLockedFeatures,
...
function normalizeOutfitStages(value) {
  return (Array.isArray(value) ? value : []).map((entry, index) => {
    ...
    return {
      ...
      referenceAssetFile: nonEmptyString(source.referenceAssetFile ?? source.reference_asset_file),
      referenceAssetSha256: nonEmptyString(source.referenceAssetSha256 ?? source.reference_asset_sha256),
    };
  }).filter((entry) => entry.description || entry.invariants.length > 0).slice(0, 12);
}

// Declared eye-open differentials (開眼差分). Each variant gets its own 2x2
// default/open sheet; its id travels as storyStage on the job, the staged
// sheet, the identity review and the registry asset. Casts without a
// declaration keep the single unkeyed sheet.
function normalizeEyeOpenVariants(value) {
  const seen = new Set();
  return (Array.isArray(value) ? value : []).map((entry, index) => {
    const source = entry && typeof entry === "object" ? entry : {};
    const id = slugPart(source.id ?? source.storyStage ?? source.story_stage, `variant-${index + 1}`);
    return {
      id,
      label: nonEmptyString(source.label ?? source.name) || id,
      description: nonEmptyString(source.description),
      invariants: stringList(source.invariants, 20),
      referenceAssets: (Array.isArray(source.referenceAssets ?? source.reference_assets) ? (source.referenceAssets ?? source.reference_assets) : [])
        .map((asset) => ({ path: nonEmptyString(asset?.path), sha256: nonEmptyString(asset?.sha256) }))
        .filter((asset) => asset.path)
        .slice(0, 4),
    };
  }).filter((entry) => {
    if (!entry.description || seen.has(entry.id)) return false;
    seen.add(entry.id);
    return true;
  }).slice(0, 8);
}
...
function normalizeIdentityPack(value) {
  ...
  return {
    ...
    eyeOpenSheets: (Array.isArray(value.eyeOpenSheets) ? value.eyeOpenSheets : []).map(normalizeGeneratedAsset).filter((entry) => entry.assetFile),
    ...
    repairHistory: (Array.isArray(value.repairHistory) ? value.repairHistory : []).slice(-20).map((entry) => ({
      ...
      findingsPath: nonEmptyString(entry?.findingsPath),
      findingsSha256: nonEmptyString(entry?.findingsSha256),
      repairPlanPath: nonEmptyString(entry?.repairPlanPath),
      repairPlanSha256: nonEmptyString(entry?.repairPlanSha256),
      repairPlanDigest: nonEmptyString(entry?.repairPlanDigest),
      findingIds: stringList(entry?.findingIds, 40),
      ...
      entry.findingsSha256 && entry.repairPlanSha256 && entry.repairPlanDigest && entry.findingIds.length > 0
    ))),
    refreshHistory: (Array.isArray(value.refreshHistory) ? value.refreshHistory : []).slice(-20).map((entry) => ({
      refreshId: nonEmptyString(entry?.refreshId),
      frozenFaceSha256: nonEmptyString(entry?.frozenFaceSha256),
      priorIdentityReviewPath: nonEmptyString(entry?.priorIdentityReviewPath),
      priorIdentityReviewSha256: nonEmptyString(entry?.priorIdentityReviewSha256),
      generationCheckpointPath: nonEmptyString(entry?.generationCheckpointPath),
      generatorContextId: nonEmptyString(entry?.generatorContextId),
      refreshedAt: nonEmptyString(entry?.refreshedAt),
    })).filter((entry) => entry.refreshId && entry.frozenFaceSha256 && entry.priorIdentityReviewSha256),
  };
}

function normalizeCorrectiveSupersedeHistory(value) {
  return (Array.isArray(value) ? value : []).slice(-20).map((entry) => ({
    roundId: nonEmptyString(entry?.roundId),
    supersededByRoundId: nonEmptyString(entry?.supersededByRoundId),
    reason: nonEmptyString(entry?.reason),
    supersededAt: nonEmptyString(entry?.supersededAt),
    priorStatus: nonEmptyString(entry?.priorStatus),
    selectedCandidateId: nonEmptyString(entry?.selectedCandidateId),
    identityReviewDraftPath: nonEmptyString(entry?.identityReviewDraftPath),
    identityReviewPath: nonEmptyString(entry?.identityReviewPath),
    stylingSelection: normalizeStylingSelection(entry?.stylingSelection),
    approval: entry?.approval && typeof entry.approval === "object" ? structuredClone(entry.approval) : null,
    identityPack: normalizeIdentityPack(entry?.identityPack),
  })).filter((entry) => entry.roundId && entry.supersededByRoundId && entry.reason && entry.supersededAt);
}
...
function mergeCastEntry(base, override = {}) {
  ...
  return {
    ...
    sideLockedFeatures: normalizeSideLockedFeatures(
      override.sideLockedFeatures ?? override.side_locked_features ?? base.sideLockedFeatures,
    ),
    eyeOpenVariants: normalizeEyeOpenVariants(override.eyeOpenVariants ?? override.eye_open_variants),
...
function normalizeWorkflowCast(entry, index) {
  ...
  return {
    ...
    invariants: stringList(source.invariants, 30),
    // 左右が固定された特徴（片肩だけ外したスカジャン、隠す側の目、片側の柄）
    // は明示宣言する。文章から左右を推測しないのは、推測が外れた瞬間に検査が
    // 黙って消えるため。宣言があるキャラは identity-pack レビューで
    // asymmetricFeatureChecks を書かない限り合格できない。
    sideLockedFeatures: normalizeSideLockedFeatures(source.sideLockedFeatures ?? source.side_locked_features),
    ...
    eyeOpenVariants: normalizeEyeOpenVariants(source.eyeOpenVariants ?? source.eye_open_variants),
    ...
    correctiveSupersedeHistory: normalizeCorrectiveSupersedeHistory(source.correctiveSupersedeHistory),
    ...
    eyeOpenSheet: source.eyeOpenSheet && typeof source.eyeOpenSheet === "object"
      ...
      : null,
    eyeOpenSheets: (Array.isArray(source.eyeOpenSheets) ? source.eyeOpenSheets : [])
      .map(normalizeGeneratedAsset)
      .filter((entry) => entry.assetFile),
  };
}
...
export async function prepareCharacterWorkflow(args = {}) {
  ...
  await upsertCharacterWorkflow(args, workflow);
  return workflow;
}

/**
 * workflow を1件、ストアへ入れる（同じ id があれば差し替える）。読み・差し替え・書き込みを
 * 全部ストアの錠の中で行う。
 *
 * 以前はストアを錠の外で読み、書くときだけ revision を比べていた。別の話数の `images` が
 * 同時に workflow を足すと、後から書く側が必ず `Stale character workflow revision` で落ちる
 * ので、`images` はチャンネル全体で同時1本に縛られていた。ここで足すのは自分の workflow
 * 1件だけなので、錠の中で読み直してから差し替えれば、他の workflow を消さずに並べられる。
 */
async function upsertCharacterWorkflow(args, workflow) {
  const file = resolveCharacterWorkflowFile(args);
  return withCanvasFileLock(file, async () => {
    const store = await readCharacterWorkflowStoreFile(file);
    ...
    return writeCharacterWorkflowStoreFile(file, store, store.revision);
  });
}
...
function characterAppearanceDescription(cast, workflow, styleReferenceCount = 0) {
  ...
  const details = [
    ...
    // 左右は「本人基準」で渡す。画面の左右で書くと、向きが変わったビューで
    // 反転して指示の意味が壊れる。
    (cast.sideLockedFeatures || []).length > 0
      ? `Side-locked features, expressed as the character's own anatomical side and never mirrored between views: ${cast.sideLockedFeatures.map((feature) => `${feature.feature} is on the character's ${feature.expectedSide === "subject-left" ? "own left" : "own right"} side`).join("; ")}.`
      : "",
...
export async function importCharacterCandidateRebuild(args = {}) {
  ...
  if (!["codex", "claude", "legacy-migration"].includes(generatorHost)) throw new Error("Candidate rebuild import generatorHost must be codex, claude, or legacy-migration.");
  const canvasDir = resolveCanvasDir(args);
  ...
  for (const [index, mapping] of mapEntries.entries()) {
    ...
    const sourceGenerator = source?.generator && typeof source.generator === "object"
      ? source.generator
      : sourceManifest.generator;
    if (!["codex", "claude"].includes(nonEmptyString(sourceGenerator?.host))
      || !nonEmptyString(sourceGenerator?.id)
      || !nonEmptyString(sourceGenerator?.contextId)
      || !nonEmptyString(sourceGenerator?.model)) {
      throw new Error(`Candidate ${label} requires complete per-entry codex/claude source generator provenance.`);
    }
    if (generatorHost !== "legacy-migration" && (
      nonEmptyString(sourceGenerator.host) !== generatorHost
      || nonEmptyString(sourceGenerator.id) !== generatorId
      || nonEmptyString(sourceGenerator.contextId) !== generatorContextId
      || nonEmptyString(sourceGenerator.model) !== nonEmptyString(sourceManifest.generator.model)
    )) throw new Error(`Candidate ${label} source generator provenance does not match the import invocation.`);
    ...
    resolvedImports.push({ label, source, sourceGenerator: structuredClone(sourceGenerator), declared, assetFile, sha256 });
  }
  ...
  await writeJsonAtomic(evidencePath, {
    ...
    previousCandidates,
    importedCandidates: resolvedImports.map((entry) => ({ label: entry.label, path: entry.assetFile, sha256: entry.sha256, axis: entry.declared.axis, sourceGenerator: entry.sourceGenerator })),
  });
  const jobs = resolvedImports.map((entry) => {
    const previous = currentByLabel.get(entry.label);
    return {
      prompt: entry.source.prompt,
      model: entry.sourceGenerator.model,
...
export function isUnselectedCharacterStylingConsolidation({ target, supersededRound } = {}) {
  return supersededRound?.status === "awaiting-selection"
    && !supersededRound.selectedOptionId
    && target?.stylingSelection?.roundId !== supersededRound.id;
}
...
export async function markCharacterStylingVariationsGenerating(args = {}, workflowId, round) {
  return updateCharacterWorkflow(args, workflowId, (workflow) => {
    ...
    const correctiveSupersedeReason = nonEmptyString(args.correctiveSupersedeReason ?? args.corrective_supersede_reason);
    const supersededRound = supersedeRoundId ? findStylingVariationRound(target, supersedeRoundId) : null;
    const selectedCorrection = Boolean(correctiveSupersedeReason
      && supersededRound?.status === "selected"
      && supersededRound.selectedOptionId
      && target.stylingSelection?.roundId === supersededRound.id
      && target.stylingSelection?.optionId === supersededRound.selectedOptionId);
    assertStylingRoundBaseIsCurrent(selectedCorrection ? { ...target, stylingSelection: null } : target, round);
    if (supersedeRoundId) {
      ...
      const unselectedConsolidation = isUnselectedCharacterStylingConsolidation({ target, supersededRound });
      if (!selectedCorrection && !unselectedConsolidation) throw new Error(`Styling round ${supersedeRoundId} can be superseded only while awaiting an unmade human selection, or by an explicit corrective supersede of the exact current selection.`);
      ...
      if (eligibleOptions.length < 1 || (unselectedConsolidation && eligibleOptions.length !== supersededRound.options.length)) {
        throw new Error(`Styling round ${supersedeRoundId} can be superseded only after independently passed SHA-bound evidence exists.`);
      }
      if (selectedCorrection && correctiveSupersedeReason.length < 12) throw new Error("Corrective styling supersede requires a concrete reason of at least 12 characters.");
    }
    ...
    workflow.cast = workflow.cast.map((cast) => cast.id === target.id
      ? {
        ...
        ...(selectedCorrection ? {
          ...
          eyeOpenSheet: null,
          correctiveSupersedeHistory: [
            ...(cast.correctiveSupersedeHistory || []),
            {
              roundId: supersededRound.id,
              supersededByRoundId: nextRound.id,
              reason: correctiveSupersedeReason,
              supersededAt,
              priorStatus: cast.status,
              selectedCandidateId: cast.selectedCandidateId,
              identityReviewDraftPath: cast.identityReviewDraftPath,
              identityReviewPath: cast.identityReviewPath,
              stylingSelection: cast.stylingSelection,
...
async function measureStylingColorAxis(options) {
  ...
  const python = await requirePythonRuntime({
    ...
    projectDir: resolve(MODULE_DIR, ".."),
    requiredModules: ["cv2", "numpy"],
  });
  const { stdout } = await execFile(
    python.command,
    [...python.args, STYLING_COLOR_MEASUREMENT_SCRIPT, JSON.stringify(payload)],
...
export async function validateCharacterStylingReview({ reviewPath, workflow, cast, round, allowFailedOutcome = false } = {}) {
  ...
  if (round.kind === "hairColor") {
    try {
      ...
      if (freshMachineAxisMeasurement.pass !== true && !allowFailedOutcome) {
  ...
  for (const expected of expectedPairs) {
    ...
    if (round.kind === "hairColor") {
      ...
      if (measuredPair?.pass !== true && !allowFailedOutcome) failures.push(`Styling pair ${expected.pairId} cannot pass below ΔE76 ${MIN_STYLING_COLOR_DELTA_E76}.`);
...
function identityPackJobFileName(castId, role, options = {}) {
  const suffix = nonEmptyString(options.fileNameSuffix ?? options.file_name_suffix);
  return `${slugPart(castId)}-${slugPart(role, "identity")}${suffix ? `-${slugPart(suffix, "generation")}` : ""}.png`;
}
...
export function buildApprovedTurnaroundJob(workflow, cast, candidate, options = {}) {
  ...
  const prompt = [
    ...
    "APPROVED ACCESSORY CONTINUITY: preserve every wearable accessory visible in reference image 1 or named by the human-approved selected design axis in every applicable full-body angle. Render its view-correct front/side/back visibility; never make a shoulder towel, scarf, glasses, headband, jewelry or other approved wearable disappear merely because the camera angle changes. A one-sided shoulder towel or similar asymmetric wearable must stay on the same anatomical side in all views and must remain visible whenever that shoulder, neck edge, or upper back is visible. In a strict far-side profile where the approved shoulder is genuinely fully occluded by the body, do not move or mirror the wearable merely to expose it; adjacent front, opposite-profile, and back views must prove its continuous placement on the correct side. NEAR-SIDE VISIBILITY IS MANDATORY: in the profile that shows the same side of the body the wearable sits on, that wearable is the MOST visible it ever is — it must be drawn large and unmistakable on the near shoulder, never omitted, never reduced to a sliver. Omitting an approved wearable from its own near-side profile is as serious a failure as putting it on the wrong side. Before finalising the two profiles, check each one: name which anatomical side it shows, then confirm the wearable is prominent on the near side in one profile and only partially visible past the silhouette in the other. The two profiles must therefore NOT contain equal amounts of that wearable. Do not mirror it to the other side. Do not add any accessory that is absent from both authorities.",
    "NEUTRAL PRODUCTION STANCE: a turnaround is a production reference, not a character illustration. Every human full-body view uses the same relaxed neutral stance: standing upright, weight even on both feet, arms hanging straight down at the sides, hands relaxed and empty. Do not copy a posed gesture from reference image 1: no crossed arms, no hand on hip, no pointing, no raised or bent arm, no leaning or contrapposto. HELD OBJECTS ARE EXCLUDED: any object the reference figure holds in a hand (phone, game device, cup, bag, tool, fan, document) must be omitted entirely and that hand rendered empty; this exclusion never applies to an object WORN on the body, which stays under the accessory-continuity rule above. Non-human animals keep their species-appropriate stance instead.",
    "STANCE PRECEDENCE: this stance rule outranks every posture, gesture, or held-object phrase that appears anywhere else in this prompt, including the character description, the human-approved design axis, and the preserved-trait list below. Those texts are authoritative for WHO the character is and WHAT they wear; they are never authoritative for how the figure stands or what it holds in this sheet. When a preserved trait names a held prop or a posed gesture, satisfy it by keeping the character's identity and wardrobe exactly and still rendering the neutral empty-handed stance.",
    "GARMENT DRAPE FIDELITY: reproduce exactly how each garment sits on the body in reference image 1 rather than an idealized or tidied version of it. If outerwear hangs off one shoulder, off both shoulders, is draped, slipped down, unzipped, open, half-tucked, rolled at the cuffs, or worn asymmetrically, keep that exact wearing state, the exact same anatomical side, and the same amount of exposed underlayer in every view. Never promote loosely worn or off-shoulder outerwear into a properly worn garment, never pull a fallen collar or sleeve back onto a shoulder, and never close an open garment. DO NOT SYMMETRISE: if the reference wears the garment off exactly one shoulder, keep that one shoulder bare and the other shoulder still covered — do not slide it off both shoulders and do not lift it back onto both. Read the reference shoulder by shoulder before drawing, and reproduce the same covered/bare pattern on the same anatomical sides in every view where those shoulders are visible. Changing how a garment is worn is a clothing drift failure even when the garment itself is correct.",
    ...
    cast.stylePrompt ? `\n${nonEmptyString(cast.stylePrompt)}` : "",
    workflow.channelStylePrompt,
    buildChannelVisualStylePrompt(workflow.visualProfile, {
  ...
  return {
    ...
    imageSize: nonEmptyString(options.imageSize ?? options.image_size) || workflow.imageSize,
    quality: nonEmptyString(options.quality) || workflow.quality,
    fileName: identityPackJobFileName(cast.id, "turnaround", options),
...
export function buildExpressionSheetJob(workflow, cast, candidate, options = {}) {
  ...
  const prompt = [
    ...
    "NEUTRAL PRODUCTION STANCE FOR ROW 3: the Row 3 coverage cells are camera-coverage references, not acted poses. Each human coverage figure stands upright in the same relaxed neutral stance with arms hanging down at the sides and hands empty. Do not copy a posed gesture from reference image 1 and do not include any object held in a hand; a held phone, game device, cup, bag or tool is omitted and the hand rendered empty. Objects WORN on the body stay under the accessory-continuity rule above. Non-human animals keep their species-appropriate stance.",
    "STANCE PRECEDENCE: the Row 3 stance rule outranks every posture, gesture, or held-object phrase that appears anywhere else in this prompt, including the character description, the human-approved design axis, and the preserved-trait list below. Those texts are authoritative for identity and wardrobe, never for how a coverage figure stands or what it holds. Row 1 and Row 2 facial expressions are unaffected by this rule.",
    "GARMENT DRAPE FIDELITY: reproduce exactly how each garment sits on the body in reference image 1 rather than an idealized or tidied version of it. If outerwear hangs off one shoulder, off both shoulders, is draped, slipped down, unzipped, open, half-tucked, or rolled at the cuffs, keep that exact wearing state, the exact same anatomical side, and the same amount of exposed underlayer in every cell. Never promote loosely worn or off-shoulder outerwear into a properly worn garment, never pull a fallen collar or sleeve back onto a shoulder, and never close an open garment. DO NOT SYMMETRISE: if the reference wears the garment off exactly one shoulder, keep that one shoulder bare and the other shoulder still covered — do not slide it off both shoulders and do not lift it back onto both. Read the reference shoulder by shoulder before drawing, and reproduce the same covered/bare pattern on the same anatomical sides in every view where those shoulders are visible.",
  ...
  return {
    ...
    imageSize: nonEmptyString(options.imageSize ?? options.image_size) || workflow.imageSize,
    quality: nonEmptyString(options.quality) || workflow.quality,
    fileName: identityPackJobFileName(cast.id, "expressions", options),
    referenceImagePaths: [candidate.assetFile, ...styleReferences.map((reference) => reference.path)],
    customData: {
...
export function buildOutfitSheetJob(workflow, cast, candidate, outfitStage, options = {}) {
  ...
  const approvedOutfitReference = nonEmptyString(outfitStage.referenceAssetFile);
  const includeApprovedOutfitReference = approvedOutfitReference
    && resolve(approvedOutfitReference) !== resolve(candidate.assetFile);
  const strictAuthorityOnly = new Set(Array.isArray(options.strictOutfitAuthorityStages)
    ? options.strictOutfitAuthorityStages.map(nonEmptyString).filter(Boolean)
    : []).has(nonEmptyString(outfitStage.id));
  const identityReferenceCount = includeApprovedOutfitReference ? 2 : 1;
  const styleReferenceStart = identityReferenceCount + 1;
  const styleReferenceEnd = identityReferenceCount + styleReferences.length;
  const prompt = [
    `Create a clean 2D Japanese manga OUTFIT CONTINUITY SHEET for ${cast.name}, story stage '${outfitStage.id}'.`,
    includeApprovedOutfitReference
      ? "Reference image 1 is the selected CHARACTER IDENTITY. Reference image 2 is the exact independently reviewed, SHA-bound APPROVED OUTFIT AUTHORITY for this story stage. Preserve the identity from reference 1 and reproduce the garment design, color blocks, accessories and anatomical left/right placement from reference 2; do not redesign either."
      : "Reference image 1 is both the selected CHARACTER IDENTITY and the exact independently reviewed, SHA-bound APPROVED OUTFIT AUTHORITY for this story stage. Preserve its face, head, hair, age, build, skin tone, accessories and garment design; do not redesign it.",
    styleReferences.length > 0
      ? `Reference images ${styleReferenceStart}-${styleReferenceEnd} are CHANNEL STYLE-ONLY. Never copy a person or clothing design from them.`
      : "",
    strictAuthorityOnly
      ? "STRICT OUTFIT AUTHORITY MODE: derive the garment only from the exact approved outfit reference pixels above. Do not interpret, paraphrase, or redesign it from a generic garment name or prose description."
      : `Required outfit: ${outfitStage.label}. ${outfitStage.description}`,
    !strictAuthorityOnly && outfitStage.invariants.length > 0 ? `Exact garment invariants: ${outfitStage.invariants.join("; ")}.` : "",
    textAuthority.description,
  ...
  return {
    ...
    imageSize: nonEmptyString(options.imageSize ?? options.image_size) || workflow.imageSize,
    quality: nonEmptyString(options.quality) || workflow.quality,
    fileName: identityPackJobFileName(cast.id, `outfit-${slugPart(outfitStage.id, "stage")}`, options),
    referenceImagePaths: [
      candidate.assetFile,
      ...(includeApprovedOutfitReference ? [approvedOutfitReference] : []),
      ...styleReferences.map((reference) => reference.path),
    ],
    customData: {
      ...
      buzzassistCharacterOutfitAuthorityPath: approvedOutfitReference,
      buzzassistCharacterOutfitAuthoritySha256: nonEmptyString(outfitStage.referenceAssetSha256),
      buzzassistStyleReferencePaths: styleReferences.map((reference) => reference.path),
    },
    pipeline: {
      identityRole: "outfit",
      storyStage: outfitStage.id,
      outfitAuthorityPath: approvedOutfitReference,
      outfitAuthoritySha256: nonEmptyString(outfitStage.referenceAssetSha256),
    },
  };
}
...
export function castEyeOpenVariants(cast) {
  return normalizeEyeOpenVariants(cast?.eyeOpenVariants);
}
...
// not guarantee that differential. Declared eyeOpenVariants always need one.
export function castNeedsEyeOpenVariant(cast) {
  if (castEyeOpenVariants(cast).length > 0) return true;
  const selection = normalizeStylingSelection(cast?.stylingSelection);
...
// Review/repair key of an identity-pack job: outfit and eye-open sheets are
// keyed per storyStage (outfit stage or eye-open variant).
export function identityPackJobRoleKey(job) {
  const role = nonEmptyString(job?.pipeline?.identityRole);
  const storyStage = nonEmptyString(job?.pipeline?.storyStage);
  if (role === "outfit") return `outfit:${storyStage}`;
  if (role === "eye-open") return eyeOpenReviewKey(storyStage);
  return role;
}

export function buildEyeOpenVariantJob(workflow, cast, candidate, options = {}, eyeOpenVariant = null) {
  if (!candidate?.assetFile) throw new Error(`Candidate ${candidate?.id ?? ""} has no generated asset.`);
  const model = nonEmptyString(options.model) || workflow.model;
  const variant = eyeOpenVariant ? normalizeEyeOpenVariants([eyeOpenVariant])[0] : null;
  if (eyeOpenVariant && !variant) throw new Error(`${cast.name} eye-open variant requires an id and a description.`);
  const authorityReferences = variant?.referenceAssets || [];
  if (authorityReferences.some((asset) => !/^[a-f0-9]{64}$/u.test(asset.sha256))) {
    throw new Error(`${cast.name} eye-open variant ${variant.id} authority references require lowercase SHA-256 provenance.`);
  }
  const styleReferences = identityPackStyleReferences(workflow, cast);
  const textAuthority = identityPackTextAuthority(cast);
  const authorityStart = 2;
  const authorityEnd = 1 + authorityReferences.length;
  const styleStart = authorityEnd + 1;
  const styleEnd = authorityEnd + styleReferences.length;
  const prompt = [
    `Create a clean 2D Japanese manga EYES-OPEN DIFFERENTIAL SHEET (開眼差分) for ${cast.name}${variant ? ` — variant '${variant.label}'` : ""}.`,
    ...
    authorityReferences.length > 0
      ? `Reference image${authorityReferences.length > 1 ? `s ${authorityStart}-${authorityEnd} are` : ` ${authorityStart} is`} the exact SHA-bound APPROVED EYES-OPEN AUTHORITY for this variant. Reproduce their open-eye shape, iris, brow tension and expression intensity in the RIGHT COLUMN only; take identity from reference image 1 and nothing else from them.`
      : "",
    styleReferences.length > 0
      ? `Reference images ${styleStart}-${styleEnd} are CHANNEL STYLE-ONLY. They control rendering style and visual information density only. Never copy a person or identity from them.`
      : "",
    variant
      ? `On a pure white 16:9 canvas show a 2x2 grid of the same person. LEFT COLUMN = DEFAULT STATE: the eyes drawn as the character's usual narrow closed-arc slit eyes (top: relaxed front bust-up, bottom: relaxed 3/4 bust-up). RIGHT COLUMN = EYES-OPEN STATE (開眼): the SAME person with visible irises in the approved '${variant.label}' variant: ${variant.description} (top: front bust-up, bottom: 3/4 bust-up). This approved variant is the only expression change allowed. The open eyes must belong to this exact face; keep nose, mouth placement, age and hair identical between the two columns.`
      : "On a pure white 16:9 canvas show a 2x2 grid of the same person. LEFT COLUMN = DEFAULT STATE: the eyes drawn as the character's usual narrow closed-arc slit eyes (top: relaxed front bust-up, bottom: relaxed 3/4 bust-up). RIGHT COLUMN = EYES-OPEN STATE (開眼): the SAME person with visible irises and the eyelid opening and expression specified by the approved character-specific instructions below (top: front bust-up, bottom: 3/4 bust-up). Opening the eyes does not itself mean anger, menace, a glare, or enlarged eyes. Without an explicitly approved expression change, preserve relaxed brows and facial muscles. The open eyes must belong to this exact face; keep nose, mouth placement, age and hair identical between the two columns.",
    variant && variant.invariants.length > 0 ? `Eyes-open variant invariants: ${variant.invariants.join("; ")}.` : "",
    "HARD CELL CONTAINMENT: use four equal cells. Keep the entire head, hair and portrait silhouette inside each cell with at least 8% pure-white clearance on every edge. No grid lines, text or background scenery.",
  ...
  return {
    ...
    imageSize: nonEmptyString(options.imageSize ?? options.image_size) || workflow.imageSize,
    quality: nonEmptyString(options.quality) || workflow.quality,
    fileName: identityPackJobFileName(cast.id, variant ? `eyes-open-${variant.id}` : "eyes-open", options),
    referenceImagePaths: [
      candidate.assetFile,
      ...authorityReferences.map((asset) => asset.path),
      ...styleReferences.map((reference) => reference.path),
    ],
    customData: {
      ...
      buzzassistCharacterCandidateId: candidate.id,
      buzzassistCharacterName: cast.name,
      buzzassistCharacterLabel: `${cast.name}｜開眼差分${variant ? `｜${variant.label}` : ""}`,
      ...(variant ? {
        buzzassistCharacterEyeOpenVariant: variant.id,
        buzzassistCharacterEyeOpenAuthorityPaths: authorityReferences.map((asset) => asset.path),
        buzzassistCharacterEyeOpenAuthoritySha256s: authorityReferences.map((asset) => asset.sha256),
      } : {}),
      buzzassistStyleReferencePaths: styleReferences.map((reference) => reference.path),
    },
    pipeline: variant
      ? {
          identityRole: "eye-open",
          storyStage: variant.id,
          eyeOpenAuthorities: authorityReferences.map((asset) => ({ path: asset.path, sha256: asset.sha256 })),
        }
      : { identityRole: "eye-open" },
  };
}
...
// rules from the Channel Pack show bible (e.g. "cast A is the shortest adult; cast B
// and cast C are the same height"); no cast names are hard-coded here.
...
export function buildApprovedIdentityPackJobs(workflow, cast, candidate, options = {}) {
  ...
  const eyeOpenVariants = castEyeOpenVariants(cast);
  if (eyeOpenVariants.length > 0) {
    jobs.push(...eyeOpenVariants.map((variant) => buildEyeOpenVariantJob(workflow, cast, candidate, options, variant)));
  } else if (castNeedsEyeOpenVariant(cast)) {
    jobs.push(buildEyeOpenVariantJob(workflow, cast, candidate, options));
  }
...
export function buildApprovedIdentityPackRepairJobs(workflow, cast, candidate, failedRoles, options = {}) {
  ...
  const repairFindings = Array.isArray(options.repairFindings) ? options.repairFindings : [];
  const repairPlan = options.repairPlan && typeof options.repairPlan === "object" ? options.repairPlan : null;
  ...
  return allJobs.filter((job) => requested.has(keyFor(job))).map((job) => {
    ...
    const roleFindings = repairFindings.filter((finding) => nonEmptyString(finding.role) === role);
    const rolePlanEntries = (repairPlan?.entries || []).filter((entry) =>
      roleFindings.some((finding) => nonEmptyString(finding.findingId) === nonEmptyString(entry.findingId)));
    if (repairFindings.length > 0 && (roleFindings.length === 0 || rolePlanEntries.length !== roleFindings.length)) {
      throw new Error(`Identity-pack ROI repair role ${role} is not fully bound to findings and repair-plan entries.`);
    }
    const extension = extname(job.fileName) || ".png";
    const stem = basename(job.fileName, extension);
    const repairSource = role === "turnaround" ? cast.identityPack?.turnaround?.assetFile
      : role === "expression" ? cast.identityPack?.expression?.assetFile
        : job.pipeline?.identityRole === "eye-open"
          ? identityPackEyeOpenSheets(cast.identityPack).find((asset) => asset.storyStage === nonEmptyString(job.pipeline?.storyStage))?.assetFile
          : cast.identityPack?.outfitSheets?.find((asset) => `outfit:${asset.storyStage}` === role)?.assetFile;
    if (roleFindings.length > 0 && !nonEmptyString(repairSource)) {
      throw new Error(`Identity-pack ROI repair role ${role} has no staged source image to edit.`);
    }
    const referenceImagePaths = [...(job.referenceImagePaths || [])];
    if (repairSource && !referenceImagePaths.includes(repairSource)) referenceImagePaths.push(repairSource);
    const angleReference = role === "expression" ? nonEmptyString(cast.identityPack?.turnaround?.assetFile) : "";
    if (angleReference && !referenceImagePaths.includes(angleReference)) referenceImagePaths.push(angleReference);
    return {
      ...job,
      referenceImagePaths,
      fileName: `${stem}-repair-${repairId}${extension}`,
      prompt: [
        ...(repairSource ? [`IMAGE EDIT: reference image ${referenceImagePaths.indexOf(repairSource) + 1} is the existing ${role} sheet to repair. Preserve its successful drawings, layout and identity. Reference image 1 remains the approved identity authority. Edit the existing sheet according to the findings below; do not create a new character or replace already correct cells.`] : []),
        ...(angleReference ? [`Reference image ${referenceImagePaths.indexOf(angleReference) + 1} is the current turnaround: use its corresponding view to resolve the character's anatomical sides and camera angle. It is an angle reference, not the sheet being edited.`] : []),
        ...(roleFindings.length > 0 && repairSource ? [
          `Repair the existing ${role} production sheet, preserving the approved face, age, body, hairstyle, anatomical asymmetry, wardrobe and drawing style from the references.`,
          "The existing correct cells are the starting artwork. The finding-specific edits below are authoritative for this repair; do not import poses or layouts from the identity candidate card.",
          "Grid: turnaround = 4 columns x 2 rows; expression = 4 columns x 3 rows; eye-open = 2 columns x 2 rows. Equal cells, at least 8% white clearance on every edge. Keep the same required view in every cell unless its finding explicitly requests a correction. No text or visible grid rules.",
        ] : [job.prompt]),
        "REPAIR ROUND: the previous parent sheet failed independent original-scale cell QA. The final import is restricted to the declared repair ROI; pixels outside it are protected and any outside change fails the round.",
        "PURE-WHITE BACKGROUND IS MANDATORY for this and every later setting-sheet generation. No room, office, hotel, furniture, floor, wall, window, scenery, texture, or environmental shadow.",
        ...roleFindings.flatMap((finding) => [
          `FINDING ${finding.findingId}: ${finding.problem}`,
          `EXACT REPAIR INSTRUCTION: ${finding.repair}`,
        ]),
        ...rolePlanEntries.map((entry) => `AUTHORIZED REPAIR ROI ${entry.findingId}: ${JSON.stringify(entry.repairRegion)}; everything outside this ROI is immutable.`),
      ...
      pipeline: {
        ...
        identityRepairSourceRole: role,
        identityRepairSourcePath: nonEmptyString(repairSource),
        repairPlanDigest: nonEmptyString(repairPlan?.digest),
        repairFindingIds: roleFindings.map((finding) => finding.findingId),
        repairRegions: rolePlanEntries.map((entry) => entry.repairRegion),
      },
    };
  });
}
...
export async function stageApprovedCharacterIdentityPack(args = {}) {
  ...
  const projectRoot = resolve(nonEmptyString(args.projectDir) || process.cwd());
  for (const job of jobs) {
    for (const authority of Array.isArray(job.pipeline?.eyeOpenAuthorities) ? job.pipeline.eyeOpenAuthorities : []) {
      const authorityPath = resolve(projectRoot, nonEmptyString(authority?.path));
      let actualSha256 = "";
      try { actualSha256 = await characterAssetSha256(authorityPath); } catch (error) {
        throw new Error(`Eyes-open authority reference is unreadable: ${authorityPath} (${error.message}).`);
      }
      if (actualSha256 !== nonEmptyString(authority?.sha256)) {
        throw new Error(`Eyes-open authority reference bytes changed after declaration: ${authorityPath}.`);
      }
    }
  }
  ...
  const eyeOpenRows = byRole.get("eye-open") || [];
  const declaredEyeOpenVariants = castEyeOpenVariants(cast).map((variant) => variant.id);
  const stagedEyeOpenKeys = eyeOpenRows.map((row) => eyeOpenReviewKey(row.storyStage));
  if (new Set(stagedEyeOpenKeys).size !== stagedEyeOpenKeys.length) {
    throw new Error(`Identity-pack results contain more than one eyes-open sheet for the same variant: ${stagedEyeOpenKeys.join(", ")}.`);
  }
  if (declaredEyeOpenVariants.length > 0) {
    const generatedVariants = new Set(eyeOpenRows.map((row) => row.storyStage));
    const missingVariants = declaredEyeOpenVariants.filter((id) => !generatedVariants.has(id));
    if (missingVariants.length > 0) throw new Error(`Missing required eyes-open differential sheets for variant: ${missingVariants.join(", ")}.`);
    const undeclared = [...generatedVariants].filter((id) => !declaredEyeOpenVariants.includes(id));
    if (undeclared.length > 0) throw new Error(`Eyes-open sheets do not match a declared variant: ${undeclared.map((id) => id || "(unkeyed)").join(", ")}.`);
    eyeOpenRows.sort((left, right) => declaredEyeOpenVariants.indexOf(left.storyStage) - declaredEyeOpenVariants.indexOf(right.storyStage));
  } else {
    if (castNeedsEyeOpenVariant(cast) && !eyeOpenRows[0]?.assetFile) {
      throw new Error("This character requires a generated eyes-open differential before identity QA.");
    }
    if (eyeOpenRows.length > 1 || eyeOpenRows.some((row) => row.storyStage)) {
      throw new Error("Keyed or multiple eyes-open sheets require eyeOpenVariants declared on the character.");
    }
  }
  if ((cast.outfitStages || []).length > 1) {
  ...
  for (const asset of [selectedFace, turnaround, expression, ...eyeOpenRows, ...(byRole.get("outfit") || [])]) {
  ...
  const approval = {
    ...
    approvedAt: nonEmptyString(args.approvedAt ?? args.approved_at) || new Date().toISOString(),
    ...
    selectedVariationAxis: nonEmptyString(args.selectedVariationAxis ?? args.selected_variation_axis) || nonEmptyString(candidate.variationAxis),
  ...
  const identityPack = {
    ...
    turnaround,
    expression,
    // eyeOpen mirrors the first sheet for readers that predate eyeOpenSheets.
    eyeOpen: eyeOpenRows[0] || null,
    eyeOpenSheets: eyeOpenRows,
    ...
    repairHistory: [
      ...
      ...(args.repairEvidence && typeof args.repairEvidence === "object" ? [{
        ...
        findingsPath: nonEmptyString(args.repairEvidence.findingsPath),
        findingsSha256: nonEmptyString(args.repairEvidence.findingsSha256),
        repairPlanPath: nonEmptyString(args.repairEvidence.repairPlanPath),
        repairPlanSha256: nonEmptyString(args.repairEvidence.repairPlanSha256),
        repairPlanDigest: nonEmptyString(args.repairEvidence.repairPlanDigest),
        findingIds: stringList(args.repairEvidence.findingIds, 40),
    ...
    refreshHistory: [
      ...(cast.identityPack?.refreshHistory || []),
      ...(args.refreshEvidence && typeof args.refreshEvidence === "object" ? [{
        refreshId: nonEmptyString(args.refreshEvidence.refreshId),
        frozenFaceSha256: nonEmptyString(args.refreshEvidence.frozenFaceSha256),
        priorIdentityReviewPath: nonEmptyString(args.refreshEvidence.priorIdentityReviewPath),
        priorIdentityReviewSha256: nonEmptyString(args.refreshEvidence.priorIdentityReviewSha256),
        generationCheckpointPath: nonEmptyString(args.refreshEvidence.generationCheckpointPath),
        generatorContextId,
...
export async function finalizeApprovedCharacter(args = {}) {
  ...
  const eyeOpenDestinations = [];
  for (const sheet of identityPackEyeOpenSheets(cast.identityPack)) {
    const variantSuffix = sheet.storyStage ? `-${slugPart(sheet.storyStage, "variant")}` : "";
    const destination = await availableDestination(characterDir, `${slugPart(cast.id)}-eyes-open${variantSuffix}${extname(sheet.assetFile) || ".png"}`);
    await copyFile(resolve(sheet.assetFile), destination);
    eyeOpenDestinations.push({ sheet, destination });
  }
  ...
  const approval = {
    ...
    // 登録の前に品質ループの合格を確かめた記録（呼び出し側のゲートが渡したときだけ。形は台帳が検める）。
    ...(args.assetQualityApproval && typeof args.assetQualityApproval === "object" ? { assetQuality: args.assetQualityApproval } : {}),
  };
  const referenceRows = [
    ...
    // The unkeyed sheet keeps id "eye-open"; variants are keyed by storyStage
    // in declaration order. With two or more, callers must name the variant.
    ...eyeOpenDestinations.map(({ sheet, destination }) => ({
      id: sheet.storyStage ? `eye-open-${sheet.storyStage}` : "eye-open",
      role: "eye-open",
      destination,
      sha256: sheet.sha256,
  ...
  const characterEntry = {
    ...
    // 宣言は台帳側にも持たせる。ここで落とすと、登録済みキャラを土台に
    // 作り直すときに左右検査が復元されない。
    sideLockedFeatures: cast.sideLockedFeatures || [],
  ...
  const updatedWorkflow = await updateCharacterWorkflow(args, workflow.id, (current) => {
    current.cast = current.cast.map((entry) => entry.id === cast.id
      ? {
          ...
          eyeOpenSheet: eyeOpenDestinations[0] ? { ...eyeOpenDestinations[0].sheet, assetFile: eyeOpenDestinations[0].destination } : null,
          eyeOpenSheets: eyeOpenDestinations.map(({ sheet, destination }) => ({ ...sheet, assetFile: destination })),
        }
      : entry);
    current.status = workflowStatusForCast(current.cast);
    return current;
  ...
  return {
    ...
    copiedAssets: [faceDestination, turnaroundDestination, expressionsDestination, ...eyeOpenDestinations.map((entry) => entry.destination), ...outfitDestinations.map((entry) => entry.destination)],
  };
}
...
export function buildCharacterStoryboardJobs(workflow, scenes, options = {}) {
  ...
  return normalizedScenes.map((scene, index) => {
    ...
    const eyeOpenVariant = nonEmptyString(scene.eyeOpenVariant ?? scene.eye_open_variant);
    ...
    return {
      ...
      characterIds,
      referenceIntent,
      productionReferenceIndexPath: nonEmptyString(scene.productionReferenceIndexPath ?? scene.production_reference_index_path ?? options.productionReferenceIndexPath),
      storyStage,
      ...(eyeOpenVariant ? { eyeOpenVariant } : {}),
      referenceImagePaths: styleReferences.map((reference) => reference.path),
      ...
      customData: {
        ...
        buzzassistProductionReferenceIndexPath: nonEmptyString(scene.productionReferenceIndexPath ?? scene.production_reference_index_path ?? options.productionReferenceIndexPath),
        buzzassistCharacterStoryStage: storyStage,
        ...(eyeOpenVariant ? { buzzassistCharacterEyeOpenVariant: eyeOpenVariant } : {}),
