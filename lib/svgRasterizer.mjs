...
async function rasterizeSvgWithChrome(chromePath, svgPath, pngPath, { env = process.env } = {}) {
  ...
  try {
    ...
    await runChromeScreenshot(chromePath, [
      ...
      // This disposable SVG-only profile must never prompt for the user's macOS keychain.
      "--use-mock-keychain",
...
export async function probeSvgRasterizer({
  ...
  try {
    ...
    try {
      ...
      // ブラウザーの初回の起動は遅いことがあり（Windows の CI で、1回目だけ screenshot が時間切れになり代替経路へ
      // 落ちた。2026-09-26）、起動し直すと描ける。ブラウザーがあるのに代替経路へ落ちたときは、ブラウザーで1回だけ
      // 描き直してから判定する（本番の描画も同じブラウザーを使うので、描けるなら描ける版で測る）。
      if (resolved.path && result?.backend !== "chrome") {
        const retried = await rasterize(svgPath, pngPath, { env, platform, log: () => {} }).catch(() => null);
        if (retried?.backend === "chrome") result = retried;
      }
    } catch (error) {
