...
import { resolveLearningTarget } from "./harnessLearningTargets.mjs";
...
export function normalizeHarnessFeedbackTarget(value) {
  ...
  // 旧名の対応表は harness-learn と同じ定義（lib/harnessLearningTargets.mjs）を読む。
  // ここにだけ別の表を持つと、同じ提案が台帳側と bundle 側で別の宛先に数えられる。
  const target = resolveLearningTarget(original);
