...
export { NARRATED_CAMERA_AUDIT_ID, NARRATED_REVIEW_LAYOUT_AUDIT_ID, NARRATED_SCENE_TRANSITION_AUDIT_ID, NARRATED_SUBTITLE_AUDIT_ID };
...
export async function loadNarratedVisualConfig(source = {}, { render, channelPackDir }) {
  ...
  return { config: { subtitles: subtitleConfig, camera: camera.config, sceneTransition: sceneTransition.config }, blockers: [...new Set(blockers)] };
}
...
export function planNarratedReviewLayout({ config, bookendPlan, storyScenes = [], operatorVideos = null }) {
  ...
  if (content === "scene-motion") {
    const moveByScene = new Map();
    for (const shot of partCameraShots(bookendPlan.segments.filter((segment) => segment.part === "story"), config)) {
      if (!moveByScene.has(shot.imageKey)) moveByScene.set(shot.imageKey, shot.move);
    }
    for (const section of planned.sections.filter((entry) => entry.layout === "tv-left-presenter-right")) {
      const move = moveByScene.get(section.tvScene) || config.camera.sequence[0];
      const motion = planCameraMove(move, config.camera.moves[move], section.frames / config.render.fps);
      section.tvMotion = { move, frames: section.frames, from: motion.from, to: motion.to, easing: config.camera.easing };
    }
  }
  let tvVideoPath = "";
...
export async function narratedVisualAuditChecks({ ffmpeg, videoPath, config, subtitleLayer, renderGraphs = {}, cameraPlan = null, operatorVideos = null, bookendPlan = null, reviewLayout = null }) {
  ...
  const sceneJoins = cameraPlan && cameraPlan.problems.length === 0
    ...
    : null;
  const staticExclusions = [subtitleExclusionRegion(subtitleLayer, config.subtitles)].filter(Boolean);
  if (!sceneJoins) {
