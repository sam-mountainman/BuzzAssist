...
import { appliedCutVideoBinding } from "./mangaCutVideoSubstitutionPolicy.mjs";
...
export function createKoyaRenderedCameraPlan(manifest = {}) {
  ...
  const rows = [];
  const staticRows = [];
  const videoSubstitutedCuts = [];
  for (const cut of manifest.cuts || []) {
    // A generated clip's optical flow is subject motion, not page-camera
    // motion. Its real frames are audited by the video-substitution audit;
    // the camera grammar (including all three families) must still be
    // visible in the cuts that remain stills.
    const videoSubstitution = appliedCutVideoBinding(cut);
    if (videoSubstitution) {
      videoSubstitutedCuts.push({ cutId: cut.id, clipSha256: videoSubstitution.clipSha256 });
      continue;
    }
  ...
  return { version: "koya-rendered-camera-plan-v1", rows, staticRows, videoSubstitutedCuts };
}
...
export function evaluateKoyaRenderedCamera({ manifest, plan, motion, fullDecodePass = true }) {
  ...
  return {
    ...
    videoSubstitutedCuts: plan.videoSubstitutedCuts || [],
    gates,
...
export async function auditKoyaRenderedCamera(options = {}) {
  ...
  await runCommand(python.command, [...python.args,
  ...
  try {
    await runCommand("ffmpeg", ["-v", "error", "-xerror", "-i", videoPath, "-f", "null", "-"], {
