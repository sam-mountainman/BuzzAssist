...
/** Job を読む（動画 Job の層は重いので、結び付けを扱うときだけ読む。置き場の規則は1か所: videoHarnessJobPath）。 */
export async function readThumbnailBindingJob({ projectDir, jobId }) {
  const { readVideoHarnessJob } = await import("./videoHarnessJob.mjs");
  return readVideoHarnessJob({ projectDir, jobId });
}

const defaultReadJob = readThumbnailBindingJob;

/** Job から結び付けの値（jobId・回の id・完成動画の SHA）を取る。完成動画がまだ無ければ SHA は空。 */
export function thumbnailJobBindingFromJob(job) {
  const finalVideo = (Array.isArray(job?.artifacts) ? job.artifacts : [])
  ...
  return {
    ...
    videoSha256: finalVideo ? nonEmpty(finalVideo.sha256).toLowerCase() : "",
  };
}
...
export async function auditThumbnailPlan(options = {}) {
  ...
  let jobBinding = null;
  // 下書きの空の欄（jobId・episodeId・videoSha256 が全部空）は、まだ結び付けていないのと同じに扱う。
  const blankBinding = plainObject(plan.jobBinding)
    && ["jobId", "episodeId", "videoSha256"].every((key) => !nonEmpty(plan.jobBinding[key]))
    && Object.keys(plan.jobBinding).every((key) => ["jobId", "episodeId", "videoSha256"].includes(key));
  if (plan.jobBinding !== undefined && !blankBinding) {
