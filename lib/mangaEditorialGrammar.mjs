...
export function classifyMangaEditorialBeat(input = {}) {
  ...
  const splitType = temporalMontage && finiteNumber(input.montageBeatCount, 0) >= 3
    ...
    : confrontationSplit || input.parallelLocations === true
      ? "vertical-2"
      : null;
  // A graphic plate is one characterless field, while a split page is a
  // composite of generated story panels. They cannot both be true for one
  // beat. Prefer the concrete montage/contrast when both heuristics fire.
  if (splitType) plateType = null;
  const resolvedBackgroundOnly = Boolean(plateType);
