...
// opening their eyes in too many scenes of one episode is flagged.
export function auditKoyaEyeOpenPlan({ policy, eyeOpenPlan } = {}) {
  ...
  return {
    ...
    warnings: eyeOpenSceneWarnings(boundImages.map((entry) => ({ key: entry.characterId, scene: entry.cutId })), labelFor),
  };
}
...
function storyEyeOpenScenes({ showBible, review, parsed, byId }) {
  if (Array.isArray(review?.eyeOpenBeats)) {
    return review.eyeOpenBeats.map((entry) => ({
      key: nonEmpty(entry?.castId),
      scene: byId.get(nonEmpty(entry?.utteranceId))?.cutId || nonEmpty(entry?.utteranceId),
    }));
  }
  const policy = buildKoyaEyeOpenPolicy({ showBible });
  ...
  return detectMangaEyeOpenBeats({ cuts: Array.isArray(parsed?.cuts) ? parsed.cuts : [], candidates: policy.candidates })
    .beats.map((beat) => ({ key: memberIdFor(beat.characterId), scene: beat.cutId }));
}
...
export function auditKoyaStory(options = {}) {
  ...
  warnings.push(...eyeOpenSceneWarnings(storyEyeOpenScenes({ showBible, review, parsed, byId })));
...
export function createKoyaStoryReviewDraft(options = {}) {
  ...
  return {
    ...
    eyeOpenBeatWarnings: eyeOpenSceneWarnings(detectedEyeOpen.beats.map((beat) => ({ key: memberIdFor(beat.characterId), scene: beat.cutId }))),
