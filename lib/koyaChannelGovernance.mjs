...
import { detectMangaEyeOpenBeats } from "./mangaEyeOpenBeats.mjs";
...
export function buildKoyaEyeOpenPolicy({ showBible, registry, storyReview } = {}) {
  ...
  const reviewedBeats = Array.isArray(storyReview?.eyeOpenBeats)
    ...
    : null;
  const recurringMemberId = nonEmpty(showBible?.storyGrammar?.castSemantics?.recurringEyeOpen?.castId);
  return {
    candidates,
    reviewedBeats,
    recurringMemberId,
    recurringCharacterId: recurringMemberId ? characterIdFor(recurringMemberId) : "",
  };
}

// The every-episode eye-open member must have at least one generated image
// bound to an eye-open beat whenever they are in the episode's active cast.
export function auditKoyaEyeOpenPlan({ policy, eyeOpenPlan, activeCastIds = [] } = {}) {
  const recurringMemberId = nonEmpty(policy?.recurringMemberId);
  const recurringRequired = Boolean(recurringMemberId) && activeCastIds.includes(recurringMemberId);
  const boundImages = Array.isArray(eyeOpenPlan?.boundImages) ? eyeOpenPlan.boundImages : [];
  const recurringImages = boundImages.filter((entry) => entry.characterId === policy?.recurringCharacterId);
  const failures = recurringRequired && recurringImages.length === 0
    ? [`${recurringMemberId} must open their eyes in every episode, but no generated image binds an eye-open beat for them. Add the beat to the script or to the reviewed eyeOpenBeats.`]
    : [];
  return {
    pass: failures.length === 0,
    ...
    recurringImageCount: recurringImages.length,
    failures,
  };
}

function auditReviewedEyeOpenBeats({ showBible, review, byId, activeCastIds, failures }) {
  ...
  const recurringId = nonEmpty(showBible?.storyGrammar?.castSemantics?.recurringEyeOpen?.castId);
  if (recurringId && activeCastIds.has(recurringId)) {
    storyReviewFailure(
      failures,
      entries.some((entry) => nonEmpty(entry?.castId) === recurringId),
      `eyeOpenBeats must include at least one eye-open beat for the every-episode eye-open member ${recurringId}.`,
    );
  }
}
...
export function auditKoyaStory(options = {}) {
  ...
  auditReviewedEyeOpenBeats({
    showBible,
    review,
    byId,
    activeCastIds: new Set([...fixedCastIds, ...declaredFixedCastIds]),
    failures,
  });
...
export function createKoyaStoryReviewDraft(options = {}) {
  ...
  return {
    ...
    instructions: "Fill every required beat with one utteranceInventory id, complete the human checks, and remove utteranceInventory only if desired. Confirm eyeOpenBeats against the meaning of each line: every image utterance where a listed member has their eyes open, with the variant id when the member declares two or more; resolve eyeOpenBeatFindings. Never change scriptSha256 by hand.",
  };
}
