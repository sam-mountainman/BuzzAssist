...
import { assertKoyaDeclaredProtagonist } from "./koyaMangaProductionContract.mjs";
...
const LOCATION_IMPORT_REFERENCE_ROLES = Object.freeze(["anchor", "style", "other"]);
...
function locationProvenanceGapShapeFailures(gap, label) {
  const failures = exactKeyFailures(gap, LOCATION_PROVENANCE_GAP_KEYS, [], `${label} provenanceGap`);
  if (failures.length > 0) return failures;
  if (gap.promptRecorded !== false || gap.generatorContextRecorded !== false) {
    failures.push(`${label} provenanceGap is only for records that do not exist: promptRecorded and generatorContextRecorded must both be false.`);
  }
  if (typeof gap.reason !== "string" || !gap.reason.trim() || gap.reason.length > MAX_LOCATION_PROVENANCE_GAP_REASON) {
    failures.push(`${label} provenanceGap.reason must say in at most ${MAX_LOCATION_PROVENANCE_GAP_REASON} characters why the prompt and the conversation were never recorded.`);
  }
...
function importedLocationEntryFailures({ entry, job, isAnchor, expectedAnchorSha256, label }) {
  ...
  if (Object.hasOwn(block, "provenanceGap")) {
    ...
    if (nonEmpty(entry?.generator?.contextId)) {
  ...
  if (!references || references.some((reference) => !nonEmpty(reference?.path)
    ...
    if (!isAnchor && (anchorReferences.length !== 1 || anchorReferences[0].sha256 !== nonEmpty(expectedAnchorSha256))) {
...
export async function createKoyaLocationAnchorReviewDraft(options = {}) {
  ...
  const instructions = [
    ...
    ...(anchorGap ? [`This anchor declares a provenance gap: its prompt and generator conversation were never recorded. Read the gap reason and the written specification named in the generation manifest (${nonEmpty(anchorGap.specificationPath)}), then set ${KOYA_LOCATION_PROVENANCE_GAP_CHECK} to true only if the anchor satisfies that specification.`] : []),
...
export async function auditKoyaLocationAnchorReview(options = {}) {
  ...
  // 出所欠落を宣言したボードには会話 id が無い。host と id と生成時刻は
  // 欠落を宣言しても残るので、そこは従来どおり要る。
  if (!nonEmpty(generated?.generator?.host) || !nonEmpty(generated?.generator?.id) || (!anchorGap && !nonEmpty(generated?.generator?.contextId))) failures.push("Location anchor generation provenance is incomplete.");
...
async function validateKoyaLocationImportMap({ plan, locationBible, importMapPath, mapBytes }) {
  ...
  for (const [position, board] of (Array.isArray(map.boards) ? map.boards : []).entries()) {
    ...
    const row = { position, label, jobIndex, board };
    ...
    const generatorFailures = exactKeyFailures(
      board.generator,
      hasGap ? ["host", "id", "generatedAt"] : ["host", "id", "contextId", "generatedAt"],
      hasGap ? ["contextId"] : [],
      `${label}.generator`,
    );
    ...
    if (hasGap) {
      ...
      const gapFailures = exactKeyFailures(board.provenanceGap, LOCATION_PROVENANCE_GAP_KEYS, [], `${label}.provenanceGap`);
  ...
  for (const row of rows) {
    ...
    if (row.jobIndex > 0 && (anchorReferences.length !== 1 || !anchorSha256 || anchorReferences[0].sha256 !== anchorSha256)) {
      failures.push(`${row.label} is a continuity view; list the imported anchor board (sha256 ${anchorSha256 || "unknown"}) exactly once as its role "anchor" reference.`);
    }
...
async function importKoyaLocationBoardsUnlocked({ authority, plan, importMapPath, canvasDir }) {
  ...
  const entries = validated.rows.map((row) => {
    ...
    return {
      ...
      promptSha256: gap ? "" : sha256(row.promptText),
      anchorSha256: row.jobIndex === 0 ? "" : validated.anchorSha256,
      generator: {
        host: nonEmpty(board.generator.host),
        id: nonEmpty(board.generator.id),
        ...(gap ? {} : { contextId: nonEmpty(board.generator.contextId) }),
      },
      ...
      import: {
        ...
        ...(gap ? { provenanceGap: gap } : { promptText: row.promptText, promptPath: row.promptPath }),
...
export async function auditKoyaLocationReview(options = {}) {
  ...
  for (const job of plan.jobs) {
    ...
    // 出所欠落を宣言したボードには会話 id が無い。残りの生成情報は要る。
    if (!nonEmpty(generated?.generator?.host) || !nonEmpty(generated?.generator?.id) || (!gap && !nonEmpty(generated?.generator?.contextId))) failures.push(`${job.boardId} generation provenance is incomplete.`);
...
export async function createKoyaLocationReviewDraft(options = {}) {
  ...
  for (const job of plan.jobs) {
    ...
    const gap = koyaLocationEntryProvenanceGap(generated);
    if (gap) gapBoards.push({ boardId: job.boardId, specificationPath: nonEmpty(gap.specificationPath) });
    boards.push({
      boardId: job.boardId,
      path: job.outputPath,
      sha256: buffer ? sha256(buffer) : "",
  ...
  const entry = {
    ...
    approval: {
      ...
      reason: importedBoards
        ? `All four SHA-bound environment boards passed independent original-scale and continuity review. The boards were imported from an external image tool with SHA-bound prompt, reference and context provenance.${provenanceGaps.length > 0 ? ` ${provenanceGaps.length} of them declare a provenance gap: no prompt and no generator conversation were ever recorded, and the independent reviewer acknowledged each gap against its written specification.` : ""}`
