...
export function koyaThumbnailAssetQualitySubjectId(plan, index, artworkPath) {
  // 組み立て方はジャンル共通（lib/thumbnailPlan.mjs）。ナレーション物語のサムネも同じ id になる。
  return thumbnailArtworkQualitySubjectId(plan, index, artworkPath);
}
...
// ---------------------------------------------------------------------------------------------
// サムネ: 計画・検査の本体はジャンル共通の lib/thumbnailPlan.mjs。ここは漫画の決まり（Channel Pack の
// thumbnail contract）をその「決まり」へ写し、今の出力（検査の結果・失敗の文言・下書き）を変えずに包むだけ。
// 変わらないことは test/koyaThumbnailParity.test.mjs が、寄せる前の出力（test/fixtures/koya-thumbnail-parity.golden.json）
// との比較で見る。
// ---------------------------------------------------------------------------------------------

const KOYA_THUMBNAIL_AUDIT_VERSION = "koya-thumbnail-audit-v1";
const KOYA_THUMBNAIL_HARNESS_ID = "koya-manga-video";
// 漫画の計画で、配置の理由を書く欄（3コマのときの3つ目の場面の理由）。
const KOYA_THUMBNAIL_LAYOUT_REASON_FIELD = "thirdBeatReason";
const KOYA_THUMBNAIL_FINAL_CHECKS = Object.freeze([
  "original1280x720", "mobile320x180", "textCropZero", "faceCropZero", "primaryEmotionReadable", "approvedCharacterReferencesOnly", "realLogoZero",
]);
const KOYA_THUMBNAIL_DRAFT_INSTRUCTIONS = "Approve exact copy and record koyaThumbnailCopySha256(plan) only after band/font tokens are human-approved. Change stage to final and add dedicated artwork plus main-video source images for reuse audit.";
// 契約の commonChecks 節で入れられる共通の検査（入れなければ今の漫画の検査のまま）。
const KOYA_THUMBNAIL_COMMON_CHECK_KEYS = Object.freeze([
  "lettering", "characterReferences", "composite", "jobBinding", "distinctPanelArtwork", "idea", "minimumDistinctIdeaAxes",
]);

/** 契約の配置の宣言（structure の *-image の数がコマの数、onlyWhenThemeNeedsThirdBeat が理由の要る配置）。 */
function koyaThumbnailLayouts(contract) {
  const layouts = {};
  let defaultLayout = "";
  for (const [id, spec] of Object.entries(contract?.layouts && typeof contract.layouts === "object" ? contract.layouts : {})) {
    const panels = Array.isArray(spec?.structure) ? spec.structure.filter((part) => /-image$/u.test(String(part || ""))).length : 0;
    if (panels < 1) throw new Error(`Koya thumbnail layout ${id} must list at least one *-image part in structure.`);
    layouts[id] = { panels, reasonRequired: spec?.onlyWhenThemeNeedsThirdBeat === true };
    if (spec?.default === true && !defaultLayout) defaultLayout = id;
  }
  if (Object.keys(layouts).length === 0) throw new Error("Koya thumbnail contract must declare its layouts.");
  return { layouts, defaultLayout: defaultLayout || Object.keys(layouts)[0] };
}

/**
 * 漫画の thumbnail contract（koya-thumbnail-contract-v1）を、ジャンル共通の決まりへ写す。
 * 帯・吹き出し・テロップの行数と字数、禁止語、ブランドの語（帯色・書体）、配置の種類、使い回しの距離は
 * 契約から。1コマあたりの吹き出しの数は依頼者の未決の判断なので警告に留める（今の漫画の検査と同じ）。
 * 共通の新しい検査（文字の設計・承認済みの参照・完成画・Job との結び付け・別のコマの使い回し・案の軸）は、
 * 契約に commonChecks 節を足したときだけ必須になる。計画にその欄を足せば、節が無くても検査は効く。
 */
export function koyaThumbnailRules(contract) {
  validateKoyaThumbnailContract(contract);
  const common = contract?.commonChecks === undefined ? {} : contract.commonChecks;
  if (!common || typeof common !== "object" || Array.isArray(common)) throw new Error("Koya thumbnail contract commonChecks must be an object.");
  const unknown = Object.keys(common).filter((key) => !KOYA_THUMBNAIL_COMMON_CHECK_KEYS.includes(key));
  if (unknown.length > 0) throw new Error(`Koya thumbnail contract commonChecks has unsupported keys: ${unknown.join(", ")}.`);
  const { layouts, defaultLayout } = koyaThumbnailLayouts(contract);
  return normalizeThumbnailRules({
    harnessId: KOYA_THUMBNAIL_HARNESS_ID,
    planVersion: KOYA_THUMBNAIL_PLAN_VERSION,
    auditVersion: KOYA_THUMBNAIL_AUDIT_VERSION,
    status: contract.status,
    canvas: { width: contract.canvas.width, height: contract.canvas.height },
    layouts,
    defaultLayout,
    layoutReasonField: KOYA_THUMBNAIL_LAYOUT_REASON_FIELD,
    copy: {
      band: { lines: contract.copy.band.lines, maxCharactersPerLine: contract.copy.band.maxCharactersPerLine },
      speechBubble: { lines: contract.copy.speechBubble?.lines, maxCharactersPerLine: contract.copy.speechBubble?.maxCharactersPerLine, perPanel: "pending" },
      telop: { minCharacters: contract.copy.telop?.minCharacters, maxCharacters: contract.copy.telop?.maxCharacters, concreteNounReviewRequired: true },
      forbiddenTerms: (contract.copy.forbiddenAbstractHooks || []).map((hook) => ({ text: String(hook), kind: "abstract hook" })),
      forbiddenTermSlots: ["band", "speechBubble"],
      exactTextApproval: "always",
    },
    brandTokens: [
      { id: "bandColorToken", value: contract.visual?.bandColorToken },
      { id: "bandFontToken", value: contract.visual?.bandFontToken },
    ],
    source: {
      dedicatedArtworkRequired: true,
      reuseDistance: Number(contract?.sourcePolicy?.normalizedGray32MaximumReuseDistance ?? 0.025),
      distinctPanelArtwork: common.distinctPanelArtwork === true,
    },
    finalChecks: [...KOYA_THUMBNAIL_FINAL_CHECKS],
    finalChecksBinding: common.composite === true ? "composite" : "none",
    composite: { required: common.composite === true },
    lettering: { required: common.lettering === true },
    characterReferences: { required: common.characterReferences === true },
    ideas: { required: common.idea === true, minimumDistinctAxes: common.minimumDistinctIdeaAxes },
    jobBinding: { required: common.jobBinding === true },
    draftInstructions: KOYA_THUMBNAIL_DRAFT_INSTRUCTIONS,
  });
}

export function koyaThumbnailCopySha256(plan = {}) {
  return thumbnailCopySha256(plan, { reasonField: KOYA_THUMBNAIL_LAYOUT_REASON_FIELD });
}

/** 登録簿で承認済みの参照画と、人が選んだ顔の SHA（ファイル名は使わない）。 */
async function koyaThumbnailApprovedReferences(projectDir) {
  const collected = await collectKoyaApprovedReferences({ projectDir });
  return new Map(collected.references.map((row) => [row.sha256, row.kind]));
}
...
export async function auditKoyaThumbnailPlan(options = {}) {
  ...
  const projectDir = resolve(options.projectDir || process.cwd());
  const result = await auditThumbnailPlan({
    rules,
    plan,
    projectDir,
    approvedReferences: () => koyaThumbnailApprovedReferences(projectDir),
    ...
    // 自己申告の checks は残すが、それだけでは合格にしない。v54 より前の契約では照合しない（null）。
    assetQualityGate: async () => {
      const productionContract = await koyaProductionContractForAssetQuality(options, projectDir);
      if (!koyaAssetQualityGateInForce(productionContract)) return null;
      const workDir = koyaAssetQualityWorkDir({ projectDir });
      return {
        check: ({ path, subjectId }) => checkKoyaAssetQuality({ contract: productionContract, workDir, stage: "thumbnail", subjectId, assetPath: path }),
      };
    },
  });
  // 今の漫画の出力の形（欄と順番）を保つ。共通の新しい欄は、計画にその欄があるときだけ足す。
  return {
    ...
    ...(plan.previousThumbnailPaths !== undefined ? { previousThumbnails: result.previousThumbnails } : {}),
    ...(plan.compositePath !== undefined || rules.composite.required ? { composite: result.composite } : {}),
    ...(result.lettering ? { lettering: result.lettering } : {}),
    ...(result.idea ? { idea: result.idea } : {}),
    ...(result.characterReferences ? { characterReferences: result.characterReferences } : {}),
    ...(result.jobBinding ? { jobBinding: result.jobBinding } : {}),
  };
}

export function createKoyaThumbnailPlanDraft(options = {}) {
  const rules = koyaThumbnailRules(options.thumbnailContract);
  return createThumbnailPlanDraft({
    rules,
    layout: options.layout,
    includeOptionalFields: false,
    ...(options.jobBinding ? { jobBinding: options.jobBinding } : {}),
  });
}
