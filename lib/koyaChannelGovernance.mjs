...
export function validateKoyaShowBible(showBible) {
  ...
  const cast-ad7e = cast.find((member) => member.id === "cast-ad7e");
  const cast-7806 = cast.find((member) => member.id === "cast-7806");
  const cast-8c15 = cast.find((member) => member.id === "cast-8c15");
  if (cast-ad7e?.requiredEveryEpisode !== true || !cast-ad7e?.requiredReferenceRoles?.includes("eye-open")) throw new Error("Show bible must require Horo in every episode with an eye-open asset.");
  if (!cast-7806?.requiredReferenceRoles?.includes("eye-open")) throw new Error("Show bible must require Tatsu's eye-open asset.");
  if (cast-8c15?.episodeRoleSelectionRequired !== true || !cast-8c15?.requiredReferenceRoles?.includes("outfit")) throw new Error("Show bible must require Ema's episode role and outfit evidence.");
 ...
 */
export function assertProductionChannelAuthority(authority, context = "production") {
...
export async function readKoyaChannelAuthority(options = {}) {
  ...
  const projectSources = Object.fromEntries(
    Object.entries(AUTHORITY_FILES).map(([key, path]) => [key, resolveChannelPackSource(projectDir, path)]),
  );
  ...
  const usedFixture = Object.values(projectSources).some((entry) => entry.kind === "fixture");
  const source = present.every(Boolean) ? (usedFixture ? "test-fixture" : "project") : "runtime-template";
  ...
  const root = present.every(Boolean) ? projectDir : runtimeRoot;
...
export function auditKoyaStory(options = {}) {
  ...
  storyReviewFailure(failures, castMemberForSpeaker(showBible, ibukiBeat)?.id === "cast-be83", "ibukiSignal must be spoken by the cast member registered as cast-be83.");
  ...
  const tatsuRows = fixedRows.filter((row) => row.cast.id === "cast-7806");
