...
export async function readKoyaChannelAuthority(options = {}) {
  ...
  const projectPaths = Object.fromEntries(Object.entries(AUTHORITY_FILES).map(([key, path]) => [key, join(projectDir, path)]));
  ...
  const paths = Object.fromEntries(Object.entries(AUTHORITY_FILES).map(([key, path]) => [key, join(root, path)]));
  ...
  for (const member of showBible.cast || []) {
    ...
    for (const relativePath of declarations) {
      ...
      if (!inside(root, absolutePath)) throw new Error(`${member.id} styling spec path escapes the authority root: ${relativePath}`);
