...
function importedLocationEntryFailures({ entry, job, isAnchor, expectedAnchorSha256, label }) {
  ...
  if (typeof block.promptText !== "string" || !block.promptText.trim() || sha256(block.promptText) !== entry.promptSha256) {
...
export async function createKoyaLocationAnchorReviewDraft(options = {}) {
  ...
  return {
    ...
    anchor: {
      ...
      dimensions,
      checks: Object.fromEntries(koyaLocationAnchorCheckKeys(location).map((key) => [key, false])),
    },
    instructions,
  };
}
...
export async function auditKoyaLocationAnchorReview(options = {}) {
  ...
  if (nonEmpty(generated?.generator?.contextId) === reviewerContextId) failures.push("Location anchor must be reviewed in a context different from its generator.");
  if (entries.some(isImportedLocationEntry)) {
    // 取り込みでは4枚が審査より前に揃っているので、アンカーの審査者も
    // 取り込んだ人と全ての外部生成 context から独立していなければならない。
    for (const entry of entries) {
      if (entry !== generated && nonEmpty(entry?.generator?.contextId) === reviewerContextId) {
  ...
  for (const key of koyaLocationAnchorCheckKeys(locationBibleEntry(options.locationBible, plan.location.id))) {
...
async function validateKoyaLocationImportMap({ plan, locationBible, importMapPath, mapBytes }) {
  ...
  for (const [position, board] of (Array.isArray(map.boards) ? map.boards : []).entries()) {
    ...
    const shapeFailures = exactKeyFailures(
      ...
      ["boardIndex", "boardLabel", "promptText", "promptPath", "promptSha256", "note"],
      label,
    );
    ...
    const generatorFailures = exactKeyFailures(board.generator, ["host", "id", "contextId", "generatedAt"], [], `${label}.generator`);
    ...
    if (generatorFailures.length === 0) {
      ...
      if (!nonEmpty(board.generator.contextId)) failures.push(`${label}.generator.contextId must name the conversation that produced the image.`);
    ...
    if (hasPromptText === hasPromptPath) {
...
async function importKoyaLocationBoardsUnlocked({ authority, plan, importMapPath, canvasDir }) {
  ...
  return {
    ...
    results: entries.map((entry) => ({
      ...
      generatorContextId: entry.generator.contextId,
    })),
...
export async function auditKoyaLocationReview(options = {}) {
  ...
  for (const job of plan.jobs) {
    ...
    if (nonEmpty(generated?.generator?.contextId) === reviewerContextId) failures.push(`${job.boardId} must be reviewed in a context different from its generator.`);
    ...
    for (const key of koyaLocationBoardCheckKeys(location)) {
...
export async function createKoyaLocationReviewDraft(options = {}) {
  ...
  for (const job of plan.jobs) {
    ...
    boards.push({
      ...
      sha256: buffer ? sha256(buffer) : "",
      dimensions,
      checks: Object.fromEntries(koyaLocationBoardCheckKeys(location).map((key) => [key, false])),
    });
  }
