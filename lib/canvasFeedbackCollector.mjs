...
/**
 * Job を作ったチャンネル（start が metadata.channel に残した台帳の id）。channel-pack 宛の提案は、そのチャンネルの
 * 保存先へ積む（同じハーネスの別のチャンネルと混ぜない）。共有層（genre / platform）へ一般化する提案には付けない。
 */
function channelCaptureOptions(job, target) {
  const id = typeof job?.metadata?.channel?.id === "string" ? job.metadata.channel.id.trim() : "";
  return id && String(target).startsWith("channel-pack:") ? { channelId: id } : null;
}

async function assertIsolatedLearningLedger(target, ledgerResolver, channelOptions = null) {
  if (!target.startsWith("channel-pack:")) return;
  let channelLedger;
  let sharedLedger;
  try {
    channelLedger = await canonicalPotentialPath(channelOptions
      ? ledgerResolver(target, "proposals", { channel: channelOptions.channelId })
      : ledgerResolver(target, "proposals"));
...
export async function collectCanvasFeedback({
  ...
  return withCanvasFileLock(feedbackStateFile, () => withCanvasFileLock(canvasRunStateFile, async () => {
    ...
    for (const candidate of ordered) {
      ...
      const channelOptions = channelCaptureOptions(job, candidate.target);
      await assertIsolatedLearningLedger(candidate.target, ledgerResolver, channelOptions);
      const proposalInput = {
      ...
      let output;
      try {
        output = channelOptions ? await proposalCapture(proposalInput, channelOptions) : await proposalCapture(proposalInput);
      } catch {
