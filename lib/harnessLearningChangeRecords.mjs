...
/**
 * 誰が当てた（戻した・外した）か。applied / rolled-back / rejected の行の actor 欄。
 * 欄の無い行（2026-09-26 より前）は、人の確認でしか当てられなかった時期の記録なので human と読む。
 */
export const CHANGE_ACTOR_AGENT = "agent";
export const CHANGE_ACTOR_HUMAN = "human";
/** 機械が機械として記録した印（harness-learn の attestationFor と同じ語）。 */
export const AGENT_SELF_ATTESTED = "agent-self-attested";
...
/** 行の actor。欄の無い古い行は、人の確認でしか当てられなかった時期の記録なので human。 */
export function changeActorOf(row) {
  if (row?.actor === CHANGE_ACTOR_AGENT || row?.actor === CHANGE_ACTOR_HUMAN) return row.actor;
  return row?.actor === undefined ? CHANGE_ACTOR_HUMAN : "";
}
 ...
 * 反映記録（apply が書くのと同じ形）として数える。エージェントが当てた変更も数える
 * （actor: agent。変更前後の sha256 と巻き戻しが残り、人の確認はリリースで1回）。
 * 巻き戻した変更は数えない。巻き戻しの行そのものは反映記録ではないので外す。それ以外の行は
 * そのまま返す。
 */
