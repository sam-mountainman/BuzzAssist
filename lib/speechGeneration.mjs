...
import {
  BUZZASSIST_SPEECH_PROVIDER,
  generateBuzzAssistSpeech,
  listBuzzAssistVoices,
} from "./buzzassistVoice.mjs";
import {
...
/**
 * 音声プロバイダーの選択。既定は BuzzAssist（自前基盤）。
 * 移行期間のみ SPEECH_PROVIDER=elevenlabs で従来動作に戻せる。
 */
export function resolveSpeechProvider(input = {}) {
  const requested = nonEmptyString(input.provider) || nonEmptyString(process.env.SPEECH_PROVIDER);
  if (requested === "elevenlabs") return "elevenlabs";
  return BUZZASSIST_SPEECH_PROVIDER;
}

/**
 * 台本から音声を作る統一エントリ。呼び出し側はプロバイダーを意識しなくてよい。
 * 返り値の形はどちらのプロバイダーでも同じ（alignment は既存の
 * speechBoundsFromAlignment にそのまま渡せる）。
 */
export async function generateSpeech(input = {}) {
  if (resolveSpeechProvider(input) === "elevenlabs") {
    return generateElevenLabsSpeech(input);
  }
  return generateBuzzAssistSpeech(input);
}

/** 声の一覧。BuzzAssist ではライブラリ声＋自分の専用声を返す */
export async function listVoices(input = {}) {
  if (resolveSpeechProvider(input) === "elevenlabs") {
    const voices = await listAllElevenLabsVoices(input);
    return Array.isArray(voices) ? voices : [];
  }
  return listBuzzAssistVoices(input);
}
...
export async function generateElevenLabsSpeech(input = {}) {
  ...
  const mediaJobBroker = input.mediaJobBroker || createPaidMediaJobBroker({
    stateDir: input.mediaJobStateDir,
    apiBase: input.mediaJobApiBase,
    apiFetch: input.mediaJobFetch,
    sleepFn: input.sleepFn,
    maxAttempts: input.maxAttempts,
  });
  let job = await mediaJobBroker.start({
    kind: "voice.synthesis",
    provider: "elevenlabs",
    model,
    adapterVersion: "elevenlabs-speech-server-v1",
    voiceId,
    requestKey: nonEmptyString(input.requestKey) || undefined,
    input: requestBody,
    output: {
      format: outputFormat,
    ...
    reservation: {
      unit: "characters",
      estimatedUnits: [...text].length,
      estimatedSeconds: Math.max(1, [...text].length / 5),
    },
  }, {
    signal: input.signal,
    timeoutMs: 300_000,
  ...
  if (job.status !== "completed") {
    job = await mediaJobBroker.waitFor({ requestKey: job.requestKey }, {
      signal: input.signal,
      timeoutMs: finiteNumber(input.jobTimeoutMs, 15 * 60_000),
      pollIntervalMs: finiteNumber(input.jobPollIntervalMs, 2_000),
  ...
  const artifactReader = input.artifactReader || readPaidMediaJobArtifact;
  const audioBuffer = await artifactReader(job, { fetchImpl: input.artifactFetch, signal: input.signal });
  const alignment = normalizeSpeechAlignment(job.result?.alignment);
  const rawAlignment = normalizeSpeechAlignment(job.result?.rawAlignment || job.result?.alignment);
  const durationSeconds = Math.max(
    0,
    finiteNumber(job.result?.durationSeconds, 0)
      || alignment.characterEndTimesSeconds.at(-1)
      || rawAlignment.characterEndTimesSeconds.at(-1)
      || finiteNumber(job.usage?.seconds, 0),
  );
  const speechBounds = speechBoundsFromAlignment(alignment, durationSeconds);
  const mediaJobReceipt = typeof mediaJobBroker.receipt === "function"
  ...
  return {
    ...
    requestId: nonEmptyString(job.result?.requestId),
    characterCost: finiteNumber(job.usage?.units, null),
    characterCount: [...text].length,
    mediaJobId: nonEmptyString(job.jobId),
...
export async function writeSpeechAsset(input = {}) {
  // プロバイダーは resolveSpeechProvider で決まる（既定は BuzzAssist）。
  const generated = input.generated || await generateSpeech(input);
  ...
  const sidecar = {
    version: 3,
    inputHash: nonEmptyString(input.inputHash ?? input.input_hash) || nonEmptyString(generated.inputHash),
    ...
    speechStartSeconds: generated.speechStartSeconds ?? speechBoundsFromAlignment(generated.alignment, generated.durationSeconds).startSeconds,
    speechEndSeconds: generated.speechEndSeconds ?? speechBoundsFromAlignment(generated.alignment, generated.durationSeconds).endSeconds,
    ...
    mediaJobId: generated.mediaJobId,
    providerJobId: generated.providerJobId,
    requestKey: generated.requestKey,
    mediaJobReceipt: generated.mediaJobReceipt,
...
export function speechAssetPublicResult(result = {}) {
  return {
    ...
    mediaJobReceipt: result.mediaJobReceipt,
