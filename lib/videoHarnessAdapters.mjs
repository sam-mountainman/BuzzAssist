...
import { PAID_CALL_GUARD_ENV, readPaidCallGuardLedger } from "./paidCallGuard.mjs";
...
/**
 * 子プロセスの環境。更新をまたいで確定させる実行（Job 層が context.paidCallGuardPath を渡す）では、
 * 新しい有料の呼び出しを止める関所（lib/paidCallGuard.mjs）の台帳を子の環境にだけ入れる。
 */
function childEnv(context = {}) {
  const guard = nonEmpty(context.paidCallGuardPath);
  return guard ? { ...process.env, [PAID_CALL_GUARD_ENV]: resolve(guard) } : process.env;
}
...
export async function prepareVideoHarnessJob({
  ...
  if (job.harness.id === EXPLAINER_HARNESS_ID) {
    if (verified.payloadKind !== EXPLAINER_PAYLOAD_KIND) {
      return { ok: false, blockers: ["explainer-channel-pack-payload-required"], evidence };
    }
    // 中身の形（パス・納品の記録・品質ループ・表示の決まり・制作のコマンドの雛形）と、台帳のチャンネルとの一致。
    // どちらも有料の処理ではなく、ここで止めれば Job は doctor へ進まない。
    let pack;
    try {
      pack = await loadExplainerChannelPack(verified.payloadDir);
      assertExplainerStartOptions(job.options || {});
    } catch (error) {
      return {
        ok: false,
        blockers: [nonEmpty(error?.code) || "explainer-channel-pack-invalid"],
        evidence,
        error: String(error?.message || error),
      };
    }
    ...
    if (channelOfJob && channelOfJob !== pack.channelId) {
      return {
        ok: false,
        blockers: ["explainer-channel-pack-channel-mismatch"],
        evidence,
        error: `Channel Pack のチャンネル（${pack.channelId}）が Job のチャンネル（${channelOfJob}）と違う。`,
      };
    }
    let acceptance;
    try {
...
async function runKoya(job, context) {
  ...
  // 更新をまたいで確定させる実行では、Job に固定した制作契約の写し（Job 層が中身の digest を確かめた
  // もの）で制作・監査する。いまのリポジトリの新しい版の契約を後から当てない。
  pushOption(args, "contract-path", nonEmpty(context.pinnedProductionContractPath) || job.options.contractPath);
  ...
  // 台本の品質ループの作業フォルダ（start が台本のあるフォルダを既定で入れる。Job の識別子に入る）。子は
  // Job に保存した台本の写しのバイト列について、このフォルダのループの答えを有料の処理の前に問う。
...
const EXPLAINER_ARTIFACT_KINDS = Object.freeze({
  auditReport: "audit-report",
  runReceipt: "genre-run-receipt",
  contactSheet: "contact-sheet",
  finalVideo: "final-video",
...
/**
 * 解説動画。Job の中の runner（配置表の scripts/explainer-video.mjs full）を、Job の束縛つきで起動する。
 * 取り込み（import-delivery）では、子の環境に有料の呼び出しの関所（lib/paidCallGuard.mjs）の台帳を入れ、子が
 * 終わったあとに読み直す。止めた呼び出しが1件でもあれば、子がどう報告しても完成にも人待ちにもしない（failed）。
 * 取り込んだ完成版のファイルは Job の成果物に写さない（チャンネルのフォルダの元のファイルを指す）。人待ちの間は
 * 完成 MP4 から作った contact sheet だけを成果物に載せ、完成 MP4 と納品の記録は、人の評価が合格して確定する
 * （final-audited）ときだけ載せる（共通の RunReceipt が検証し直し、Canvas はそのとき完成 MP4 を1回だけ投影する）。
 * 字幕・サムネ・投稿用情報の SHA と大きさは監査の報告と RunReceipt に残る。
 */
async function runExplainer(job, context) {
  let route;
  try {
    route = resolveHarnessDeploymentCommand(job.deployment);
    await access(route.entrypointPath, fsConstants.R_OK);
    ...
    return {
      status: "failed",
      error: `解説動画の正規入口が無い: ${job.deployment?.entrypoint || "(未設定)"}`,
      knownRemainingIssues: ["canonical-explainer-entrypoint-missing"],
    };
  }
  const mode = assertExplainerStartOptions(job.options || {});
  const args = [...route.args, "full", "--script-path", job.script.path, "--job-id", job.id, "--project-dir", job.projectDir];
  pushOption(args, "channel-pack-dir", context.prepareResult?.payloadDir);
  // 上位 Job 層で運営者 env と照合済みの信頼リスト path（人の評価の署名の照合用。ナレーション物語と同じ規則）。
  pushOption(args, "reviewer-trust-path", context.reviewerTrustPath);
  args.push(...commonUpstreamExecutionArgs(job));
  // 関所: 更新をまたいで確定させる実行では Job 層の台帳、取り込みでは Job の run フォルダの台帳。
  const guardPath = nonEmpty(context.paidCallGuardPath)
    || (mode === "import-delivery" ? join(resolve(job.runDir), "explainer", `paid-call-guard-r${job.revision}.jsonl`) : "");
  if (guardPath && !nonEmpty(context.paidCallGuardPath)) {
  ...
  const child = await (context.runChild || runChild)(route.command, args, {
    cwd: route.cwd,
    isCancellationRequested: context.isCancellationRequested,
    env: childEnv({ paidCallGuardPath: guardPath }),
  });
  const parsed = parseJsonOutput(child.stdout);
  if (child.termination?.reason === "cancelled" || (child.signal === "SIGTERM" && await context.isCancellationRequested?.())) {
    return { status: "cancelled", knownRemainingIssues: [], result: { signal: child.signal, termination: child.termination || null } };
  }
  const refused = guardPath ? readPaidCallGuardLedger(guardPath) : [];
  if (refused.length > 0) {
    const detail = `explainer-paid-call-refused: 取り込みの実行で、BuzzAssist の有料の送り口を通る呼び出しを ${refused.length} 件止めた（課金はしていない）。取り込みは有料の呼び出しをしない前提なので、完成にも人待ちにもしない。`;
    return { status: "failed", blockers: ["explainer-paid-call-refused"], error: detail, knownRemainingIssues: [detail] };
  }
  const artifacts = [];
  for (const [rawKind, value] of Object.entries(parsed?.artifacts || {})) {
    const path = nonEmpty(value?.path);
    if (path) artifacts.push(await verifiedArtifact(EXPLAINER_ARTIFACT_KINDS[rawKind] || rawKind, path, nonEmpty(value?.sha256)));
  }
  const status = nonEmpty(parsed?.status);
  const evidence = {
    mediaJobs: [],
    auditChecks: plainAuditChecks(parsed?.auditChecks),
    runReceiptPath: nonEmpty(parsed?.runReceiptPath),
  };
  if (child.code === 3 || /awaiting|waiting/iu.test(status)) {
    const issues = Array.isArray(parsed?.knownRemainingIssues) && parsed.knownRemainingIssues.length
      ? parsed.knownRemainingIssues.map(String)
      : [status || "production-paused"];
    return { status: "awaiting-human-review", blockers: issues, knownRemainingIssues: issues, artifacts, ...evidence, result: parsed };
  }
  if (child.code !== 0 || !parsed) {
    const issues = Array.isArray(parsed?.knownRemainingIssues) && parsed.knownRemainingIssues.length
      ? parsed.knownRemainingIssues.map(String)
      : [`解説動画の正規入口が失敗した (${child.code ?? child.signal}):${childTerminationSummary(child)} ${child.stderr.slice(-2000)}`];
    return { status: "failed", error: issues[0], knownRemainingIssues: issues, artifacts, ...evidence, result: parsed };
  }
  const issues = Array.isArray(parsed.knownRemainingIssues) ? parsed.knownRemainingIssues.map(String) : [];
  if (status !== "final-audited" || issues.length > 0) {
    return {
  ...
  return { status: "completed", knownRemainingIssues: [], artifacts, ...evidence, result: parsed };
}

function plainAuditChecks(value) {
...
export async function executeVideoHarnessAdapter(context = {}) {
  ...
  if (job?.harness?.id === EXPLAINER_HARNESS_ID) return runExplainer(job, context);
