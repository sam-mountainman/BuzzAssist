...
function narratedEvidence(parsed) {
  return {
...
function koyaBlockers(job) {
  const required = [
    ...
    ["protagonistSpeakerId", "protagonist-speaker-required"],
    ["characterBiblePath", "character-bible-required"],
    ["storyReviewPath", "independent-story-review-required"],
  ];
  return required.filter(([key]) => !nonEmpty(job.options?.[key])).map(([, blocker]) => blocker);
}
...
async function runKoya(job, context) {
  ...
  if (job.options.retryFailed === true) args.push("--retry-failed");
