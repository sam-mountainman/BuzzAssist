 ...
 * Pack が本編の場面の間（pacing。lib/narratedStoryPacing.mjs）を宣言していれば、切り替えは間の真ん中にあり、
 * 重なりは間の中（前の語りの終わりの後ろから次の語りの始まりの前まで）に収める。フレームの丸めで溢れる分は
 * 縮め、計画に reduced: "scene-pause" と残す（間より長い crossfade の宣言は有料生成の前に止めてある）。
 *
...
export function planNarratedSceneJoins({ shots = [], transition = null } = {}) {
  ...
  for (let index = 1; index < shots.length; index += 1) {
    ...
    let outgoingAllowance = overlapAllowance(outgoing.frames);
    let incomingAllowance = overlapAllowance(incoming.frames);
    const shotLimited = Math.min(requested, outgoingAllowance + incomingAllowance);
    // 本編の場面の間（Pack の pacing。lib/narratedStoryPacing.mjs）がある切り替えは、重なりを間の中に収める
    // （出る側は語りの終わりの後ろ、入る側は次の語りの始まりの前まで。字幕も語りも無いフレームだけで混ぜる）。
    const paused = Number.isInteger(outgoing.pauseTailFrames) || Number.isInteger(incoming.pauseHeadFrames);
    if (paused) {
      outgoingAllowance = Math.min(outgoingAllowance, outgoing.pauseTailFrames || 0);
      incomingAllowance = Math.min(incomingAllowance, incoming.pauseHeadFrames || 0);
    }
    ...
    const reduced = frames < requested ? (frames < shotLimited ? "scene-pause" : "shots-too-short") : "";
    if (frames < 2) {
      joins.push({ index, cutFrame, type: "cut", requestedFrames: requested, frames: 0, before: 0, after: 0, reduced: reduced || "shots-too-short" });
      continue;
    }
    joins.push({ index, cutFrame, type: "crossfade", requestedFrames: requested, frames, before, after, ...(reduced ? { reduced } : {}) });
  }
  return joins;
}
