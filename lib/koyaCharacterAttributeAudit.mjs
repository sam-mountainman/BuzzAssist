...
export function buildAttributeChecksFromInventory(inventory, baseDir) {
  ...
  for (const asset of assets) {
    ...
    if (reference) {
      const hairColorImage = asset.hairColorImage ? absolutize(baseDir, asset.hairColorImage) : file;
      const hairColorReference = asset.hairColorReference ? absolutize(baseDir, asset.hairColorReference) : reference;
      checks.push({
        id: `${id}:hairColorDelta`,
        type: "hairColorDelta",
        image: hairColorImage,
        reference: hairColorReference,
        ...(asset.hairColorImageCell ? { imageCell: asset.hairColorImageCell } : {}),
        ...(asset.hairColorReferenceCell ? { referenceCell: asset.hairColorReferenceCell } : {}),
        ...(asset.hairColorCellTopFraction ? { hairCellTopFraction: asset.hairColorCellTopFraction } : {}),
      });
    }
    if (asset.base) {
      checks.push({
    ...
    if (asset.requireWhiteBackground === true) {
      checks.push({
        id: `${id}:whiteBackground`,
        type: "whiteBackground",
        image: file,
        region: asset.whiteBackgroundRegion ?? [0, 0, 1, 1],
        minimumWhiteRatio: asset.minimumWhiteRatio ?? 0.90,
        whiteThreshold: asset.whiteThreshold ?? 245,
      });
    }
  }
...
export async function runKoyaCharacterAttributeGate(options = {}) {
  ...
  const decision = {
    ...
    machinePass: failed.length === 0 && coverage.complete,
