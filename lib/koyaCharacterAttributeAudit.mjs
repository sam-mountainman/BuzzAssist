...
export function buildAttributeChecksFromInventory(inventory, baseDir) {
  ...
  for (const asset of assets) {
    ...
    checks.push({
      ...
      type: "neckOrnament",
      image: file,
      ...(asset.cleanReference ? { reference: absolutize(baseDir, asset.cleanReference) } : {}),
    });
  }
  // One duplicate-takes check spans the whole set: near-identical options are
  // a property of the SET, not of a single asset (ledger R190).
  if (assets.length >= 2) {
    checks.push({
...
export async function runKoyaCharacterAttributeGate(options = {}) {
  ...
  const attestedHuman = new Set(humanGates.filter((gate) => gate.status === "pass").map((gate) => gate.id));
