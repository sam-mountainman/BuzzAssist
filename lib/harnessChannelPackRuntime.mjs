...
import { NARRATED_CAMERA_FIELDS, NARRATED_CAMERA_MOVES } from "./narratedStoryCamera.mjs";
import { NARRATED_REVIEW_LAYOUT_FIELDS } from "./narratedStoryReviewLayout.mjs";
import { NARRATED_OVERLAY_FIELDS } from "./narratedStoryOverlays.mjs";
import { NARRATED_PACING_FIELDS } from "./narratedStoryPacing.mjs";
import { NARRATED_SCENE_TRANSITION_FIELDS } from "./narratedStorySceneTransitions.mjs";
...
const TOP_LEVEL_FIELDS = new Set([
  ...
  "thumbnail",
  // 焼き込み字幕（lib/narratedStorySubtitles.mjs）とカメラ（lib/narratedStoryCamera.mjs）。
  "subtitles", "camera",
  // 本編の場面の切り替え（lib/narratedStorySceneTransitions.mjs）と固定の重ね物（lib/narratedStoryOverlays.mjs）。
  "sceneTransition", "overlays",
  // 本編の文と文・場面と場面の間（lib/narratedStoryPacing.mjs）。
  "pacing",
]);
...
const BOOKEND_OPENING_FIELDS = new Set([
  ...
  // 回ごとの OP 映像（kind: episode-video）の尺の範囲と音の扱い。
  "minSeconds", "maxSeconds", "useEmbeddedAudio",
]);
// voice: 感想パートの声の有無（voiced / none。値の検査は lib/narratedStoryBookends.mjs）。
const BOOKEND_REVIEW_FIELDS = new Set(["scriptMarker", "presenter", "music", "musicGain", "operatorReplacementMarker", "layout", "voice"]);
const BOOKEND_PRESENTER_FIELDS = new Set(["required", "video", "backgroundColor", "episodeVideo"]);
// 感想パートの配置（lib/narratedStoryReviewLayout.mjs）。
const REVIEW_LAYOUT_FIELDS = new Set(NARRATED_REVIEW_LAYOUT_FIELDS.top);
const REVIEW_LAYOUT_TV_FIELDS = new Set(NARRATED_REVIEW_LAYOUT_FIELDS.tv);
const REVIEW_LAYOUT_PRESENTER_FIELDS = new Set(NARRATED_REVIEW_LAYOUT_FIELDS.presenterFrame);
...
const CAMERA_FIELDS = new Set(NARRATED_CAMERA_FIELDS.top);
const CAMERA_MOVE_FIELDS = Object.fromEntries(NARRATED_CAMERA_MOVES.map((kind) => [kind, new Set(
  kind === "slow-push-in" || kind === "slow-pull-out"
    ? NARRATED_CAMERA_FIELDS.zoomMove
    : (kind === "pan-left" || kind === "pan-right" ? NARRATED_CAMERA_FIELDS.panMove : NARRATED_CAMERA_FIELDS.staticMove),
)]));
const CAMERA_EASING_FIELDS = new Set(NARRATED_CAMERA_FIELDS.easing);
const SCENE_TRANSITION_FIELDS = new Set(NARRATED_SCENE_TRANSITION_FIELDS);
const OVERLAY_FIELDS = new Set(NARRATED_OVERLAY_FIELDS.top);
const OVERLAY_ITEM_FIELDS = new Set(NARRATED_OVERLAY_FIELDS.item);
const OVERLAY_REGION_FIELDS = new Set(NARRATED_OVERLAY_FIELDS.region);
const PACING_FIELDS = new Set(NARRATED_PACING_FIELDS);
...
function validateSourceConfigShape(source) {
  ...
  if (source.bookends !== undefined) {
    ...
    if (review !== undefined && review !== null) {
      ...
      if (review.layout !== undefined && review.layout !== null) {
        assertAllowedFields(review.layout, REVIEW_LAYOUT_FIELDS, "narrated-story.json.bookends.review.layout");
        if (review.layout.tv !== undefined) assertAllowedFields(review.layout.tv, REVIEW_LAYOUT_TV_FIELDS, "narrated-story.json.bookends.review.layout.tv");
        if (review.layout.presenterFrame !== undefined) {
          assertAllowedFields(review.layout.presenterFrame, REVIEW_LAYOUT_PRESENTER_FIELDS, "narrated-story.json.bookends.review.layout.presenterFrame");
        }
      }
    }
  ...
  if (source.camera !== undefined) {
    assertAllowedFields(source.camera, CAMERA_FIELDS, "narrated-story.json.camera");
    if (source.camera.moves !== undefined) {
      assertAllowedFields(source.camera.moves, new Set(NARRATED_CAMERA_MOVES), "narrated-story.json.camera.moves");
      for (const [kind, move] of Object.entries(source.camera.moves)) {
        assertAllowedFields(move, CAMERA_MOVE_FIELDS[kind], `narrated-story.json.camera.moves.${kind}`);
      }
    }
    if (source.camera.easing !== undefined) assertAllowedFields(source.camera.easing, CAMERA_EASING_FIELDS, "narrated-story.json.camera.easing");
  }
  if (source.sceneTransition !== undefined) assertAllowedFields(source.sceneTransition, SCENE_TRANSITION_FIELDS, "narrated-story.json.sceneTransition");
  if (source.pacing !== undefined) assertAllowedFields(source.pacing, PACING_FIELDS, "narrated-story.json.pacing");
  if (source.overlays !== undefined) {
    assertAllowedFields(source.overlays, OVERLAY_FIELDS, "narrated-story.json.overlays");
    if (!Array.isArray(source.overlays.items)) throw new Error("narrated-story.json.overlays.items must be an array.");
    source.overlays.items.forEach((item, index) => assertAllowedFields(item, OVERLAY_ITEM_FIELDS, `narrated-story.json.overlays.items[${index}]`));
    if (source.overlays.faceRegions !== undefined) {
      assertAllowedFields(source.overlays.faceRegions, new Set(["story", "review"]), "narrated-story.json.overlays.faceRegions");
      for (const [part, list] of Object.entries(source.overlays.faceRegions)) {
        if (!Array.isArray(list)) throw new Error(`narrated-story.json.overlays.faceRegions.${part} must be an array.`);
        list.forEach((region, index) => assertAllowedFields(region, OVERLAY_REGION_FIELDS, `narrated-story.json.overlays.faceRegions.${part}[${index}]`));
      }
    }
  }
