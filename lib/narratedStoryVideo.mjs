...
import { assertScriptQualityWorkDirBoundToJob } from "./scriptQualityUseGate.mjs";
...
/** 子へ渡した動画の manifest が、Job の options.operatorVideoManifestPath と違うときの error.code。 */
export const OPERATOR_VIDEO_MANIFEST_UNBOUND_CODE = "operator-video-manifest-not-bound-to-job";

async function readUpstreamJob(upstreamJobPath) {
  try {
    return JSON.parse(await readFile(resolve(String(upstreamJobPath)), "utf8"));
  } catch {
    return null;
  }
}

function assertManifestOption(job, { optionKey, passedPath, code, what, how }) {
  const declared = text(job?.options?.[optionKey]);
  const passed = text(passedPath);
  if (!declared && !passed) return;
  if (!declared || !passed || resolve(declared) !== resolve(passed)) {
    const error = new Error(`${code}: the ${what} passed to the narrated runner must be the one the outer Job declares in options.${optionKey} (${how}).`);
    error.code = code;
    throw error;
  }
}
...
export async function assertOperatorImageManifestBoundToJob({ upstreamJobPath, operatorImageManifestPath }) {
  assertManifestOption(await readUpstreamJob(upstreamJobPath), {
    optionKey: "operatorImageManifestPath",
    passedPath: operatorImageManifestPath,
    code: OPERATOR_IMAGE_MANIFEST_UNBOUND_CODE,
    what: "operator image manifest",
    how: "start the Job with --operator-image-manifest",
  });
}

/**
 * 運営者の動画の取り込みの記録（回ごとの OP 映像など）も同じ規則: 子 CLI の --operator-video-manifest と、
 * 上位 Job の options.operatorVideoManifestPath（start の --options-json / MCP の options で渡す）が同じ path。
 */
export async function assertOperatorVideoManifestBoundToJob({ upstreamJobPath, operatorVideoManifestPath }) {
  assertManifestOption(await readUpstreamJob(upstreamJobPath), {
    optionKey: "operatorVideoManifestPath",
    passedPath: operatorVideoManifestPath,
    code: OPERATOR_VIDEO_MANIFEST_UNBOUND_CODE,
    what: "operator video manifest",
    how: "start the Job with options.operatorVideoManifestPath via --options-json or the MCP options",
  });
}
...
export async function runNarratedStoryVideo({
  ...
  if (upstreamCount === 0) {
    ...
    await assertOperatorVideoManifestBoundToJob({ upstreamJobPath, operatorVideoManifestPath });
    // 台本の品質ループの作業フォルダも同じ規則（lib/scriptQualityUseGate.mjs。漫画の子と同じ照合）。
    assertScriptQualityWorkDirBoundToJob({ job: await readUpstreamJob(upstreamJobPath), scriptQualityWorkDir });
  }
  ...
  const outcome = await runNarratedStoryPipeline({
    ...
    // 運営者の動画の取り込みの記録（回ごとの OP 映像など。Pack が枠を宣言したときだけ）。
    operatorVideoManifestPath: text(operatorVideoManifestPath) ? resolve(text(operatorVideoManifestPath)) : "",
    // 台本の品質ループの作業フォルダ（Job の options.scriptQualityWorkDir。無ければ台本のあるフォルダ）。
    scriptQualityWorkDir: text(scriptQualityWorkDir) ? resolve(text(scriptQualityWorkDir)) : "",
    // 台本の品質ループの答えの差し替えは in-process の試験だけ（CLI からは渡せない）。
    ...(runtime.scriptQualityVerdict ? { scriptQualityVerdict: runtime.scriptQualityVerdict } : {}),
    env,
  });
