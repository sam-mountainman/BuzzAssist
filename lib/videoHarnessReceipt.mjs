...
// 解説動画（explainer-video）の監査の報告と、人の評価の signoff の版（lib/explainerVideo.mjs と同じ値）。
const EXPLAINER_AUDIT_VERSION = "buzzassist-explainer-audit-v1";
const EXPLAINER_SIGNOFF_VERSION = "buzzassist-explainer-video-human-review-signoff-v1";
// 承認した人の評価の signoff に結合されていなければならない監査（どちらも監査契約 v2 から）。
const EXPLAINER_SIGNOFF_AUDIT_IDS = Object.freeze(["humanReviewSigned", "qualityLoopPassed"]);
// signoff 本文（reviewerAttestation を除く全体）を署名する subject。evidence document の pass 欄ではなく
// 本文の approved などを検証器が見る。
const BODY_SIGNED_ATTESTATION_SUBJECTS = new Set(["narrated-story-video", "explainer-video"]);
...
function effectiveAuditContract({ declaration, declarationSha256, report, job, pinnedContract = null }) {
  ...
  // 更新をまたいで確定させる Job は、計画時の宣言で測る（付け替えた今の宣言の版で引き上げない。新しい版の
  // 必須監査を後から求めない）。
  const plannedDeclarationSha256 = String(plannedCanonicalIdentityOf(job)?.harnessDeclaration?.sha256 || "").trim().toLowerCase();
...
function completionRequirements(declaration) {
  ...
  // 解説動画は取り込んだ納品の記録（DELIVERY.json）も成果物に要る。人の評価の署名がその SHA に結び付いている。
  if (producesKind === "explainer-video") {
    requiredArtifacts.push({ id: "delivery-manifest", matches: (artifact) => String(artifact?.kind || "") === "delivery-manifest" });
  }
  return {
    requiredArtifacts,
...
function signoffCandidates(report, { attestationSubject = "" } = {}) {
  ...
  if (attestationSubject === "explainer-video") {
    // 解説動画の人の確認（全編の試聴と初見の評価）。合格した回の signoff だけが report に載る。
    const signoff = report?.independentSignoff;
    if (!signoff || typeof signoff !== "object" || Array.isArray(signoff) || !signoff.sha256) return [];
    return [{
      type: "human",
      scope: "full-length-human-review",
      status: report?.status === "pass" ? "approved" : "rejected",
      evidencePath: signoff.path,
      evidenceSha256: signoff.sha256,
      reviewer: signoff.reviewer,
      reviewerContextId: signoff.reviewerContextId,
      evidenceContract: EXPLAINER_SIGNOFF_VERSION,
    }];
  }
...
async function assertCurrentKoyaResolvedContract(job) {
  ...
  // 更新をまたいで確定させる Job は、リポジトリの今の契約を読み直さず、Job に固定した写しの中身（版・digest を
  // 固定値と照合済み）の requiredAudits で測る。回の上書き（入力）が変わっていれば確定しない。
  const pinned = await readRebindPinnedProductionContract(job);
  if (pinned) {
    if (await episodeOverrideChanged(job)) throw new Error("Koyaの回の上書きが固定した後に変わった。");
    const pinnedAudits = pinned.contract.requiredAudits;
    if (!Array.isArray(pinnedAudits) || pinnedAudits.length === 0 || pinnedAudits.some((id) => typeof id !== "string" || !id.trim())) {
      throw new Error("Koyaの固定した制作契約に必須監査requiredAuditsが無い。");
    }
    return { ...expected, requiredAudits: [...pinnedAudits] };
  }
...
/**
 * 解説動画（explainer-video）: 人の全編の試聴・初見の評価の signoff を、今の Job・完成 MP4・完成 MP4 から作った
 * contact sheet・取り込んだ納品の記録（どれも disk から読み直して SHA を検証した成果物）と、監査の報告の
 * humanReviewSigned・qualityLoopPassed の証跡に結合し、信頼リストの鍵の署名をナレーション物語と同じ共通の手順で
 * 検証し直す。期待 subject は signoff の写しではなく、Job の真値と成果物の SHA から組む。
 */
function validateExplainerSignoffBinding({
  report,
  evidenceDocument,
  evidenceSha256,
  verifiedArtifacts,
  ...
  const inForce = new Set(auditContract?.requiredAuditIds || EXPLAINER_SIGNOFF_AUDIT_IDS);
  if (report?.version !== EXPLAINER_AUDIT_VERSION || report?.status !== "pass"
    || !Array.isArray(report?.knownRemainingIssues) || report.knownRemainingIssues.length > 0) {
    throw new Error(`signoffs[${index}]のexplainer-video audit reportがpassではない。`);
  }
  if (String(report?.jobId || "") !== String(job?.id || "")) {
    throw new Error(`signoffs[${index}]のaudit-reportが現在のJobに結合されていない。`);
  }
  if (evidenceDocument?.version !== EXPLAINER_SIGNOFF_VERSION
    || String(evidenceDocument?.jobId || "") !== String(job?.id || "")
    || evidenceDocument?.approved !== true
    || evidenceDocument?.fullLengthViewed !== true
    || !Array.isArray(evidenceDocument?.findings)
    || evidenceDocument.findings.length > 0
    || !Array.isArray(evidenceDocument?.knownRemainingIssues)
    || evidenceDocument.knownRemainingIssues.length > 0) {
    throw new Error(`signoffs[${index}]のexplainer-video 人の評価の signoff が承認・全編視聴の形ではない。`);
  }
  const finalVideo = singleArtifact(verifiedArtifacts, (artifact) => FINAL_VIDEO.test(artifactText(artifact)), "final-video");
  const contactSheet = singleArtifact(verifiedArtifacts, (artifact) => CONTACT_SHEET.test(artifactText(artifact)), "contact-sheet");
  const delivery = singleArtifact(verifiedArtifacts, (artifact) => String(artifact?.kind || "") === "delivery-manifest", "delivery-manifest");
  const bound = (value, label) => normalizedSha(value, `signoffs[${index}].${label}`);
  if (bound(report?.videoSha256, "report.videoSha256") !== finalVideo.sha256
    || bound(evidenceDocument?.videoSha256, "videoSha256") !== finalVideo.sha256) {
    throw new Error(`signoffs[${index}]が現在のfinal-video SHAに結合されていない。`);
  }
  if (bound(report?.contactSheetSha256, "report.contactSheetSha256") !== contactSheet.sha256
    || bound(evidenceDocument?.contactSheetSha256, "contactSheetSha256") !== contactSheet.sha256) {
    throw new Error(`signoffs[${index}]が現在のcontact-sheet SHAに結合されていない。`);
  }
  if (bound(report?.delivery?.sha256, "report.delivery.sha256") !== delivery.sha256
    || bound(evidenceDocument?.deliverySha256, "deliverySha256") !== delivery.sha256) {
    throw new Error(`signoffs[${index}]が現在の納品の記録（DELIVERY.json）の SHA に結合されていない。`);
  }
  for (const auditId of EXPLAINER_SIGNOFF_AUDIT_IDS.filter((id) => inForce.has(id))) {
    const observed = report?.checks?.[auditId];
    if (observed?.pass !== true
      || normalizedSha(observed?.signoffSha256, `${auditId}.signoffSha256`) !== evidenceSha256
      || normalizedSha(observed?.videoSha256, `${auditId}.videoSha256`) !== finalVideo.sha256
      || normalizedSha(observed?.contactSheetSha256, `${auditId}.contactSheetSha256`) !== contactSheet.sha256
      || normalizedSha(observed?.deliverySha256, `${auditId}.deliverySha256`) !== delivery.sha256) {
      throw new Error(`signoffs[${index}]がchecks.${auditId}の証跡に結合されていない。`);
    }
  }
  // 合格した回の採点が、品質ループの契約（評価項目と下限）に対するものであること。
  const qualityDigest = String(report?.checks?.qualityLoopPassed?.contractDigest || "");
  if (inForce.has("qualityLoopPassed")
    && (!/^[a-f0-9]{64}$/u.test(qualityDigest) || evidenceDocument?.qualityReview?.contractDigest !== qualityDigest)) {
    throw new Error(`signoffs[${index}]の採点が品質ループの契約に結合されていない。`);
  }
  const attestation = verifyExplainerReviewSignoff(evidenceDocument, {
    jobId: job?.id,
    identityDigest: job?.identityDigest,
    videoSha256: finalVideo.sha256,
    contactSheetSha256: contactSheet.sha256,
...
const SIGNOFF_BINDING_VALIDATORS = Object.freeze({
  ...
  "explainer-video": validateExplainerSignoffBinding,
});
...
async function validateSignoffs(report, {
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    if (!evidenceDocument || typeof evidenceDocument !== "object" || Array.isArray(evidenceDocument)
      || (!BODY_SIGNED_ATTESTATION_SUBJECTS.has(attestationSubject) && (
    ...
    const reviewerAttestation = await SIGNOFF_BINDING_VALIDATORS[attestationSubject]({
      report,
      reportPath,
      evidenceDocument,
      evidencePath,
...
export async function createVideoHarnessRunReceipt({
  ...
  const receipt = openRunReceipt({
    ...
    // 計画したコードと確定したコード。BuzzAssist の更新をまたいで確定した（resume --finalize-after-update）なら、
    // 付け替えの履歴・固定した制作契約の版・再利用だけの実行で止めた有料の呼び出しの数を残す（digest つき）。
    codeIdentity: runReceiptCodeIdentityRecord(job),
    timing: {
