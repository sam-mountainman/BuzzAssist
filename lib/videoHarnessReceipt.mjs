...
function assertReportAuditRoster(report, requiredAuditIds) {
  ...
  if (actual.size !== reported.length || missing.length > 0 || unexpected.length > 0) {
    throw new Error(
      "audit-reportの必須監査集合がHarness宣言と一致しない。"
...
function validateNarratedSignoffBinding({
  ...
  for (const auditId of NARRATED_STORY_SIGNOFF_AUDIT_IDS) {
  ...
  if (!/^[a-f0-9]{64}$/u.test(qualityDigest) || evidenceDocument?.qualityReview?.contractDigest !== qualityDigest) {
  ...
  if (identity?.contractDigest !== qualityDigest
...
export async function createVideoHarnessRunReceipt({
  ...
  const declaration = JSON.parse(await readFile(
    join(harnessDeclarationsDir, `${job.harness.id}.harness.json`),
    "utf8",
  ));
  ...
  const requiredAuditIds = declaredAuditIds(declaration);
  assertReportAuditRoster(report, requiredAuditIds);
  ...
  if (job.harness.id === "koya-manga-video") {
    ...
    await assertCurrentKoyaResolvedContract(job);
  }
  ...
  if (observed.kind === "steps") {
    recordGatesFromAuditSteps(receipt, {
      declaration,
      steps: observed.value,
      requiredAuditIds,
      contractVersion: String(report.contractVersion || declaration.version || ""),
    });
  } else {
