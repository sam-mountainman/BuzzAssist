...
// Same reason as the voice gate: PATH resolves python3 differently for a Node
// child than for an interactive shell, and cv2/onnxruntime live in a specific
// interpreter. Explicit beats ambient.
export const DEFAULT_ATTRIBUTE_GATE_PYTHON = process.env.KOYA_GATE_PYTHON || "/usr/bin/python3";
...
export async function auditCandidateAttributes(input = {}) {
  ...
  const python = typeof input.python === "string" && input.python ? input.python : DEFAULT_ATTRIBUTE_GATE_PYTHON;
