...
export const QUALITY_ACCEPTANCE_MODES = Object.freeze(["average"]);
...
export function normalizeQualityAcceptance(value) {
  ...
  return { declared: true, mode, evaluators: raw, minimumEvaluatorScore: null };
}
...
function evaluateAcceptance({ acceptance, normalizedReviews, reviews, contract }) {
  ...
  const evaluators = normalizedReviews.map((review, index) => ({
    ...
    score: review.score,
    floorFailures: rubricFloorFailures([reviews[index]], contract),
  }));
  return {
    failures,
    record: {
