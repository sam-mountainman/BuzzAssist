 ...
 *
 * 止まる条件: 目標到達・人の判断が要る状態・費用・時間・回数・改善の停滞。回の外では、人が採点なしで止める
 * （stopQualityLoopByHuman。評価項目の改定を決めたときなど。回も費用も時間も数えない）。
 * 合格: 機械ゲートが全部通り、加重平均が目標以上で、どの評価項目も下限を下回らないこと。
 *
 * 契約が任意で持てるもの（持たなければ今までと同じ動き）:
 *   - acceptance: 1回に要る評価者の宣言と、受け入れ方（average / each-evaluator）
 * 状態が任意で持てるもの:
 *   - carriedOver: 始め直す前のループから持ち越した回数・費用・時間。止まる条件は累計で判定する
 * 回の入力が任意で持てるもの:
 *   - unresolvedFindingIds: 採用したのにまた出た指摘。点が上がっていても停滞として数える
 */
...
/**
 * 前のループ（始め直す前の同じ仕事）から持ち越す回数・費用・時間。止まる条件は、このループの値に
 * これを足した累計で判定する。始め直しで回数・費用・時間の上限を消せないようにするため。
 * unpricedCount は費用の分からなかった呼び出しの件数（0円として数えていない件数）。
 */
export function normalizeCarriedOver(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const count = (entry) => Math.max(0, Math.round(finiteNumber(entry, 0)));
  return {
    loops: count(value.loops),
    rounds: count(value.rounds),
    cost: Math.max(0, finiteNumber(value.cost, 0)),
    elapsedMs: Math.max(0, finiteNumber(value.elapsedMs, 0)),
    unpricedCount: count(value.unpricedCount),
  };
}

const NO_CARRY = Object.freeze({ loops: 0, rounds: 0, cost: 0, elapsedMs: 0, unpricedCount: 0 });
...
export function createQualityLoopState(input = {}) {
  ...
  return {
    ...
    totalCost: 0,
    elapsedMs: 0,
    // 持ち越しがあるときだけ置く（無いループの状態の形は今までと同じ）。
    ...(normalizeCarriedOver(input.carriedOver) ? { carriedOver: normalizeCarriedOver(input.carriedOver) } : {}),
...
export function recordQualityRound(input = {}) {
  ...
  // 直したと言った（採用した）指摘が、この回の評価でも出た。点が上がっていても、その指摘については
  // 進んでいないので停滞として数える。呼び出し側が渡さなければ今までと同じ（点の伸びだけで見る）。
  const unresolvedFindingIds = uniqueStrings(input.unresolvedFindingIds).sort();
  const stagnantRounds = previousBest === null
    || (improvement >= contract.limits.minimumImprovement && unresolvedFindingIds.length === 0)
    ? 0
  ...
  const round = {
    ...
    ...(unresolvedFindingIds.length > 0 ? { unresolvedFindingIds } : {}),
  ...
  // 止まる条件は、前のループから持ち越した分を足した累計で判定する（持ち越しが無ければ今までと同じ）。
  const carried = normalizeCarriedOver(previous.carriedOver) || NO_CARRY;
  if (passingTarget) {
    state.status = "passed";
  ...
  } else if (state.totalCost + carried.cost >= contract.limits.maximumCost) {
    state.status = "budget-exhausted";
    state.stopReason = "cost-limit";
    state.nextAction = "human-review";
  } else if (state.elapsedMs + carried.elapsedMs >= contract.limits.maximumElapsedMs) {
    state.status = "budget-exhausted";
    state.stopReason = "time-limit";
    state.nextAction = "human-review";
  } else if (state.rounds.length + carried.rounds >= contract.limits.maximumReviewRounds) {
...
/** 人が採点なしで止めたときの止まった理由（stopReason）。状態は blocked（人の判断で止まった）を使う。 */
export const HUMAN_STOP_REASON = "human-stopped";
/** 止めの記録に数える確認の種類。対話端末から --human-verified を付けたものだけ（scripts/harness-learn.mjs の attestationFor）。 */
export const HUMAN_STOP_ATTESTATION = "human-verified";

/**
 * 続いているループを、人が採点なしで止める。評価項目の改定（Channel Pack の改定など）を決めた、方向を変えると
 * 決めた、というときの出口。次の版を古い契約で採点して blocking-condition で止めると、同じ版を新しい契約で
 * もう一度採点することになり、回数の上限を1回無駄にする。契約が変わって record が受け付けなくなったループ
 * （contract-changed）は active のまま始め直せないので、その出口も兼ねる。
 *
 * - 回を足さない。回数・費用・時間（totalCost・elapsedMs・rounds）は触らない。始め直すときの持ち越しは
 *   止めた時点までの記録どおり
 * - 状態は blocked、止まった理由は human-stopped。止めた人・理由・時刻は humanStop に残す
 * - 合格にはならない。回があれば、最も点の高かった回を bestRound に残す（納品の判断の材料で、合格ではない）
 * - 人の確認の判定（対話端末＋--human-verified）は呼び出し側が attestationFor で行い、ここには結果だけを渡す。
 *   human-verified 以外は受けない（機械は止めの記録を作れない）
 * - 止まっているループ（合格・人待ち・上限）は止め直さない（それらは既に start --restart で始め直せる）
 */
export function stopQualityLoopByHuman({ state, reason = "", reviewer = "", attestedBy = "", stoppedAt = "" } = {}) {
  if (!state || typeof state !== "object" || Array.isArray(state)) throw new Error("A quality loop state is required.");
  if (state.status !== "active") throw new Error(`Quality loop is already ${state.status}; only an active loop can be stopped.`);
  if (attestedBy !== HUMAN_STOP_ATTESTATION) {
    throw new Error("Only a human-verified operator (interactive terminal with --human-verified) can stop a quality loop.");
  }
  const who = nonEmptyString(reviewer);
  if (!who) throw new Error("Stopping a quality loop requires the reviewer name.");
  const text = sanitizeEvidence(reason, 500);
  if (Array.from(text).length < 4) throw new Error("Stopping a quality loop requires a concrete reason.");
  const at = nonEmptyString(stoppedAt) || new Date().toISOString();
  if (!Number.isFinite(Date.parse(at))) throw new Error("stoppedAt must be a valid timestamp.");
  const rounds = Array.isArray(state.rounds) ? state.rounds : [];
  const bestRound = bestNonPassingRound(rounds);
  const { bestRound: _previousBest, ...rest } = state;
  return deepFreeze({
    ...rest,
    status: "blocked",
    stopReason: HUMAN_STOP_REASON,
    nextAction: "restart-with-reason",
    humanStop: {
      stoppedAt: new Date(at).toISOString(),
      reason: text,
      reviewer: who,
      attestedBy,
      // 止めた時点までに記録した回の数（止めることで増えない）。
      roundsAtStop: rounds.length,
    },
    ...(bestRound ? { bestRound } : {}),
  });
}
