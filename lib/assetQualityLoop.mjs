 ...
 * （本編に差し込む画・声のテイク・サムネ）を同じ品質ループにかけるための口。解説動画のハーネスの宣言
 * （config/harnesses）はまだ無く、制作エンジンもここには無い。ここで決めるのは使える工程だけで、評価項目・
 * 下限・機械ゲートは各工程の共有の宣言をそのまま使う（解説動画だけの採点制度は作らない）。学習の台帳の宛先は
 * 決まっていないので、不合格の回は推測で別の台帳へ積まない（lib/assetQualityLearning.mjs の unknown-harness-route）。
 */
