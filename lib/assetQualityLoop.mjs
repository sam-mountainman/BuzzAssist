 ...
 * （scene-image）、サムネ（thumbnail）、声のテイク（voice-take）、動画クリップ（video-clip: 漫画のカットを差し替える
 * 短い動画と、ナレーション物語の回ごとの OP 映像・感想パートの人物の映像）。これまでは機械の検査と人の承認、
 ...
 *   - 機械ゲート（ファイル・参照の承認一覧・音声品質ゲートの報告・動画の測定〔lib/videoClipMeasurement.mjs〕だけから決まる）
 ...
 *   - 続いているループを採点なしで止められるのは、理由と人の確認つきの stop だけ（中核の stopQualityLoopByHuman。
 *     評価項目の改定を決めたときなど）。回も費用も時間も数えず、止めても合格にはならない
 */
...
import { VIDEO_CLIP_GATE_IDS, evaluateVideoClipMeasurement } from "./videoClipMeasurement.mjs";
...
export const ASSET_STAGES = Object.freeze(["character", "location", "scene-image", "thumbnail", "voice-take", "video-clip"]);

/**
 * ハーネスごとに使える工程。場所（location）は漫画固有（下の STAGE_SPECS.location.genreOnly）。
 *
 * explainer-video は、BuzzAssist の外の制作の仕組み（HTML/CSS/JS で映像を組む解説動画）から、途中の成果物
 * （本編に差し込む画・声のテイク・サムネ）を同じ品質ループにかけるための口。解説動画のハーネスの宣言は
 * config/harnesses/explainer-video.harness.json で、制作エンジンはここには無い（チャンネルの手元の制作を取り込む）。
 * ここで決めるのは使える工程だけで、評価項目・下限・機械ゲートは各工程の共有の宣言をそのまま使う（解説動画だけの
 * 採点制度は作らない）。不合格の回は解説動画のチャンネルの非公開台帳（channel-pack:explainer。
 * lib/harnessLearningTargets.mjs の HARNESS_LEARNING_ROUTES）へ積む。
 */
export const ASSET_QUALITY_HARNESSES = Object.freeze({
  "koya-manga-video": Object.freeze({
    id: "koya-manga-video",
    genre: "manga",
    stages: Object.freeze(["character", "location", "scene-image", "thumbnail", "voice-take", "video-clip"]),
  }),
  "narrated-story-video": Object.freeze({
    id: "narrated-story-video",
    genre: "narrated-story",
    stages: Object.freeze(["character", "scene-image", "thumbnail", "voice-take", "video-clip"]),
  }),
  "explainer-video": Object.freeze({
    id: "explainer-video",
    genre: "explainer",
    stages: Object.freeze(["scene-image", "voice-take", "thumbnail"]),
  }),
});
...
export const ASSET_GENERATION_ROUTES = Object.freeze({
  ...
  "media-generation": "BuzzAssist の生成層（lib/mediaGeneration.mjs の generateVideoMedia）で提供元の API を呼んで作った（漫画のカットの動画差し替え）",
  "human-edit": "人が作画・編集ソフトで作った・直した",
  recorded: "人が撮った映像（生成していない。lib/operatorVideoImport.mjs の経路 recorded と同じ）",
});
...
export const ASSET_MACHINE_GATES = Object.freeze({
  ...
  // 動画クリップ。測定は ffprobe / ffmpeg（lib/videoClipMeasurement.mjs の measureVideoClip。CLI は measure-video）で、
  // 合否は測定の数値から品質ループが毎回決め直す（測定のファイルに書いた合否は読まない）。
  "video-measurement-bound": "動画の測定（buzzassist-video-clip-measurement-v1）がこのクリップの SHA に結び付き、宣言（尺・fps・解像度・音声の有無）が読める",
  "video-format-readable": "ffprobe がコンテナ（MP4/MOV・WebM/Matroska）と映像のコーデックを読める",
  "video-full-decode": "ffmpeg -xerror で映像（と音声）を最後までデコードでき、デコードしたフレーム数が尺 × fps に届く（途中で切れた動画を通さない）",
  "video-duration-declared": "尺が宣言の範囲にある",
  "video-frame-rate-declared": "フレームレートが宣言の範囲にある",
  "video-resolution-declared": "幅・高さ（と縦横比）が宣言の範囲にある",
  "video-audio-declared": "音声トラックの有無が宣言どおり（required / forbidden / optional）",
});
...
// 動画クリップ（漫画のカットを差し替える短い動画・ナレーション物語の回ごとの OP 映像と感想パートの人物の映像）。
// 形式・デコード・尺・fps・解像度・音声の有無は機械ゲート（ASSET_MACHINE_GATES の video-*）で落とし、評価者は
// 見た印象だけを採点する。画素の指標は本人性も歪み・溶けも見分けない（lib/mangaCutVideoSubstitution.mjs の
// 人の目視、.agents/skills/manga-video-production/SKILL.md「選択カットの動画差し替え」の「人物の同一性は機械で判定しない」）。
const VIDEO_CLIP_RUBRIC = Object.freeze([
  {
    // 出典: .agents/skills/manga-video-production/SKILL.md「選択カットの動画差し替え」（開始フレームは承認済み静止画を
    // カメラ設計の t=0 で切った1枚・motionPrompt は「何がどう動くか」だけ・静止の水増しで尺を埋めない）、
    // lib/mangaCutVideoSubstitutionPolicy.mjs の buildCutVideoSubstitutionPrompt（The first frame must be identical to the
    // provided image）、契約の videoSubstitution.minimumStartFrameSimilarity（機械が見るのは開始フレームの一致だけで、動きが
    // 意図どおりかは見ない）。意味の項目なので下限 80（manga SKILL「絶対条件」の「台本との意味の一致 80」）。
    id: "source-intent-match",
    label: "元の画・場面の意図との一致",
    weight: 20,
    minimumScore: 80,
    description: "最初のフレームが元の画（承認済みの静止画・参照）と同じ絵で始まり、動きが場面の意図（何がどう動くか）のとおり。別の場面・別の構図・別の時間帯へ流れていない。動きが無く静止の水増しになっていない",
  },
  {
    // 出典: SCENE_IDENTITY と同じ。加えて config/harnesses/koya-manga-video.harness.json の保証 cut-video-substitution の
    // human（人物の同一性は、差し替え区間の前・中・後の実フレームを人が見て判断する）、
    // .agents/skills/manga-video-production/references/final-review-ja.md（差し替えカットは区間の前・中・後の実フレームを
    // characterContinuity で見る）、buildCutVideoSubstitutionPrompt（Keep every character's face, hairstyle, eye shape,
    // outfit, body proportions … exactly the same for the whole clip）。
    id: "character-identity",
    label: "登場人物の同一性",
    weight: 25,
    minimumScore: 80,
    description: "写っている全員が、参照した承認済みの設定画（と元の画）と同じ人に見え、最初から最後まで顔・髪・体型・服が崩れない。動いている途中のフレームの顔も参照と並べて見る（属性が合っているだけでは合格にしない）",
  },
  {
    // 出典: .agents/skills/manga-video-production/SKILL.md「選択カットの動画差し替え」の最終監査 video-substitution
    // （動きがあり静止の水増しが無いこと・色の大崩れが無いこと。機械が見るのはそこまで）と「人物の同一性は機械で判定しない」、
    // references/final-review-ja.md（区間の前・中・後の実フレームを見る）、buildCutVideoSubstitutionPrompt（Mouths stay as
    // drawn: no talking, no lip-sync）。歪み・溶け・ちらつきは画素の指標では見分けないので評価者が見る。下限は方針値の 60
    // （manga SKILL「絶対条件」の「ほかが60」）。顔や手が溶けたクリップは同一性・手指の項目の下限でも落ちる。
    id: "motion-integrity",
    label: "動きの自然さ",
    weight: 20,
    minimumScore: 60,
    description: "歪み・溶け（顔や手や物の形が流れて崩れる）・ちらつき（色・明るさ・線が瞬く）・破綻（人や物が増える・消える・途中で別の絵へ飛ぶ・口が勝手に動く）が無い。始まり・中ほど・終わりのフレームと、通しの再生の両方で見る",
  },
  {
    // 出典: HAND_SAFETY と同じ。動画は動いている途中の手も公開面に出る。
    ...HAND_SAFETY,
    weight: 15,
    description: "猥褻・侮辱と読める手や指のジェスチャーが無い。動いている途中も含めて、指の融合・欠損・本数の破綻、不自然な持ち方が無い。手の写るフレームを拡大して見る。該当が無ければ 100、1つでもあれば 0",
  },
  {
    // 出典: ART_STYLE と同じ。加えて buildCutVideoSubstitutionPrompt（Keep … line art and art style exactly the same）。
    ...ART_STYLE,
    weight: 10,
    description: "元の画とチャンネルが宣言した画風（線・塗り・色）のまま動き、途中で実写調・3D 調・別の画風へ変わらない。同じ回の他の画と並べて浮かない",
  },
  {
    // 出典: .agents/skills/manga-video-production/SKILL.md「選択カットの動画差し替え」の最終監査 video-substitution
    // （文字混入が無いこと。tesseract 必須）、references/final-review-ja.md の generatedTextArtifacts（疑似文字・不要ロゴなし）、
    // buildCutVideoSubstitutionPrompt（Do not add any … text, letters, numbers, signs, subtitles, captions, speech bubbles,
    // watermarks or logos）、THUMBNAIL_RUBRIC の no-real-brand-logo。1つでもあれば致命的な二値の項目なので下限 100。
    id: "no-text-or-logo",
    label: "文字・ロゴの混入が無い",
    weight: 10,
    minimumScore: 100,
    description: "元の画に無い文字・数字・字幕・吹き出し・透かし・ロゴ（実在の意匠に似た形を含む）が、どのフレームにも出ていない。無ければ 100、1つでもあれば 0",
  },
]);
 ...
 *                when=characters-or-references は参照がある版か、評価者が人物が写っていると答えた版
...
const STAGE_SPECS = Object.freeze({
  ...
  "video-clip": Object.freeze({
    id: "video-clip",
    label: "動画クリップ",
    media: "video",
    rubric: VIDEO_CLIP_RUBRIC,
    // 漫画のクリップは元の静止画を、人物が写る動画は承認済みの設定画を参照にする。人物の写らない OP の映像などは
    // 参照しない理由を書く（評価者が人物が写っていると答えれば reference-declared で落ちる）。
    referencePolicy: "required-or-exempt",
    referenceApproval: true,
    // 人物が写る動画は、人物の設定画・サムネと同じく同一性と手指を人が見る（動画はそのまま公開面に出る）。
    humanChecks: Object.freeze([
      { id: "identity", when: "characters-or-references" },
      { id: "hand-safety", when: "characters-or-references" },
    ]),
    reviewRequirements: Object.freeze(["charactersVisible", "comparedReferenceSha256s", "identityComparison", "framesReviewed"]),
    // 形式・全フレームのデコード・尺・fps・解像度・音声の有無（lib/videoClipMeasurement.mjs）と、参照の承認。
    machineGates: Object.freeze([
      "asset-readable", ...VIDEO_CLIP_GATE_IDS, "reference-declared", "reference-approved", "human-rejection-absent",
    ]),
  }),
});
...
/** ファイルの sha256 を読みながら数える（動画クリップは数百 MB になるので、全体をメモリへ読まない）。 */
...
/** ファイルの先頭（形式の判定に使う分だけ）。 */
async function fileHead(file, bytes = 64) {
...
function detectMedia(bytes) {
  ...
  // 動画クリップ: ISO BMFF（MP4・MOV。音声だけの M4A は上で分けた）と EBML（WebM・Matroska）。
  if (bytes.length >= 12 && ascii(4, 8) === "ftyp") return { media: "video", format: ascii(8, 10) === "qt" ? "mov" : "mp4" };
  if (bytes.length >= 4 && bytes[0] === 0x1a && bytes[1] === 0x45 && bytes[2] === 0xdf && bytes[3] === 0xa3) return { media: "video", format: "matroska" };
  return { media: "unknown", format: "" };
}

async function inspectAsset(file) {
  const detected = detectMedia(await fileHead(file));
  if (detected.media === "image") {
    ...
    return { sha256: sha256(bytes), bytes: bytes.length, ...detectMedia(bytes), dimensions };
  }
  const info = await stat(file);
  return { sha256: await fileSha256(file), bytes: info.size, ...detected, dimensions: null };
}
...
export async function resolveReferenceSha256s(values = []) {
  ...
  for (const raw of Array.isArray(values) ? values : []) {
    ...
    if (!value) continue;
    const sha = SHA256.test(value.toLowerCase()) ? value.toLowerCase() : await fileSha256(path.resolve(value));
 ...
 * ページ単位の人の確認（lib/assetQualityVerifyPages.mjs）も、ページに並べる設定画をこの一覧で照合する。
 */
export async function loadApprovedReferences(file) {
...
/**
 * 動画クリップ: 測定のファイル（lib/videoClipMeasurement.mjs の measureVideoClip が書く形）から、このクリップの
 * SHA に結び付いた機械ゲートを決める。合否は数値から決め直す（ファイルに書かれた合否は読まない）。測定が無い・
 * 読めない・別のクリップのものなら、動画のゲートは全部落ちる。
 */
async function videoMeasurementVerdict(file, assetSha) {
  const failed = (reason, reportSha256 = "") => ({
    gates: Object.fromEntries(VIDEO_CLIP_GATE_IDS.map((id) => [id, false])),
    reportSha256,
    metrics: {},
    problems: [reason],
  });
  if (!file) return failed("measurement-missing");
  let bytes;
  try {
    bytes = await readFile(file);
  } catch {
    return failed("measurement-unreadable");
  }
  const reportSha256 = sha256(bytes);
  let measurement;
  try {
    measurement = JSON.parse(bytes.toString("utf8"));
  } catch {
    return failed("measurement-invalid", reportSha256);
  }
  const verdict = evaluateVideoClipMeasurement(measurement, { assetSha256: assetSha });
  return { gates: verdict.gates, reportSha256, metrics: verdict.metrics, problems: verdict.problems };
}
...
export function requiredHumanChecks(stage, version) {
  ...
  const charactersVisible = version?.charactersVisible === true;
  return spec.humanChecks
    .filter((row) => row.when === "always"
      || (row.when === "references" && hasReferences)
      || (row.when === "characters-or-references" && (hasReferences || charactersVisible)))
    .map((row) => row.id);
}
...
export function assetQualityReviewSheet(contract, { assetSha256 = "", referenceSha256s = [] } = {}) {
  ...
  return {
    ...
    independence: "この成果物を作った文脈（作った会話・タスク）と、前の回で採点した文脈では採点できない。新しい文脈で、成果物を原寸で見て（声は聞いて、動画は通しで再生して）採点する",
  };
}
...
const REVIEW_REQUIREMENT_TEXT = Object.freeze({
  ...
  // 出典: .agents/skills/manga-video-production/references/final-review-ja.md（差し替えカットは区間の前・中・後の実フレームを
  // 見る）、lib/mangaCutVideoSubstitution.mjs の auditSubstitutedCutReviewCoverage（前・中・後の3区間の実フレーム）。
  framesReviewed: "framesReviewed: { start, middle, end } それぞれ、始まり・中ほど・終わりのフレームで何を見たか（通しの再生も見る。動いている途中の顔と手を含める）",
});

/** 動画の始まり・中ほど・終わりのフレームを見た記録（各 4 文字以上）。 */
function framesReviewedComplete(value) {
  return plainObject(value) && ["start", "middle", "end"].every((key) => Array.from(sanitizeEvidence(value[key], 500)).length >= 4);
}
...
function nextStepDetail(state, paths, version = lastOf(state?.asset?.versions)) {
  ...
  if (status === "awaiting-human-verification") {
    ...
    return `評価者の採点は通った。人の確認（${human.missing.join(", ")}）が要る。確認した人が自分の端末から`
      ...
      + "対象が多いときは verify-pages で数枚ずつのページにして見る）";
  }
  if (status === "human-rejected") {
    return "人の確認で否とされた。この版は合格にならない。直した版で続けるには start --restart --reason \"何を直すか\" で新しいループを始める（前の状態と人の確認の記録は残る）";
  }
  if (state.stopReason === HUMAN_STOP_REASON) {
    return `人が採点なしで止めた（${state.humanStop?.reviewer || "不明"}: ${state.humanStop?.reason || "理由なし"}）。合格ではない。`
      + "新しい契約で続けるなら start --restart --reason \"何が変わったか\"（Pack を変えたなら --channel-pack も）で始め直す"
      + "（前の状態と人の確認の記録は残る）";
  }
...
function checkFor(state, extra = {}) {
  ...
  return {
    ...
    // 人が採点なしで止めたループ: 止めた人・理由・時刻。
    ...(state?.humanStop ? { humanStop: { ...state.humanStop } } : {}),
    ...extra,
  };
}
    ...
    if (existing) {
      ...
      if (existing.status === "active") {
        return {
          ...
          detail: "続いているループは始め直せない（回数と停滞の上限を消せてしまう）。合格・人待ち・上限で止まってから始め直す。"
            + "採点なしで止めるなら、止めると決めた人が自分の端末から stop --reason \"...\" --reviewer <名前> --human-verified を打つ",
        };
      }
      ...
      history = [
        ...(previousAsset?.history || []),
        {
          ...
          // 人が採点なしで止めたループは、止めた人・理由・時刻を history の行にも出す（中の state にも残る）。
          ...(existing.humanStop ? { humanStop: existing.humanStop } : {}),
...
function reviewRequirementProblems(review, spec, referenceSha256s) {
  ...
  for (const id of spec.reviewRequirements) {
    ...
    if (id === "framesReviewed" && !framesReviewedComplete(review?.framesReviewed)) problems.push("frames-not-reviewed");
...
async function recordVersionLocked({
  ...
  if (!contract || contract.digest !== existing.contractDigest) {
    return waiting(existing, ["asset-quality-contract-changed"], "このループは別の契約（評価項目・下限・上限・Pack の設定）で始まっている。続けるか始め直すかは人が決める"
      + (existing.status === "active"
        ? "（新しい契約で始め直すなら、決めた人が自分の端末から stop --reason \"...\" --reviewer <名前> --human-verified で止めてから start --restart）"
        : "（新しい契約で始め直すなら start --restart --reason \"...\"）"));
  }
  ...
  // 機械ゲート（ファイル・承認一覧・音声品質ゲートの報告・動画の測定だけから決まる）。
  ...
  const video = spec.machineGates.includes("video-measurement-bound") ? await videoMeasurementVerdict(measurementFile?.full || "", assetInfo.sha256) : null;
  ...
  const evidence = [
    ...
    ...(measurementFile && video?.reportSha256 ? [{ path: measurementFile.rel, sha256: video.reportSha256, note: "動画の測定（ffprobe・ffmpeg。尺・fps・解像度・音声の有無・全フレームのデコード）" }] : []),
  ];
  let recorded;
  try {
  ...
  const version = {
    ...
    // 動画の測定（版の sha256 に結び付いた測定のファイルの sha256・数値・落ちた理由）。
    ...(video ? { videoMeasurement: { sha256: video.reportSha256 || null, metrics: video.metrics, problems: video.problems } } : {}),
  ...
  if (next.status !== "passed" && typeof captureLearning === "function") {
    ...
    try {
      ...
      learning = await captureLearning({ event: "round", state: next, round, version, contract, workDir: paths.workDir });
    } catch (error) {
...
/** 人の確認の欄の id を検める（verify と verify-pages で同じ）。 */
function humanCheckIdsOrThrow(spec, checks, option = "--check") {
  ...
  if (wanted.length === 0) throw new Error(`${option} に確認した欄を1つ以上（${allowed.join(" / ")}）。`);
  for (const id of wanted) if (!allowed.includes(id)) throw new Error(`${option} ${id} はこの工程の欄に無い（${allowed.join(" / ")}）。`);
  return wanted;
}

/** 人の確認として数えるか。判定は scripts/harness-learn.mjs の attestationFor（一つの実装）。 */
async function humanAttestation({ reviewer, isInteractive, agentAttested, humanVerified, attest }) {
  ...
  return verdictOfAttestation.attestation;
}

/**
 * 続いているループを、人が採点なしで止める（評価項目の改定を決めた、など）。中核の stopQualityLoopByHuman で
 * blocked（human-stopped）にし、止めた人・理由・時刻を humanStop に残す。回も費用も時間も数えない。
 * 止めた後は start --restart --reason で始め直せる（前の状態は history に、人の確認の記録は引き継ぐ）。
 * 止めても合格にはならない（理由コードは loop-stopped:blocked / before-use の not-passed）。
 * 数えるのは、止めると決めた人の対話端末から --human-verified を付けた記録だけ（verify と同じ attestationFor）。
 * --agent-attested は数えず何も変えない。対象は1つのループ（工程と対象 id）ずつ。
 */
export async function stopAssetQualityLoop({
  workDir,
  stage,
  subjectId,
  reviewer = "",
  ...
  if (Array.from(text).length < 4) throw new Error("--reason に、ループを止める理由（何を決めたか）を書いてください。");
  const attestation = await humanAttestation({ reviewer, isInteractive, agentAttested, humanVerified, attest });
  return withCanvasFileLock(paths.statePath, async () => {
    const existing = await readJsonIfExists(paths.statePath, null);
    if (!existing?.asset) return { stopped: false, ...waiting(null, ["asset-quality-loop-not-started"], "先に start でループを始める") };
    if (existing.asset.stage !== spec.id) throw new Error("状態ファイルの工程が --stage と違います。");
    if (existing.status !== "active") {
      return {
        stopped: false,
        ...waiting(existing, [`asset-quality-stop-loop-not-active:${existing.status}`],
          `このループは ${existing.status}（${existing.stopReason || "-"}）で既に止まっている。止め直さない。始め直すなら start --restart --reason "..."`),
      };
    }
    if (attestation.attestedBy !== HUMAN_STOP_ATTESTATION) {
      return {
        stopped: false,
        counted: false,
        ...waiting(existing, [`asset-quality-stop-not-counted:${attestation.attestedBy}`],
          "ループを止めるのは、止めると決めた人の確認（その人の対話端末から --human-verified）があるときだけ。何も変えていない"),
      };
    }
    const core = stopQualityLoopByHuman({
      state: existing, reason: text, reviewer: attestation.reviewer, attestedBy: attestation.attestedBy, stoppedAt: new Date(now()).toISOString(),
    });
    const next = { ...core, asset: existing.asset };
    await writeJsonAtomic(paths.statePath, next);
    ...
    return {
      ...
      state: next,
      issues: issuesFromState(next, version),
      detail: `品質ループを採点なしで止めた（${next.rounds.length} 回目まで記録。回・費用・時間は数えていない）。${nextStepDetail(next, paths, version)}`,
      check: checkFor(next),
    };
  });
}

/** 人の確認の記録の1行（verify と verify-pages で同じ形。verify-pages は page を足す）。 */
function humanVerificationRow({ check, verdict, assetSha256, assetPath, attestation, note, recordedAt, page = null }) {
  return {
    check,
    verdict,
    assetSha256,
...
/** 状態へ人の確認の記録を足し、数える否があれば学習を捕捉する（呼び出し側が状態ファイルのロックを取る）。 */
async function appendHumanVerificationsLocked({ paths, existing, rows, counted, captureLearning }) {
  const next = { ...existing, asset: { ...existing.asset, humanVerifications: [...(existing.asset.humanVerifications || []), ...rows] } };
  await writeJsonAtomic(paths.statePath, next);
  const rejections = rows.filter((row) => row.verdict === "reject");
  let learning = null;
  if (counted && rejections.length > 0 && typeof captureLearning === "function") {
    try {
      const version = (next.asset.versions || []).find((row) => row.assetSha256 === rejections[0].assetSha256) || null;
      learning = await captureLearning({
        event: "human-rejection", state: next, version, verifications: rejections, contract: next.asset.contract, workDir: paths.workDir,
      });
    } catch (error) {
      learning = { captured: 0, skippedReason: "capture-failed", detail: sanitizeEvidence(error?.message || String(error), 200) };
    }
...
export async function recordAssetHumanVerification({
  ...
  const spec = assetQualityStage(stage);
  const paths = assetQualityPaths(workDir, spec.id, subjectId);
  const wanted = humanCheckIdsOrThrow(spec, checks);
  ...
  const attestation = await humanAttestation({ reviewer, isInteractive, agentAttested, humanVerified, attest });
  const asset = workDirRelative(paths.workDir, assetPath, "--asset");
  const assetSha256 = await fileSha256(asset.full);
  return withCanvasFileLock(paths.statePath, async () => {
    const existing = await readJsonIfExists(paths.statePath, null);
    if (!existing?.asset) return waiting(null, ["asset-quality-loop-not-started"], "先に start でループを始める");
    const recordedAt = new Date(now()).toISOString();
    const rows = wanted.map((check) => humanVerificationRow({ check, verdict, assetSha256, assetPath: asset.rel, attestation, note: text, recordedAt }));
    const counted = attestation.attestedBy === ASSET_HUMAN_VERIFIED;
    const { next, learning } = await appendHumanVerificationsLocked({ paths, existing, rows, counted, captureLearning });
    const version = lastOf(next.asset.versions);
    return {
      recorded: true,
      counted,
...
const PAGE_ID = /^[a-f0-9]{16}$/u;

function isoTimeOrThrow(value, label) {
  const at = Date.parse(String(value || ""));
  if (!Number.isFinite(at)) throw new Error(`ページの ${label} が時刻として読めない。`);
  return new Date(at).toISOString();
}

/**
 * ページ単位の人の確認（lib/assetQualityVerifyPages.mjs の verify-pages）が、1つの対象への答えを記録する口。
 * 記録の形は verify と同じ（欄ごとに1行・成果物の SHA に結ぶ）で、page（ページの id・ページ画像の sha256・置き場・
 * ページの中の番号・見せた時刻・答えた時刻）を足す。
 *
 * 守ること:
 *   - 数えるのは human-verified だけで、それ以外（--agent-attested・対話端末でない）は記録もしない（ページで
 *     見たことを機械が申告する道を作らない）
 *   - ページに並べた版（expectedAssetSha256）が今も最新の版で、ファイルも同じバイト列のときだけ記録する。
 *     ページを作った後で画が変わった・新しい版が記録されたなら、見た画と今の画が同じと言えないので記録しない
 *   - 1つの対象の記録は、その対象の状態ファイルだけに書く（同じページの他の対象の記録に触らない）
 */
export async function recordAssetPageVerification({
  workDir,
  stage,
  subjectId,
  expectedAssetSha256 = "",
  ...
  const spec = assetQualityStage(stage);
  const paths = assetQualityPaths(workDir, spec.id, subjectId);
  const answers = (Array.isArray(verdicts) ? verdicts : []).map((row) => ({
    check: nonEmpty(row?.check),
    verdict: nonEmpty(row?.verdict),
    note: sanitizeEvidence(row?.note, 500),
  }));
  const checks = humanCheckIdsOrThrow(spec, answers.map((row) => row.check), "verdicts[].check");
  if (checks.length !== answers.length) throw new Error("ページの答えに同じ欄が2回ある。");
  for (const row of answers) {
    if (!["pass", "reject"].includes(row.verdict)) throw new Error("ページの答えは pass か reject。");
    if (Array.from(row.note).length < 4) throw new Error("ページの答えに、何を見てどう判断したか（落とすなら理由）が要ります。");
  }
  const attestation = await humanAttestation({ reviewer, isInteractive, agentAttested: false, humanVerified, attest });
  if (attestation.attestedBy !== ASSET_HUMAN_VERIFIED) {
    throw new Error("ページ単位の人の確認は、確認した人が自分の対話端末から --human-verified を付けたときだけ記録する（機械は人の確認を記録できない）。");
  }
  const expected = nonEmpty(expectedAssetSha256).toLowerCase();
  if (!SHA256.test(expected)) throw new Error("ページに並べた画の sha256 が要ります。");
  const slot = Number(page?.slot);
  if (!plainObject(page) || !PAGE_ID.test(nonEmpty(page.id)) || !SHA256.test(nonEmpty(page.sha256)) || !nonEmpty(page.path)
    || !Number.isInteger(slot) || slot < 1) {
    throw new Error("ページの記録（id・ページ画像の sha256・置き場・ページの中の番号）が要ります。");
  }
  const shownAt = isoTimeOrThrow(page.shownAt, "見せた時刻");
  const answeredAt = isoTimeOrThrow(page.answeredAt, "答えた時刻");
  if (Date.parse(answeredAt) < Date.parse(shownAt)) throw new Error("ページの答えた時刻が見せた時刻より前。");
  const pageRow = { id: nonEmpty(page.id), sha256: nonEmpty(page.sha256), path: nonEmpty(page.path), slot, shownAt, answeredAt };
  return withCanvasFileLock(paths.statePath, async () => {
    const existing = await readJsonIfExists(paths.statePath, null);
    if (!existing?.asset) return waiting(null, ["asset-quality-loop-not-started"], "先に start でループを始める");
    const version = lastOf(existing.asset.versions);
    if (!version || version.assetSha256 !== expected) {
      return waiting(existing, ["asset-quality-page-version-changed"], "ページを作った後に、この対象の版が変わった（新しい版は新しいページで見る）。記録していない");
    }
    let current = "";
    try {
      current = await fileSha256(path.resolve(paths.workDir, version.assetPath));
    } catch {
      current = "";
    }
    if (current !== expected) {
      return waiting(
        existing,
        [current ? "asset-quality-asset-changed-since-page" : "asset-quality-asset-missing"],
        "ページに並べた後で画のファイルが変わった（sha256 が違う）。見た画と今の画が同じと言えないので記録していない",
      );
    }
    const recordedAt = new Date(now()).toISOString();
    const rows = answers.map((row) => humanVerificationRow({
      check: row.check, verdict: row.verdict, assetSha256: expected, assetPath: version.assetPath, attestation, note: row.note, recordedAt, page: pageRow,
    }));
    const { next, learning } = await appendHumanVerificationsLocked({ paths, existing, rows, counted: true, captureLearning });
    const latest = lastOf(next.asset.versions);
    return {
      recorded: true,
      counted: true,
      verifications: rows,
      state: next,
      issues: issuesFromState(next, latest),
      detail: `ページ ${pageRow.id} の [${slot}] の答え（${rows.map((row) => `${row.check}: ${row.verdict === "pass" ? "可" : "否"}`).join(", ")}）を成果物 ${expected.slice(0, 12)} に記録した。`,
      check: checkFor(next),
      learning,
    };
  });
...
export async function assetQualityStatus({ workDir, stage, subjectId, assetPath = "" } = {}) {
  ...
  if (version) {
    try {
      currentSha256 = await fileSha256(path.resolve(paths.workDir, version.assetPath));
    } catch {
      currentSha256 = "";
    }
  }
  ...
  if (nonEmpty(assetPath)) {
    ...
    try {
      candidateSha256 = await fileSha256(candidate);
    } catch {
      candidateSha256 = "";
    }
...
function reviewTemplateFields(contract, reviewRequirements, assetSha256, notes) {
  ...
  return {
    ...
    ...(requirement.has("framesReviewed") ? { framesReviewed: { start: "", middle: "", end: "" } } : {}),
 ...
 *     "references"?, "referenceExemptReason"?, "measurement"?（声のテイク・動画クリップ）,
...
export async function assetQualityBatchReviewTemplate({ workDir, stage = "", manifestPath = "", manifest = null } = {}) {
  ...
  const sheet = {
    ...
    perItem: "対象ごとに原寸で見て（声は直前の地の文と続けて聞いて、動画は通しで再生して）採点し、所見もその対象について具体的に書く。同じ所見の写しを複数の対象に貼った採点は記録しない。人の確認（人物の同一性・手指の安全）は batch では記録できず、要る対象は1件ずつ人が確かめる",
  };
  const template = {
...
export async function recordAssetQualityBatch({
  ...
  for (const item of batch.items) {
    ...
    const assetSha256 = await fileSha256(item.asset.full).catch(() => {
  ...
  return {
    ...
    detail: `batch ${recordId}（${spec.label}・${results.length} 件）: 記録 ${recorded} 件・記録済み ${alreadyRecorded} 件・人待ち/直し待ち ${waitingCount} 件。`
      + " 合格した対象は記録し直さない。人の確認が要る対象は、人が対象ごとに verify する（多いときは verify-pages）",
  };
}
