 ...
 * BUZZASSIST_LEARNING_DIR があればその下、開発用チェックアウト（.git と両ホストのアダプターが
 * ある）なら docs/learning/evals、それ以外（運営者の端末）なら ~/.buzzassist/learning/evals。
 */
  ...
  for (const skill of loaded.sandboxSkills) {
    ...
    fs.cpSync(source, destination, {
      ...
      filter: (entry) => {
        ...
        if (first === "evals") return false;
        if (path.basename(entry) === ".DS_Store") return false;
