...
// Ed25519 署名で結合する。Koya（漫画動画）・narrated-story-video・explainer-video（解説動画）の
// ハーネスがこの 1 つの実装を使う。ハーネスごとに subject の形（schema）だけが違い、
// 信頼リスト・署名・検証の規則は同じ（解説動画はナレーション物語の形に納品の記録の SHA を足す）。
//
...
export const EXPLAINER_REVIEW_ATTESTATION_VERSION = "explainer-video-review-attestation-v1";
...
export const EXPLAINER_VIDEO_HARNESS_ID = "explainer-video";
...
/**
 * explainer-video（解説動画）の署名対象。形はナレーション物語の subject と同じ考え方（Job の真値の jobId と
 * identityDigest、disk から計算した完成 MP4 と contact sheet の SHA、signoff 本文の canonical SHA、申告した
 * reviewer）に、取り込んだ納品の記録（DELIVERY.json）の SHA を足す。解説動画は BuzzAssist の外で作った完成版を
 * 取り込むので、人が試聴した MP4 が「どの納品の版か」まで署名に入れる（納品の記録を差し替えたら署名が落ちる）。
 */
export const EXPLAINER_REVIEW_ATTESTATION_SUBJECT_FIELDS = Object.freeze([
  "version",
  "harnessId",
  "jobId",
  "identityDigest",
...
const SUBJECT_SCHEMAS = new Map([
  ...
  [EXPLAINER_VIDEO_HARNESS_ID, Object.freeze({
    harnessId: EXPLAINER_VIDEO_HARNESS_ID,
    version: EXPLAINER_REVIEW_ATTESTATION_VERSION,
    jobId: /^video-explainer-video-[a-f0-9]{16}$/u,
    fields: EXPLAINER_REVIEW_ATTESTATION_SUBJECT_FIELDS,
    shaFields: Object.freeze(["identityDigest", "videoSha256", "contactSheetSha256", "deliverySha256", "signoffBodySha256"]),
    textFields: Object.freeze(["reviewerId"]),
    hostField: "",
  })],
]);
...
/**
 * explainer-video: Job の真値（id / identityDigest）、disk から計算した完成 MP4・contact sheet・納品の記録の SHA、
 * signoff 本文の canonical SHA、signoff が申告した reviewer から組む。
 */
export function createExplainerReviewAttestationSubject({
  jobId,
  identityDigest,
  videoSha256,
  contactSheetSha256,
...
/**
 * signoff 本文の canonical SHA（ナレーション物語と解説動画の共通）。reviewerAttestation だけを除く（署名が
 * 自分自身を含めない）。JSON 往復で undefined を落とし、file から読んだ写しと in-memory の原本が同じ digest に
 * なるようにする。
 */
...
/** signoff の reviewer 申告（文字列 / object の両形を受ける。ナレーション物語と解説動画の共通）。 */
...
/**
 * 本文ごと署名する signoff（ナレーション物語・解説動画）に reviewer attestation を付ける共通の手順。
 * subjectOf(body, bodySha256) がハーネスの subject を組む。既存の reviewerAttestation は本文から除いて
 * 署名し直し、署名した直後に同じ subject で検証し直す。
 */
async function signBodyBoundSignoff({ signoff, subjectOf, privateKeyPem, privateKeyPath, trust, signedAt, invalidCode }) {
  if (!isPlainObject(signoff)) throw new Error(invalidCode);
  const body = JSON.parse(JSON.stringify(signoff));
  delete body.reviewerAttestation;
  const subject = subjectOf(body, narratedSignoffBodySha256(body));
...
/**
 * 本文ごと署名する signoff の attestation を、呼び出し側の真値から組んだ期待 subject で再検証する共通の手順。
 * signoff の申告値（videoSha256 等）は期待側に使わない（subjectOf には本文の SHA と reviewer の申告だけを渡す）。
 */
function verifyBodyBoundSignoff(signoff, { subjectOf, trust, invalidCode }) {
  const failed = (codes) => ({
    pass: false,
  ...
  if (!isPlainObject(signoff)) return failed([invalidCode]);
  let expectedSubject;
  try {
    expectedSubject = subjectOf(narratedSignoffBodySha256(signoff), narratedSignoffReviewer(signoff));
  } catch (error) {
...
export async function signNarratedReviewSignoff({
  ...
  return signBodyBoundSignoff({
    signoff,
    privateKeyPem,
    privateKeyPath,
    trust,
    signedAt,
    invalidCode: "narrated-signoff-invalid",
    subjectOf: (body, bodySha256) => createNarratedReviewAttestationSubject({
      jobId,
      identityDigest,
      videoSha256: body.videoSha256,
      contactSheetSha256: body.contactSheetSha256,
      signoffBodySha256: bodySha256,
...
export function verifyNarratedReviewSignoff(signoff, {
  ...
  return verifyBodyBoundSignoff(signoff, {
    trust,
    invalidCode: "narrated-signoff-invalid",
    subjectOf: (bodySha256, reviewer) => createNarratedReviewAttestationSubject({
      jobId,
      identityDigest,
      videoSha256,
      contactSheetSha256,
...
/**
 * explainer-video（解説動画）の人の評価の signoff に reviewer attestation を付ける。ナレーション物語と同じ
 * 共通の手順で、subject に納品の記録の SHA が加わる。生成側はこの関数を呼ばない（signoff を書くのは別の文脈の
 * 評価者だけ）。
 */
export async function signExplainerReviewSignoff({
  signoff,
  jobId,
  identityDigest,
  privateKeyPem = "",
  ...
  return signBodyBoundSignoff({
    ...
    trust,
    signedAt,
    invalidCode: "explainer-signoff-invalid",
    subjectOf: (body, bodySha256) => createExplainerReviewAttestationSubject({
      jobId,
      identityDigest,
      videoSha256: body.videoSha256,
      contactSheetSha256: body.contactSheetSha256,
      deliverySha256: body.deliverySha256,
      signoffBodySha256: bodySha256,
      reviewer: narratedSignoffReviewer(body),
    }),
  });
...
/**
 * 解説動画の signoff の attestation を、呼び出し側の真値（Job id / identityDigest、disk から計算した完成 MP4・
 * contact sheet・納品の記録の SHA）から組んだ期待 subject で再検証する。signoff の申告値は期待側に使わない。
 */
export function verifyExplainerReviewSignoff(signoff, {
  jobId,
  identityDigest,
  videoSha256,
  contactSheetSha256,
  ...
  return verifyBodyBoundSignoff(signoff, {
    trust,
    invalidCode: "explainer-signoff-invalid",
    subjectOf: (bodySha256, reviewer) => createExplainerReviewAttestationSubject({
      jobId,
      identityDigest,
      videoSha256,
      contactSheetSha256,
