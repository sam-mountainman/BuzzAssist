...
//   - approve: エージェントも当てられる（運営者の決定 2026-09-26: 正本スキルの人の確認は、運営者へ
//     配る版を出すリリースのときの1回にまとめる）。--reviewer を省けば「エージェントが当てた」
//     （actor: agent）、人が自分の端末から --reviewer <名前> --human-verified で打てば「人が当てた」
//     （actor: human）として記録を分ける。名前だけ（--human-verified なし）は受け付けない——黙って
//     人を名乗れる形を残さない。対象の今の sha256 が base と一致するときだけ差分を当て（読んでから
//     書く）、違えば base-changed で拒否する。当てたら applied 台帳へ「適用した変更」を1行残す
//     （対象・変更前後の sha256・誰が当てたか・時刻・元の提案 ID・差分）。この行は提案ごとの反映記録
//     として数えられるので、approve のあとに apply は要らない
//   - reject: キューから外す（記録は消さない）
//   - rollback: approve と同じく、エージェントも人も打てて、どちらが戻したかを分けて残す。対象の
//     今の sha256 が「変更後」と一致するときだけ変更前へ戻し、戻したことも applied 台帳へ残す。
//     一致しなければ rollback-conflict で拒否する
//
// ただし Channel Pack の台帳（review-only）は、リリースを通らず「何を人が決めたか」の記録そのものなので、
// 今までどおり人の確認でだけ当てて戻す（エージェントは pending に置くまで）。
//
// 人が確かめる関門は、ここではなくリリース（skills:check:release と release.yml が、在庫の承認が
// 版と内容 SHA に付いていることを要求する。承認は承認者の端末の skill-inventory --approve だけが
// 記録できる）。ここで守るのは「何を・どの版から・誰が当てたか」を読める形で残すことと、1件ずつ
// 戻せること。
//
// apply は、正本を直接直した変更の記録として残る（キューを通さない経路）。同じ検証
// （assertPromotableProposal・canonicalHasPromotionEvidence）を approve も通す。
//
 ...
 *
 * channel（台帳のチャンネルの id か、harness-learn の resolveChannelLearningScope の結果）を渡せば、共有台帳と
 * そのチャンネルの保存先だけを読み書きする。キュー・反映記録・正本はそのチャンネルの保存先のもので、別の
 * チャンネルやチャンネルの無い保存先の変更は見えない（見えても channel-scope-mismatch で当てない）。
 */
export function learningChangeContext({
  ...
  const scope = channelLearningScopeOf(channel, { repoRoot: root, env });
  let resolveLedger = ledgerPath;
  let readAll = readLedger;
  if (root === DEFAULT_REPO_ROOT) {
    resolveLedger ??= (target, kind) => ledgerPathFor(target, kind, { channel: scope });
    readAll ??= (kind) => readLearningLedgerRows(kind, { channel: scope });
  } else {
    resolveLedger ??= (target, kind) => {
      const resolved = resolveTarget(String(target || ""));
      if (isChannelPackTarget(resolved)) {
        if (scope) {
          const store = resolveChannelLearningStore(resolved, targetMap[resolved] || {}, { repoRoot: root, channel: scope });
          return path.join(store.root, "docs", "learning", `${kind}.jsonl`);
        }
    ...
    readAll ??= (kind) => {
      const shared = sharedLedgerPath(state, kind);
      const files = new Set([shared]);
      const channelTargets = scope
        ? [...scope.stores.keys()]
        : Object.entries(targetMap).filter(([, definition]) => definition?.scope === "channel-pack").map(([target]) => target);
      for (const target of channelTargets) files.add(resolveLedger(target, kind));
      return [...files].flatMap((file) => {
        const rows = readLearningJsonl(file);
        return !scope || file === shared ? rows : rows.filter((row) => rowInChannelScope(row, scope));
      });
    };
  }
  const readers = createCanonicalReaders({ repoRoot: root, targets: targetMap, channel: scope });
  return {
    repoRoot: root,
    targets: targetMap,
    state,
...
/** 変更の記録に残すチャンネルの印（チャンネルの範囲で置いた変更だけ）。 */
function channelFields(context, target) {
  return context.channel && isChannelPackTarget(resolveTarget(String(target || ""))) ? { channel: context.channel.channelId } : {};
}

/**
 * 変更を置いたときのチャンネルと、今のチャンネルの範囲が同じか。違えば当てない・外さない・戻さない
 * （別のチャンネルの保存先の正本を、今のチャンネルの記録で書き換えない）。
 */
function assertChangeChannel(context, record) {
  const recorded = typeof record?.channel === "string" ? record.channel : "";
  const current = isChannelPackTarget(resolveTarget(String(record?.target || ""))) ? context.channel?.channelId || "" : "";
  if (recorded === current) return;
  throw changeError(
    "channel-scope-mismatch",
    recorded
      ? `${record.changeId || record.id} はチャンネル ${recorded} の保存先の変更です。--channel ${recorded} を付けて実行してください。`
      : `${record.changeId || record.id} はチャンネルの無い保存先の変更です。--channel を外して実行してください。`,
  );
}

function writableOptions(context) {
  return { repoRoot: context.repoRoot, targets: context.targets, channel: context.channel };
}
...
/**
 * エージェントが当てて（戻して）よい宛先か。運営者の決定（2026-09-26）で機械が直してよくなったのは
 * 正本スキル（SKILL.md・references）で、人の確認は配る版のリリースで1回受ける。Channel Pack の台帳
 * （review-only）はリリースを通らず、しかも「何を人が決めたか」の記録そのものなので、今までどおり
 * 人の確認でだけ当てて戻す。
 */
function assertActorMayWrite(context, target, actor, verb) {
  if (actor.actor !== CHANGE_ACTOR_AGENT) return;
  const definition = context.targets?.[target] || {};
  if (isChannelPackTarget(target) || definition.mode === "review-only") {
    throw changeError(
      "human-verification-required",
      `${target} は Channel Pack の台帳（review-only。人の決定の記録そのもので、リリースの確認も通らない）なので、`
        + `エージェントは${verb}ません。人が差分を読んだ端末から --reviewer <名前> --human-verified で実行してください。`,
    );
  }
}

function resolveWritable(context, target) {
  const resolved = requireWritableTarget(target, writableOptions(context));
...
/**
 * 誰がこの操作をしたか（approve / reject / rollback の記録に残す）。
 *
 * - --reviewer を省く（または --agent-attested）: エージェントがした操作。reviewer は "agent"、
 *   attestedBy は agent-self-attested、actor は agent。名前を添えていれば claimedReviewer に残す
 * - --reviewer <名前> --human-verified（対話端末から）: 人がした操作。actor は human
 * - 名前だけ・非対話の --human-verified: 拒否する。黙って人を名乗れる形を残さない
 *   （TTY は人の証明にならないので、人の印は二手を要る。本当の関門はリリースの承認）
 */
export function changeActorFor({ reviewer, humanVerified = false, agentAttested = false, isInteractive = false } = {}) {
  const name = String(reviewer || "").trim();
  // 明示が推測に勝つ。--agent-attested を先に見る（attestationFor と同じ順）。
  if (agentAttested === true || (!name && humanVerified !== true)) {
    return {
      actor: CHANGE_ACTOR_AGENT,
      reviewer: "agent",
      attestedBy: AGENT_SELF_ATTESTED,
      ...(name ? { claimedReviewer: name } : {}),
    };
  }
  const attested = attestationFor({ reviewer: name, isInteractive: Boolean(isInteractive), agentAttested: false, humanVerified: humanVerified === true });
  if (!attested.ok) {
    throw changeError(
      "human-verification-required",
      `${attested.message}（エージェントとして当てるなら --reviewer を省いてください。記録は「エージェントが当てた」になります）`,
    );
  }
  if (attested.attestation.attestedBy !== HUMAN_VERIFIED) {
    throw changeError(
      "human-verification-required",
      "名前だけでは「人が当てた」として記録しません。人が差分を読んだ端末から --reviewer <名前> --human-verified で"
        + "実行するか、エージェントとして当てるなら --reviewer を省いてください。",
    );
  }
  return { actor: CHANGE_ACTOR_HUMAN, reviewer: attested.attestation.reviewer, attestedBy: HUMAN_VERIFIED };
}
...
function actorFields(actor) {
  return {
    ...
    attestedBy: actor.attestedBy,
    ...(actor.claimedReviewer ? { claimedReviewer: actor.claimedReviewer } : {}),
  };
}
...
export function enqueueLearningChange({
  ...
  const row = {
    ...
    changeId,
    target: changeTarget,
    ...channelFields(context, changeTarget),
...
export function listLearningChanges({ includeClosed = false, ...options } = {}) {
  ...
  for (const change of changesState(context).values()) {
    ...
    if (record?.target) {
      try {
        const resolved = requireWritableTarget(record.target, writableOptions(context));
...
/**
 * キューの変更を正本へ当てる。エージェントも人も当てられ、どちらが当てたかを actor で分けて残す。
...
export function approveLearningChange({
  ...
  const actor = changeActorFor({ reviewer, humanVerified, agentAttested, isInteractive });
  const change = findChange(context, changeId);
  ...
  assertChangeChannel(context, queued);
  assertActorMayWrite(context, queued.target, actor, "当て");
  ...
  const record = withLearningFileLock(appliedPath, () => {
    ...
    const row = {
      ...
      ...channelFields(context, queued.target),
      targetPath: queued.targetPath,
...
/** キューから外す（記録は消さない）。approve と同じく、誰が外したかを actor で分けて残す。 */
export function rejectLearningChange({
  ...
  assertChangeChannel(context, change.queued);
  const row = {
    version: CHANGE_RECORD_VERSION,
    ...
    ...channelFields(context, change.queued.target),
    ...actorFields(actor),
    reason: text,
 ...
 * エージェントも人も戻せ、どちらが戻したかを actor で分けて残す（当てた側と戻す側は別でもよい）。
 */
export function rollbackLearningChange({
  ...
  assertChangeChannel(context, applied);
  assertActorMayWrite(context, applied.target, actor, "戻し");
  ...
  const record = withLearningFileLock(appliedPath, () => {
    ...
    const row = {
      ...
      ...channelFields(context, applied.target),
