  ...
  if (!plainObject(inputs)) {
    ...
    if (!Array.isArray(inputs.startOptions)) {
      ...
      inputs.startOptions.forEach((entry, index) => {
        ...
        if (!nonEmpty(entry.what)) errors.push(`${label}.what が要る`);
        // 条件つきの引数（例: Pack の宣言しだいで要る）。条件は plan-request からは確かめられないので、
        // 足りなくても blockers にせず、条件つきとして見せる。
        if (entry.requiredWhen !== undefined && !nonEmpty(entry.requiredWhen)) errors.push(`${label}.requiredWhen は条件の説明（空にしない）`);
      });
    }
  }
...
export function checkHarnessInputs(view, { script = { provided: false, status: "missing" }, options = {}, channelPack = { provided: false } } = {}) {
  ...
  const requiredOptions = view.inputs.startOptions.filter((entry) => !nonEmpty(entry.requiredWhen));
  const conditionalOptions = view.inputs.startOptions.filter((entry) => nonEmpty(entry.requiredWhen));
  const missingOptions = requiredOptions.filter((entry) => !nonEmpty(given[entry.key]));
  ...
  return {
    ...
    script: scriptResult,
    startOptions: {
      required: requiredOptions.map((entry) => ({ key: entry.key, cliFlag: entry.cliFlag })),
      missing: missingOptions.map((entry) => entry.key),
      conditional: conditionalOptions.map((entry) => ({
        key: entry.key,
        cliFlag: entry.cliFlag,
        requiredWhen: entry.requiredWhen,
        provided: Boolean(nonEmpty(given[entry.key])),
      })),
    },
