...
import { projectCanvasRunProgress } from "./canvasRunProgressProjection.mjs";
import { readKoyaMangaProgressSnapshot } from "./koyaMangaProgressSnapshot.mjs";

// 途中の成果物（本編の画・人物の候補と設定画・採用テイク・工程の DAG）を読むジャンルごとの読み取り側。
// 無いハーネスは途中の投影をしない（Run と media の投影はそのまま）。
const PROGRESS_SNAPSHOT_READERS = Object.freeze({
  "koya-manga-video": readKoyaMangaProgressSnapshot,
});
...
function canvasArtifactKind(kind, path = "") {
  // 手がかりは種類名とファイル名だけにする。以前は完全なパスの文字列を見ていたので、
  // 一時フォルダや作業フォルダの名前に「png」「image」「reference」などが偶然入ると、
  // JSON まで画像と判定され、投影が「未対応のCanvas image形式」で落ちた（CI の macOS で、
  // 仮想機械ごとに乱数で決まる一時フォルダ名に当たった）。
  const fileName = basename(String(path || "").replaceAll("\\", "/"));
...
/**
 * 途中の成果物を Canvas へ出す。表示のための投影なので、ここで失敗しても Job の状態遷移
 * （completed への確定を含む）は止めない。失敗は戻り値の progressProjection.ok=false と error に残す。
 * 作業場（job.executionProjectDir）の外の原本は読まない。
 */
async function projectVideoHarnessJobProgress(job, { dryRun = false, snapshotReader, projector = projectCanvasRunProgress } = {}) {
  const reader = snapshotReader || PROGRESS_SNAPSHOT_READERS[job?.harness?.id];
  if (typeof reader !== "function") {
    return { ok: true, skipped: true, skippedReason: "no-progress-reader-for-harness" };
  }
  try {
    const snapshot = await reader(job);
    const sourceRoot = typeof job.executionProjectDir === "string" && job.executionProjectDir.trim()
      ? resolve(job.executionProjectDir)
      : "";
    return await projector({ projectDir: job.projectDir }, snapshot, { dryRun, sourceRoot });
  } catch (error) {
...
export async function projectVideoHarnessJob(job, options = {}) {
  const {
    ...
    progressSnapshotReader = null,
    progressProjector = projectCanvasRunProgress,
    ...projectionOptions
  } = options;
  ...
  // Run の状態カードを進めた後に、途中の成果物を出す（こちらは失敗しても Run の投影を戻さない）。
  const progressProjection = await projectVideoHarnessJobProgress(job, {
    dryRun: projectionOptions.dryRun === true,
    snapshotReader: progressSnapshotReader,
    projector: progressProjector,
  });
  return { ...projected, feedbackCollection, mediaProjection, progressProjection };
}

export const _testing = Object.freeze({ canvasArtifactKind, canvasArtifactSources, canvasJobStatus, canvasRunStatus, projectVideoHarnessJobProgress, providerVersions, readVerifiedRunReceipt, verifiedSignoffs });
