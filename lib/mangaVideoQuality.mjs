...
export async function auditMangaVideoQuality(options = {}) {
  ...
  const audioResult = await run(options.ffmpegPath || "ffmpeg", [
    ...
    "-af", `ebur128=peak=true,silencedetect=noise=${options.silenceThresholdDb ?? -45}dB:d=${options.minimumSilenceSeconds ?? 0.6}`,
    "-f", "null", "-",
  ]);
  ...
  const report = {
    ...
    loudness,
    silence: {
      thresholdDb: finiteNumber(options.silenceThresholdDb, -45),
