...
export async function auditMangaVideoQuality(options = {}) {
  ...
  // 全デコード。この監査ステップは最終監査から "full-decode" という名前で
  // 呼ばれているのに、実際には ffprobe でメタデータを読み、音声を1回
  // 解析するだけで、映像を一度もデコードしていなかった。壊れたフレームを
  // 含むMP4がそのまま合格しうる状態だったので、実際に全編を通す。
  // -xerror が要るのは、ffmpeg が復旧可能なデコードエラーを報告しながら
  // 終了コード0で終えることがあるため（ナレーション物語側は元からこれを付けていた）。
  // 終了コードが0でも、誤りの報告があれば落とす（fullDecodeVerdict の説明を参照）。
  let fullDecode;
  try {
    const decoded = await run(options.ffmpegPath || "ffmpeg", [
      "-v", "error", "-xerror", "-i", videoPath, "-f", "null", "-",
    ]);
    fullDecode = fullDecodeVerdict({ stderr: decoded?.stderr });
  } catch (error) {
    fullDecode = fullDecodeVerdict({ error });
  }
  ...
  const gates = {
    ...
    // 最終監査がこのステップを "full-decode" として呼ぶ以上、
    // 全編が本当にデコードできることを合否に含める。
    fullDecode: fullDecode.pass,
  };
  const report = {
    version: 1,
