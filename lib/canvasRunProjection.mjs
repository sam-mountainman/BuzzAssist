...
/**
 * 状態色つきのカード（枠＋中の文字）。Run の投影と、途中の成果物の投影
 * （lib/canvasRunProgressProjection.mjs）が同じ形のカードを使う。要素 ID と customData は
 * 呼ぶ側が決め、ここは形と投影 hash だけを受け持つ。clock は updatedAt / revision を持つ物で、
 * 要素の updated を決める（Run 自身を渡してよい）。
 */
export function buildCanvasStatusCard({ cardId, labelId, groupId, customData, title, lines = [], status = "pending", x, y, width = CARD_WIDTH, height = CARD_HEIGHT, link = null, clock }, sequence) {
  const run = clock;
...
function makeCard({ run, logicalKind, logicalId, title, lines = [], status = "pending", x, y, width = CARD_WIDTH, height = CARD_HEIGHT, link = null }, sequence) {
  return buildCanvasStatusCard({
    cardId: stableCanvasRunId(run.runId, `${logicalKind}-card`, logicalId),
    labelId: stableCanvasRunId(run.runId, `${logicalKind}-label`, logicalId),
    groupId: stableCanvasRunId(run.runId, `${logicalKind}-group`, logicalId),
    customData: {
...
/** カードの右端から次のカードの左端へ引く矢印。from/to は { id, x, y, width, height }。 */
export function buildCanvasArrow({ id, customData, from, to, clock }, sequence) {
  const run = clock;
...
function makeArrow({ run, logicalKind, logicalId, from, to }, sequence) {
  return buildCanvasArrow({
    id: stableCanvasRunId(run.runId, `${logicalKind}-edge`, logicalId),
    customData: {
      [CANVAS_RUN_PROJECTION_TAG]: true,
      buzzassistHarnessRun: true,
      buzzassistRunId: run.runId,
...
/** 投影の要素に付ける fractional index の列（投影の中だけで使う。scene 上の位置は reconcile が決める）。 */
export function canvasProjectionIndexSequence() {
  return indexSequence();
}

/** 要素の投影 hash（version・index・updated を除いた形の SHA-256）を customData に付け直す。 */
export function finalizeCanvasProjectionElement(element) {
  return finalizeProjectionHash(element);
}
...
// 途中の成果物の投影（lib/canvasRunProgressProjection.mjs）も、同じ規則で既存要素を更新・墓標化する:
// 投影 hash が同じなら触らない、違えば version を上げて index は保つ、消えたものは isDeleted にする。
export {
  nextProjectedElement as nextCanvasProjectedElement,
  tombstoneElement as tombstoneCanvasProjectedElement,
};
