...
export function createMangaProductionDag(input = {}) {
  ...
  for (const speaker of speakers) {
    const candidateIds = [1, 2, 3].map((candidate) => {
      ...
      add("character-candidate", id, [preflightId], "image", { episodeId: manifest.id, speaker, candidate }, { candidate });
      return id;
    });
    const approvalId = `character-approval:${speaker.id}`;
    add("character-approval", approvalId, candidateIds, "planning", { episodeId: manifest.id, speaker, candidateIds });
  ...
  }, { previewConfirmedRequired: true, explicitApprovalRequired: true });
