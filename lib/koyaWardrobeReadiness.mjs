...
const REVIEW_KEYS = Object.freeze(["decisions", "episodeId", "inventoryDigest", "note", "reviewedAt", "reviewer", "scriptDigest", "verdictDigest", "version"]);
...
/**
 * 場面の文からキーワード検出の前に伏せてよいのは「人の名前」だけ。
 *
 * 台帳には人物のほかに場所（kind "location"）と小物（kind "prop"）も入る。
 * 場所の名前は場面の説明そのもの（見出し「市民プール・昼」＝登録済みの場所名）
 * なので、これを伏せるとゲートが見るべき根拠が消えて、水着の要る回が
 * 「スロット0件で pass」になる（＝閉じているはずのゲートが黙って開く）。
 * 伏せるのは、照合対象の判定（koyaWardrobeCheckedCharacters）と同じ述語で選ぶ
 * 人物だけにする。人物はエピソード専用・未承認でも伏せる（名前は名前なので、
 * 「雪村」が冬の場面タグを作ってはいけない）。
 */
export function koyaWardrobeMaskableNames(registry, parsed = null) {
  const people = (Array.isArray(registry?.characters) ? registry.characters : [])
    .filter((character) => character?.kind === undefined || character.kind === "character");
  return significantNames([
    ...people.flatMap((character) => [character?.name, ...(Array.isArray(character?.aliases) ? character.aliases : [])]),
    ...(Array.isArray(parsed?.cast) ? parsed.cast.map((entry) => entry?.name ?? entry) : []),
    ...(Array.isArray(parsed?.utterances) ? parsed.utterances : [])
      .filter((utterance) => !isNarration(utterance))
      .map((utterance) => utterance?.speakerName),
  ]);
}
...
function detectSceneTags(scene, policy, maskedNames) {
  ...
  for (const source of scene.attributeSources) {
    const { masked, used } = maskNames(source.text, maskedNames);
    for (const tag of policy.detectedTags) {
      for (const keyword of policy.keywords[tag]) {
        if (masked.includes(keyword)) {
          evidence.push({ tag, keyword, source: source.source });
        } else if (source.text.includes(keyword)) {
          // 人の名前を伏せたせいで消えたキーワード。スロットは作らない
          // （「雪村」が冬の場面を作らないための伏せ字なので）が、黙って
          // 落とさずに在庫へ残す。
          const naming = used.filter((name) => name.includes(keyword));
          maskedHits.push({
            tag,
            keyword,
            source: source.source,
            // 普通は「その語を名前に含む人」。名前の境界をまたいで消えた稀な形でも
            // 落とさず、その文で伏せた名前を全部書く。
            maskedBy: naming.length > 0 ? naming : used,
          });
        }
      }
    }
  ...
  return { tags, evidence, maskedHits };
}
...
export function validateKoyaWardrobeReview(review, context = {}) {
  ...
  // レビューは判定（verdictDigest）に結び付ける。verdictDigest を書いた記録は
  // そちらで、書いていない記録は在庫全体（inventoryDigest）で照合する。在庫は、
  // この回の判定に関係の無い台帳の書き込みでも変わり、判定が同じなら
  // レビューの根拠は変わっていないため。
  if (text(review.verdictDigest)) {
    if (review.verdictDigest !== context.verdictDigest) {
      reviewFailure(failures, `verdictDigest does not match the current verdict (${context.verdictDigest}); rerun wardrobe-readiness without the review and review that inventory`);
    }
  } else if (review.inventoryDigest !== context.inventoryDigest) {
    reviewFailure(failures, `inventoryDigest does not match the current inventory (${context.inventoryDigest}); rerun wardrobe-readiness without the review and review that inventory, or bind the review to verdictDigest`);
  }
...
export function evaluateKoyaWardrobeReadiness(input = {}) {
  ...
  const allNames = koyaWardrobeMaskableNames(registry, input.parsed);
  const scenes = koyaWardrobeScenes(input.parsed);
  const sceneRows = [];
  const assignments = [];
  const slotGroups = new Map();
  for (const scene of scenes) {
    const { tags, evidence, maskedHits } = detectSceneTags(scene, policy, allNames);
    ...
    sceneRows.push({
      ...
      sceneTags: tags,
      evidence,
      ...(maskedHits.length > 0 ? { maskedKeywordHits: maskedHits } : {}),
  ...
  // 判定そのものを決めた入力だけの digest。在庫（inventoryDigest）は台帳の
  // 承認済み人物と全 reference asset を丸ごと畳み込むので、この回に関係の無い
  // 登録（来期のキャストを1人足す、別人の表情シートを1枚足す）でも変わる。
  // それで画像工程を止めると、季節を通して台帳へ書き続ける運用では、ただの
  // 登録作業が Job の停止になる。この回の判定に効く入力——場面（タグと根拠）、
  // 割り当て（誰がどの服を着るか・その sha256）、スロット、検出規則、台本——
  // だけを畳み込み、画像工程の再確認はこちらで行う。衣装が台帳から消えれば
  // matchedOutfit が変わり、承認が外れれば割り当てが消えるので、ベース衣装の
  // まま有料生成へ進む経路は塞がったままになる。
  // 注意: レビュー適用の前に取る（レビューは slots の status を書き換える）。
  const verdictDigest = koyaWardrobeDigest({
    version: KOYA_WARDROBE_READINESS_VERSION,
    episodeId,
    scriptDigest,
    policy: policySummary,
  ...
  const report = {
    ...
    summary: {
      ...
      maskedKeywordHitCount: sceneRows.reduce((total, scene) => total + (scene.maskedKeywordHits || []).length, 0),
    },
  };
