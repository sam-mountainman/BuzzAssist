...
import { installedManagedFfmpeg, installedManagedPython } from "./prerequisiteTools.mjs";
...
export function pythonRuntimeCandidates({
  ...
  // setup が ~/.buzzassist/tools/python-venv に入れた venv（opencv<5・numpy・pillow）。
  // PATH の Python が使えるならそちらが先に選ばれる。使えないときの受け皿で、
  // 運営者の interpreter を上書きしない。
  const managedPython = installedManagedPython({ platform, env, toolsDir });
  const managed = managedPython ? [{ command: managedPython, args: [], source: "buzzassist-tools-venv" }] : [];
  ...
  return (explicit.length > 0 ? uniqueSpecs(explicit) : uniqueSpecs([...local, ...ambient])).map(withUtf8Mode);
}

// Windows の Python は、ファイルとパイプの既定の文字コードがロケール（cp1252 など）に
// なる。日本語の台本や JSON を読んだ時点、または日本語を print した時点で落ちる
// （Windows の CI で原画の顔検出が UnicodeDecodeError になった）。スクリプト側でも
// 文字コードを明示しているが、起動する側で UTF-8 モードにして取りこぼしを無くす。
const UTF8_MODE_ARGS = Object.freeze(["-X", "utf8"]);

function withUtf8Mode(spec) {
  if (!spec) return spec;
  const args = [...(spec.args ?? [])];
  const already = args.some((value, index) => value === "-X" && /^utf8(?:=1)?$/u.test(String(args[index + 1] ?? "")));
  return already ? { ...spec, args } : { ...spec, args: [...args, ...UTF8_MODE_ARGS] };
}
...
export async function requirePythonRuntime(options = {}) {
  ...
  if (runtime?.ok === false || !runtime?.command) {
    ...
    // 直し方を言うのは、import は通ったのに属性だけ無かったときだけ。interpreter の起動
    // 自体が失敗したとき（機械の飽和でタイムアウトした等）に版の固定を勧めるのは誤誘導。
    const attributeMissing = new Set();
    for (const attempt of runtime?.attempts ?? []) {
      for (const [name, present] of Object.entries(attempt.modules ?? {})) {
        const [moduleName, attr] = name.split(":");
        if (attr && present === false && attempt.modules?.[moduleName] === true) attributeMissing.add(name);
      }
    }
    const hints = [...new Set([...attributeMissing].map((name) => PYTHON_REQUIREMENT_HINTS[name]).filter(Boolean))];
...
export function normalizePythonRuntime(value, fallback) {
  if (value && typeof value === "object" && nonEmpty(value.command)) {
    return withUtf8Mode({ command: nonEmpty(value.command), args: [...(value.args ?? [])], source: value.source || "explicit" });
  }
  if (nonEmpty(value)) return withUtf8Mode({ command: nonEmpty(value), args: [], source: "explicit" });
...
// execFile は timeout で子を殺したとき killed: true を付ける（spawnSync 系は ETIMEDOUT）。
function probeTimedOut(error) {
 ...
 *
 * A probe that ran out of time is not evidence that the interpreter is broken: a broken install fails its imports
 * quickly (the probe catches them and reports the modules as missing), whereas a saturated machine only makes the
 * imports slow. With several sessions and test runs at once (load average 250+), twelve concurrent probes took
 * 3-5 s each, and a character approval stopped with "cannot resolve Python" and a bare "Command failed" for every
 * candidate while the same interpreter passed the next run. So a timed-out candidate is retried once with
 * `saturatedTimeoutMs` before it counts as unusable, and the attempt says that it timed out.
 */
export async function resolvePythonRuntime({
  ...
  for (const candidate of requested) {
    ...
    if (!spec) continue;
    try {
      const probe = (timeout) => runCommand(spec.command, [...spec.args, "-c", script], { timeout, env });
      let result;
      try {
        result = await probe(timeoutMs);
      } catch (error) {
        if (!probeTimedOut(error) || !(saturatedTimeoutMs > timeoutMs)) throw error;
        try {
          result = await probe(saturatedTimeoutMs);
        } catch (retryError) {
          if (probeTimedOut(retryError)) throw new Error(`probe timed out after ${timeoutMs}ms and again after ${saturatedTimeoutMs}ms`);
          throw retryError;
        }
      }
...
function binaryCandidates(name, envNames, env, { toolsDir } = {}) {
  const explicit = envNames
    .map((envName) => ({ command: nonEmpty(env?.[envName]), args: [], source: `env:${envName}` }))
    .filter((entry) => entry.command);
  if (explicit.length > 0) return uniqueSpecs(explicit);
  // 明示の指定が無ければ PATH、次に setup が ~/.buzzassist/tools に入れた固定版。
  const managed = ["ffmpeg", "ffprobe"].includes(name) ? installedManagedFfmpeg({ env, toolsDir })[name] : null;
  return uniqueSpecs([
    { command: name, args: [], source: "path" },
    ...(managed ? [{ command: managed, args: [], source: "buzzassist-tools" }] : []),
  ]);
}
...
export async function resolveBinaryRuntime({
  ...
  const attempts = [];
  for (const spec of binaryCandidates(name, envNames, env, { toolsDir })) {
    try {
