...
// --- 音声（BuzzAssist voice） ---------------------------------------------------
// 課金は「生成された音声の長さ（秒）」。分パック 50分 ¥2,000 = ¥40/分 を単価とする。
export const BUZZASSIST_VOICE_YEN_PER_MINUTE = 40;
// 日本語の読み上げ速度の目安（文字/秒）。payloadPreview の事前見積りに使う。
export const BUZZASSIST_VOICE_CHARS_PER_SECOND = 6;

/**
 * 台本から「約◯分・¥◯」を見積もる。実際の課金は生成後の実測秒数で行われる。
 * 同じ行の作り直しは3回まで無料なので、その分はここには含めない。
 */
export function estimateVoiceGenerationCost(input = {}, config) {
  const text = typeof input.text === "string" ? input.text : "";
  const speed = Number.isFinite(Number(input.speed)) ? Math.min(2, Math.max(0.5, Number(input.speed))) : 1;
  const explicitSeconds = Number(input.durationSeconds);
  const chars = text.replace(/\s+/g, "").length;
  const pauses = (text.match(/[。．！？!?\n]/g) || []).length * 0.35
    + (text.match(/[、，,]/g) || []).length * 0.15;
  const seconds = Number.isFinite(explicitSeconds) && explicitSeconds > 0
    ? explicitSeconds
    : Math.max(0, (chars / BUZZASSIST_VOICE_CHARS_PER_SECOND + pauses) / speed);
  const minutes = seconds / 60;
  const estimatedCostYen = Math.round(minutes * BUZZASSIST_VOICE_YEN_PER_MINUTE * 10) / 10;
  return {
    kind: "voice",
    characterCount: chars,
    estimatedSeconds: Math.round(seconds * 10) / 10,
    estimatedMinutes: Math.round(minutes * 100) / 100,
    yenPerMinute: BUZZASSIST_VOICE_YEN_PER_MINUTE,
    estimatedCostYen,
    // 分数はアカウントの残高から引かれるため credits 換算は行わない
    credits: 0,
    summary: `約${Math.round(minutes * 10) / 10}分・約¥${estimatedCostYen}`,
  };
}
