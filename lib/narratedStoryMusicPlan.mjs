 ...
 *
 * ブロックごとに `-stream_loop -1 -i <曲>` を並べる呼び出しは、ブロックが多い・曲の置き場が深いと Windows の
 * コマンド行の上限に当たる。重ねは共有層の lib/ffmpegSequenceRender.mjs（mixAudioPieces）で描き、1回で
 * 収まらなければ塊に分ける（1回で収まるときの graph は今までと同じ）。
 */
...
import { mixAudioPieces } from "./ffmpegSequenceRender.mjs";
 ...
 * - 1回の ffmpeg で収まらないブロック数・曲の置き場では、塊に分けて描いて足す（budget。既定は共有層の上限）
 */
export async function buildNarratedMusicBed({
  ...
  // ブロックごとの断片: 曲を繰り返して断片の長さに切り、区分の倍率を掛け、境目に等パワーの fade を付ける
  // （この順番は変えない）。置く位置への遅らせと番組の長さへの揃え、断片どうしの足し合わせは mixAudioPieces がする。
  const pieces = timed.map((block, index) => {
    ...
    const fades = [
      ...(index > 0 && fade > 0 ? [`afade=t=in:st=0:d=${fade.toFixed(6)}:curve=qsin`] : []),
      ...(index < timed.length - 1 && fade > 0 ? [`afade=t=out:st=${(pieceSeconds - fade).toFixed(6)}:d=${fade.toFixed(6)}:curve=qsin`] : []),
    ];
    ...
    return {
      ...
      startSample: Math.round(start * SAMPLE_RATE),
      samples: pieceSamples,
      chain: (input) => [
        `[${input}:a]aresample=${SAMPLE_RATE},aformat=sample_fmts=fltp:sample_rates=${SAMPLE_RATE}:channel_layouts=stereo`,
  ...
  const graph = await mixAudioPieces({
    ffmpeg,
    pieces,
    totalSamples,
    outputPath,
  ...
  const filterGraph = graph.filterGraph;
  return {
    path: outputPath,
    receipts,
    graph,
    manifest: {
      ...
      ...(graph.assembly ? { assembly: graph.assembly, chunkCount: graph.chunks.length } : {}),
    },
  };
}
