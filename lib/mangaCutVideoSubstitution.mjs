...
//
// 契約 v55 から、結び付けたクリップは途中の成果物の品質ループの工程 video-clip で合格（別文脈の評価者の
// 採点・要る人の確認・使うファイルの sha256 が合格した版と同じ）していなければレンダーしない
// （lib/koyaAssetQualityGatePolicy.mjs の koyaVideoClipGateInForce）。この工程はクリップを ffprobe / ffmpeg で
// 測って測定のファイルを書き、合格していなければ理由コードと次の一手を返して止まる。有料の再生成はしない。
//
...
import { checkKoyaVideoClipAssetQuality, koyaAssetQualityWorkDir } from "./koyaAssetQualityGate.mjs";
import {
  KOYA_APPROVED_REFERENCES_RELATIVE_PATH,
  KOYA_ASSET_QUALITY_HARNESS_ID,
  KOYA_VIDEO_CLIP_BLOCK_REASON,
  KOYA_VIDEO_CLIP_STAGE,
  koyaVideoClipGateInForce,
} from "./koyaAssetQualityGatePolicy.mjs";
import { VIDEO_CLIP_DECLARATION_VERSION, writeVideoClipMeasurement } from "./videoClipMeasurement.mjs";
import {
...
/**
 * 差し替えのクリップの宣言（工程 video-clip の測定に渡す。lib/videoClipMeasurement.mjs）。
 *   - 尺: カットを覆う長さ（足りないフレームの許容つき。生成直後の検査と同じ）から、依頼した尺 + 1 秒まで
 *   - fps: 12〜60（提供元は 24 か 30 で返す。レンダーは回の fps へ揃える）
 *   - 解像度: 高さ 480 以上（生成直後の検査と同じ）、縦横比は回の映像と同じ（許容 2%）
 *   - 音声: optional。レンダーはクリップの映像だけを使い、音は承認済みの台詞だけ（契約の forbidGeneratedAudio は
 *     生成の依頼 generateAudio: false で守り、完成 MP4 の音声の監査が確かめる）。提供元が無音のトラックを
 *     付けて返しただけで、課金済みのクリップを使えなくしない
 */
export function cutVideoClipDeclaration({ plan, row }) {
  const shortfall = Number(plan?.policy?.maximumClipShortfallFrames) || 0;
  const minSeconds = Math.max(0.04, (Number(row.frameCount) - shortfall) / plan.fps);
  const maxSeconds = Math.max(minSeconds, Number(row.requestDurationSeconds || row.durationSeconds || 0) + 1);
  const minHeight = 480;
  return {
    version: VIDEO_CLIP_DECLARATION_VERSION,
    durationSeconds: { min: round3(minSeconds), max: round3(maxSeconds) },
    frameRate: { min: 12, max: 60 },
    width: { min: Math.floor(minHeight * (plan.width / plan.height) * 0.98) },
    height: { min: minHeight },
    aspectRatio: { width: plan.width, height: plan.height, tolerance: 0.02 },
    audio: "optional",
  };
}

/** 作業フォルダの中なら "/" 区切りの相対パス（record の --asset・--measurement に渡す値）、外なら絶対パス。 */
function workDirRel(workDir, file) {
  const rel = relative(workDir, file);
  if (!rel || rel === ".." || rel.startsWith(`..${sep}`) || isAbsolute(rel)) return resolve(file);
  return rel.split(sep).join("/");
}

/**
 * 結び付けたクリップの工程 video-clip の品質ループ（契約 v55 から）。クリップを測って測定のファイルを
 * カットの置き場に書き（同じクリップ・同じ宣言なら測り直さない）、ループの合格を確かめる。例外は投げない。
 */
async function cutVideoClipAssetQuality({ contract, workDir, episodeId, plan, row, binding, cutDir, ffmpegPath, ffprobePath }) {
  const measurementPath = join(cutDir, `measurement-${String(binding.clipSha256).slice(0, 16)}.json`);
  let measurement = null;
  let measurementError = "";
  try {
    measurement = await writeVideoClipMeasurement({
      assetPath: binding.clipPath,
      declaration: cutVideoClipDeclaration({ plan, row }),
      outputPath: measurementPath,
      ffprobe: ffprobePath,
      ffmpeg: ffmpegPath,
    });
  } catch (error) {
    measurementError = redactedError(error).slice(0, 300);
  }
  const checked = await checkKoyaVideoClipAssetQuality({ contract, workDir, episodeId, cutId: row.cutId, clipPath: binding.clipPath });
  const route = /^grok-/u.test(String(binding.model || "")) ? "grok" : "media-generation";
  const clipRel = workDirRel(workDir, binding.clipPath);
  const measurementRel = workDirRel(workDir, measurementPath);
  return {
    ...
    measurementPath: measurementRel,
    measurementSha256: measurement?.sha256 || "",
    measurementGatesPass: measurement ? Object.values(measurement.verdict.gates).every(Boolean) : false,
    ...(measurement && !Object.values(measurement.verdict.gates).every(Boolean) ? { measurementProblems: measurement.verdict.problems } : {}),
    ...(measurementError ? { measurementError } : {}),
    next: checked.pass === true ? "" : [
      `node scripts/asset-quality-loop.mjs start --work-dir <canvas> --harness ${KOYA_ASSET_QUALITY_HARNESS_ID} --stage ${KOYA_VIDEO_CLIP_STAGE} --subject ${checked.subjectId} --generator-context <クリップを作った文脈>`,
      `sheet / record --asset ${clipRel} --reference ${binding.sourceImageSha256} --approved-references ${KOYA_APPROVED_REFERENCES_RELATIVE_PATH} --measurement ${measurementRel} --route ${route}（採点は別の評価文脈）`,
      "人物が写るなら verify --check identity --check hand-safety --human-verified（対話端末の人）",
      "承認一覧は koya-manga-video.mjs asset-quality-references で書き直す（元の画の本編の画のループの合格を載せる）。合格したら同じ Job を resume する。クリップを作り直さない",
    ].join(" → "),
  };
}
...
async function runCutVideoSubstitutionsLocked({
  ...
  // 工程 video-clip の品質ループ（契約 v55 から）。結び付けるクリップを測り、ループが合格していなければ
  // 理由コードと次の一手を返して止める。結び付け自体は manifest に残す（合格した後の再開で作り直さない）。
  // 有料の再生成はしない。
  if (koyaVideoClipGateInForce(contract)) {
    const workDir = koyaAssetQualityWorkDir({ canvasDir: canvasDir || resolve(episodeDir || dirname(manifestPath), "..", "..") });
    const rowsById = new Map(plan.rows.map((row) => [row.cutId, row]));
    for (const out of rowsOut) {
      if (out.status !== "applied" || !out.binding) continue;
      const quality = await cutVideoClipAssetQuality({
        contract,
        workDir,
        episodeId: manifest.id,
        plan,
        row: rowsById.get(out.cutId),
        binding: out.binding,
        cutDir: paths.cutDir(out.cutId),
        ffmpegPath,
        ffprobePath,
      });
      out.assetQuality = quality;
      if (!quality.pass) block(out.cutId, KOYA_VIDEO_CLIP_BLOCK_REASON, `${quality.code}（${quality.detail}）`, quality.next);
    }
  }
 ...
 */
export async function verifyCutVideoSubstitutionBindings({
  manifest,
  manifestPath,
  contract,
  durationOptionsFor = imageToVideoDurationOptions,
  // レンダーの前（assertCutVideoSubstitutionsReadyForRender）だけ、工程 video-clip の品質ループの合格も求める
  // （契約 v55 から）。最終監査は asset-quality-loops がクリップを照らし直すので、video-substitution の監査では見ない。
  requireAssetQuality = false,
  canvasDir = "",
}) {
  ...
  for (const row of plan.rows) {
    ...
    if (requireAssetQuality && koyaVideoClipGateInForce(contract)) {
      const checked = await checkKoyaVideoClipAssetQuality({
        contract,
        workDir: koyaAssetQualityWorkDir({ canvasDir: canvasDir || resolve(episodeDir, "..", "..") }),
        episodeId: manifest.id,
        cutId: row.cutId,
        clipPath: binding.clipPath,
      });
      summary.assetQuality = { stage: checked.stage, subjectId: checked.subjectId, pass: checked.pass, reason: checked.reason, code: checked.code };
      if (!checked.pass) fail(row.cutId, KOYA_VIDEO_CLIP_BLOCK_REASON, `${checked.code}（${checked.detail}）`);
    }
  }
...
export async function assertCutVideoSubstitutionsReadyForRender(options) {
  const verification = await verifyCutVideoSubstitutionBindings({ ...options, requireAssetQuality: true });
...
export async function auditCutVideoSubstitutions({
  ...
  outputDir,
  pythonPath = "python3",
  // 最終監査は解決済みの実行環境（UTF-8 モードつき）を渡す。直接呼ばれたときも
  // Windows の既定の文字コードで日本語を読み落とさないよう、UTF-8 モードで起動する。
  pythonArgs = ["-X", "utf8"],
  ffprobePath = "ffprobe",
  cascadePath = "",
  runAnalysis = null,
