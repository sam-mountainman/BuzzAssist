...
export function channelLearningTargets(channel) {
  ...
  if (script) targets.push({ area: "script", target: script });
  return targets;
}
 ...
 */
function normalizeChannel(row, index, { root, pathApi, harnessIds, genres, issues }) {
...
export function validateChannelRegistry(parsed, {
  ...
  const channels = rows.map((row, index) => normalizeChannel(row, index, { root, pathApi, harnessIds, genres, issues }));
...
export function loadChannelRegistry({
  ...
  const channels = validateChannelRegistry(parsed, { repoRoot, sourcePath, harnessIds, genres });
  return {
