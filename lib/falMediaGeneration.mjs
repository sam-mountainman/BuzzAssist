...
export const FAL_IMAGE_MODELS = [
  { id: "nano-banana-2", label: "Nano Banana 2 (BuzzAssist)", provider: "nano-banana" },
  { id: "gpt-image-2", label: "GPT Image 2 (BuzzAssist)", provider: "openai" },
  { id: "seedream-v5-lite", label: "Seedream 5.0 Lite (BuzzAssist)", provider: "seedream" },
  { id: "grok-imagine-image-api", label: "Grok Imagine (BuzzAssist)", provider: "grok" },
];

export const FAL_VIDEO_MODELS = [
  { id: "seedance-2", label: "Seedance 2 (BuzzAssist)", provider: "seedance" },
  { id: "seedance-2-fast", label: "Seedance 2 Fast (BuzzAssist)", provider: "seedance" },
  { id: "kling-v3", label: "Kling v3 (BuzzAssist)", provider: "kling" },
  { id: "kling-o3", label: "Kling o3 (BuzzAssist)", provider: "kling" },
  { id: "kling-v2-6", label: "Kling v2.6 (BuzzAssist)", provider: "kling" },
  { id: "grok-imagine-video-api", label: "Grok Imagine (BuzzAssist)", provider: "grok" },
];
...
/**
 * Whole-second image-to-video durations this model accepts as-is.
 * sanitizeVideoDuration() clamps anything else silently, so callers that must
 * cover an exact edit length pick from this list instead of trusting a clamp.
 */
export function falVideoDurationOptions(model) {
  if (!isFalVideoModel(model)) return [];
...
function previewMediaInputUrl(value, kind) {
  const raw = nonEmptyString(value);
  if (!raw) return null;
  if (/^https?:\/\//i.test(raw)) return raw;
  if (raw.startsWith("data:")) return `https://preview.invalid/${kind}/inline-data-url`;
  return `https://preview.invalid/${kind}/${encodeURIComponent(basename(raw.startsWith("file://") ? new URL(raw).pathname : raw))}`;
}

function previewMediaInputUrls(values, kind) {
  return [...new Set((Array.isArray(values) ? values : []).map((value) => previewMediaInputUrl(value, kind)).filter(Boolean))];
}

export function previewFalImageRequest(input = {}) {
  ...
  const referenceImageUrls = previewMediaInputUrls(
    [
  ...
  return { model, endpoint, submitUrl: `https://fal.run/${endpoint}`, body };
}

export function previewFalVideoRequest(input = {}) {
  const prompt = nonEmptyString(input.prompt);
  if (!prompt) throw new Error("prompt is required.");
  const model = isFalVideoModel(input.model) ? input.model : "kling-v2-6";
  const media = {
    startFrameUrl: previewMediaInputUrl(
    ...
    endFrameUrl: previewMediaInputUrl(input.endFrameDataURL ?? input.end_frame_data_url ?? input.endFramePath ?? input.end_frame_path, "image"),
    referenceImageUrls: previewMediaInputUrls(
      [...(Array.isArray(input.referenceImages) ? input.referenceImages : []), ...(Array.isArray(input.referenceImagePaths) ? input.referenceImagePaths : [])],
      "image",
    ),
    referenceVideoUrls: previewMediaInputUrls(
      [...(Array.isArray(input.referenceVideos) ? input.referenceVideos : []), ...(Array.isArray(input.referenceVideoPaths) ? input.referenceVideoPaths : [])],
      "video",
    ),
    referenceAudioUrls: previewMediaInputUrls(
      [...(Array.isArray(input.referenceAudios) ? input.referenceAudios : []), ...(Array.isArray(input.referenceAudioPaths) ? input.referenceAudioPaths : [])],
      "audio",
    ),
  };
  const { endpoint, body } = buildFalVideoRequest({ ...input, prompt, model }, media);
  return { model, endpoint, submitUrl: `https://queue.fal.run/${endpoint}`, body };
}
