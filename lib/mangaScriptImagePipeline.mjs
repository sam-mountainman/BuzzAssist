...
export const MANGA_SCRIPT_IMAGE_PIPELINE_VERSION = 14;
...
function balancedCharacterReferencePaths(registry, characterIds, canvasDir, limit = 7) {
  const groups = characterIds.map((characterId) => (
    resolveCharacterReferencePaths(registry, [characterId], { canvasDir })
  ));
  const selected = [];
  const seen = new Set();
  ...
  for (const cut of parsed.cuts) {
    ...
    for (const utterance of cut.utterances) {
      ...
      const activeSpeakerDirective = activeSpeakerName
        ...
        : "";
      const allowUnnamedBackgroundPeople = exactBeatRequiresUnnamedBackgroundPeople(cut, utterance);
      ...
      for (let panelIndex = 0; panelIndex < splitCount; panelIndex += 1) {
        const rawPanelBeat = splitCount > 1 ? compositionForPanel(beat, panelIndex, panelJobs.map((job) => job.composition.setup.id)) : beat;
        const panelBeat = applyVisibleCastCountCompositionOverride(rawPanelBeat, castNames.length);
        ...
        }, editorial, splitCount > 1 ? panelRoles[panelIndex] : ""), activeSpeakerDirective, settingDirective, splitCount > 1 ? splitPanelSafeZoneDirective(editorial.split.type, panelIndex) : "", `Story age and wardrobe stage: ${storyStage}`, textFreeProps, dialogueRepair, bibleGuidance ? `Character bible authority:\n${bibleGuidance}` : "", narrationIdentityAnchor].filter(Boolean).join("\n");
        ...
        if (cutCharacterIds.length > characterReferenceCapacity) {
          throw new Error(
            `Cut ${cut.id} has ${cutCharacterIds.length} visible characters but the image provider can bind only ${characterReferenceCapacity} character anchors while preserving the location reference. Split the cut or reduce its visible cast before paid generation.`,
          );
        }
        const characterRefs = balancedCharacterReferencePaths(
          registry,
          cutCharacterIds,
          canvasDir,
        ...
        const job = {
          ...
          castNames,
          expectedVisibleCastCount: castNames.length,
          allowUnnamedBackgroundPeople,
          storyStage,
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        ...
        try {
          if (job.kind === "editorial-plate") {
            ...
            const previousCandidateAttached = isCorrectionPass && await fileExists(job.outputPath);
            const correctionReferenceImagePaths = previousCandidateAttached
              ? unique([job.outputPath, ...(job.referenceImagePaths || [])]).slice(0, 5)
              : job.referenceImagePaths;
            const fallbackCorrectionReferenceImagePaths = previousCandidateAttached
