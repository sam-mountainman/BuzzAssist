...
async function runGenerationJobs(items, concurrencySpec, worker, options = {}) {
  ...
  const outcomes = await runWithAdaptiveConcurrency(
    items.map((item, index) => (slot) => worker(item, index, slot)),
    controller,
...
async function writeGeneratedMedia(media, outputPath) {
  ...
  // 一時の名前に書いてから置き換える。途中で落ちても、書きかけの画が出力の名前に残らない
  // （書きかけの画を、次の修正の回が「直前の候補」として参照に添付しないように）。
  const temp = `${outputPath}.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2)}.tmp`;
  try {
...
export async function executeMangaScriptImagePlan(plan, options = {}) {
  ...
  // 生成の枠と QA の枠を分ける（2段）。生成の枠（AIMD のプール。チャンネルの上限）は生成から
  // 保存までだけ持ち、QA は qaQueue の枠で数える。前の画の QA を待つ間も次の画の生成が進む。
  // 端末全体の枠（paid-image）は有料の生成器（generateImageMedia）の中で取るので、ここでは重ねて取らない
  // （重ねると1枚に2枠を数える）。生成の同時数は、この2つの小さい方になる。
  const generationStage = { active: 0, peak: 0 };
  const qaStage = { active: 0, peak: 0 };
  const stageEvent = typeof options.onStageEvent === "function" ? options.onStageEvent : null;
  const emitStage = (event) => {
    if (!stageEvent) return;
    try { stageEvent({ ...event, at: Date.now() }); } catch { /* 観測の失敗で制作を止めない */ }
  };
  ...
  const semanticQa = primarySemanticQa && fallbackSemanticQa
    ...
    : primarySemanticQa;
  // QA（意味の判定）を待つ間は生成の枠を返す。stageOverlap: false なら従来どおり、1本が生成から
  // QA まで枠を持つ（直列と比べる試験と、切り分けのための逃げ道）。
  const releaseSlotForQa = Boolean(semanticQa) && options.stageOverlap !== false;
  ...
  // 途中の成果物の品質ループ（呼び出し側が渡したときだけ。漫画の公式経路は契約 v54 から渡す）。
  // 機械の QA（技術＋意味）に合格した行を complete にする前に呼び、合格していなければ行を
  // awaiting-human-review で止める。止まった行は有料の再生成を回さない（入力が変わったときだけ作り直す）。
  const assetQualityGate = typeof options.assetQualityGate === "function" ? options.assetQualityGate : null;
  const applyAssetQualityGate = async (job, state) => {
    if (!assetQualityGate) return null;
    const verdict = await assetQualityGate({ job, outputPath: job.outputPath });
    if (!verdict || verdict.required === false) {
      delete state.assetQuality;
      return null;
    }
    state.assetQuality = {
      required: true,
      ...
      detail: String(verdict.detail || "").slice(0, 500),
      assetSha256: verdict.assetSha256 || "",
      checkedAt: new Date().toISOString(),
    };
    return state.assetQuality;
  };
  const runMachineQa = async (job, state, attempt) => {
    const technical = await defaultTechnicalQa(job);
    const semantic = semanticQa
      ? await qaQueue.add(
          async () => {
            qaStage.active += 1;
            qaStage.peak = Math.max(qaStage.peak, qaStage.active);
            emitStage({ stage: "qa", phase: "start", jobId: job.id, attempt });
            try {
              let lastError;
              ...
              qaStage.active -= 1;
              emitStage({ stage: "qa", phase: "end", jobId: job.id, attempt });
            }
          },
  ...
  // 品質ループ待ちで止めた行: 入力が同じなら作り直さず、今のファイルでループを見直すだけ。
  // ファイルが機械の QA を通った版から差し替わっていれば（別の経路で作った新しい版）、先にその版へ
  // 機械の QA を掛け直す（生成はしない）。入力が変わった行は作り直しへ戻す。
  for (const job of plan.jobs) {
    const state = ledger.jobs[job.id];
    if (state?.status !== "awaiting-human-review") continue;
    if (state.inputHash !== job.inputHash || !await fileExists(job.outputPath)) {
      state.previousAssetQualityHold = { inputHash: state.inputHash || "", assetQuality: state.assetQuality || null };
      state.status = "pending";
      continue;
    }
    // ゲートを持たない呼び出し（汎用の経路）は、止めた行を合格扱いにも作り直しにもしない。
    // ここで complete にすると、ゲートの外から回し直すだけで品質ループを飛ばせてしまう。
    if (!assetQualityGate) continue;
    try {
      const currentSha256 = sha256(await readFile(job.outputPath));
      if (currentSha256 !== state.machineQaAssetSha256) {
        const qa = await runMachineQa(job, state, Number(state.retries || 0));
        state.qa = qa;
        delete state.qaInfrastructureError;
        if (!qa.pass) {
          state.status = "failed";
          state.error = qa.issues.join("; ") || "visual QA failed";
          state.failedAt = new Date().toISOString();
          continue;
        }
        state.machineQaAssetSha256 = currentSha256;
      }
      const verdict = await applyAssetQualityGate(job, state);
      delete state.assetQualityRecheckError;
      ...
      state.assetQualityRecheckError = error instanceof Error ? error.message : String(error);
    }
  }
  ...
  while (completed.size < plan.jobs.length) {
    const ready = plan.jobs.filter((job) => !completed.has(job.id)
      ...
      && ledger.jobs[job.id]?.status !== "awaiting-human-review");
    if (ready.length === 0) break;
    const outcomes = await runGenerationJobs(ready, concurrencySpec, async (job, _index, slot) => {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        // QA に落ちた作り直しは、生成の枠を取り直してから（新しい画の開始より先に回る）。
        if (attempt > 0) await slot?.reacquire?.();
        state.attempts += 1;
        ...
        try {
          if (job.kind === "editorial-plate") {
            ...
            if (!(reuseGeneratedForQa && attempt === 0)) {
              ...
              generationStage.active += 1;
              generationStage.peak = Math.max(generationStage.peak, generationStage.active);
              emitStage({ stage: "generation", phase: "start", jobId: job.id, attempt });
              try {
                let media;
                ...
                generationStage.active -= 1;
                emitStage({ stage: "generation", phase: "end", jobId: job.id, attempt });
              }
              delete state.qa;
              delete state.qaGenerationAttempt;
            }
          }
          // 保存が済んだら生成の枠を返し、QA の枠で待つ。
          if (releaseSlotForQa) slot?.release?.();
          const qa = await runMachineQa(job, state, attempt);
          state.qa = qa;
          state.qaGenerationAttempt = attempt;
          delete state.qaInfrastructureError;
          if (!qa.pass) throw new Error(qa.issues.join("; ") || "visual QA failed");
          if (assetQualityGate) {
            // 機械の QA に通った版の SHA を残す（品質ループ待ちの見直しで、差し替えを見分けるため）。
            state.machineQaAssetSha256 = sha256(await readFile(job.outputPath));
            const verdict = await applyAssetQualityGate(job, state);
            if (verdict && !verdict.pass) {
              state.status = "awaiting-human-review";
              state.awaitingHumanReviewSince = new Date().toISOString();
              delete state.error;
              await save();
              return { job, reused: false, awaitingReview: true };
            }
          }
          state.status = "complete";
          state.completedAt = new Date().toISOString();
    ...
    for (const outcome of outcomes) {
      if (outcome.ok && outcome.value.awaitingReview !== true) completed.add(outcome.value.job.id);
    }
  ...
  ledger.summary = {
    ...
    qaConcurrency,
    // 生成の枠と QA の枠を分けた2段の実測（この回に同時に走った最大数）。上限を越えていないことの証跡。
    stagePipeline: {
      version: "manga-image-two-stage-v1",
      generationLimit: generationController?.limit ?? concurrencySpec.fixedLimit,
      qaConcurrency,
      releaseSlotForQa,
      peakGeneration: generationStage.peak,
      peakQa: qaStage.peak,
    },
  ...
  const awaitingReviewStates = states.filter((entry) => entry.status === "awaiting-human-review");
  if (assetQualityGate) {
    ledger.summary.awaitingHumanReview = awaitingReviewStates.length;
    ledger.summary.assetQualityAwaiting = awaitingReviewStates.map((entry) => ({
      jobId: entry.id,
  ...
  ledger.status = ledger.summary.complete === plan.jobs.length
    ...
    : hasWaitingJobs
      ? "waiting"
      : ledger.summary.failed === 0 && awaitingReviewStates.length > 0
...
export async function runMangaScriptImagePipeline(args = {}) {
  ...
  // 計画を書いた後・有料の生成の前に呼び出し側へ知らせる（漫画の full は台詞の音声をここから並べて始める）。
  // 呼び出し側は待たせる仕事をここで抱え込まないこと（渡すだけにする）。
  if (typeof args.onScenePlanReady === "function") await args.onScenePlanReady({ episodeId, plan, planPath });
  const execution = await executeMangaScriptImagePlan(plan, {
    ...
    stageOverlap: args.stageOverlap,
    onStageEvent: args.onStageEvent,
    ...
    assetQualityGate: typeof args.assetQualityGateFactory === "function"
      ? args.assetQualityGateFactory({ episodeId, canvasDir, plan })
      : args.assetQualityGate,
  });
