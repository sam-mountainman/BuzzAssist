...
export const MANGA_SCRIPT_IMAGE_PIPELINE_VERSION = 2;
...
function textFreePropDirective(utterance = {}) {
  if (!/(?:英語|大学|学歴|就活|内定|講義|ディスカッション|会議|スマホ|ライン|資料|勉強|授業|仕事|職場|欠勤|解雇|アルバイト|レシート)/u.test(utterance.text || "")) return "";
  return "Text-free prop rule: every book cover, brochure, worksheet, notebook, phone screen, sign, badge, and document must be completely blank or use only simple non-linguistic color blocks. Draw no letters, pseudo-text, numbers, notation, logos, or glyph-like lines. Communicate meaning through character action, color, shape, and composition instead.";
}
...
function splitPanelSafeZoneDirective(splitType, panelIndex) {
  ...
  if (panelIndex === 0) {
    return "Final-page crop safety: this source becomes a narrow full-height LEFT panel occupying about 39% of the page. Keep exactly one primary character's complete face, hands, and torso centered inside the middle 25% horizontal band of the source with generous margin on both sides. No required body part may touch a source edge. Do not place empty furniture or background in that band.";
  }
  if (panelIndex === 1) {
    return "Final-page crop safety: this source becomes the UPPER-RIGHT panel with a rising diagonal lower edge. Keep all required faces in the upper-center safe area, away from every edge and diagonal; any foreground shoulder must cover less than 15% of the frame.";
  }
  return "Final-page crop safety: this source becomes the LOWER-RIGHT panel with a rising diagonal upper edge. Keep all required faces and hands in the lower-center/right safe area with generous headroom, never touching the diagonal boundary.";
}
...
function visualQaPrompt(payload) {
...
async function runCodexVisualQa(payload, options = {}) {
  ...
  const prompt = visualQaPrompt(payload);
...
async function runGrokVisualQa(payload, options = {}) {
  ...
  const blocks = [{ type: "text", text: visualQaPrompt(payload) }];
