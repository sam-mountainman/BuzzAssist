...
export async function insertExcalidrawVideo(args = {}) {
  ...
  const posterDataURL = args.posterDataURL || videoPlaceholderDataURL({ width: pixelWidth, height: pixelHeight, label: "Video" });
  const customData = {
...
export async function insertExcalidrawSubtitle(args = {}) {
  ...
  const previewLines = subtitleLines
    .slice(0, 4)
    .map((line) => String(line?.text ?? "").trim())
    .filter(Boolean);
  const lineCountLabel = subtitleLines.length > 0 ? `${subtitleLines.length} cues` : "SRT";
  const cardText = [
    `字幕 ${fileName}`,
    lineCountLabel,
    ...previewLines.map((line) => (line.length > 24 ? `${line.slice(0, 24)}…` : line)),
  ].join("\n");
  ...
  const textElementId = uniqueId(existingIds, "subtitle_text", recordSeed);
  ...
  const textElement = newTextElementRecord({
    id: textElementId,
    index: chooseIndexAfter(scene.elements.concat(cardElement), cardIndex),
    bounds,
    groupId,
    assetUrl,
    text: cardText,
    customData: { codexGeneratedSubtitleText: true, codexAssetUrl: assetUrl },
  });
...
export async function insertExcalidrawMediaBatch(args = {}) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    if (kind === "video") {
      ...
      const posterDataURL = item.posterDataURL || videoPlaceholderDataURL({ width: pixelWidth, height: pixelHeight, label: "Video" });
      const customData = {
        codexInsertedVideo: true,
        codexGeneratedVideo: true,
        codexMediaKind: "video",
      ...
      element = newImageElementRecord({ id: elementId, fileId, index, bounds, customData, link: assetUrl });
      fileRecord = {
        id: fileId,
        mimeType: "image/svg+xml",
        dataURL: posterDataURL,
      ...
      const dataURL = item.dataURL || `data:${mimeType};base64,${item.mediaBuffer.toString("base64")}`;
      const customData = {
        codexInsertedImage: true,
        codexAssetPath: filePath,
        codexAssetUrl: assetUrl,
