...
export const SPEECH_GENERATOR_FRAME_TAG = "buzzassist.speechGenerator.frame";
...
// 実装は外部依存の無い atomicJsonFile.mjs にある（実行系がこのファイル経由で
// 外部パッケージを読まないため）。既存の呼び出し側のために同じ名前で再公開する。
// `export { … } from` の形はこのファイルの中に名前を作らないので、ここで使う分も含めて
// import してから再公開する（最初は再公開だけにして、このファイル内の呼び出しが壊れた）。
import { readJsonIfExists, writeJsonAtomic } from "./atomicJsonFile.mjs";
export { readJsonIfExists, writeJsonAtomic };
...
export async function insertExcalidrawImage(args = {}) {
  ...
  const reuseExistingAsset = args.reuseExistingAsset === true && sourceImagePath && isSafeChildPath(assetsDir, sourceImagePath);
  const { fileName, filePath } = reuseExistingAsset
    ? { fileName: basename(sourceImagePath), filePath: sourceImagePath }
  ...
  if (!args.dryRun) {
    await mkdir(assetsDir, { recursive: true });
    if (!reuseExistingAsset) await writeFile(filePath, fileData);
    if (replaceAnchor) {
      scene.elements = scene.elements.map((element) =>
...
function escapeSpeechCardText(value) {
  return String(value ?? "")
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
function speechCardPreviewDataUrl(args = {}) {
  const name = escapeSpeechCardText(args.fileName || "speech.mp3");
  const voice = escapeSpeechCardText(args.voiceName || args.voiceId || "ElevenLabs default");
  const model = escapeSpeechCardText(args.model || "eleven_v3");
  const seconds = Math.max(0, Number(args.durationSeconds) || 0).toFixed(1);
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="320" height="180" viewBox="0 0 320 180">
  <rect width="320" height="180" rx="18" fill="#fff"/>
  <rect x="20" y="22" width="76" height="76" rx="18" fill="#e8ddf5"/>
  <g fill="#7048b8"><rect x="37" y="54" width="5" height="16" rx="2.5"/><rect x="47" y="43" width="5" height="38" rx="2.5"/><rect x="57" y="49" width="5" height="26" rx="2.5"/><rect x="67" y="38" width="5" height="48" rx="2.5"/><rect x="77" y="52" width="5" height="20" rx="2.5"/></g>
  <text x="116" y="48" font-family="Inter,Arial,sans-serif" font-size="17" font-weight="700" fill="#1f2937">${name}</text>
  <text x="116" y="75" font-family="Inter,Arial,sans-serif" font-size="13" fill="#6b7280">${voice}</text>
  <text x="116" y="97" font-family="Inter,Arial,sans-serif" font-size="12" fill="#8a94a6">${model} · ${seconds}s</text>
  <path d="M20 126H300" stroke="#edf0f5" stroke-width="2"/>
  <text x="20" y="153" font-family="Inter,Arial,sans-serif" font-size="12" font-weight="700" fill="#7048b8">ELEVENLABS SPEECH</text>
</svg>`;
  return `data:image/svg+xml;base64,${Buffer.from(svg).toString("base64")}`;
}

export async function insertExcalidrawAudioResult(args = {}) {
  const audioPath = pathResolve(nonEmptyString(args.audioPath || args.assetPath) || "");
  const sourceStat = await stat(audioPath);
  if (!sourceStat.isFile()) throw new Error(`Audio path is not a file: ${audioPath}`);
  ...
  const bounds = replaceAnchor && !matchAnchor && anchorBounds
    ? { x: anchorBounds.x + (anchorBounds.width - width) / 2, y: anchorBounds.y + (anchorBounds.height - height) / 2, width, height }
    : placementBounds;

  const canvasDir = resolveCanvasDir(args);
  const assetsDir = join(canvasDir, "assets");
  if (!isSafeChildPath(assetsDir, audioPath)) throw new Error(`Unsafe speech asset path: ${audioPath}`);
  const relativeAssetPath = relative(assetsDir, audioPath).split(sep).join("/");
  const fileName = sanitizeFileName(args.fileName || basename(audioPath), "speech.mp3");
  const assetUrl = nonEmptyString(args.assetUrl) || `${ASSETS_ROUTE}${relativeAssetPath.split("/").map(encodeURIComponent).join("/")}`;
  const existingIds = new Set([...scene.elements.map((element) => element.id), ...Object.keys(scene.files ?? {})]);
  const recordSeed = sanitizeIdPart(fileName, "speech");
  const elementId = uniqueId(existingIds, "speech", recordSeed);
  const fileId = uniqueId(existingIds, "file", `${recordSeed}-preview`);
  const customData = {
    ...
    codexFileName: fileName,
    codexAssetPath: audioPath,
    codexAssetUrl: assetUrl,
    codexAssetMimeType: args.mimeType || mimeTypeForFile(audioPath),
    codexAssetDuration: Math.max(0, Number(args.durationSeconds) || 0),
    speechProvider: args.provider || "elevenlabs",
    speechModel: args.model || "eleven_v3",
    speechVoiceId: args.voiceId || "",
    speechVoiceName: args.voiceName || "",
    speechText: args.text || "",
    speechAlignmentPath: args.alignmentPath || "",
    ...(anchorElementId ? { codexAnchorElementId: anchorElementId } : {}),
    ...(args.customData && typeof args.customData === "object" ? args.customData : {}),
  };
  const element = newImageElementRecord({
    id: elementId,
    fileId,
    index: chooseIndex(scene.elements),
    bounds,
    customData,
    link: assetUrl,
  });
  const fileRecord = {
    id: fileId,
    name: `${fileName}-preview.svg`,
    mimeType: "image/svg+xml",
    dataURL: speechCardPreviewDataUrl(args),
    created: Date.now(),
    lastRetrieved: Date.now(),
  };
  ...
  if (!args.dryRun) {
    if (replaceAnchor) {
      scene.elements = scene.elements.map((candidate) => candidate.id === anchorElementId
        ? { ...candidate, isDeleted: true, version: (Number(candidate.version) || 1) + 1, versionNonce: Math.floor(Math.random() * 2 ** 31), updated: Date.now() }
        : candidate);
    }
    scene.files = scene.files && typeof scene.files === "object" ? scene.files : {};
    scene.files[fileId] = fileRecord;
    scene.elements.push(element);
    scene.appState = { ...scene.appState, selectedElementIds: { [elementId]: true } };
    await saveScene(args, scene);
  }

  return { elementId, fileId, anchorElementId, assetFile: audioPath, assetUrl, bounds, replacedAnchor: replaceAnchor, dryRun: Boolean(args.dryRun) };
}
...
// 寸法の読み方は外部パッケージに依存しない lib/imageDimensions.mjs の1か所。既存の import のために再公開する。
export { getImageDimensionsFromBuffer };
