...
export async function insertGeneratorFrameBatch(args = {}) {
  ...
  const columns = Math.max(1, Math.round(finiteNumber(Number(args.columns), 2)));
  // Batch chunks contain at most ten jobs. Fill downward first so the default
  // layout is two columns x five rows: item 1-5 in the first column and item
  // 6-10 in the second column.
  const rowsPerColumn = Math.max(1, Math.round(finiteNumber(Number(args.rowsPerColumn), Math.ceil(10 / columns))));
  ...
  const usedColumns = Math.max(1, Math.min(columns, Math.ceil(frames.length / rowsPerColumn)));
  const usedRows = Math.max(1, Math.min(rowsPerColumn, frames.length));
