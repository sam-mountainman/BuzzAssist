...
/** 捕捉が止まった理由のコード（本文やパスは返さない）。 */
function captureFailureReason(error, channelId) {
  if (error?.code === "LEARNING_WRITE_FORBIDDEN_IN_CHILD_AGENT") return "child-agent";
  // チャンネルの保存先を決められない（台帳に無い・壊れている・共有台帳と重なる）。チャンネルの無い保存先へは落とさない。
  if (channelId && (error?.code === CHANNEL_LEARNING_SCOPE_ERROR || error?.code === CHANNEL_LEARNING_STORE_ERROR)) {
    return "channel-learning-store-unresolved";
  }
...
async function captureSettledJobLearningCore({
  ...
  for (const candidate of summary.candidates) {
    try {
      ...
      return { ...result, skippedReason: captureFailureReason(error, channelId) };
    }
  }
  return result;
}
