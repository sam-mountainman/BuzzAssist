...
//     ハーネス・状態・RunReceipt の path と sha256・決着時刻だけを持つ
 ...
 * Job の id・ハーネス・状態・RunReceipt の path と sha256・決着時刻だけ。失敗しても Job は止めない。
 */
...
/**
 * 決着した Job から学習候補を捕捉する。Job を止めないため、例外は投げずに理由を返す。
 * 捕捉の前に、決着を Receipt の索引へ残す（BUZZASSIST_LEARNING_AUTO_CAPTURE=0 でも残す。
 * 索引は集計の読み先で、提案ではないため）。
 */
export async function captureSettledJobLearning({
  job,
  env = process.env,
