...
// ここで守ること:
//
//   - **書くのは提案台帳への追記だけ**。正本（SKILL.md・台帳）と overlay には一切触らない。
//     捕捉は何も書き換えない、という自己改善の大原則のまま
...
export async function locateJobRunReceipt(job, { readReceipt = readReceiptFile } = {}) {
  ...
  if (receiptArtifact?.path) {
    try {
      ...
      return { receipt, digest, source: "run-receipt" };
    } catch {
  ...
  if (adapterPath) {
    try {
      ...
      return { receipt, digest, source: "adapter-run-receipt" };
    } catch {
      // 同上。
    }
  }
