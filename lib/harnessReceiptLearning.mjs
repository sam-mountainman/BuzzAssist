...
//     ハーネス・状態・動かしたホスト・Job を作ったチャンネル（metadata.channel の id）・RunReceipt の path と
//     sha256・決着時刻だけを持つ
...
import { issueCodeOf } from "./harnessRunReceipt.mjs";
...
// issue をコードへ丸める関数は RunReceipt の集計と共有する（lib/harnessRunReceipt.mjs）。
export { issueCodeOf, jobChannelId };
 ...
 * Job の id・ハーネス・状態・動かしたホスト・Job を作ったチャンネルの id（metadata.channel。無い Job は欄を足さない）・
 * RunReceipt の path と sha256・決着時刻だけ。失敗しても Job は止めない。
 */
export function indexSettledJobReceipt({ job, located = null, indexPath, now = () => new Date().toISOString() } = {}) {
  try {
    ...
    // Job を作ったチャンネル（rollup --channel とチャンネルごとの内訳が使う）。
    const channel = jobChannelId(job);
