...
function bubbleSegmentBoundaryScore(characters, index, target) {
  ...
  const punctuationPenalty = /[.!?。！？]/u.test(previous)
    ...
    : /[,、]/u.test(previous)
      ...
      : /\s/u.test(previous) || /\s/u.test(next)
        ? 12
        : 40;
  return Math.abs(index - target) + punctuationPenalty;
}

function splitBubbleTextNaturally(text, segmentCount) {
  ...
  for (let segmentIndex = 1; segmentIndex < segmentCount; segmentIndex += 1) {
    ...
    const maximumCharactersPerSegment = 16;
    ...
    for (let index = minimum; index <= maximum; index += 1) {
      const score = bubbleSegmentBoundaryScore(characters, index, target);
      if (score < bestScore) {
        best = index;
        bestScore = score;
      }
...
function renderBubbleOrSequentialSegments({ bubble, width, height, avoidRegions, profileId, title }) {
  ...
  for (const cut of manifest.cuts) {
    ...
    for (const [utteranceIndex, utteranceId] of cut.utteranceIds.entries()) {
      ...
      if (bubbleSegments.length > 1 && bubbleSegments.some((segment) => (
        ...
        const weights = bubbleSegments.map((segment) => Math.max(
          1,
        ...
        for (const [segmentIndex, segment] of bubbleSegments.entries()) {
          const previousBoundary = speechStart + speechDuration * cumulativeWeight / totalWeight;
          cumulativeWeight += weights[segmentIndex];
          const nextBoundary = speechStart + speechDuration * cumulativeWeight / totalWeight;
          segment.startOffsetSeconds = Number((segmentIndex === 0
            ? bubbleStartOffset
            : previousBoundary + segmentGap / 2).toFixed(4));
          segment.endOffsetSeconds = Number((segmentIndex === bubbleSegments.length - 1
            ? bubbleEndOffset
            : nextBoundary - segmentGap / 2).toFixed(4));
          segment.timingPolicy = "proportional-character-count-v1";
        }
