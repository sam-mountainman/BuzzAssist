...
import {
  ...
  cutFrameCount,
  cutVideoBindingHashInput,
  cutVideoClipBaseLayerFilter,
...
import {
  CUT_HEADING_FORMAT,
  SCENE_SCRIPT_DEFAULT_MAX_UTTERANCES_PER_CUT,
  SCENE_SCRIPT_FORMAT,
  detectMangaScriptFormat,
  planSceneCutBoundaries,
  readSceneScript,
} from "./mangaSceneScript.mjs";
...
import { rasterizeSvg } from "./svgRasterizer.mjs";
...
// Scene-script speakers only. Slugging keeps just ASCII letters and digits,
// so for a name such as 「客1」 it would return "1" and merge 「客1」 with
// 「男1」. A name that is not all ASCII gets the hash id instead. The legacy
// parser keeps speakerIdForName as it is: its ids are already part of
// existing episodes' jobs.
function sceneScriptSpeakerIdForName(name, registry) {
  const normalized = nonEmptyString(name);
  if (/^[\x20-\x7E]+$/u.test(normalized.normalize("NFKC"))) return speakerIdForName(normalized, registry);
  if (/^(?:ナレーション|ナレーター|地の文)$/u.test(normalized)) return "narration";
  return findCharacter(registry, normalized)?.id
    || `speaker-${createHash("sha256").update(normalized).digest("hex").slice(0, 10)}`;
}

function dialoguePreset(text) {
  return /！|!$/u.test(text)
    ? "shout"
...
/**
 * Parse a manga script into cuts and utterances.
 *
 * Two input formats are accepted. The scene-script format (front matter with
 * タイトル/登場人物, or 「#場面 N」 headings) is parsed by parseSceneScript; every
 * other script keeps the original 【カット N：…】 parser, whose output is
 * unchanged apart from `format: "cut-heading"`.
 */
export function parseMangaScript(scriptText, options = {}) {
  const registry = options.registry && typeof options.registry === "object"
    ? options.registry
    : { characters: [], voices: [] };
  if (detectMangaScriptFormat(scriptText) === SCENE_SCRIPT_FORMAT) {
    return parseSceneScript(scriptText, options, registry);
  }
  return { ...parseCutHeadingScript(scriptText, options, registry), format: CUT_HEADING_FORMAT };
}

function parseCutHeadingScript(scriptText, options, registry) {
  ...
  for (const rawLine of markdown.body.split("\n")) {
    ...
    currentCut.utterances.push({
      ...
      preset: isNarration ? "narration" : dialoguePreset(text),
    });
  }
...
function sceneScriptMaximumUtterancesPerCut(value) {
  if (value === undefined || value === null) return SCENE_SCRIPT_DEFAULT_MAX_UTTERANCES_PER_CUT;
  const number = Number(value);
  if (!Number.isInteger(number) || number < 1) {
    throw new Error(`maxUtterancesPerCut must be a positive integer (received ${JSON.stringify(value)}).`);
  }
  return number;
}

function parseSceneScript(scriptText, options, registry) {
  const maximumUtterancesPerCut = sceneScriptMaximumUtterancesPerCut(options.maxUtterancesPerCut);
  const script = readSceneScript(scriptText);
  const { frontMatter, protagonistName } = script;
  // Compare by speaker id so a registry alias of the protagonist counts too.
  const protagonistSpeakerId = protagonistName ? sceneScriptSpeakerIdForName(protagonistName, registry) : "";
  const title = nonEmptyString(options.title)
    || nonEmptyString(frontMatter["タイトル"])
    || nonEmptyString(frontMatter.title)
    || script.bodyTitle
    || script.markdownTitle
    || "漫画動画";

  const cuts = [];
  for (const scene of script.scenes) {
    if (scene.entries.length === 0) continue; // reported as an empty-scene warning
    const boundaries = planSceneCutBoundaries(scene.entries.length, scene.blankBoundaries, { maxUtterancesPerCut: maximumUtterancesPerCut });
    const edges = [0, ...boundaries, scene.entries.length];
    for (let part = 0; part < edges.length - 1; part += 1) {
      const number = cuts.length + 1;
      const id = `cut-${pad2(number)}`;
      const cut = {
        id,
        number,
        purpose: scene.heading,
        utterances: [],
        scene: { number: scene.number, heading: scene.heading, place: scene.place, timeOfDay: scene.timeOfDay },
        ...(scene.place ? { location: { name: scene.place } } : {}),
      };
      scene.entries.slice(edges[part], edges[part + 1]).forEach((line, index) => {
        const order = index + 1;
        const utteranceId = `${id}-u${pad2(order)}`;
        const isNarration = line.kind === "narration";
        const speakerName = isNarration ? "ナレーション" : line.speakerName;
        const speakerId = isNarration ? "narration" : sceneScriptSpeakerIdForName(speakerName, registry);
        const text = isNarration ? line.text : normalizeDialogueText(line.text);
        cut.utterances.push({
          id: utteranceId,
          cutId: id,
          order,
          ...
          text,
          bubbleId: `bubble-${utteranceId}`,
          preset: isNarration ? "narration" : line.innerVoice ? "thought" : dialoguePreset(text),
          isProtagonist: !isNarration && Boolean(protagonistSpeakerId) && speakerId === protagonistSpeakerId,
          innerVoice: !isNarration && line.innerVoice === true,
        });
      });
      cuts.push(cut);
    }
...
const JAPANESE_CONNECTIVE_PARTICLE = /^(?:の)$/u;
...
const JAPANESE_CLOSING_PUNCTUATION = /^[.!?。！？、，,]/u;
const JAPANESE_INFLECTION_FRAGMENT = /^[ぁ-ゖ]{1,4}$/u;
...
function japaneseBubbleBoundaryProfile(text) {
  ...
  for (let index = 0; index < units.length - 1; index += 1) {
    ...
    // Closing punctuation belongs to the phrase before it.  A timed bubble
    // replacement beginning with "。" or "！" reads as a broken subtitle even
    // when Intl.Segmenter exposes a boundary immediately before that mark.
    if (JAPANESE_CLOSING_PUNCTUATION.test(next.text)) continue;
    ...
    if (JAPANESE_BOUNDARY_PARTICLE.test(previous.text)) {
      // A genitive の must keep its following noun, while directional へ and
      // the other case/topic particles can close a readable bunsetsu.
      if (!JAPANESE_CONNECTIVE_PARTICLE.test(previous.text)) allowed.set(boundary, 6);
      continue;
    }
    const inflectionBoundary = previous.isWordLike && next.isWordLike
      && JAPANESE_INFLECTION_FRAGMENT.test(next.text);
    // Intl.Segmenter intentionally exposes morphological pieces such as
    // 戻/っ/た, 送り/まし/た and いつ/だって. Those are useful tokenizer
    // units, but a timed balloon must not reveal an inflection fragment by
    // itself on the next frame.
    if (inflectionBoundary) continue;
    const attributiveBoundary = previous.isWordLike && next.isWordLike
      && /な$/u.test(previous.text)
      && /^[\p{Script=Han}\p{Script=Katakana}]/u.test(next.text);
    // Keep a na-adjective with the noun it modifies: 派手な/腕時計.
    if (attributiveBoundary) continue;
    const mixedScriptCompoundBoundary = previous.isWordLike && next.isWordLike
      && (
        (/[\p{Script=Katakana}ー]$/u.test(previous.text) && /^\p{Script=Han}/u.test(next.text))
        || (/\p{Script=Han}$/u.test(previous.text) && /^[\p{Script=Katakana}ー]/u.test(next.text))
        || (/\p{Script=Han}.*\p{Script=Hiragana}$/u.test(previous.text) && /^\p{Script=Han}/u.test(next.text))
      );
    // Japanese compounds and attributive predicates often cross the script
    // boundaries that Intl exposes (スポンサー/契約, 切り/欠き, 使える/方法).
    // They remain one readable phrase in sequential balloon replacement.
    if (mixedScriptCompoundBoundary) continue;
...
/**
 * 台本の解析と声の割り当て（画像に依らない部分）。createEpisodeManifest と、画像より先に音声の入力だけを
 * 作る createEpisodeDialogueDefinition が同じ結果を得るために1か所に置く。
 * allowVoiceCasting: false なら声の自動割り当て（ネットワークと登録簿の書き込み）をしない。
 */
async function prepareEpisodeScript(args = {}, { allowVoiceCasting = true } = {}) {
  const canvasDir = resolveCanvasDir(args);
  ...
  if (allowVoiceCasting && args.autoCastVoices !== false && args.auto_cast_voices !== false) {
  ...
  return { canvasDir, registry, scriptText, parsedScript, episodeId, voiceCasting, speechPronunciations };
}

/** 1発話の、音声の入力に要る欄（表示文・読み・声）。画像にも吹き出しの配置にも依らない。 */
function episodeUtteranceSpeechFields(utterance, { registry, episodeId, speechPronunciations, model, stripTerminalJapanesePeriod }) {
  ...
  return {
    bubbleDisplayText: mangaBubbleDisplayText(utterance.text, { stripTerminalJapanesePeriod: stripTerminalJapanesePeriod === true }),
    ...
    model: registryVoice?.modelId || nonEmptyString(model) || DEFAULT_SPEECH_MODEL,
  };
}
...
export async function createEpisodeManifest(args = {}) {
  const {
    ...
    episodeId,
    voiceCasting,
    speechPronunciations,
  } = await prepareEpisodeScript(args);
  ...
  for (const cut of parsedScript.cuts) {
    ...
    for (const utterance of cut.utterances) {
      const speechFields = episodeUtteranceSpeechFields(utterance, {
        registry,
        episodeId,
        speechPronunciations,
        model: args.model,
        stripTerminalJapanesePeriod: args.stripTerminalJapanesePeriod,
      });
      const { bubbleDisplayText } = speechFields;
  ...
  const manifest = episodeManifestDocument(args, {
    episodeId,
    parsedScript,
    scriptText,
    speechPronunciations,
...
/**
 * 画像を待たずに、台本から音声の入力（対話の定義）だけを作る。createEpisodeManifest と同じ解析・同じ声の
 * 割り当て・同じ文書の形で、画像・吹き出しの配置・overlay の欄だけを持たない。ファイルは1つも書かない。
 * 声の自動割り当て（ネットワーク・登録簿の書き込み）はしない。自動割り当てをしない呼び出し
 * （autoCastVoices: false。Koya の公式経路）の createEpisodeManifest と、発話と cut の音声の欄が一致する。
 */
export async function createEpisodeDialogueDefinition(args = {}) {
  const {
    registry,
    scriptText,
    parsedScript,
    ...
    voiceCasting,
    speechPronunciations,
  } = await prepareEpisodeScript(args, { allowVoiceCasting: false });
  const cuts = [];
  const utterances = [];
  for (const cut of parsedScript.cuts) {
    const cutUtteranceIds = [];
    for (const utterance of cut.utterances) {
      utterances.push({
        ...utterance,
        ...episodeUtteranceSpeechFields(utterance, {
          registry,
          episodeId,
          speechPronunciations,
          model: args.model,
  ...
  return {
    manifest: episodeManifestDocument(args, {
      episodeId,
      parsedScript,
      scriptText,
      speechPronunciations,
...
function episodeManifestDocument(args, { episodeId, parsedScript, scriptText, speechPronunciations, voiceCasting, cuts, utterances }) {
  ...
  return {
    ...
    scriptText,
    ...(parsedScript.format === SCENE_SCRIPT_FORMAT
      ? { scriptFormat: parsedScript.format, declaredProtagonistName: parsedScript.protagonistName }
      : {}),
    video: {
...
export async function refreshEpisodeBubbleOverlays(args = {}) {
  ...
  for (const utterance of manifest.utterances || []) {
    ...
    const faceConstrainedMaximumCharactersPerSegment = 13;
    ...
    const shouldResegment = (panelLayoutEnabled && panelCharacterCount > panelMaximumCharactersPerSegment)
      ...
      // frames can have a safe pocket too short for a legacy 14–16 character
    ...
    // If the Japanese boundary model cannot form a <=13 character sequence,
    ...
    const refreshSegments = shouldResegment && naturalSegmentTexts.length > 0
      ? naturalSegmentTexts.map((text, index) => {
          ...
          const naturalColumns = characterCount >= 6 && characterCount <= 13
          ...
          const columns = characterCount >= 6 && characterCount <= 13
...
export function bubbleSegmentSpeechBoundaries(utterance, segments) {
  ...
  for (let index = 1; index < rows.length; index += 1) {
    ...
    // Two balloons cannot share an instant: a zero-length segment renders no
    // frame at all and leaves the transition audit with endFrame < startFrame.
    if (rows[index].startSeconds - rows[index - 1].endSeconds < -0.001) return null;
  }
  if (rows.some((row) => row.endSeconds - row.startSeconds < 0.05)) return null;
  return rows;
}
...
// SVG→PNG の描画（ブラウザーの探索・起動・代替経路）は lib/svgRasterizer.mjs に1つだけ置く。
// doctor の svg-rasterizer 検査が、本番と同じ関数で縦書きを描いて読み返すため。
...
function panelSlots(type, width, height, gutter, ratios, value) {
  if (type === "story-3") {
    if (value.layoutVariant === "grid-3") {
      const cellWidth = Math.floor((width - gutter) / 2);
      const cellHeight = Math.floor((height - gutter) / 2);
      return {
        layoutVariant: "grid-3",
        slots: [
          { x: 0, y: 0, width: cellWidth, height: cellHeight },
          { x: cellWidth + gutter, y: 0, width: cellWidth, height: cellHeight },
          { x: Math.floor((width - cellWidth) / 2), y: cellHeight + gutter, width: cellWidth, height: cellHeight },
        ],
      };
    }
...
export async function renderCutInputHash(manifest, cut, utterances) {
  ...
  const sourcePaths = shotSequence.length > 0
    ...
    : [cut.imagePath];
  // A substituted cut's pixels come from the generated clip. Hash its bytes
  // too; still cuts add nothing, so their existing cache keys stay valid.
  const videoSubstitution = appliedCutVideoBinding(cut);
  const inputPaths = [
    ...sourcePaths,
    ...(videoSubstitution ? [videoSubstitution.clipPath] : []),
  ...
  return mangaVideoJobInputHash("render-cut", {
    ...
    cut: {
      ...
      timing: cut.timing,
      videoSubstitution: cutVideoBindingHashInput(videoSubstitution),
    },
...
async function renderCutVideo({ manifest, cut, utterances, workDir, ffmpegPath }) {
  ...
  // Opt-in video substitution replaces only this cut's visual base. The
  // authored camera stays in the manifest as the approved still design (and
  // defines the clip's start frame); bubbles and audio below are unchanged.
  const videoSubstitution = appliedCutVideoBinding(cut);
  if (videoSubstitution) {
    if (panelLayout) throw new Error(`${cut.id}: a split page cannot be replaced by a generated clip.`);
    if (utterances.some((utterance) => utterance.preset === "thought")) {
      throw new Error(`${cut.id}: a thought cut cannot be replaced by a generated clip (its spotlight is baked into the still).`);
    }
    const clipSignature = await fileContentSignature(videoSubstitution.clipPath);
    if (clipSignature.sha256 !== videoSubstitution.clipSha256) {
      throw new Error(`${cut.id}: generated clip bytes changed since they were bound (${clipSignature.sha256} != ${videoSubstitution.clipSha256}).`);
    }
    const requiredFrames = cutFrameCount(duration, fps);
    const clipFrames = Math.floor(finiteNumber(videoSubstitution.clipDurationSeconds, 0) * fps + 1e-6);
    if (clipFrames + videoSubstitution.maximumClipShortfallFrames < requiredFrames) {
      throw new Error(`${cut.id}: generated clip has ${clipFrames} frames but the cut needs ${requiredFrames}; regenerate instead of padding.`);
    }
  }
  ...
  const sourcePaths = videoSubstitution
    ? [videoSubstitution.clipPath]
  ...
  for (const sourcePath of sourcePaths) {
    // A generated clip keeps its own frame clock; only stills get -framerate.
    if (videoSubstitution) args.push("-i", sourcePath);
    else args.push("-framerate", String(fps), "-i", sourcePath);
  }
  ...
  if (videoSubstitution) {
    filters.push(cutVideoClipBaseLayerFilter({
      inputIndex: 0,
      width,
      height,
      fps,
      frameCount,
      shortfallFrames: videoSubstitution.maximumClipShortfallFrames,
    }));
  } else if (shotSequence.length > 0) {
