...
function speakerIdForName(name, registry) {
  ...
  return matched?.id || slug(normalized, "speaker");
}
...
export function parseMangaScript(scriptText, options = {}) {
  ...
  for (const rawLine of markdown.body.split("\n")) {
    ...
    const text = normalizeDialogueText(dialogueMatch[2]);
    if (!text) continue;
    ...
    currentCut.utterances.push({
      ...
      preset: /^(?:ナレーション|ナレーター|地の文)$/u.test(speakerName)
        ? "narration"
        : /！|!$/u.test(text)
          ? "shout"
...
export async function createEpisodeManifest(args = {}) {
  ...
  for (const cut of parsedScript.cuts) {
    ...
    for (const utterance of cut.utterances) {
      const bubble = bubbleForUtterance(template, utterance, cut.utterances);
  ...
  for (const utterance of manifest.utterances || []) {
    ...
    if (reflowPlacement && !panelLayoutEnabled) {
      ...
      const shotSequence = normalizeCameraShotSequence(
        cut,
        cutUtterances,
        finiteNumber(cut.timing?.durationSeconds, 0),
      );
    ...
    const entries = existingSegments.length > 0
    ...
    for (const [entryIndex, entry] of entries.entries()) {
      ...
      const invalidTypography = rendered.quality.find((quality) => (
        quality.overflow || quality.textLoss || quality.tooSmall || quality.exactTextMatch === false
      ));
      if (invalidTypography) {
        throw new Error(`Speech bubble override failed typography gates for ${utterance.id}${entries.length > 1 ? ` segment ${entryIndex + 1}` : ""}.`);
      }
      ...
      renderedEntries.push({ ...entry, bubble, rendered, plannedBubble });
    }
  ...
  for (const cut of manifest.cuts) {
    ...
    for (const [utteranceIndex, utteranceId] of cut.utteranceIds.entries()) {
      ...
      utterance.timing = {
        ...
        bubbleStartInCutSeconds: Math.max(0, cutCursor + speechStart - bubbleLead),
        bubbleEndInCutSeconds: Math.min(cutCursor + duration + bubbleHold, cutCursor + speechEnd + bubbleHold),
        audioStartSeconds: episodeCursor + cutCursor,
        audioEndSeconds: episodeCursor + cutCursor + duration,
        bubbleStartSeconds: episodeCursor + Math.max(0, cutCursor + speechStart - bubbleLead),
        bubbleEndSeconds: episodeCursor + Math.min(cutCursor + duration + bubbleHold, cutCursor + speechEnd + bubbleHold),
      };
    ...
    const durationSeconds = Math.max(1, cutCursor + cutTail);
    cut.timing = {
...
function ffmpegSeconds(value) {
  return Math.max(0, finiteNumber(value, 0)).toFixed(3);
}
...
async function renderCutInputHash(manifest, cut, utterances) {
...
async function renderCutVideo({ manifest, cut, utterances, workDir, ffmpegPath }) {
  ...
  for (const [index, sourcePath] of sourcePaths.entries()) {
    const sourceDuration = shotSequence[index]?.durationSeconds || duration;
    args.push("-loop", "1", "-framerate", String(fps), "-t", ffmpegSeconds(sourceDuration), "-i", sourcePath);
  }
  for (const overlay of visualOverlays) {
    args.push("-loop", "1", "-framerate", String(fps), "-t", ffmpegSeconds(duration), "-i", overlay.path);
  }
  ...
  if (shotSequence.length > 0) {
    ...
    panelLayout.panels.forEach((panel, index) => {
      ...
      filters.push(
        ...
        `scale=${slot.width}:${slot.height}:flags=lanczos,fps=${fps},trim=end_frame=${frameCount},` +
        `setsar=1,setpts=PTS-STARTPTS,format=rgba${alphaFilter}[p${index}]`,
      );
    });
    ...
    filters.push(
      `[0:v]scale=${width}:${height}:force_original_aspect_ratio=increase,crop=${width}:${height},` +
      `eq=saturation=${ffmpegDecimal(camera.saturation)}:contrast=${ffmpegDecimal(camera.contrast)}:brightness=${ffmpegDecimal(camera.brightness)},setsar=1,format=rgba[base0]`,
    );
  } else {
    const progress = cameraProgressExpression(
      frameCount,
  ...
  if (audioLabels.length > 0) {
    filters.push(
      ...
      + `apad=whole_dur=${ffmpegSeconds(duration)},atrim=0:${ffmpegSeconds(duration)}[aout]`,
    );
  } else {
    filters.push(`anullsrc=channel_layout=stereo:sample_rate=48000,atrim=0:${ffmpegSeconds(duration)}[aout]`);
  }
  ...
  args.push(
    ...
    "-t", ffmpegSeconds(duration), "-movflags", "+faststart", outputPath,
  );
...
export async function renderEpisodeVideo(args = {}) {
  ...
  for (const [index, cut] of manifest.cuts.entries()) {
    ...
    const hashCacheHit = existingCut
      && previousJob?.status === "complete"
      && previousJob.inputHash === inputHash;
    const legacyReuse = existingCut
      && !previousJob?.inputHash
      && (reuseRenderedCuts || excludedBySelection);
    // --cut-ids is an explicit targeted-render contract.  A cut outside that
    // selection must keep its existing encoded file even when a manifest-wide
    // policy/version change invalidates its hash; otherwise a one-cut visual
    // correction unexpectedly rebuilds the entire episode.
    const selectionReuse = existingCut && excludedBySelection;
    const canReuse = !forceRender
      && !explicitlySelected
      && (selectionReuse
        || hashCacheHit
        || legacyReuse
        || (reuseRenderedCuts && existingCut && previousJob?.inputHash === inputHash));
    const job = {
      ...(previousJob?.inputHash === inputHash ? previousJob : {}),
  ...
  const concatText = ["ffconcat version 1.0", ...cutFiles.map((filePath) => `file '${filePath.replace(/'/g, "'\\''")}'`)].join("\n");
  ...
  if (bgmPath) {
    ...
    const analysis = await runCommand(ffmpegPath, [
      "-hide_banner", "-f", "concat", "-safe", "0", "-i", concatPath,
      "-map", "0:a", "-af",
      `loudnorm=I=${masterTargetLufs.toFixed(1)}:LRA=${masterLoudnessRange.toFixed(1)}:TP=${masterTruePeakDb.toFixed(1)}:print_format=json`,
      "-f", "null", "-",
    ]);
    ...
    await runCommand(ffmpegPath, [
      ...
      "-af", constantGainFilter(measuredMasterLoudness),
      "-c:a", "aac", "-b:a", "192k", "-ar", "48000", "-ac", "2",
      "-t", ffmpegSeconds(manifest.metrics.videoDurationSeconds), "-movflags", "+faststart", outputPath,
    ]);
