 ...
 */
export function strategyBriefReviewSheet(contract, { briefRel = "", briefSha256 = "", label = "", inspection = [], previousVersion = null, brief = null } = {}) {
 ...
 */
export async function evaluateStrategyBriefGates({ workDir, brief, previousVersion = null, contract = createStrategyBriefQualityContract() }) {
  ...
  return { validation, inspection, gates, failedGateIds };
}
...
export async function recordStrategyBriefRound({
  ...
  return withCanvasFileLock(paths.statePath, async () => {
    ...
    const evaluated = await evaluateStrategyBriefGates({ workDir: paths.workDir, brief, previousVersion, contract });
