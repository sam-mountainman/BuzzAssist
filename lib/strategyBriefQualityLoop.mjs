 ...
 *   - 前提（動画の問い・見る人・制作条件）を前の版から変えたら、その前提で集めた根拠は「当てはまりの確認待ち」。
 *     上位の AI が根拠ごとに今の前提へ当てはまるかを記録する（strategy-brief.mjs applicability）。判定の無い根拠と
 *     当てはまらない根拠が残る版は機械ゲート evidence-premise-current が落ち、verdict は理由コード
 *     （strategy-evidence-applicability-review-required:<id> / strategy-evidence-refresh-required:<id>）を返す。
 *     入口の約束（表現）を変えただけでは古くしない。日数では決めない（問い・対象・条件の変更で決める）
 *   - 前の版で未確認だった事項を確認済みにする・仮説の状態を変えるには、その版で新しく足した根拠が要る
 *     （機械ゲート claims-updated-with-new-evidence）。制作を止める未確認事項が open のまま残るブリーフは、
 *     合格していても verdict が制作へ渡さない（strategy-brief-open-question-blocks-production:<id>）
 *   - 続いているループを採点なしで止められるのは、理由と人の確認つきの stop だけ（中核の stopQualityLoopByHuman。
 *     企画の方向を変えると決めた・評価項目が改定されたときなど）。回も時間も数えず、止めても合格にはならない
 */
...
export const STRATEGY_BRIEF_MACHINE_GATES = Object.freeze({
  ...
  "evidence-premise-current": "前提（動画の問い・見る人・制作条件）を変えた版で、前の前提で集めた根拠のそれぞれに今の前提への当てはまりの判定があり、当てはまらない根拠が残っていない（入口の表現だけの変更では古くしない）",
  "claims-updated-with-new-evidence": "前の版で未確認だった事項を確認済みにした・仮説の状態を変えた版は、その版で新しく足した根拠を指す。未確認事項を黙って消していない",
});
 ...
 */
export function strategyBriefReviewSheet(contract, { briefRel = "", briefSha256 = "", label = "", inspection = [], previousVersion = null, claimBase = previousVersion, brief = null, staleness = null } = {}) {
  const premiseChanged = previousVersion && brief
    ? premiseChangedFields(previousVersion.premise, strategyBriefPremiseDigests(brief))
    : [];
  const entryChanged = Boolean(previousVersion?.entryDigest && brief && previousVersion.entryDigest !== strategyBriefEntryDigest(brief));
  return {
    ...
    entryChangedFromPrevious: entryChanged,
    evidenceStateUpgradesFromPrevious: brief ? upgradesSince(previousVersion, brief) : [],
    evidenceStaleByPremise: (staleness?.stale || []).map((row) => ({ id: row.id, status: row.status, changedFields: row.changedFields })),
    // 前提の変更のあと、今の前提に当てはまると判定して使い続ける根拠（判定の理由を評価者が確かめる）。
    evidenceReusedAfterApplicabilityReview: (staleness?.reused || []).map((row) => ({
      id: row.id,
      changedFields: row.changedFields,
      reason: row.applicability?.reason || "",
    })),
    openQuestions: brief ? openQuestionSummary(brief) : null,
    claimChangesFromPrevious: brief ? claimChangesSince(claimBase, brief) : [],
...
function nextStepDetail(state, paths) {
  ...
  if (state.stopReason === HUMAN_STOP_REASON) {
    return `人が採点なしで止めた（${state.humanStop?.reviewer || "不明"}: ${state.humanStop?.reason || "理由なし"}）。合格ではない。`
      + "続けるなら start --restart --reason \"何が変わったか\" で始め直す";
  }
...
/**
 * 未確認事項・仮説の変更を比べる基準の版。claims-updated-with-new-evidence が落ちた版は基準にしない
 * （落ちた版の「確認済み」を基準にすると、次の版で同じ書き換えを繰り返すだけで変化が消えて通ってしまう）。
 */
export function claimBaseline(versions = []) {
  const list = Array.isArray(versions) ? versions : [];
  for (let index = list.length - 1; index >= 0; index -= 1) {
    if (list[index]?.machineGates?.["claims-updated-with-new-evidence"] !== false) return list[index];
  }
  return null;
}
...
export async function evaluateStrategyBriefGates({ workDir, brief, versions = [], contract = createStrategyBriefQualityContract() }) {
  ...
  const staleness = plainObject(brief)
    ...
    : { stale: [], reviewRequired: [], refresh: [], reused: [], refreshRequired: false, applicabilityReviewRequired: false };
  const previous = brief?.changes?.previous;
  const claimChanges = plainObject(brief) ? claimChangesSince(claimBaseline(versions), brief) : [];
  const gates = {
    ...
    "claims-updated-with-new-evidence": claimChanges.every((row) => row.ok),
  };
  const failedGateIds = contract.machineGates.filter((id) => gates[id] !== true).sort();
  return { validation, inspection, staleness, claimChanges, gates, failedGateIds };
}
...
export async function recordStrategyBriefRound({
  ...
  return withCanvasFileLock(paths.statePath, async () => {
    ...
    const version = {
      ...
      entryDigest: strategyBriefEntryDigest(brief),
      ...
      staleEvidence: evaluated.staleness.stale.map((row) => ({ id: row.id, status: row.status, reasonCode: row.reasonCode, changedFields: row.changedFields })),
      reusedEvidence: evaluated.staleness.reused.map((row) => ({ id: row.id, changedFields: row.changedFields })),
      ...claimRecords(brief),
      claimChanges: evaluated.claimChanges,
    ...
    if (next.status !== "passed" && typeof captureLearning === "function") {
      ...
      try {
        // 作業フォルダとブリーフの channel.id は、学習を積むチャンネルを台帳から引く手がかり（lib/learningChannelResolver.mjs）。
        learning = await captureLearning({
          state: next, round, version, contract, workDir: paths.workDir, briefChannelId: nonEmpty(brief?.channel?.id),
        });
      } catch (error) {
...
/**
 * 続いているループを、人が採点なしで止める（企画の方向を変えると決めた、評価項目が改定された、など）。中核の
 * stopQualityLoopByHuman で blocked（human-stopped）にし、止めた人・理由・時刻を humanStop に残す。回も時間も
 * 数えない。止めた後は start --restart --reason で始め直せる。止めても合格にはならず、verdict は制作へ渡さない
 * （strategy-brief-loop-not-passed:blocked など）。
 * 数えるのは、止めると決めた人の対話端末から --human-verified を付けた記録だけ（判定は scripts/harness-learn.mjs の
 * attestationFor。台本・途中の成果物のループと同じ一つの実装）。--agent-attested は数えず何も変えない。
 */
export async function stopStrategyBriefLoop({
  workDir,
  reviewer = "",
  reason = "",
  humanVerified = false,
  ...
  return withCanvasFileLock(paths.statePath, async () => {
    const existing = await readJsonIfExists(paths.statePath, null);
    if (!existing?.strategy) return { stopped: false, ...waiting(null, ["strategy-brief-loop-not-started"], "先に start でループを始める") };
    if (existing.status !== "active") {
      return {
        stopped: false,
        ...waiting(existing, [`strategy-brief-stop-loop-not-active:${existing.status}`],
    ...
    if (attestation.attestedBy !== HUMAN_STOP_ATTESTATION) {
      return {
        stopped: false,
        counted: false,
        ...waiting(existing, [`strategy-brief-stop-not-counted:${attestation.attestedBy}`],
    ...
    const next = { ...core, strategy: existing.strategy };
    await writeJsonAtomic(paths.statePath, next);
    return {
      stopped: true,
      counted: true,
      ...
      state: next,
      issues: issuesFromState(next),
      detail: `企画の品質ループを採点なしで止めた（${next.rounds.length} 回目まで記録。回・時間は数えていない）。${nextStepDetail(next, paths)}`,
      check: checkFor(next),
    };
  });
}
...
export async function strategyBriefReviewTemplate({ workDir, briefPath } = {}) {
  ...
  const sheet = strategyBriefReviewSheet(state.strategy.contract, {
    ...
    claimBase: claimBaseline(state.strategy.versions),
    brief,
    staleness,
  });
  return {
...
export const STRATEGY_EVIDENCE_APPLICABILITY_VERSION = "buzzassist-strategy-evidence-applicability-v1";

async function readBriefForApplicability(workDir, briefPath) {
  const paths = strategyBriefQualityPaths(workDir);
  if (!nonEmpty(briefPath)) throw new Error("--brief にブリーフのファイルが要ります。");
  const full = resolve(briefPath);
  const bytes = await readFile(full);
  let brief;
  try {
    brief = JSON.parse(bytes.toString("utf8"));
  } catch {
    const error = new Error("ブリーフが JSON として読めない。");
    error.code = "strategy-brief-unreadable";
    throw error;
  }
  const state = await readJsonIfExists(paths.statePath, null);
  const boundPremiseFor = boundPremiseFromVersions(state?.strategy?.versions || []);
  return { paths, full, sha256: sha256Hex(bytes), brief, boundPremiseFor };
}

function applicabilitySummary(staleness) {
  return {
    premiseDigest: staleness.premise.digest,
    reviewRequired: staleness.reviewRequired.map((row) => ({ id: row.id, kind: row.kind, path: row.path, changedFields: row.changedFields, boundBy: row.boundBy })),
    refreshRequired: staleness.refresh.map((row) => ({ id: row.id, kind: row.kind, path: row.path, changedFields: row.changedFields, reason: row.applicability?.reason || "" })),
    reused: staleness.reused.map((row) => ({ id: row.id, kind: row.kind, path: row.path, changedFields: row.changedFields, reason: row.applicability?.reason || "" })),
  };
}

/**
 * 前提（問い・見る人・制作条件）を変えた後に、根拠ごとの当てはまりの判定が要る行の一覧。何も書かない。
 * 集めたときの前提は、行の collected.premise か、作業フォルダの品質ループの記録（最初に現れた版）から引く。
 */
export async function strategyEvidenceApplicabilityStatus({ workDir = "", briefPath } = {}) {
  const root = nonEmpty(workDir) ? resolve(workDir) : resolve(resolve(briefPath || "."), "..");
  const read = await readBriefForApplicability(root, briefPath);
  const staleness = evidencePremiseStaleness(read.brief, { boundPremiseFor: read.boundPremiseFor });
  return {
    version: STRATEGY_EVIDENCE_APPLICABILITY_VERSION,
    modelCallsAttempted: false,
    briefSha256: read.sha256,
    ...applicabilitySummary(staleness),
    detail: staleness.reviewRequired.length > 0
      ? `当てはまりの判定が要る根拠 ${staleness.reviewRequired.length} 件。根拠を開いて、今の前提（問い・見る人・制作条件）に当てはまるかを applicability --evidence <id> --applies yes|no --reason "..." で記録する`
      : staleness.refresh.length > 0
        ? `当てはまらないと判定した根拠 ${staleness.refresh.length} 件は取り直す（新しいファイルと SHA で行を差し替えるか、行を外す）`
        : "当てはまりの判定が要る根拠は無い",
  };
}

/**
 * 1つの根拠に、今の前提へ当てはまるかの判定（applies・理由・判定した文脈）を書く。判定はブリーフの行に入る
 * （ブリーフの SHA が変わるので、判定を含めて品質ループの採点を受け直す）。書く先は outPath（既存のファイルは
 * 上書きしない）か、ブリーフそのもの（原子的に書き換える）。モデルは呼ばない。判定そのものは上位の AI がする。
 */
export async function recordStrategyEvidenceApplicability({
  workDir = "",
  briefPath,
  evidenceId,
  applies,
  ...
  const root = nonEmpty(workDir) ? resolve(workDir) : resolve(resolve(briefPath || "."), "..");
  const read = await readBriefForApplicability(root, briefPath);
  const recorded = recordEvidenceApplicability(read.brief, {
    evidenceId: nonEmpty(evidenceId),
    applies,
    reason,
    decidedBy,
    decidedAt: new Date(now()).toISOString(),
    boundPremiseFor: read.boundPremiseFor,
  });
  ...
  if (nonEmpty(outPath)) {
    ...
    if (exists) {
      const error = new Error(`--out のファイルが既にある（上書きしない）: ${target.split(sep).pop()}`);
      error.code = "strategy-evidence-applicability-out-exists";
      throw error;
    }
  }
  await writeJsonAtomic(target, recorded.brief);
  const bytes = await readFile(target);
  const staleness = evidencePremiseStaleness(recorded.brief, { boundPremiseFor: read.boundPremiseFor });
  return {
    version: STRATEGY_EVIDENCE_APPLICABILITY_VERSION,
    modelCallsAttempted: false,
    recorded: true,
    evidence: recorded.row,
    previousBriefSha256: read.sha256,
    briefSha256: sha256Hex(bytes),
    written: (() => {
      const rel = relative(root, target);
      return rel && !rel.startsWith("..") && !isAbsolute(rel) ? rel.split(sep).join("/") : target.split(sep).pop();
    })(),
    ...applicabilitySummary(staleness),
    detail: `根拠 ${recorded.row.id} を${recorded.row.applies ? "今の前提に当てはまる（再利用する）" : "今の前提に当てはまらない（取り直す）"}と記録した。`
      + "ブリーフの SHA が変わったので、判定を含めて品質ループの採点を受ける",
  };
}

export const STRATEGY_BRIEF_HANDOFF_VERSION = "buzzassist-strategy-brief-handoff-v1";
export const STRATEGY_BRIEF_NOT_A_BRIEF_CODE = "strategy-brief-not-a-brief";

/**
 * 制作へ渡す口（run-video-harness の plan-request / start）が読むブリーフの要約。ブリーフは必須にしない
 * （渡されたときだけ読む。必須にするかはチャンネルの設定で後から選ぶ）。verdict の合否と根拠の状態を返し、
 * 合否で止めない。buzzassist-strategy-brief-v1 のブリーフとして読めないものだけは例外にする。
 */
export async function strategyBriefHandoff({ briefPath, workDir = "" } = {}) {
  const verdict = await strategyBriefVerdict({ workDir, briefPath });
  if (verdict.reasonCodes.includes("strategy-brief-unreadable") || (verdict.validationIssues || []).includes("invalid:version")) {
    const error = new Error(`${STRATEGY_BRIEF_NOT_A_BRIEF_CODE}: --strategy-brief は buzzassist-strategy-brief-v1 のブリーフにする。`);
    error.code = STRATEGY_BRIEF_NOT_A_BRIEF_CODE;
    throw error;
  }
  const counts = verdict.evidence?.counts || { verified: 0, provisional: 0, unverified: 0 };
  const stale = verdict.evidence?.stale?.length || 0;
  const applicability = verdict.evidence?.applicability || { reviewRequired: 0, refreshRequired: 0, reused: 0 };
  const questions = verdict.openQuestions || { total: 0, open: 0, resolved: 0, blocking: [] };
  const label = verdict.label || "?";
  const status = verdict.pass ? "合格" : `未合格（${verdict.reasonCodes.slice(0, 4).join(", ")}${verdict.reasonCodes.length > 4 ? " ほか" : ""}）`;
  return {
    version: STRATEGY_BRIEF_HANDOFF_VERSION,
    provided: true,
    briefSha256: verdict.briefSha256,
    label: verdict.label || null,
    harnessId: verdict.harnessId || null,
    pass: verdict.pass,
    loopStatus: verdict.loop?.status || "not-started",
    evidence: { ...counts, stale },
    applicability: { ...applicability },
    openQuestions: { open: questions.open, blocking: [...questions.blocking] },
    refreshRequired: verdict.refreshRequired === true,
    reasonCodes: [...verdict.reasonCodes],
    summary: `企画ブリーフ（${label}）: ${status}・根拠 verified ${counts.verified} / provisional ${counts.provisional} / unverified ${counts.unverified}`
      + (applicability.reviewRequired > 0 ? `・前提の変更で当てはまりの確認待ちの根拠 ${applicability.reviewRequired} 件` : "")
      + (applicability.refreshRequired > 0 ? `・今の前提に当てはまらず取り直しが要る根拠 ${applicability.refreshRequired} 件` : "")
      + (questions.blocking.length > 0 ? `・制作を止める未確認事項 ${questions.blocking.length} 件` : ""),
  };
}
...
export async function strategyBriefVerdict({ workDir = "", briefPath } = {}) {
  ...
  if (!state?.strategy) reasons.push("strategy-brief-loop-not-started");
    ...
    if (latest) {
      ...
      // 採点した版から、未確認を確認済みにした・仮説の状態を変えた・未確認事項を消した書き換えを名指しする。
      claimChanges = claimChangesSince(claimBaseline(versions), brief);
      for (const row of claimChanges) reasons.push(claimChangeReasonCode(row));
    }
  ...
  if (staleness.applicabilityReviewRequired) reasons.push(APPLICABILITY_REVIEW_REQUIRED_CODE);
  if (staleness.refreshRequired) reasons.push(EVIDENCE_REFRESH_REQUIRED_CODE);
  // 制作を止める未確認事項が open のまま残るブリーフは、品質ループで合格していても制作へ渡さない。
  const questions = openQuestionSummary(brief);
  for (const id of questions.blocking) reasons.push(`${OPEN_QUESTION_BLOCKS_PRODUCTION_CODE}:${id}`);
  ...
  return {
    ...
    evidence: {
      ...
      stale: staleness.stale.map((row) => ({ id: row.id, status: row.status, reasonCode: row.reasonCode, changedFields: row.changedFields })),
      reused: staleness.reused.map((row) => ({ id: row.id, changedFields: row.changedFields, reason: row.applicability?.reason || "" })),
      applicability: {
        reviewRequired: staleness.reviewRequired.length,
        refreshRequired: staleness.refresh.length,
        reused: staleness.reused.length,
      },
      upgradedWithoutReview: upgrades,
    },
    openQuestions: questions,
    claimChangesWithoutReview: claimChanges,
    refreshRequired: staleness.refreshRequired,
    applicabilityReviewRequired: staleness.applicabilityReviewRequired,
    reasonCodes,
    ...
    detail: pass
      ...
      : staleness.applicabilityReviewRequired || staleness.refreshRequired
        ? `前提（${staleFields.join("・") || "問い・見る人・制作条件"}）を変えた。前の前提で集めた根拠のうち、当てはまりの確認待ち ${staleness.reviewRequired.length} 件`
          + `（strategy-brief.mjs applicability で根拠ごとに判定を記録する）・当てはまらず取り直しが要る ${staleness.refresh.length} 件`
        : questions.blocking.length > 0 && reasonCodes.every((code) => code.startsWith(`${OPEN_QUESTION_BLOCKS_PRODUCTION_CODE}:`))
          ? `制作を止める未確認事項 ${questions.blocking.length} 件（${questions.blocking.join(", ")}）が残っている。確かめた根拠を足して resolved にし、品質ループで採点を受ける`
          : `制作へ渡す前に要ること: ${reasonCodes.join(", ")}`,
  };
}

export const OPEN_QUESTION_BLOCKS_PRODUCTION_CODE = "strategy-brief-open-question-blocks-production";

/** 採点した版からの、未確認事項・仮説の書き換えの理由コード。 */
export function claimChangeReasonCode(row) {
  if (row.kind === "hypothesis") {
    return row.ok ? `strategy-brief-hypothesis-changed-without-review:${row.id}` : `strategy-brief-hypothesis-changed-without-new-evidence:${row.id}`;
  }
  if (row.change === "dropped") return `strategy-brief-open-question-dropped-without-review:${row.id}`;
  if (row.change === "unblocked") return `strategy-brief-open-question-unblocked-without-review:${row.id}`;
  return row.ok ? `strategy-brief-open-question-resolved-without-review:${row.id}` : `strategy-brief-open-question-resolved-without-new-evidence:${row.id}`;
}
