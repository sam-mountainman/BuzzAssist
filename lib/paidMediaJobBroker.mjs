...
export function createPaidMediaJobBroker(options = {}) {
  ...
  const apiFetch = typeof options.apiFetch === "function" ? options.apiFetch : buzzAssistFetch;
  ...
  const redactionSecrets = Array.isArray(options.redactionSecrets) ? options.redactionSecrets : [];
  ...
  async function start(spec, callOptions = {}) {
    ...
    return withLock(normalized.requestKey, async () => {
      ...
      if (existing) {
        ...
        const locallyReservedBeforeDispatch = existing.status === "reserved"
          && nonEmptyString(existing.jobId).startsWith("local-")
        ...
        if (!locallyReservedBeforeDispatch) return existing;
        job = existing;
      } else {
        job = await persist({
          version: PAID_MEDIA_JOB_VERSION,
