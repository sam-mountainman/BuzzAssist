...
export function createPaidMediaJobBroker(options = {}) {
  ...
  // 再利用だけの実行（lib/paidCallGuard.mjs）。新しく送る直前に見る。既存の記録へ接続するだけなら見ない。
  const guardEnv = options.env && typeof options.env === "object" ? options.env : process.env;
  ...
  async function acquire(requestKey) {
    ...
    for (;;) {
      try {
        ...
        // Windows では、別のプロセスが消したばかりの錠のディレクトリ（削除待ち）へ mkdir すると
        // EEXIST ではなく EPERM / EACCES が返る（CI の windows-latest で観測、2026-09-25）。
        // 一時的なものなので、同じ待ち時間の上限の中で待って取り直す。
        if (process.platform === "win32" && (error?.code === "EPERM" || error?.code === "EACCES")) {
          if (Date.now() - started > lockTimeoutMs) throw error;
  ...
  function guardNewDispatch(normalized) {
    assertPaidCallAllowed({
      route: "paid-media-broker",
      kind: normalized.kind,
      provider: normalized.provider,
      model: normalized.model,
      requestKey: normalized.requestKey,
    }, { env: guardEnv, now: isoNow });
  }
