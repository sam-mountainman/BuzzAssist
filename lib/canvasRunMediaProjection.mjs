...
// 途中の成果物の投影（lib/canvasRunProgressProjection.mjs）は、原本を読んで SHA を確かめ、
// content-addressed に複製する部分をここと共有する（2つ目の複製器を作らない）。
// verifyCanvasRunMediaAsset の第2引数は runId を持つ物であればよい。
export {
  verifiedAsset as verifyCanvasRunMediaAsset,
  assetUrlFor as canvasRunMediaAssetUrl,
  previewDataUrl as canvasRunMediaPreviewDataUrl,
  mediaBaseElement as canvasRunMediaBaseElement,
};
