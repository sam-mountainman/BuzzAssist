...
const LOVART_MODE_QUERY_PATH = `${LOVART_PATH_PREFIX}/mode/query`;
const LOVART_MODE_SET_PATH = `${LOVART_PATH_PREFIX}/mode/set`;
...
let lovartGenerationModeQueue = Promise.resolve();
...
function normalizeLovartGenerationPreference(value, fallback = "fast") {
  return value === "unlimited" || value === "fast" ? value : fallback;
}

function normalizeUnlimitedModelList(value) {
  if (!Array.isArray(value)) return [];
  return value
    .map((entry) => {
      if (typeof entry === "string") return entry.trim();
      if (!entry || typeof entry !== "object") return "";
      return nonEmptyString(entry.name ?? entry.tool ?? entry.model ?? entry.model_name);
    })
    .filter(Boolean);
}

export function normalizeLovartGenerationMode(detail = {}, preference) {
  const source = detail?.detail && typeof detail.detail === "object" ? detail.detail : detail;
  const activeMode = source?.unlimited === true ? "unlimited" : "fast";
  const unlimitedEnabled =
    typeof source?.unlimited_enable === "boolean" ? source.unlimited_enable : null;
  return {
    mode: activeMode,
    preference: normalizeLovartGenerationPreference(preference, "unlimited"),
    unlimitedEnabled,
    unlimitedModels: normalizeUnlimitedModelList(source?.unlimited_list),
  };
}

export function getLovartUnlimitedEligibility(modeStatus, model) {
  if (!modeStatus || modeStatus.unlimitedEnabled === false) return "unavailable";
  const tools = Array.isArray(modeStatus.unlimitedModels) ? modeStatus.unlimitedModels : [];
  if (tools.length === 0) return "unknown";
  const tool = lovartToolForModel(model);
  return tool && tools.includes(tool) ? "available" : "unavailable";
}
...
export async function resolveLovartCredentials() {
...
async function lovartRequest(
  method,
  path,
  { body, params, timeoutMs = 60_000, retries, credentials } = {},
) {
  const { accessKey, secretKey } = credentials ?? (await resolveLovartCredentials());
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorCode = Number(payload.code) || 0;
      ...
      if (payload && typeof payload === "object" && (payload.code ?? 0) !== 0) {
        const requestError = new Error(payload.message || `Lovart API error code ${payload.code}`);
        requestError.code = Number(payload.code) || 0;
        throw requestError;
      }
...
async function writeLovartGenerationPreference(preference) {
  const state = await readLovartState();
  await writeLovartState({
    ...state,
    generation_policy: normalizeLovartGenerationPreference(preference),
  });
}

async function requestLovartGenerationMode(unlimited, { credentials } = {}) {
  return lovartRequest("POST", LOVART_MODE_SET_PATH, {
    body: { unlimited: unlimited === true },
    credentials,
  });
}

async function restoreLovartUnlimitedPreference() {
  const state = await readLovartState();
  if (normalizeLovartGenerationPreference(state?.generation_policy) !== "unlimited") return;
  await requestLovartGenerationMode(true);
}

export async function queryLovartGenerationMode({ credentials, preference } = {}) {
  const detail = await lovartRequest("POST", LOVART_MODE_QUERY_PATH, {
    body: {},
    retries: 3,
    credentials,
  });
  const state = preference ? null : await readLovartState();
  return normalizeLovartGenerationMode(
    detail,
    preference ?? state?.generation_policy,
  );
}

export async function setLovartGenerationPreference(preference) {
  const normalized = normalizeLovartGenerationPreference(preference, "");
  if (!normalized) throw new Error("Lovart generation preference must be fast or unlimited.");

  const current = await queryLovartGenerationMode();
  if (normalized === "fast") {
    const detail = current.mode === "fast"
      ? { unlimited: false, unlimited_enable: current.unlimitedEnabled, unlimited_list: current.unlimitedModels }
      : await requestLovartGenerationMode(false);
    await writeLovartGenerationPreference("fast");
    return normalizeLovartGenerationMode(detail, "fast");
  }

  // The preference remains "unlimited" even when the current plan cannot
  // enable it. Generation then falls back to fast automatically, and the next
  // request re-checks eligibility so a later plan upgrade works immediately.
  if (current.unlimitedEnabled === false) {
    if (current.mode !== "fast") await requestLovartGenerationMode(false);
    await writeLovartGenerationPreference("unlimited");
    return { ...current, mode: "fast", preference: "unlimited" };
  }

  try {
    const detail = current.mode === "unlimited"
      ? { unlimited: true, unlimited_enable: current.unlimitedEnabled, unlimited_list: current.unlimitedModels }
      : await requestLovartGenerationMode(true);
    await writeLovartGenerationPreference("unlimited");
    return normalizeLovartGenerationMode(detail, "unlimited");
  } catch (error) {
    // A plan can lose Unlimited access between query and set. Keep the user's
    // Unlimited-first policy, but leave Lovart in Fast mode so generation is
    // still usable. The next request queries the account again.
    if (!shouldRetryLovartInFastMode(error)) throw error;
    await requestLovartGenerationMode(false).catch(() => {});
    await writeLovartGenerationPreference("unlimited");
    return {
      ...current,
      mode: "fast",
...
function shouldRetryLovartInFastMode(error) {
  return (
    error?.status === 402 ||
    error?.code === 2012 ||
    /unlimited|relax|無制限|not available in this mode|not supported in this mode/i.test(String(error?.message || ""))
  );
}

function runLovartGenerationModeExclusive(task) {
  const previous = lovartGenerationModeQueue;
  let releaseQueue;
  lovartGenerationModeQueue = new Promise((resolve) => {
...
async function generateLovartMediaOnce(input, kind, { onSubmitted } = {}) {
  ...
  for (const path of Array.isArray(input.referenceImagePaths) ? input.referenceImagePaths : []) {
    await uploadWithRole(path, "a binding reference image — preserve its character/subject identity and recognizable appearance, or match its style when it is a style reference; do not ignore it");
  }
  if (kind === "video") {
  ...
  if (typeof onSubmitted === "function") await onSubmitted({ threadId, projectId, model });
...
async function generateLovartMedia(input, kind) {
  return runLovartGenerationModeExclusive(async (releaseMode) => {
    const generateAndRelease = (afterSubmit) => generateLovartMediaOnce(input, kind, {
      onSubmitted: async (submitted) => {
        if (typeof afterSubmit === "function") await afterSubmit(submitted);
        releaseMode();
      },
    });

    const generateTemporarilyInFastMode = async ({ restoreUnlimited }) => {
      let restored = !restoreUnlimited;
      const restore = async () => {
        if (restored) return;
        restored = true;
        await restoreLovartUnlimitedPreference().catch(() => {});
      };
      try {
        return await generateAndRelease(restore);
      } finally {
        // Submission errors happen before onSubmitted. Restore the account in
        // that case too, and never leave another queued generation blocked.
        await restore();
        releaseMode();
      }
    };
    ...
    let modeStatus;
    try {
      modeStatus = await queryLovartGenerationMode();
    } catch {
      // Mode discovery must never make an otherwise valid Lovart generation
      // unavailable during a transient mode endpoint outage.
      return generateAndRelease();
    }
    const preference = modeStatus.preference;

    if (preference === "fast") {
      if (modeStatus.mode !== "fast") await requestLovartGenerationMode(false);
      return generateAndRelease();
    }

    const eligibility = getLovartUnlimitedEligibility(modeStatus, input.model);
    const shouldUseFastImmediately =
      modeStatus.unlimitedEnabled === false || eligibility === "unavailable";

    if (shouldUseFastImmediately) {
      if (modeStatus.mode !== "fast") await requestLovartGenerationMode(false);
      return generateTemporarilyInFastMode({
        restoreUnlimited: modeStatus.unlimitedEnabled !== false,
      });
    }

    try {
      if (modeStatus.mode !== "unlimited") await requestLovartGenerationMode(true);
      return await generateAndRelease();
    } catch (error) {
      // Some accounts return an empty unlimited_list, so model eligibility can
      // only be learned by attempting the generation. Retry exactly once in
      // fast mode for Lovart's mode/quota rejection, then restore Unlimited.
      if (!shouldRetryLovartInFastMode(error)) throw error;
      // The first request may already have been submitted and released the
      // lock. Re-enter the queue before changing the account-wide mode.
      releaseMode();
      return runLovartGenerationModeExclusive(async (releaseFallback) => {
        await requestLovartGenerationMode(false);
        let restored = false;
        const restore = async () => {
          if (restored) return;
        ...
        try {
          return await generateLovartMediaOnce(input, kind, {
            onSubmitted: async () => {
              await restore();
              releaseFallback();
            },
...
export async function saveLovartCredentials({ accessKey, secretKey }) {
  ...
  // Validate against a read-only authenticated endpoint before replacing an
  // existing working key. This also gives the UI the current generation mode.
  const modeStatus = await queryLovartGenerationMode({ credentials });
  ...
  return { ...(await getLovartAuthStatus()), modeStatus };
}
