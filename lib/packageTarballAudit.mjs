...
export const PACKAGE_RUNTIME_REQUIRED_PATHS = Object.freeze([
  // 自動更新（scripts/update-current.mjs）と 1 行導入は、この tgz だけから setup-agents を走らせる。
  // npm pack は package-lock.json を入れないので prepack が写した lockfile を、Canvas は src/ を
  // 配らないのでビルド済みのものを必須にする。setup-agents が配布物の検査で要るものも並べる。
  ...
  // 導入後の検証が依存の無い置き場を確かめるのに使う（古い update-current からも呼ばれる）。
  ...
  // 能力カード。plan-request / plan_video_request がハーネスを選ぶ材料として読む。
  "config/harnesses/koya-manga-video.capabilities.json",
  "config/harnesses/narrated-story-video.capabilities.json",
  "lib/harnessCapabilities.mjs",
  "lib/videoRequestPlan.mjs",
  ...
  // harness-feedback-ingest.mjs が読む（自動の bundle v3 の検証と、受け取り口の書き出しの取り込み）。
  "lib/harnessFeedbackAutoBundle.mjs",
  "lib/harnessFeedbackExportImport.mjs",
  ...
  "lib/narratedStoryQualityLoop.mjs",
