...
const DEFAULT_PROFILE_ID = PROFILE_CATALOG.defaultProfileId || "reference-video-locked-v2";
...
function indexedBubbleText(bubble) {
  const rawColumns = Array.isArray(bubble?.columns)
    ? bubble.columns
    : String(bubble?.text ?? "").split(/\r?\n/);
  const hasExplicitColumns = rawColumns.length > 1;
  let index = 0;
...
function naturalBubbleSize({ bubble, width, height, profile }) {
  ...
  const perColumn = explicitColumns
    ? Math.max(...explicitColumns.map((column) => column.length), 1)
    : Math.ceil(characters.length / estimatedColumns);
  const fontSize = clamp(height * profile.fontSizeRatio, 30, 68);
  const minimumWidth = width * profile.minimumBubbleWidthRatio;
  const maximumWidth = width * profile.maximumBubbleWidthRatio;
  ...
  const bubbleHeight = clamp(
    ...
    height * profile.minimumBubbleHeightRatio,
    height * profile.maximumBubbleHeightRatio,
  );
...
export function planSpeechBubbleLayout({ width, height, bubbles = [], avoidRegions = [], profileId = DEFAULT_PROFILE_ID }) {
  ...
  for (const [index, bubble] of bubbles.entries()) {
    const characters = indexedBubbleText(bubble).characters;
...
function fitTypography(bubble, frameHeight, profile) {
  ...
  const configuredMaxColumns = clamp(
    Math.max(explicitColumns?.length || 1, Math.round(finiteNumber(bubble.maxColumns, profile.maxColumns))),
    1,
    DEFAULT_MAX_COLUMNS,
  );
  ...
  const columns = explicitColumns
    ? explicitColumns.slice(0, columnCount)
    : canSplitAtEmphasis
