...
// Latin letters and digits stay upright in vertical Japanese composition:
// 「T大」 reads as an upright T stacked above 大, never as a T lying on its
// side. Hiragino's 'vrt2' feature (vertical alternates *and rotation*) rotates
// every rotatable glyph, Latin included, so the feature has to be withheld
// from these characters instead of applied blanket-wide to the column.
const UPRIGHT_VERTICAL_CHARACTER = /^[0-9A-Za-z０-９Ａ-Ｚａ-ｚ]$/u;

export function requiresUprightVerticalForm(char) {
  return UPRIGHT_VERTICAL_CHARACTER.test(String(char ?? ""));
}

// Rotating features are safe for kana/kanji/punctuation, which is where the
// vertical alternates actually live.
export const VERTICAL_ROTATING_FEATURES = "font-synthesis:none;font-kerning:none;font-feature-settings:'vert' 1,'vrt2' 1,'palt' 0";
const VERTICAL_UPRIGHT_FEATURES = "font-synthesis:none;font-kerning:none;font-feature-settings:'palt' 0";
...
function renderBubbleGroup(bubble, frame) {
  ...
  const verticalGlyphPosition = (entry, columnX, cellY, size) => {
    ...
    if (requiresUprightVerticalForm(entry.char)) {
      return { x: columnX, y: cellY, kind: "upright-latin" };
    }
  ...
  const text = typography.columns.map((column, index) => {
    ...
    return column.map((entry, characterIndex) => {
      ...
      const glyphStyle = glyph.kind === "upright-latin"
        ? VERTICAL_UPRIGHT_FEATURES
        : VERTICAL_ROTATING_FEATURES;
      return `<text x="${glyph.x.toFixed(2)}" y="${glyph.y.toFixed(2)}" fill="${emphasized ? accent : ink}" data-layout="explicit-vertical-glyph" data-glyph-kind="${glyph.kind}" xml:lang="ja" text-anchor="middle" dominant-baseline="central" alignment-baseline="central" font-family="${emphasized ? emphasisFontFamily : fontFamily}" font-size="${typography.fontSize.toFixed(2)}" font-weight="${emphasized ? emphasisFontWeight : fontWeight}" style="${glyphStyle}">${escapeXml(entry.char)}</text>`;
    }).join("");
  }).join("");
