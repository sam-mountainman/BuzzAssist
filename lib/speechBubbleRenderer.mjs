...
export function planSpeechBubbleLayout({ width, height, bubbles = [], avoidRegions = [], profileId = DEFAULT_PROFILE_ID }) {
  ...
  for (const [index, bubble] of bubbles.entries()) {
    ...
    if ((indexedText.explicitColumns?.length ?? 0) > DEFAULT_MAX_COLUMNS) {
      throw new Error(`Speech bubble ${index + 1} has more than ${DEFAULT_MAX_COLUMNS} explicit columns. Split it into another bubble so no dialogue is lost.`);
    }
...
function shoutBubblePath(bounds, tail) {
  const points = [];
  // The reference videos use eight broad, readable spikes rather than a busy
  // comic-book explosion. Sixteen alternating vertices produces eight tips.
  const count = 16;
  for (let index = 0; index < count; index += 1) {
    const angle = -Math.PI / 2 + (Math.PI * 2 * index) / count;
