...
const DEFAULT_PROFILE_ID = PROFILE_CATALOG.defaultProfileId || "reference-video-v1";
...
function naturalBubbleSize({ textLength, width, height }) {
  const estimatedColumns = clamp(Math.ceil(textLength / 9), 1, DEFAULT_MAX_COLUMNS);
  const bubbleWidth = clamp(width * (0.11 + estimatedColumns * 0.045), width * 0.18, width * 0.36);
  const perColumn = Math.ceil(textLength / estimatedColumns);
  const bubbleHeight = clamp(height * (0.25 + perColumn * 0.038), height * 0.36, height * 0.76);
  return { width: bubbleWidth, height: bubbleHeight };
}

function candidateBounds({ width, height, bubbleWidth, bubbleHeight, target }) {
  ...
  const sameSideRight = target.x >= width / 2;
  const sideCandidates = sameSideRight
    ? [[rightX, topY], [rightX, middleY], [rightX, lowerY], [leftX, topY], [centerX, topY]]
    : [[leftX, topY], [leftX, middleY], [leftX, lowerY], [rightX, topY], [centerX, topY]];
...
function scoreCandidate(candidate, { target, width, height, avoidRegions, preferredSide }) {
  ...
  for (const region of avoidRegions) {
    ...
    if (overlap > 0) score += (overlap / area) * region.weight;
  }
  ...
  if (preferredSide && candidateSide !== preferredSide) score += 18;
  return score;
}
...
export function planSpeechBubbleLayout({ width, height, bubbles = [], avoidRegions = [], profileId = DEFAULT_PROFILE_ID }) {
  ...
  const normalizedAvoidRegions = normalizeAvoidRegions(avoidRegions, frameWidth, frameHeight);
  const occupied = [];
  const planned = [];

  for (const [index, bubble] of bubbles.entries()) {
    const characters = graphemes(bubble.text);
    ...
    const preferredSide = bubble.side === "left" || bubble.side === "right"
      ? bubble.side
      : (target.x >= frameWidth / 2 ? "right" : "left");
    const naturalSize = naturalBubbleSize({ textLength: characters.length, width: frameWidth, height: frameHeight });
    const candidates = explicitBounds
      ? [explicitBounds]
      : candidateBounds({ width: frameWidth, height: frameHeight, bubbleWidth: naturalSize.width, bubbleHeight: naturalSize.height, target });
...
function balancedColumns(characters, count) {
  const columns = [];
  let cursor = 0;
...
function fitTypography(bubble, frameHeight, profile) {
  const characters = graphemes(bubble.text).map((char, index) => ({ char, index }));
  ...
  let fontSize = finiteNumber(bubble.fontSize, clamp(frameHeight * profile.fontSizeRatio, 38, 68));
  const configuredMaxColumns = clamp(Math.round(finiteNumber(bubble.maxColumns, profile.maxColumns)), 1, DEFAULT_MAX_COLUMNS);
  let columnCount = configuredMaxColumns;
  let usableHeight = 0;
  let usableWidth = 0;
  ...
  usableWidth = Math.max(fontSize * 1.4, bubble.bounds.width - horizontalPadding * 2);
  maxColumnsByWidth = clamp(Math.floor(usableWidth / (fontSize * profile.lineAdvance)), 1, configuredMaxColumns);
  const neededColumns = Math.ceil(characters.length / Math.max(1, Math.floor(usableHeight / (fontSize * profile.characterAdvance))));
  columnCount = clamp(neededColumns, 1, Math.min(configuredMaxColumns, maxColumnsByWidth));
  ...
  const canSplitAtEmphasis = columnCount === 2
    && firstAccentIndex > 0
  ...
  const columns = canSplitAtEmphasis
    ? [characters.slice(0, firstAccentIndex), characters.slice(firstAccentIndex)]
    : balancedColumns(characters, columnCount);
  ...
  return {
    ...
    textWidth,
    textHeight,
    tooSmall: fontSize < minimumFontSize,
...
function closedSmoothPath(points, tension = 0.18) {
...
function thoughtBubblePath(bounds) {
  const points = [];
  const count = 24;
  for (let index = 0; index < count; index += 1) {
    const angle = -Math.PI / 2 + (Math.PI * 2 * index) / count;
    const scale = index % 2 === 0 ? 1.025 : 0.925;
    points.push(ellipsePoint(bounds, angle, scale));
  }
  return closedSmoothPath(points, 0.16);
}

function shoutBubblePath(bounds, tail) {
  const points = [];
  const count = 32;
  for (let index = 0; index < count; index += 1) {
    const angle = -Math.PI / 2 + (Math.PI * 2 * index) / count;
    const scale = index % 2 === 0 ? 1.0 : 0.865;
    let point = ellipsePoint(bounds, angle, scale);
    if (tail && index % 2 === 0) {
      const distance = Math.abs(Math.atan2(Math.sin(angle - tail.attachmentAngle), Math.cos(angle - tail.attachmentAngle)));
      if (distance < Math.PI * 2 / count) point = tail.tip;
    }
    points.push(point);
  }
...
function renderBubbleGroup(bubble, frame) {
  ...
  const profileWeight = bubble.preset === "shout" ? profile.shoutFontWeight : profile.fontWeight;
  const fontWeight = clamp(Math.round(finiteNumber(bubble.fontWeight, profileWeight)), 400, 900);
  ...
  const text = typography.columns.map((column, index) => {
    ...
    return `<text x="${x.toFixed(2)}" y="${startY.toFixed(2)}" writing-mode="vertical-rl" text-orientation="mixed" dominant-baseline="central" font-family="${fontFamily}" font-size="${typography.fontSize.toFixed(2)}" font-weight="${fontWeight}" letter-spacing="0.01em" style="font-kerning:normal;font-feature-settings:'vert' 1,'vrt2' 1,'palt' 0">${runs}</text>`;
  }).join("");
  ...
  if (bubble.preset === "narration") {
    ...
    bodyPath = thoughtBubblePath(bubble.bounds);
    if (tail) {
      const beadA = {
        x: tail.attachment.x + tail.unit.x * tail.length * 0.42,
        y: tail.attachment.y + tail.unit.y * tail.length * 0.42,
      };
      const beadB = {
        x: tail.attachment.x + tail.unit.x * tail.length * 0.78,
        y: tail.attachment.y + tail.unit.y * tail.length * 0.78,
      };
      decorations = `<circle cx="${beadA.x.toFixed(2)}" cy="${beadA.y.toFixed(2)}" r="${Math.max(strokeWidth * 1.45, tail.baseHalf * 0.70).toFixed(2)}" fill="${fill}" stroke="${ink}" stroke-width="${strokeWidth.toFixed(2)}"/><circle cx="${beadB.x.toFixed(2)}" cy="${beadB.y.toFixed(2)}" r="${Math.max(strokeWidth, tail.baseHalf * 0.42).toFixed(2)}" fill="${fill}" stroke="${ink}" stroke-width="${strokeWidth.toFixed(2)}"/>`;
    }
  } else if (bubble.preset === "shout") {
    bodyPath = shoutBubblePath(bubble.bounds, tail);
  }
  ...
  return {
    svg: `<g id="${escapeXml(bubble.id)}" data-preset="${escapeXml(bubble.preset)}" data-profile="${escapeXml(profile.id)}" data-tail="${tail ? "integrated" : "none"}">${body}${text}</g>`,
    typography,
    tail,
    strokeWidth,
    profile,
