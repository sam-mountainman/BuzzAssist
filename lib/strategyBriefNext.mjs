...
export async function draftNextStrategyBrief({
  ...
  const briefDraft = {
    ...
    // 観測・仮説・未確認事項は引き継ぐ（未確認を確認済みにするのは、新しい根拠を足したときだけ）。
    ...Object.fromEntries(["observations", "hypotheses", "openQuestions"]
      .filter((key) => Array.isArray(prev[key]))
      .map((key) => [key, structuredClone(prev[key])])),
  };
