 ...
 *
 * `includeFixture: false` は合成 fixture を候補から外す。承認記録のように
 * 「実データでなければ意味が無い」ものを読む側が使う（fixture を正本として
 * 読むと、合成データに対して記録を取ってしまう）。
 */
export function resolveChannelPackSource(projectDir, relativePath, packId = "", { includeFixture = true } = {}) {
  for (const entry of channelPackRootEntries(projectDir, packId)) {
    if (!includeFixture && entry.kind === "fixture") continue;
