...
import { normalizeSideLockedFeatures } from "./characterIdentityReview.mjs";
...
// 衣装シートが覆う場面タグ（wardrobe-readiness）。台帳に残さないと、
// 次の話で同じ服を「持っていない」ことにされて作り直しになる。
const MAX_SCENE_TAGS_PER_REFERENCE_ASSET = 12;
...
// Reference asset `storyStage` is the outfit stage for role "outfit" and the
// variant id for role "eye-open" (see eyeOpenVariantId below). An outfit asset
// may also carry `sceneTags` — the scene kinds that outfit covers
// (wardrobe-readiness). The key is kept only when it has entries, so assets
// registered before the gate normalize byte-identically.
...
/**
 * 背景ボードを「記録が無い」と宣言したまま登録したときの申告。
 * 承認の中に board id・理由・満たすべき文書の SHA-256 と、どの記録が
 * 無かったのかを示す旗を残す。旗まで残すのは、引き渡し先で
 * 「会話は残っていた」と名乗る attestation を突き合わせで落とすため。
 * 1件も無いときはキー自体を作らない。
 * （lib/koyaChannelGovernance.mjs の KOYA_LOCATION_PROVENANCE_GAP_FLAGS と
 *  同じ並び。あちらがこの台帳を読むので、輸入すると循環になる。）
 */
const PROVENANCE_GAP_FLAG_SPELLINGS = Object.freeze([
  ["promptRecorded", "prompt_recorded"],
  ["generatorContextRecorded", "generator_context_recorded"],
  ["referenceImagesRecorded", "reference_images_recorded"],
]);

function normalizedProvenanceGaps(value) {
  const gaps = (Array.isArray(value) ? value : [])
    .map((entry) => ({
      boardId: nonEmptyString(plainObject(entry)?.boardId ?? plainObject(entry)?.board_id),
      reason: nonEmptyString(plainObject(entry)?.reason),
      specificationSha256: nonEmptyString(plainObject(entry)?.specificationSha256 ?? plainObject(entry)?.specification_sha256),
      ...Object.fromEntries(PROVENANCE_GAP_FLAG_SPELLINGS
        .filter(([camel, snake]) => (plainObject(entry)?.[camel] ?? plainObject(entry)?.[snake]) === false)
        .map(([camel]) => [camel, false])),
    }))
    .filter((entry) => entry.boardId && entry.reason && /^[a-f0-9]{64}$/u.test(entry.specificationSha256));
  return gaps.length > 0 ? { provenanceGaps: gaps } : {};
}

/**
 * 登録の時点で品質ループ（lib/assetQualityLoop.mjs）の合格を確かめた記録（契約 koya-manga-production-v54 から）。
 * 工程・対象 id・合格した版の SHA-256 だけを残す。形が崩れた行は捨て、1件も無ければキーを作らない
 * （ゲートより前の登録の見た目は従来のまま）。
 */
const ASSET_QUALITY_STAGES = new Set(["character", "location", "scene-image", "thumbnail", "voice-take"]);
function normalizedAssetQualityApproval(value) {
  const source = plainObject(value);
  if (!source) return {};
  const rows = (Array.isArray(source.rows) ? source.rows : [])
    .map((row) => ({
      stage: nonEmptyString(plainObject(row)?.stage),
      subjectId: nonEmptyString(plainObject(row)?.subjectId),
      assetSha256: nonEmptyString(plainObject(row)?.assetSha256),
    }))
    .filter((row) => ASSET_QUALITY_STAGES.has(row.stage)
      && /^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(row.subjectId)
      && /^[a-f0-9]{64}$/u.test(row.assetSha256));
  const version = nonEmptyString(source.version);
  const contractVersion = nonEmptyString(source.contractVersion);
  if (!version || !contractVersion || rows.length === 0) return {};
  return { assetQuality: { version, contractVersion, rows } };
}
...
function normalizeReferenceAsset(entry, index) {
  ...
  if (!path) return null;
  const sceneTags = normalizedStringList(entry.sceneTags ?? entry.scene_tags, MAX_SCENE_TAGS_PER_REFERENCE_ASSET);
  return {
...
function normalizeCharacter(entry, index) {
  ...
  return {
    ...
    // 左右が固定された特徴の宣言は台帳に残す。登録の時点で落としてしまうと、
    // 次の話数や修復で同じキャラを読み直したときに宣言が消え、左右検査が
    // 黙って無効になる（それが今日4人を鏡像のまま通した経路のひとつ）。
    // 壊れた宣言は握り潰さず例外にする。
    sideLockedFeatures: normalizeSideLockedFeatures(entry.sideLockedFeatures ?? entry.side_locked_features),
    ...
    approval: plainObject(entry.approval) ? {
      ...
      // 出所が欠けたまま登録した背景ボードの申告。空のときは書かないので、
      // 欠落の無い登録の見た目は従来のままになる。
      ...normalizedProvenanceGaps(entry.approval.provenanceGaps ?? entry.approval.provenance_gaps),
      ...
      ...normalizedAssetQualityApproval(entry.approval.assetQuality),
    } : null,
...
export function isComicalReferenceIntent(value) {
  const intent = nonEmptyString(value);
  if (/^comical/iu.test(intent) && !["comical-A", "comical-B"].includes(intent)) {
    throw new Error(`Invalid comical reference intent '${intent}'; use comical-A or comical-B explicitly.`);
  }
  return intent === "comical-A" || intent === "comical-B";
}

function verifiedReferenceBytes(asset, label) {
  if (!nonEmptyString(asset?.path) || !/^[a-f0-9]{64}$/iu.test(asset?.sha256 || "")) {
    throw new Error(`${label} requires a path and SHA-256.`);
  }
  let bytes;
  try { bytes = readFileSync(asset.path); } catch (error) {
    throw new Error(`${label} is missing or unreadable: ${asset.path}`, { cause: error });
  }
  if (createHash("sha256").update(bytes).digest("hex") !== asset.sha256.toLowerCase()) {
    throw new Error(`${label} has stale SHA-256: ${asset.path}`);
  }
}

// Recheck persisted plans before reuse and immediately before provider dispatch.
// This is supplemental routing provenance, never a registry approval operation.
export function verifyComicalReferenceBindings(bindings = []) {
  if (!bindings.length) throw new Error("Comical routing requires approved character bindings.");
  for (const binding of bindings) {
    const source = binding.productionReferenceIndex;
    if (!source || !isComicalReferenceIntent(binding.referenceIntent)) {
      throw new Error(`Character ${binding.id} has no comical routing provenance.`);
    }
    verifiedReferenceBytes(source, "Production reference index");
    const assets = binding.referenceAssets || [];
    if (binding.status !== "approved" || assets.length !== 2
      || assets[0].role !== "identity-face" || assets[1].role !== binding.referenceIntent
      || JSON.stringify(binding.referenceImagePaths) !== JSON.stringify(assets.map((asset) => asset.path))) {
      throw new Error(`Character ${binding.id} requires approved identity plus explicit comical art.`);
    }
    for (const asset of assets) verifiedReferenceBytes(asset, `${binding.id} ${asset.role}`);
  }
  return bindings;
}

function routeComicalBindings(bindings, options, intent) {
  const requestedPath = nonEmptyString(options.productionReferenceIndexPath);
  if (!requestedPath) throw new Error(`${intent} requires productionReferenceIndexPath; no supplemental fallback is allowed.`);
  if (!bindings.length) throw new Error(`${intent} requires approved character bindings.`);
  const indexPath = resolve(options.projectDir || process.cwd(), requestedPath);
  let bytes, index;
  try {
    bytes = readFileSync(indexPath);
    index = JSON.parse(bytes.toString("utf8"));
  } catch (error) {
    throw new Error(`Production reference index is missing or invalid: ${indexPath}`, { cause: error });
  }
  if (!Array.isArray(index?.characters)) throw new Error("Production reference index requires characters.");
  const indexSource = { path: indexPath, sha256: createHash("sha256").update(bytes).digest("hex") };
  const routed = bindings.map((binding) => {
    const entries = index.characters.filter((entry) => entry?.id === binding.id);
    if (binding.status !== "approved" || entries.length !== 1 || entries[0].status !== "approved") {
      throw new Error(`Production reference index requires exactly one approved entry for ${binding.id}.`);
    }
    const refs = entries[0].references;
    const one = (role) => {
      const matches = Array.isArray(refs) ? refs.filter((asset) => asset?.role === role) : [];
      if (matches.length !== 1 || !nonEmptyString(matches[0].path)) {
        throw new Error(`Production reference index requires exactly one ${role} for ${binding.id}.`);
      }
      return { ...matches[0], path: resolve(dirname(indexPath), matches[0].path) };
    };
    const indexedIdentity = one("identity-face");
    const identities = (binding.referenceAssets || []).filter((asset) => asset.role === "identity-face");
    const identity = identities[0];
    if (identities.length !== 1 || identity.path !== indexedIdentity.path
      || !identity.sha256 || identity.sha256.toLowerCase() !== indexedIdentity.sha256?.toLowerCase()) {
      throw new Error(`Production reference index has stale approved identity for ${binding.id}.`);
    }
    const supplemental = one(intent);
    if (supplemental.use !== "supplemental-expression-reference" || supplemental.path === identity.path) {
      throw new Error(`${binding.id} ${intent} must be supplemental-expression-reference art distinct from identity.`);
    }
    return {
      ...binding,
      referenceIntent: intent,
      productionReferenceIndex: indexSource,
      referenceAssets: [identity, supplemental],
      referenceImagePaths: [identity.path, supplemental.path],
    };
  });
  return verifyComicalReferenceBindings(routed);
}

// A character may carry several approved eye-open differentials (for example a
// calm and an angry open-eye state). The variant id lives in the asset's
// storyStage field, the same field outfit sheets use, so registries rewritten
// by older normalizers keep it. A character with a single eye-open sheet
// (keyed or not) uses it by default; with two or more, the caller must name
// the variant, because silently picking one shows the wrong expression.
export function eyeOpenVariantId(asset) {
  return asset?.role === "eye-open" ? nonEmptyString(asset.storyStage) : "";
}

export function selectEyeOpenReferenceAsset(assets = [], variant = "", owner = "This character") {
  const eyeOpenAssets = (Array.isArray(assets) ? assets : []).filter((asset) => asset?.role === "eye-open");
  const requested = nonEmptyString(variant);
  if (!requested) {
    if (eyeOpenAssets.length > 1) {
      const labels = eyeOpenAssets.map((asset) => eyeOpenVariantId(asset) || "(unkeyed)");
      throw new Error(`${owner} has ${eyeOpenAssets.length} eye-open sheets (${labels.join(", ")}); request one with eyeOpenVariant.`);
    }
    return eyeOpenAssets[0] || null;
  }
  const matches = eyeOpenAssets.filter((asset) => eyeOpenVariantId(asset) === requested);
  if (matches.length > 1) throw new Error(`${owner} registers eye-open variant '${requested}' ${matches.length} times; the registry must bind exactly one sheet per variant.`);
...
export function optimizeCharacterBindingsForGeneration(bindings = [], options = {}) {
  ...
  const requestedIntent = nonEmptyString(options.referenceIntent) || "default";
  const storyStage = nonEmptyString(options.storyStage);
  const eyeOpenVariant = nonEmptyString(options.eyeOpenVariant);
  if (eyeOpenVariant) {
    if (!["default", "eye-open"].includes(requestedIntent)) {
      throw new Error(`eyeOpenVariant '${eyeOpenVariant}' routes an eye-open sheet, but referenceIntent is '${requestedIntent}'.`);
    }
    if (storyStage) {
      throw new Error(`eyeOpenVariant '${eyeOpenVariant}' and storyStage '${storyStage}' select different sheets; request one task-specific sheet per generation.`);
    }
    if (normalized.length !== 1) {
      throw new Error(`eyeOpenVariant '${eyeOpenVariant}' applies to exactly one character, but this generation binds ${normalized.length}.`);
    }
  }
  const intent = eyeOpenVariant ? "eye-open" : requestedIntent;
  ...
  const route = (binding) => {
    ...
    if (normalized.length === 1) {
      if (intent === "closeup" || intent === "expression") {
        ...
        const eyeOpen = selectEyeOpenReferenceAsset(assets, eyeOpenVariant, `Character ${binding.id}`);
        if (eyeOpenVariant && !eyeOpen) {
          const available = [...new Set(assets.map(eyeOpenVariantId).filter(Boolean))];
          throw new Error(
            `Character ${binding.id} has no approved eye-open sheet for variant '${eyeOpenVariant}'. ` +
            (available.length > 0 ? `Registered variants: ${available.join(", ")}.` : "No keyed eye-open variant is registered."),
          );
        }
  ...
  const routed = isComicalReferenceIntent(intent)
    ? routeComicalBindings(normalized, options, intent)
...
export function buildCharacterIdentityPrompt(bindings = [], options = {}) {
  ...
  for (const binding of normalized) {
    ...
    if (isComicalReferenceIntent(binding.referenceIntent)) {
      lines.push(`  Reference image ${referenceIndex} locks this character's approved identity. Reference image ${referenceIndex + 1} is ${binding.referenceIntent} supplemental expression art: ${binding.referenceIntent === "comical-A" ? "exaggerate expression while retaining normal body proportions" : "use the shown chibi proportions while retaining recognizable identity"}. Do not blend A and B or copy sheet grids, captions, or unrelated props.`);
    }
