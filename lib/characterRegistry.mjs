 ...
 * 承認の中に board id・理由・満たすべき文書の SHA-256 を残し、
 * 参照画像の記録も無かったボードにはその旗を足す。
 * 1件も無いときはキー自体を作らない。
 */
function normalizedProvenanceGaps(value) {
  const gaps = (Array.isArray(value) ? value : [])
    .map((entry) => ({
      ...
      ...((plainObject(entry)?.referenceImagesRecorded ?? plainObject(entry)?.reference_images_recorded) === false
