...
export async function realignSubtitleLinesLocally(audioPath, subtitleLines, options = {}) {
  ...
  // The incoming cues already carry the deliberate offsets (pre-roll, 余韻,
  // gap floors). Snap against the SPEECH boundaries those offsets were
  // derived from, then re-apply the offsets — otherwise snapping would
  // silently undo them and the "refined" timing would feel worse, not better.
  const preRoll = SUBTITLE_PRE_ROLL_SECONDS;
  const postRoll = Math.max(0, Number(options.holdSeconds) || 0);
  const snapped = subtitleLines.map((cue) => {
    const speechStart = snap(cue.start + preRoll, starts, 0.35);
    const speechEnd = snap(Math.max(speechStart + 0.05, cue.end - postRoll), ends, 0.35);
    return { ...cue, start: speechStart, end: Math.max(speechStart + 0.05, speechEnd) };
  });
  return applySubtitleLineTiming(snapped, {
    preRollSeconds: preRoll,
    postRollSeconds: postRoll,
    minDurationSeconds: SUBTITLE_MIN_CUE_SECONDS,
    minGapSeconds: SUBTITLE_MIN_CUE_GAP_SECONDS,
    durationSeconds: normalizeDurationSeconds(options.durationSeconds),
  });
}
...
export async function generateSubtitleSrt(options = {}) {
  ...
  if (options.localRealign === true) {
    try {
      subtitleLines = await realignSubtitleLinesLocally(transcriptionSource, subtitleLines, { holdSeconds, durationSeconds });
