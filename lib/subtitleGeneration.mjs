...
/**
 * クレジット予約サービスの応答から、有償処理を始めてよいかを決める。
 *
 * 「機能が無い」と「機能が壊れている」を分ける。404/405/501 はクレジット会計を
 * 配備していない構成なので、予約なしで進むのが正しい。だが 5xx は
 * **サービスが在るのに答えられない**状態で、ここを同じに扱うと、課金制御が
 * 落ちた瞬間に有償処理が上限なしで走る。連打だけでなく、エージェントのループや
 * 壊れた再送でも共有の残高を使い切れる——止めるべき瞬間に止まらない設計だった。
 */
export function creditReservationVerdict(status, env = process.env) {
  const code = Number(status);
  if ([404, 405, 501].includes(code)) {
    return { proceedWithoutReservation: true, blocked: false, reason: "クレジット会計が配備されていない構成" };
  }
  if (code >= 500) {
    if (String(env.BUZZASSIST_ALLOW_UNRESERVED_ON_CREDIT_OUTAGE || "") === "1") {
      return { proceedWithoutReservation: true, blocked: false, reason: "障害時の続行を明示的に許可済み" };
    }
    return {
      proceedWithoutReservation: false,
      blocked: true,
      reason: "課金制御が応答しない",
      message:
        `クレジット予約サービスが応答しない（HTTP ${code}）ため、有償処理を開始しない。`
        + "課金制御が落ちている状態で走らせると、上限のないまま残高を消費する。"
        + "復旧を待つか、意図的に会計なしで進めるなら "
        + "BUZZASSIST_ALLOW_UNRESERVED_ON_CREDIT_OUTAGE=1 を明示すること。",
    };
  }
  return { proceedWithoutReservation: false, blocked: false, reason: "" };
}

export async function reserveSubtitleCredits({ model, durationSeconds, requestId }) {
  ...
  const verdict = creditReservationVerdict(response.status);
  if (verdict.proceedWithoutReservation) return undefined;
  ...
  // 2xx を受け取っただけでは「予約できた」ことにならない。
  // 欠けた値を 0 や undefined へ丸めていたので、200 {} も、全フィールド null も、
  // 負のクレジットも「予約成功」として通っていた——形だけ検査して中身を
  // 見ない型そのもの。壊れた制御プレーンの応答で有償生成が始まる。
  const checked = validateCreditReservationPayload(payload, requestId);
  if (!checked.ok) throw new Error(checked.message);
  return checked.reservation;
}

/**
 * 予約応答が本当に予約を意味しているかを見る。
 * 非空のトークン、正の整数クレジット、要求と一致する request ID を要求する。
 */
export function validateCreditReservationPayload(payload, requestId) {
  const problems = [];
  if (!payload || typeof payload !== "object") problems.push("本文が JSON オブジェクトでない");
  const token = typeof payload?.reservationToken === "string" ? payload.reservationToken.trim() : "";
  if (!token) problems.push("reservationToken が無い");
  const rawCredits = payload?.credits;
  const credits = Number(rawCredits);
  if (rawCredits === undefined || rawCredits === null) problems.push("credits が無い");
  else if (!Number.isFinite(credits) || credits <= 0 || !Number.isInteger(credits)) {
    problems.push("credits が正の整数でない");
  }
  const returnedId = typeof payload?.requestId === "string" ? payload.requestId : "";
  if (requestId && returnedId && returnedId !== requestId) problems.push("requestId が要求と一致しない");

  if (problems.length > 0) {
    return {
      ok: false,
      message:
        `クレジット予約の応答が予約になっていない（${problems.join(" / ")}）ため、有償処理を開始しない。`
        + "2xx を受け取っただけで予約できたことにすると、壊れた制御プレーンの応答で課金が始まる。",
    };
  }
  return {
    ok: true,
    reservation: {
      ...
      requestId: returnedId || requestId || "",
