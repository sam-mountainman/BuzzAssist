...
async function collectRoster({ canvasDir, showBible, registry }) {
  ...
  for (const member of members) {
    ...
    if (nonEmpty(character.episodeId)) {
      blockers.push(`${member.name}: fixed-cast roster evidence must be channel-wide, not episode-bound (${character.episodeId}).`);
      continue;
    }
    ...
    const resolvedIdentityReviewPath = identityReviewPath ? resolveRegistryAssetPath(canvasDir, identityReviewPath) : "";
    let actualReviewSha256 = "";
    try { actualReviewSha256 = resolvedIdentityReviewPath ? await sha256Path(resolvedIdentityReviewPath) : ""; } catch (error) { blockers.push(`${member.name}: identity review is unreadable (${error.message}).`); continue; }
    ...
    rows.push({
      ...
      identityReview: { path: resolvedIdentityReviewPath, sha256: identityReviewSha256 },
    });
  }
  ...
  if (rows.length > 0 && new Set(rows.map((row) => row.registryCharacterId)).size !== rows.length) {
    blockers.push("Every fixed-cast member must resolve to a distinct approved registry character.");
  }
  if (rows.length > 0 && new Set(rows.map((row) => row.identityFace.sha256)).size !== rows.length) {
    blockers.push("Every fixed-cast member must have distinct approved identity-face bytes.");
  }
...
export async function auditKoyaCharacterRosterReview(args = {}) {
  ...
  if (nonEmpty(review.episodeId)) failures.push("Fixed-cast roster review must be channel-wide and cannot be bound to an episodeId.");
