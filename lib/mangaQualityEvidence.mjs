...
function collectPathStrings(value, output = new Set()) {
  ...
  for (const [key, entry] of Object.entries(value)) {
    if (typeof entry === "string" && /(?:path|file|output|source|video|image|frame|audio)/iu.test(key)) {
      // 絶対パスの判定を "/" 始まりに決め打ちしていたので、Windows では
      // JSON が参照する証跡（C:\\...）がマニフェストから漏れていた。
      if (isAbsolute(entry)) output.add(resolve(entry));
    } else {
...
export async function createMangaEvidenceManifest({
  ...
  // 名指しされた成果物は必須。ディレクトリ走査や JSON からの芋づるで
  // 見つかったものだけが任意。読めなかったものを一律で捨てていたので、
  // **必須の成果物が1つも無くても entryCount: 0 の manifest が合格**していた。
  // 最終判断の Merkle 証拠に、根拠となるファイルが1つも入っていない状態。
  const requiredPaths = new Set((artifacts || []).filter(Boolean).map((entry) => resolve(entry)));
  ...
  for (const path of [...paths].sort()) {
    try {
      ...
      if (requiredPaths.has(path)) {
        throw new Error(
          `証拠に要るファイルを読めない: ${path}（${String(error?.message || error).slice(0, 120)}）。`
          + "名指しした成果物を黙って落とすと、根拠の無い Merkle 証拠で完成になる。",
        );
      }
      // 芋づるで拾ったものは、無くてもよい。
    }
  }
  if (requiredPaths.size > 0 && entries.length === 0) {
    throw new Error("名指しした成果物が1つも証拠に入らなかった。空の Merkle 証拠を合格にしない。");
  }
