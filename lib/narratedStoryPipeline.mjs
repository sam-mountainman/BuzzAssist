 ...
 * operatorReplacementCleared が入った。宣言の inForceSince はこの系列で書く。
 */
export const NARRATED_STORY_AUDIT_CONTRACT_VERSION = "buzzassist-narrated-story-audit-v2";
...
export async function runNarratedStoryPipeline({
  ...
  return withPipelineLock(runDir, async () => {
    ...
    // bookends の感想パートは Pack が宣言した区切り行の後ろ。区切りが無い・重複している台本は
    // 人の判断が要るので、有料生成の前に止める。
    const partition = partitionNarratedStoryScript(script, config.bookends);
    if (partition.blockers.length > 0) {
      return operatorInputOutcome(partition.blockers.map((item) => `script-structure-required:${item}`), "script-partition");
    }
    const storySegments = splitNarratedStoryScript(partition.bodyText).map((segment) => ({ ...segment, part: "story" }));
    const reviewSegments = config.bookends.enabled && config.bookends.review
    ...
    // 声にも字幕にもせず、有料生成の前に止める。最終監査の operatorReplacementCleared でも再確認する。
    const unreplaced = operatorReplacementSegments(segments, {
      extraMarker: config.bookends.review?.operatorReplacementMarker || "",
    });
    if (unreplaced.length > 0) {
      return operatorInputOutcome(unreplaced.map((id) => `operator-replacement-required:${id}`), "operator-replacement");
    }
    ...
    const tasks = segments.flatMap((segment) => ([
      ...
      ...(segment.part === "review" && presenterReview ? [] : [{ type: "image", segment }]),
      { type: "voice", segment },
    ]));
    let generated;
    try {
    ...
    await writeAtomic(generationManifestPath, {
      ...
      segments: programSegments.map((segment, index) => ({
        ...
        imageSha256: byKey.get(`image:${segment.id}`)?.receipt.artifact.sha256 || null,
