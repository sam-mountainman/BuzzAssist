...
async function renderPreview(ffmpeg, segments, masterAudioPath, srtPath, config, outputPath, { subtitleLayer = null } = {}) {
  ...
  // 焼き込み字幕の層（concat の一覧）は最後の入力。フレーム数は声の実尺の累積で丸める（字幕の頁と同じ時間割）。
  ...
  const filters = segments.map((segment, index) => {
    const frameCount = framePlan[index].frames;
  ...
  filters.push(`${segments.map((_, index) => `[v${index}]`).join("")}concat=n=${segments.length}:v=1:a=0[${subtitleLayer ? "story" : "video"}]`);
...
export function inspectNarratedStoryRenderGraphs(renderGraphs = {}, segmentCount = 0) {
  ...
  return {
    ...
    sceneTransitionOwnedByParent: Boolean(renderFilterGraph)
      ...
      && videoConcatAssembled(renderFilterGraph, { count: segmentCount, label: "story" })
      && !transitionFilterPresent,
...
async function automaticAudit({ ffmpeg, ffprobe, previewPath, voiceStemPath, bgmStemPath, expectedDuration, segments, config, hashes, renderGraphs, bookendPlan = null, renderedSegments = [], boundaryExcludeRegion = null }) {
...
export async function runNarratedStoryPipeline({
  ...
  const outcome = await withPipelineLock(runDir, async () => {
    ...
    await writeAtomic(generationManifestPath, {
      ...
      ...narratedVisualManifestFields({ config, subtitleLayer }),
