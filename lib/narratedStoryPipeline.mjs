 ...
 * operatorReplacementCleared が入った。v3 で品質ループ（qualityLoopPassed）が入った。
 * 宣言の inForceSince はこの系列で書く。
 */
export const NARRATED_STORY_AUDIT_CONTRACT_VERSION = "buzzassist-narrated-story-audit-v3";
 ...
 */
export async function inspectNarratedStoryPlan({ scriptPath, channelPackDir } = {}) {
...
async function finalizeReadyState({
  ...
  const auditChecks = { ...perceptualChecks, [NARRATED_QUALITY_AUDIT_ID]: quality.check };
...
export async function runNarratedStoryPipeline({
  ...
  const outcome = await withPipelineLock(runDir, async () => {
    ...
    const tasks = segments.flatMap((segment) => {
      ...
      const wantsImage = !(segment.part === "review" && presenterReview) && !imageKeys.has(imageKey);
        ...
        return {
          ...
          ...(await acquireMediaArtifact({
            ...
            spec: mediaSpec({
              ...
              // 字幕は表記（text）、声は読み（spokenText。台本パッケージの readings を当てた文）。
              input: { text: segment.spokenText || segment.text, language: "ja", speed: (segment.voice || config.voice).speed },
              output: narratedVoiceOutputProfile((segment.voice || config.voice).provider),
              reservation: {
                unit: "characters",
    ...
    const mediaJobs = receiptSafeMediaJobs([...generated.map((entry) => entry.receipt), ...(music.receipts || [music.receipt])]);
    ...
    await writeAtomic(generationManifestPath, {
      ...
      segments: programSegments.map((segment, index) => ({
        ...
        imageSha256: byKey.get(`image:${segment.imageKey || segment.id}`)?.receipt.artifact.sha256 || null,
