 ...
 * 人物の同一性（characterIdentityReviewed: 署名済み独立レビューの評価項目の下限）が入った。
 * 宣言の inForceSince はこの系列で書く。
 */
export const NARRATED_STORY_AUDIT_CONTRACT_VERSION = "buzzassist-narrated-story-audit-v4";
 ...
 * pipeline と plan-only の preflight が同じ判定を使う（判定を 2 か所に持たない）。
 */
async function checkNarratedOperatorImages({ config, manifest, storySegments = [], reviewSegments = [], projectDir = "", assetLoopVerifier = null }) {
...
async function renderBookendProgramFromPlan({
  ...
  renderGraphs.preview = await renderBookendProgram({ ffmpeg, clipPaths, masterAudioPath, srtPath, plan, outputPath });
  return renderGraphs;
}
 ...
 * evidenceAuditIds にはまだ無い（監査契約の版を上げるときに保証として足す）。最終化はこの監査も
 * pass でなければ進まない。
 */
...
async function automaticAudit({ ffmpeg, ffprobe, previewPath, voiceStemPath, bgmStemPath, expectedDuration, segments, config, hashes, renderGraphs, bookendPlan = null, renderedSegments = [] }) {
  ...
  const movingSegments = segments.filter((segment) => segment.camera !== "presenter-video");
  const presenterSegmentCount = segments.length - movingSegments.length;
  const cameraFrames = await mapLimit(movingSegments, Math.min(4, movingSegments.length), async (segment, index) => {
    ...
    return {
      id: segment.id,
      direction: (segment.partIndex ?? index) % 2 === 0 ? "left-to-right" : "right-to-left",
      startSeconds,
      middleSeconds,
      endSeconds,
  ...
  const graphAudit = bookendPlan
    ...
    : inspectNarratedStoryRenderGraphs(renderGraphs, segments.length);
  // 境目の実測は完成 MP4（と、それへ mux した voice stem）を decode して行う。
  const bookendMeasurement = bookendPlan
    ? await measureNarratedBookendBoundaries({ ffmpeg, ffprobe, videoPath: previewPath, voiceStemPath, plan: bookendPlan })
    : null;
...
async function finalizeReadyState({
  ...
  const perceptualChecks = applyPerceptualChecks(state.auditChecks, validation, {
  ...
  await renameWithRetry(finalTemp, finalPath);
...
async function recordSignoffRejectionInAuditReport(state, statePath, { signoffPath, signoffPresent, problems, qualityLoop = null }) {
  ...
  await writeAtomic(auditPath, { ...report, independentSignoff, qualityLoop }, { json: true });
...
export async function runNarratedStoryPipeline({
  ...
  // 途中の成果物の品質ループの検査口の差し替え（in-process の試験だけ）。
  operatorImageAssetLoopVerifier = null,
  now = () => new Date().toISOString(),
  env = process.env,
} = {}) {
  ...
  const outcome = await withPipelineLock(runDir, async () => {
    ...
    if (config.imageImport) {
      const operator = await checkNarratedOperatorImages({
        ...
        assetLoopVerifier: operatorImageAssetLoopVerifier,
      });
      if (operator.issues.length > 0) return operatorInputOutcome(operator.issues, "operator-image-import");
      ...
      segments: programSegments.map((segment, index) => ({
        ...
        camera: segment.camera === "presenter-video"
          ...
          : {
            ...
            direction: (segment.partIndex ?? index) % 2 === 0 ? "left-to-right" : "right-to-left",
            frameCount: segment.frames ?? Math.max(1, Math.round(segment.durationSeconds * config.render.fps)),
          },
    ...
    // 取り込みの記録のどちらか一方に sha256 で結び付いていること（宣言の外の監査。宣言への追加は報告に書く）。
