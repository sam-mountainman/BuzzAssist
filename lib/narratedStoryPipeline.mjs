...
import { cameraShotChains } from "./narratedStoryCamera.mjs";
 ...
 * 描かない）と、場面の画の出どころ（sceneImageProvenance）が保証になった。
 * 宣言の inForceSince はこの系列で書く。production の state は作った版を auditContractVersion に残し、
 * 確定はその版で測る（v4 以前の state は、当時無かった関門を求めずに従来どおり確定する）。
 */
export const NARRATED_STORY_AUDIT_CONTRACT_VERSION = "buzzassist-narrated-story-audit-v5";
...
export function narratedSceneImageKeys(segments = [], { presenterReview = false } = {}) {
  ...
  for (const segment of segments) {
    if (segment.part === "review" && presenterReview) continue;
 ...
 */
export async function inspectNarratedStoryPlan({ scriptPath, channelPackDir, operatorImageManifestPath = "", operatorVideoManifestPath = "", projectDir = "", ffprobe = null } = {}) {
...
async function renderPreview(ffmpeg, segments, masterAudioPath, srtPath, config, outputPath, { subtitleLayer = null } = {}) {
  ...
  // 1つの動きで通す（lib/narratedStoryCamera.mjs。文ごとに寄りを始め直さない）。
  const framePlan = segmentFramePlan(segments.map((segment) => segment.durationSeconds), config.render.fps);
  const shots = partCameraShots(segments.map((segment, index) => ({ ...segment, partStartFrame: framePlan[index].startFrame, frames: framePlan[index].frames })), config);
  const camera = cameraShotChains({ shots, render: config.render });
  const args = ["-hide_banner", "-loglevel", "error", "-y", ...camera.inputs];
  const audioIndex = shots.length;
  const subtitleIndex = shots.length + 1;
  ...
  const filters = [...camera.chains];
  filters.push(`${camera.labels.join("")}concat=n=${shots.length}:v=1:a=0[${subtitleLayer ? "story" : "video"}]`);
  if (subtitleLayer) filters.push(subtitleOverlayGraph({ inputIndex: subtitleIndex + 1, input: "story", output: "video" }));
...
async function renderBookendProgramFromPlan({
  ...
  if (reviewPart) {
    ...
    renderGraphs.reviewVideo = reviewPart.visual === "presenter-video"
      ? (await renderPresenterPart({
        ffmpeg,
        presenterPath: bookends.media["review.presenter.video"],
 ...
 */
export function inspectNarratedStoryRenderGraphs(renderGraphs = {}, segmentCount = 0, { shotCount = segmentCount } = {}) {
  ...
  const expectedVideoConcat = `concat=n=${shotCount}:v=1:a=0[video]`;
  ...
  return {
    ...
    parentAudioMapped: Boolean(renderGraphs?.voiceStem?.filterGraph)
      ...
      && renderGraphs?.preview?.audioMap === `${shotCount}:a:0`,
...
async function automaticAudit({ ffmpeg, ffprobe, previewPath, voiceStemPath, bgmStemPath, expectedDuration, segments, config, hashes, renderGraphs, bookendPlan = null, renderedSegments = [], boundaryExcludeRegion = null, cameraShotCount = segments.length }) {
  ...
  const graphAudit = bookendPlan
    ? inspectNarratedBookendRenderGraphs(renderGraphs, bookendPlan)
    : inspectNarratedStoryRenderGraphs(renderGraphs, segments.length, { shotCount: cameraShotCount });
    ...
    if (!assetLoops.pass) {
      return outcomeFromReadyState(state, {
        knownRemainingIssues: [...new Set([
          ...assetLoops.issues,
          ...NARRATED_ASSET_LOOP_AUDIT_IDS.filter((id) => productionChecks[id]?.pass !== true).map((id) => `audit-${id}-pending-or-failed`),
        ])],
...
export async function runNarratedStoryPipeline({
  ...
  const outcome = await withPipelineLock(runDir, async () => {
    ...
    const adapterProbes = await Promise.all([
      ...
      ...narratedVoiceAdapters(config, segments),
    ...
    const tasks = segments.flatMap((segment) => {
      ...
      const wantsImage = !operatorImages && !(segment.part === "review" && presenterReview) && !imageKeys.has(imageKey);
      if (wantsImage) imageKeys.add(imageKey);
      return [...(wantsImage ? [{ type: "image", segment }] : []), { type: "voice", segment }];
    });
    let generated;
    try {
    ...
    if (bookendPlan) {
      ...
      renderGraphs = {
        ...
        preview: await renderPreview(toolchain.ffmpeg, measuredSegments, masterAudioPath, srtPath, config, previewTemp, { subtitleLayer }),
      };
    }
    ...
    const voiceCasting = auditNarratedVoiceCasting({
      ...
      voiceReceipts: new Map(measuredSegments.map((segment) => [segment.id, byKey.get(`voice:${segment.id}`)?.receipt])),
      config,
    });
    await writeAtomic(generationManifestPath, {
      ...
      ...narratedVisualManifestFields({ config, subtitleLayer, cameraPlan, operatorVideos }),
    ...
    const audit = await automaticAudit({
      ...
      segments: programSegments.map((segment) => ({
        ...
        camera: staticCameraSegments.has(segment.id) ? "static" : segment.camera,
