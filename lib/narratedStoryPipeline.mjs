...
import {
  ...
  narratedReviewVoiceless,
  segmentTimelinePlan,
  EPISODE_OPENING_VIDEO_SLOT,
} from "./narratedStoryBookends.mjs";
import {
  NARRATED_PACING_AUDIT_ID,
  narratedPacingAuditCheck,
  narratedPacingManifestEntry,
  narratedPacingSummary,
  normalizeNarratedPacingConfig,
  planNarratedStoryPauses,
  renderNarratedPacedVoice,
} from "./narratedStoryPacing.mjs";
...
import { videoConcatAssembled } from "./narratedStorySubtitles.mjs";
import { overlayExclusionRegions, planNarratedOverlayLayer } from "./narratedStoryOverlays.mjs";
import { applyNarratedSceneCameraFocus } from "./narratedStoryCamera.mjs";
import {
  NARRATED_SUBTITLE_WORK_SUBDIR,
...
import { checkWindowsWorkPaths } from "./windowsWorkPath.mjs";
 ...
 * なければ有料の処理を始めない）が保証になった。v9 で本編の場面の切り替え（sceneTransitionMeasured:
 * lib/narratedStorySceneTransitions.mjs。Pack が crossfade を宣言したときだけ混ぜ、宣言していなければ cut であることを
 * 完成 MP4 で測る）と固定の重ね物（fixedOverlaysMeasured: lib/narratedStoryOverlays.mjs。宣言の区間・位置に重なり、
 * 区間の外には無いことを測る。宣言が無ければ描いていないことを確かめる）が保証になった。v10 で場面ごとの焦点
 * （cameraFocusMeasured: lib/narratedStoryCamera.mjs。台本パッケージの本編の場面の cameraFocus を指定した場面を、
 * 完成 MP4 のフレームと場面の画を計画の見せる範囲で切り出した絵とで比べる。指定が無ければ、どのショットも Pack の
 * 型ごとの焦点で計画したことを確かめる）が保証になった。焦点は運営者の画の取り込みの記録の行の cameraFocus からも
 * 渡せる（取り込みの記録 → 台本パッケージ → Pack の型ごとの焦点の順）が、監査は同じ cameraFocusMeasured で計画の焦点を
 * 測るだけなので契約は v10 のまま（入力の出どころが増えただけ。以前の記録の cameraFocus は知らない欄として有料の処理の
 * 前に止まっていたので、走った Job の計画も判定も変わらない）。v11 で本編の文と文・場面と場面の間
 * （narrationPacingMeasured: lib/narratedStoryPacing.mjs。Pack の pacing が宣言した間の区間に語りが無く BGM が流れ、足した
 * 無音が宣言どおりの長さで、字幕が語りの区切りで切り替わり、場面の切り替えが間の中にあることを、完成 MP4 と voice stem で
 * 測る。宣言が無ければ、本編の文が間を空けずに並んでいることを確かめる）が保証になった。
 ...
 * 関門を求めない）。
 */
export const NARRATED_STORY_AUDIT_CONTRACT_VERSION = "buzzassist-narrated-story-audit-v11";
...
export async function loadNarratedStoryChannelConfig(channelPackDir) {
  ...
  const visuals = await loadNarratedVisualConfig(source, { render: config.render, channelPackDir, bookends: config.bookends });
  blockers.push(...visuals.blockers);
  Object.assign(config, visuals.config);
  // 本編の文と文・場面と場面の間（lib/narratedStoryPacing.mjs）。無ければ間を足さない（今までどおり）。
  // 場面の切り替えの crossfade は場面の間の中に収めるので、間より長い crossfade の宣言はここで止める。
  const pacing = normalizeNarratedPacingConfig(source.pacing, { sceneTransition: config.sceneTransition || null });
  blockers.push(...pacing.blockers);
  config.pacing = pacing.config;
...
/**
 * 場面ごとの焦点を「運営者の画の取り込みの記録の行 → 台本パッケージの本編の場面 → Pack の型ごとの焦点」の順に
 * 決め、文の cameraFocus / cameraFocusSource に書く（lib/narratedStoryCamera.mjs の applyNarratedSceneCameraFocus）。
 * 焦点は画に属する値なので、画の後で決める焦点は取り込みの記録に書く（台本パッケージに書き足すと台本の品質ループの
 * 合格が外れる）。見た目の検査（checkNarratedVisualPlan）の前に呼ぶ。pipeline と plan-only の preflight が同じ判定を
 * 使う。manifest が無い（broker の Pack）・読めなかった行は焦点を渡さない（読めない理由は取り込みの検査が止める）。
 */
function applyNarratedImageManifestFocus({ segments = [], manifest = null }) {
  const imageFocus = new Map();
  for (const row of manifest?.rows || []) if (row.cameraFocus) imageFocus.set(row.sceneId, row.cameraFocus);
  return applyNarratedSceneCameraFocus({ segments, imageFocus });
}
 ...
 * Windows では、Job の run のフォルダの中の作業フォルダが長すぎれば（lib/windowsWorkPath.mjs）pipeline と同じ
 * windows-work-path-too-long を返す（projectDir と jobId があるときだけ測れる）。
 */
export async function inspectNarratedStoryPlan({ scriptPath, channelPackDir, operatorImageManifestPath = "", operatorVideoManifestPath = "", scriptQualityWorkDir = "", projectDir = "", jobId = "", platform = process.platform, ffprobe = null } = {}) {
  const loaded = await loadNarratedStoryChannelConfig(channelPackDir);
  const blockers = [];
  // Windows の作業フォルダの長さ（Pack にも台本にも依らない）。pipeline が有料の処理の前に止めるのと同じ判定。
  const workPath = nonEmpty(projectDir) && nonEmpty(jobId)
    ? checkWindowsWorkPaths({ workDirs: narratedStoryChildWorkDirs({ deploymentRoot: projectDir, jobId }), platform })
    : null;
  if (workPath) blockers.push(...workPath.issues);
  ...
  if (config) {
    ...
    // 本編の間（Pack の pacing）で本編がどれだけ伸びるか（件数と秒）。有料の処理の前に尺の計画に入れて見せる。
    if (loaded.ok) pacing = narratedPacingSummary(config.pacing, planNarratedStoryPauses(plan.storySegments, config.pacing));
    ...
    // 運営者の画の取り込みの記録（Pack が operator-file のとき）。場面の焦点にも使うので、見た目の検査より先に読む。
    const operatorManifest = config.imageImport ? await readOperatorImageManifest({ manifestPath: operatorImageManifestPath }) : null;
    // 場面の焦点（取り込みの記録 → 台本パッケージ → Pack）と、見た目（焼き込み字幕の書体に無い字・収まらない行など）を
    // 台本に当てて、描けない理由を先に並べる。
    if (loaded.ok) {
      const planSegments = [...plan.storySegments, ...plan.reviewSegments];
      blockers.push(...applyNarratedImageManifestFocus({ segments: planSegments, manifest: operatorManifest }).issues);
      blockers.push(...(await checkNarratedVisualPlan({ config, segments: planSegments })).issues);
    }
    ...
    if (config.imageImport) {
      if (!nonEmpty(operatorImageManifestPath)) blockers.push(...operatorManifest.problems);
        ...
        operatorImages = { scenes: operator.sceneIds.length, manifestSha256: operatorManifest.manifestSha256 || null };
  ...
  return {
    ...
    // 感想パートを声なしと宣言した Pack（感想の文は字幕だけで、声の Media Job を作らない）。
    ...(narratedReviewVoiceless(config) ? { reviewVoice: "none" } : {}),
    ...
    ...(workPath && !workPath.ok ? { windowsWorkPath: { limit: workPath.limit, longest: workPath.longest } } : {}),
    ...(pacing ? { pacing } : {}),
    segments,
    scriptInput,
...
async function renderPreview(ffmpeg, segments, masterAudioPath, srtPath, config, outputPath, { subtitleLayer = null, overlayLayer = null } = {}) {
  // 文のフレーム数は文の枠（声の実尺と本編の間）の累積で丸める（字幕の頁と同じ時間割）。同じ画が続く文は1つの
  // ショットとして1つの動きで通す（lib/narratedStoryCamera.mjs。文ごとに寄りを始め直さない）。場面の画は bookends の
  // 本編と同じ描き方（lib/narratedStoryBookends.mjs の renderSegmentImagesPart: 場面の切り替えの宣言と、
  // コマンド行の上限を越えない塊に分けた描画）で1本の動画にしてから、音と字幕を載せる。
  const timeline = segmentTimelinePlan(segments, config.render.fps);
  const story = await renderSegmentImagesPart({
    ffmpeg,
    segments: segments.map((segment, index) => ({
      ...segment,
      part: segment.part || "story",
      partStartFrame: timeline[index].startFrame,
      frames: timeline[index].frames,
      ...(timeline[index].pauseHeadFrames === undefined ? {} : { pauseHeadFrames: timeline[index].pauseHeadFrames }),
      ...(timeline[index].pauseTailFrames === undefined ? {} : { pauseTailFrames: timeline[index].pauseTailFrames }),
    })),
    config,
  ...
  // 固定の重ね物の画像と焼き込み字幕の層（concat の一覧）は、音と字幕のトラックの後の入力（bookends の番組と同じ形）。
  const layers = programLayersGraph({ assembled: "[0:v]concat=n=1:v=1:a=0", firstInput: 3, overlayLayer, subtitleLayer });
...
async function renderBookendProgramFromPlan({
  ...
  renderGraphs.preview = await renderBookendProgram({ ffmpeg, clipPaths, masterAudioPath, srtPath, plan, outputPath, subtitleLayer, overlayLayer });
  return renderGraphs;
}
...
async function bookendManifestEntry(plan, bookends) {
  ...
  return {
    ...
    // 声なしの部（感想パート）は voice: none を残す（記録から組み直した時間割でも同じ決まりで測り直せるように）。
    parts: plan.parts.map(({ id, startFrame, frames, visual, voice }) => ({ id, startFrame, frames, visual, ...(voice === "none" ? { voice } : {}) })),
...
async function automaticAudit({ ffmpeg, ffprobe, previewPath, voiceStemPath, bgmStemPath, expectedDuration, segments, config, hashes, renderGraphs, bookendPlan = null, renderedSegments = [], boundaryExcludeRegion = null, cameraShotCount = segments.length, sceneJoins = [] }) {
  ...
  // 字幕だけの文（声なしの感想パートの文を含む）は語りが無いので測らない。語りが無いことは、境目の監査
  // （audioBoundaryBreathV16）が声なしの感想パートの voice stem が無音であることとして測る。
  const captionOnlySegmentIds = segments.filter((segment) => segment.captionOnly).map((segment) => segment.id);
  checks.narrationBedSeparation = check(
    ...
    `minimum per-utterance voice/bed separation ${separationDb} dB across ${intervalSeparation.length} intervals`
      + `${captionOnlySegmentIds.length ? ` (${captionOnlySegmentIds.length} caption-only intervals carry no narration and are not measured)` : ""}`,
    { minimumSeparationDb: separationDb, intervals: intervalSeparation, ...(captionOnlySegmentIds.length ? { captionOnlySegmentIds } : {}) },
  );
  ...
  if (bookendMeasurement) {
    ...
    // 入る側が声なしの部（感想パート）の境目は、語りの代わりに最初の字幕・voice stem の無音・BGM の途切れを測った。
    const voicelessBoundaries = bookendMeasurement.audio.boundaries.filter((entry) => entry.metrics?.incomingVoice === "none").map((entry) => entry.id);
    checks.audioBoundaryBreathV16 = check(
      bookendMeasurement.audio.pass,
      bookendMeasurement.audio.pass
        ? `measured ${bookendMeasurement.audio.boundaries.length} boundaries on the MP4: narration tails intact, no dead air, caption-free lead-in carries bed only, `
          + (voicelessBoundaries.length
            ? `next narration and first caption start together where the incoming part is voiced; into the voiceless review (${voicelessBoundaries.join(", ")}) the first caption starts at the lead-in end, the mixed voice stem stays silent through the review and the bed never drops out, `
            : "next narration and first caption start together, ")
          + "no clipping, frame/audio ends match the plan"
...
/**
 * 描く工程の作業フォルダ（Job の run のフォルダの中）。pipeline と、Windows の作業フォルダの長さの見積もり
 * （narratedStoryChildWorkDirs）が同じものを使う。
 */
function narratedStoryRenderDirs(runDir) {
  const renderDir = join(runDir, "render");
  return {
    renderDir,
    bookendWorkDir: join(renderDir, "bookends"),
    subtitleWorkDir: join(renderDir, NARRATED_SUBTITLE_WORK_SUBDIR),
  };
}

/**
 * ffmpeg を作業フォルダ（cwd）を指定して起動する所のうち、Job の run のフォルダからいちばん深くなりうる
 * 作業フォルダ: 字幕の頁（lib/narratedStorySubtitles.mjs の renderNarratedSubtitleOverlay）と OP
 * （lib/narratedStoryBookends.mjs の renderOpeningPart）。Windows では長さが上限を越えると起動できない
 * （lib/windowsWorkPath.mjs）ので、doctor・plan-only の preflight・pipeline が有料の処理の前にこれで測る。
 * 別の工程で作業フォルダを指定して子を起動するようにしたら、ここに足す。
 */
export function narratedStoryChildWorkDirs({ deploymentRoot = process.cwd(), jobId } = {}) {
  const dirs = narratedStoryRenderDirs(narratedStoryRunPaths({ deploymentRoot, jobId }).runDir);
  return [
    { path: dirs.subtitleWorkDir, label: "字幕の頁の作業フォルダ" },
    { path: dirs.bookendWorkDir, label: "OP の作業フォルダ" },
  ];
}
...
export async function runNarratedStoryPipeline({
  ...
  // Windows の作業フォルダの長さを測るかどうかの OS（試験だけが差し替える）。
  ...
  const outcome = await withPipelineLock(runDir, async () => {
    ...
    // Windows では、ffmpeg を起動する作業フォルダ（字幕の頁・OP）が長すぎると起動できない（spawn が ENOENT）。
    // 有料の処理の前に、いちばん深くなりうる作業フォルダの長さで止める（lib/windowsWorkPath.mjs）。
    const workPath = checkWindowsWorkPaths({ workDirs: narratedStoryChildWorkDirs({ deploymentRoot, jobId }), platform });
    if (!workPath.ok) return operatorInputOutcome(workPath.issues, "windows-work-path-preflight");
    ...
    // 場面の焦点を「取り込みの記録 → 台本パッケージ → Pack の型ごとの焦点」の順に決め（文に書く）、見た目（焼き込み
    // 字幕の書体に無い字・画面に収まらない行など）を台本に当て、描けないなら有料生成の前に止める。
    const sceneFocus = applyNarratedImageManifestFocus({ segments, manifest: operatorManifest });
    const visualPlan = await checkNarratedVisualPlan({ config, segments });
    const visualIssues = [...new Set([...sceneFocus.issues, ...visualPlan.issues])];
    if (visualIssues.length > 0) return operatorInputOutcome(visualIssues, "visual-plan");
    ...
    const { renderDir, bookendWorkDir } = narratedStoryRenderDirs(runDir);
    ...
    // 本編の文と文・場面と場面の間（Pack の pacing。lib/narratedStoryPacing.mjs）。宣言が無ければ間は無く、文は声の
    // テイクの実尺で隙間なく並ぶ（今までどおり）。間を持つ文は、テイクの前後に間の無音を足した声のファイルを
    // voice stem へつなぎ、文の枠（画・カメラ・尺）は間を含め、声と字幕の時刻はテイクの区間にする。
    const pacingPlan = planNarratedStoryPauses(storySegments, config.pacing);
    const pacedDir = join(voiceDir, "paced");
    ...
    for (const segment of segments) {
      ...
      const pause = pacingPlan.bySegment.get(segment.id) || null;
      const before = pause?.pauseBeforeSeconds || 0;
      const after = pause?.pauseAfterSeconds || 0;
      let stemVoicePath = voicePath;
      if (before > 0 || after > 0) {
        await mkdir(pacedDir, { recursive: true });
        stemVoicePath = (await renderNarratedPacedVoice({
          ffmpeg: toolchain.ffmpeg,
          takePath: voicePath,
          beforeSamples: pause.pauseBeforeSamples,
          afterSamples: pause.pauseAfterSamples,
          outputPath: join(pacedDir, `${segment.id}.paced.wav`),
        })).path;
      }
      measuredSegments.push({
        ...segment,
        imagePath: byKey.get(`image:${segment.imageKey || segment.id}`)?.path || "",
        // voice stem へつなぐファイル（間を持つ文は間の無音を足した写し）と、採用したテイクそのもの。
        voicePath: stemVoicePath,
        takePath: voicePath,
        durationSeconds,
        ...(before > 0 ? { pauseBeforeSeconds: before } : {}),
        ...(after > 0 ? { pauseAfterSeconds: after } : {}),
        startSeconds: cursor + before,
        endSeconds: cursor + before + durationSeconds,
      });
      cursor += before + durationSeconds + after;
    }
    // 番組の本編（bookends が無ければ番組全体）の尺。本編の間を含む。
    ...
    if (bookends) {
      ...
      bookendPlan = planNarratedBookendProgram({
        ...
        // 感想パートの声の有無（Pack の bookends.review.voice）。none なら境目の実測が声なしの決まりで測る。
        reviewVoice: narratedReviewVoiceless(config) ? "none" : "voiced",
    ...
    // 固定の重ね物（Pack が宣言したときだけ。lib/narratedStoryOverlays.mjs）の番組の中の区間。
    const overlayLayer = planNarratedOverlayLayer({ config, bookendPlan, totalFrames: programFrames.totalFrames, fps: config.render.fps });
    ...
    const reviewLayout = planNarratedReviewLayout({
      ...
      // TV 枠に流す運営者の動画（layout.tv.content が operator-video のとき。取り込みの枠 review-tv）。
      operatorVideos,
    });
    ...
    if (bookendPlan) {
      ...
      const preview = await renderPreview(toolchain.ffmpeg, measuredSegments, masterAudioPath, srtPath, config, previewTemp, { subtitleLayer, overlayLayer });
    ...
    // 間の無音を足した声の写し（voice stem へつないだもの）の sha256。
    const pacedVoiceShas = new Map();
    for (const segment of measuredSegments) {
      if (segment.takePath && segment.voicePath !== segment.takePath) pacedVoiceShas.set(segment.id, await fileSha256(segment.voicePath));
    }
    await writeAtomic(generationManifestPath, {
      ...
      segments: programSegments.map((segment, index) => ({
        ...
        startSeconds: segment.startSeconds,
        endSeconds: segment.endSeconds,
        // 本編の間（Pack の pacing）: 文の枠の前後に置いた無音の秒。声と字幕は startSeconds〜endSeconds（テイクの区間）。
        ...(segment.pauseBeforeSeconds ? { pauseBeforeSeconds: segment.pauseBeforeSeconds } : {}),
        ...(segment.pauseAfterSeconds ? { pauseAfterSeconds: segment.pauseAfterSeconds } : {}),
        ...
        // voice stem へつないだ、間の無音を足した写し（テイクそのものは voiceSha256）。
        ...(segment.takePath && segment.voicePath !== segment.takePath ? { pacedVoiceSha256: pacedVoiceShas.get(segment.id) } : {}),
      })),
      // 本編の間（Pack の pacing。宣言が無ければ enabled: false で間は 0 件）。
      pacing: narratedPacingManifestEntry(config.pacing, pacingPlan),
      ...
      ...narratedVisualManifestFields({ config, subtitleLayer, cameraPlan, operatorVideos, reviewLayout, overlayLayer }),
    ...
    const audit = await automaticAudit({
      ...
      // 字幕と固定の重ね物の範囲は、出入りを転換と取り違えないように外して測る。
      boundaryExcludeRegion: [subtitleExclusionRegion(subtitleLayer, config.subtitles), ...overlayExclusionRegions(overlayLayer)].filter(Boolean),
    });
    ...
    // 本編の間（Pack の pacing。監査契約 v11 から）: 宣言した間の区間に語りが無く BGM が流れ、足した無音が宣言どおりの
    // 長さで、字幕が語りの区切りで切り替わり、場面の切り替えが間の中にあることを、完成 MP4 と voice stem で測る。
    // 宣言が無ければ、本編の文が間を空けずに並んでいることを確かめる。
    const pacingCheck = await narratedPacingAuditCheck({
      ffmpeg: toolchain.ffmpeg,
      videoPath: previewPath,
      voiceStemPath,
      pacing: config.pacing,
      segments: programSegments,
      storyPart: bookendPlan ? bookendPlan.parts.find((part) => part.id === "story") : { startSeconds: 0, endSeconds: totalDuration },
      programFrames,
      subtitleLayer,
      sceneJoins: cameraPlan.problems.length === 0 ? planNarratedProgramSceneJoins({ cameraPlan, transition: config.sceneTransition || null }) : null,
      fps: config.render.fps,
    });
    audit.checks[NARRATED_PACING_AUDIT_ID] = check(pacingCheck.pass, pacingCheck.detail, { measurement: pacingCheck.measurement });
