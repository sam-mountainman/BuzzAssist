 ...
 * lib/narratedStoryVisuals.mjs。完成 MP4 のフレームを読んで測る）が保証になった。
 ...
 */
export const NARRATED_STORY_AUDIT_CONTRACT_VERSION = "buzzassist-narrated-story-audit-v6";
...
async function renderPreview(ffmpeg, segments, masterAudioPath, srtPath, config, outputPath, { subtitleLayer = null } = {}) {
  ...
  args.push(
    // ショットごとのカメラの式は長いので、長い graph はファイルで渡す（lib/ffmpegFilterArgs.mjs）。
    ...await filterComplexArgs(ffmpeg, filterGraph, dirname(outputPath)),
