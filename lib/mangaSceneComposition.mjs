...
export const MANGA_SCENE_COMPOSITION_VERSION = 11;
...
const INTENT_RULES = [
  ...
  { intent: "object-action", pattern: /(?:前足|手|腕|指).{0,30}(?:落と|払い|払っ|差し出|置い|拾っ|開け|隠そ)|(?:落と|払い|払っ|差し出|置い|拾っ|開け|隠そ).{0,30}(?:箱|札|パス|写真|書類|腕時計|端末)/u, hardPreferred: true, setups: ["counter-level-object", "foreground-occlusion", "phone-over-shoulder", "low-dominant-close"] },
...
function isClosingResolutionMontage(utterance = {}, context = {}) {
  if (context.isFinalCut !== true) return false;
  if (utterance.speakerId !== "narration" && utterance.preset !== "narration") return false;
  const text = nonEmptyString(utterance.text);
  const sentences = text.split(/[。！？!?]+/u).map((entry) => entry.trim()).filter(Boolean);
  const clauses = text.split(/[、。！？!?]+/u).map((entry) => entry.trim()).filter(Boolean);
  const independentlyActingClauses = clauses.filter((entry) => /(?:は|が)/u.test(entry));
  const physicalActionClauses = clauses.filter((entry) => /(?:固ま|隠|突っ込|うなず|頷|歩|走|立ち|座|拾|落と|払|差し出|向か|振り向|逃げ|止ま|見送|片づ|下げ|持ち上|置い|開け|閉じ|手を|腕|前足|受付へ戻)/u.test(entry));
  return sentences.length >= 2
    && independentlyActingClauses.length >= 2
    && physicalActionClauses.length >= 2;
}
...
const INTENT_VISIBLE_ACTION = {
  ...
  "object-action": "the exact named cause-and-effect action in progress, with the acting hand or paw, the affected prop, its direction of travel, and its required destination all readable in one frame",
  "resolution-montage": "one coherent closing tableau: prioritize the main comic consequence and stage the other named end-state actions clearly in staggered depth, letting narration carry transitional biomechanical detail that cannot all read simultaneously in one still",
...
const INTENT_MOOD = {
  ...
  "object-action": "crisp cause-and-effect clarity at the decisive physical beat",
  "resolution-montage": "comic release, restored order, and a readable group sense of closure",
...
function inferIntent(utterance = {}, context = {}) {
  ...
  if (isClosingResolutionMontage(utterance, context)) {
    return { intent: "resolution-montage", hardPreferred: true, setups: ["staircase-diagonal", "triangular-confrontation", "establishing-deep"] };
  }
...
function spokenDialogueSafeSetup(setup, utterance = {}) {
  if (utterance.speakerId === "narration" || utterance.preset === "narration") return setup;
  if (!new Set(["overhead-workbench", "macro-hands"]).has(setup.id)) return setup;
  return SETUP_BY_ID.get("counter-level-object");
}
...
function inferBeatPurpose(utterance, intent) {
  ...
  if (intent === "object-action") return `Show this named physical cause-and-effect action exactly as written, including the prop's required destination: ${utterance.text}`;
  if (intent === "resolution-montage") return `Stage these simultaneous closing outcomes as one readable group tableau, prioritizing the dominant comic consequence while preserving each named character's end-state: ${utterance.text}`;
...
export function buildMangaSceneImagePrompt(plan, context = {}) {
  ...
  const castNames = Array.isArray(context.cast) ? context.cast.filter(Boolean) : [];
  const cast = castNames.length > 0 ? castNames.join(", ") : "only the story characters required for this beat";
  ...
  const allowUnnamedBackgroundPeople = context.allowUnnamedBackgroundPeople === true
    || /(?:群衆|人々|観客(?:たち)?|来場者(?:たち)?|通行人(?:たち)?|社員たち|同僚たち|同級生たち|クラスメイトたち|客たち|スタッフたち|作業員たち|大勢の人|人だかり|長い列)/u.test(`${plan.purpose || ""}\n${plan.text || ""}`);
  const visibleAction = allowUnnamedBackgroundPeople
    ? plan.visibleAction
    : String(plan.visibleAction || "").replace("crowd scale, and ", "named-cast scale and ");
  const figureCountContract = !allowUnnamedBackgroundPeople && castNames.length > 0
    ? `EXACT FIGURE COUNT: show exactly ${castNames.length} distinct named story figures total (${castNames.join(", ")}). Count every person or animal, including staff, workers, attendees, distant silhouettes, reflections, and poster figures. Add no unnamed person or animal, and never duplicate or clone a named identity.`
    : allowUnnamedBackgroundPeople
      ? "Unnamed background people are permitted only because this exact beat explicitly requires them. Keep them visually secondary and never duplicate a named identity."
      : "Show no person, animal, worker, staff member, attendee, distant silhouette, reflection, or poster figure.";
  return [
    ...
    `Mood and light: ${plan.mood}; make the light source, weather, and time-of-day continuity physically coherent without inventing rain that the story does not imply.`,
    `Editorial space: ${plan.bubbleReserve}.`,
    plan.intent === "departure" && allowUnnamedBackgroundPeople
      ...
      : "",
    plan.intent === "resolution-montage"
      ? "Closing-tableau contract: communicate the dominant consequence and every named character's end-state in one anatomically coherent group frame. A clear caught-in-the-act posture, embarrassed reaction, partially concealed prop, and readable secondary poses may represent a narrated transition; do not contort anatomy merely to literalize every intermediate micro-movement in the same instant."
      : "",
    figureCountContract,
    ...
    plan.speakerId && plan.speakerId !== "narration"
      ? "Spoken-dialogue face contract: keep the active speaker identifiable with both eyes and mouth, or at minimum one eye and the mouth in a clear profile. Do not show only the back of the head, even in an evidence or hands-led shot."
      : "",
    "No speech bubble, no captions, no readable text, no logo, no watermark, no unintended prominent bystanders, no duplicated body parts. A public venue, workplace, street, or event hall by itself does not authorize invented bystanders.",
...
export function planMangaSceneCompositions(input = {}) {
  ...
  const cutIndexById = new Map(cuts.map((cut, index) => [cut.id, index]));
  const beats = manifest.utterances.map((utterance, index) => {
    const cut = cutById.get(utterance.cutId);
    const firstInCut = cut?.utteranceIds?.[0] === utterance.id;
    const cutIndex = cutIndexById.get(cut?.id);
    const previousCut = Number.isInteger(cutIndex) && cutIndex > 0 ? cuts[cutIndex - 1] : null;
    const explicitSceneTransition = /(?:場面転換|翌日|数日後|数年後|一方、|場所を移|別の場所|会場を出|店の外)/u.test(cut?.description || "");
    const sceneStart = firstInCut && (
      !cut?.locationId
      || cutIndex === 0
      || (previousCut?.locationId && previousCut.locationId !== cut.locationId)
      || explicitSceneTransition
    );
    const rule = inferIntent(utterance, {
      cutPurpose: cut?.description || cut?.purpose,
      sceneStart,
      isFinalCut: cutIndex === cuts.length - 1,
    });
    const setup = spokenDialogueSafeSetup(selectSetup(rule, recent, utterance, index), utterance);
    const previous = recent.at(-1);
    const plan = {
