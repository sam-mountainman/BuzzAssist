...
export function createCharacterRepairPlan(input = {}) {
  ...
  const normalizedEntries = entries.map((entry, index) => {
    ...
    if (seenCells.has(cellId)) throw new Error(`duplicate repair entry for cell ${cellId}.`);
...
export function verifyRepairPlanCoverage(plan, findings = []) {
  ...
  const covered = new Set(plan.entries.map((entry) => entry.cellId));
  const missing = [];
  for (const finding of findings) {
    const cellId = nonEmptyString(finding?.cellId ?? finding);
    if (cellId && !covered.has(cellId)) missing.push(cellId);
  }
...
export function repairPlanToNextRound(plan, { imageByCell = {}, baseByCell = {} } = {}) {
  ...
  const revisionDelta = plan.entries
    ...
    .join("; ");
  const unintendedChangeChecks = plan.entries
    .filter((entry) => imageByCell[entry.cellId] && baseByCell[entry.cellId])
    .map((entry) => ({
      id: `repair-${entry.cellId}`,
      type: "unintendedChange",
      image: imageByCell[entry.cellId],
      base: baseByCell[entry.cellId],
