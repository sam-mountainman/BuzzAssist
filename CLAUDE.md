...
6. Claude Code itself does not render MCP Apps widgets. Use the local canvas URL plus MCP tools in Claude Code.
7. Claude Desktop is a separate host from Claude Code. If the user explicitly asks for Claude Desktop, run `node scripts/setup-agents.mjs --agent claude-desktop --project-dir <active-user-project-dir>`, restart Claude Desktop, then open the native widget with `render_buzzassist_canvas_widget`.
