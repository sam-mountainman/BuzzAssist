...
const CHECKS = Object.freeze({
  ...
  // 監査契約 v8: 使う台本が台本の品質ループに合格した版か、人がそのまま使うと認めた版か。
  scriptQualityAccepted: true,
...
test("監査契約を上げる前の版で確定を待っている narrated Job は、共通 Receipt でもその版の必須監査で確定し、足した保証は not-in-force になる", async (t) => {
  ...
  // 切り替えと固定の重ね物の保証を、v9→v10 で場面ごとの焦点の保証を、v10→v11 で本編の間の保証を足した。
  ...
  const v11 = ["narration-pacing-declared"];
  const cases = [
    { version: `${NARRATED_SERIES}-v10`, notInForce: [...v11] },
    { version: `${NARRATED_SERIES}-v9`, notInForce: [...v10, ...v11].sort() },
    { version: `${NARRATED_SERIES}-v8`, notInForce: [...v9, ...v10, ...v11].sort() },
    { version: `${NARRATED_SERIES}-v7`, notInForce: [...script, ...v9, ...v10, ...v11].sort() },
    { version: `${NARRATED_SERIES}-v6`, notInForce: [...video, ...script, ...v9, ...v10, ...v11].sort() },
    // v5→v6 で見た目の実測の保証を足した。
    { version: `${NARRATED_SERIES}-v5`, notInForce: [...visual, ...video, ...script, ...v9, ...v10, ...v11].sort() },
    { version: `${NARRATED_SERIES}-v4`, notInForce: ["asset-quality-loop", "scene-image-provenance", ...visual, ...video, ...script, ...v9, ...v10, ...v11].sort() },
    // v3→v4 のときも同じ穴だった（人物の同一性の signoff 結合と声の監査を後から求めていた）。
    { version: `${NARRATED_SERIES}-v3`, notInForce: ["asset-quality-loop", "character-identity", "scene-image-provenance", "voice-quality", ...visual, ...video, ...script, ...v9, ...v10, ...v11].sort() },
  ];
...
test("効力のある契約の必須監査が欠ければ、過去の版の Job でも今の版の Job でも共通 Receipt は落ちる", async (t) => {
  ...
  const v10 = await pastNarratedAudits(`${NARRATED_SERIES}-v10`);
  const v11 = Object.keys(CHECKS);
  const cases = [
    ...
    ["v10 の Job で場面ごとの焦点の監査が欠けた", plannedJob, OLDER_DECLARATION_SHA, `${NARRATED_SERIES}-v10`, v10.filter((id) => id !== "cameraFocusMeasured")],
    ["v11 の Job で本編の間の監査が欠けた", plannedJob, declarationSha256, `${NARRATED_SERIES}-v11`, v11.filter((id) => id !== "narrationPacingMeasured")],
    // 今の宣言で計画された Job が、audit-report で古い版を名乗っても、宣言が書かれた版（v11）で測る。
    ["今の宣言で計画された Job が v10 を名乗った", plannedJob, declarationSha256, `${NARRATED_SERIES}-v10`, v10],
  ...
  await t.test("v11 の Job で全部の監査が揃えば合格し、対象外は無い", async () => {
    const root = await mkdtemp(join(tmpdir(), "video-receipt-in-force-current-"));
    try {
      const outcome = await completedFixture(root, { includeRequiredAuditIds: false, reportOverrides: { contractVersion: `${NARRATED_SERIES}-v11` } });
...
test("更新をまたいで確定した narrated Job は計画時の宣言で測り、今の宣言の版の監査を後から求めず、その事実を Receipt に残す", async () => {
  ...
  try {
    ...
    // 今の宣言へ付け替えた Job（計画は古い宣言）。付け替えを知らない測り方だと今の版（v11）へ引き上げて落ちる。
    ...
    assert.equal(result.receipt.outcome, "pass", JSON.stringify(result.receipt.summary));
    assert.deepEqual([...result.receipt.summary.notInForceGates].sort(), ["camera-focus-declared", "fixed-overlays-declared", "narration-pacing-declared", "scene-transition-declared", "script-quality-accepted"], "計画時の版に無かった保証は対象外");
