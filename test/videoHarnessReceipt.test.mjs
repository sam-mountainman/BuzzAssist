...
test("監査契約を上げる前の版で確定を待っている narrated Job は、共通 Receipt でもその版の必須監査で確定し、足した保証は not-in-force になる", async (t) => {
  ...
  // v6→v7 で回ごとの運営者の動画の品質ループの保証を、v7→v8 で台本の品質ループの合格の保証を足した。
  const video = ["operator-video-asset-loop"];
  const script = ["script-quality-accepted"];
  const cases = [
    { version: `${NARRATED_SERIES}-v7`, notInForce: [...script] },
    { version: `${NARRATED_SERIES}-v6`, notInForce: [...video, ...script].sort() },
    // v5→v6 で見た目の実測の保証を足した。
    { version: `${NARRATED_SERIES}-v5`, notInForce: [...visual, ...video, ...script].sort() },
    { version: `${NARRATED_SERIES}-v4`, notInForce: ["asset-quality-loop", "scene-image-provenance", ...visual, ...video, ...script].sort() },
    // v3→v4 のときも同じ穴だった（人物の同一性の signoff 結合と声の監査を後から求めていた）。
    { version: `${NARRATED_SERIES}-v3`, notInForce: ["asset-quality-loop", "character-identity", "scene-image-provenance", "voice-quality", ...visual, ...video, ...script].sort() },
  ];
...
test("効力のある契約の必須監査が欠ければ、過去の版の Job でも今の版の Job でも共通 Receipt は落ちる", async (t) => {
  ...
  const cases = [
    ...
    ["v8 の Job で台本の品質ループの合格の監査が欠けた", plannedJob, declarationSha256, `${NARRATED_SERIES}-v8`, v8.filter((id) => id !== "scriptQualityAccepted")],
    // 今の宣言で計画された Job が、audit-report で古い版を名乗っても、宣言が書かれた版（v8）で測る。
  ...
  await t.test("v8 の Job で全部の監査が揃えば合格し、対象外は無い", async () => {
    const root = await mkdtemp(join(tmpdir(), "video-receipt-in-force-current-"));
    try {
      const outcome = await completedFixture(root, { includeRequiredAuditIds: false, reportOverrides: { contractVersion: `${NARRATED_SERIES}-v8` } });
