...
// ここは、使える工程と、共有の宣言をそのまま使うこと（解説動画だけの採点制度を作らない）と、学習の宛先を
// 推測しないことを見る。対象 id・会話 id はすべて合成の値。
...
test("explainer-video の不合格の回は、学習の宛先が決まっていないので推測で別の台帳へ積まない", async () => {
  let called = false;
  ...
  assert.equal(result.skippedReason, "unknown-harness-route");
