...
- **最終結合は旧既定版へ戻さず、直近で受入済みのreview素材のSHAと版を明示固定し、出力監査でもその入力SHAを再確認する。**
  - 根拠digest: `7129b7d5ecdf`
  - 種別: correction / 初回: 2026-09-01 / id: `5fc7cb6c8930`
- **ナレーターと同じ声IDで作ったセリフだけが地の文と違う声に聞こえるとき、速度の是正や語り口の指示を足すだけでは揃わないことがある。直前の地の文とセリフを1回の依頼で生成し、文と文の間の無音でセリフだけを切り出すと揃った。切り出し後は文字起こしで前後が混ざっていないこと、先頭が無音であること、語尾が途切れていないことを確かめ、候補テイクは却下したものも名前付きで残して耳で選んでもらう**
  - 根拠digest: `f61d8c192a16`
  - 種別: fact / 初回: 2026-09-16 / id: `fcf0c3ff6772`
- **声の配役を欠陥と呼ぶ前に、その版で効力のある配役契約と、最新の生成記録・生成スクリプトを読む。古い版の配役記録だけを見ると、後の版で差し替えた実態を見落とし、脇役をナレーターが読む設計どおりの挙動を欠陥と誤認する**
  - 根拠digest: `00610619df43`
  - 種別: correction / 初回: 2026-09-16 / id: `e5fc6bc0daff`
- **Fish Audio の model API は visibility を "unlist" と返す（"unlisted" ではない）。judgeFishVoice が unlisted だけを見ていたため、クライアントの限定公開の声3件が visibility-unknown で誤って不合格になった。両方を受け付けるよう修正。**
  - 根拠digest: `0a56d5aa3593`
  - 種別: correction / 初回: 2026-09-24 / id: `77f76b5977da`
- **Fish Audio の声の見本音声は fish.audio ではなく固定の R2 バケット c97f3361a1c971323738e24f451a0225.r2.cloudflarestorage.com から配信される。仲介サーバーの sample host 許可リストが fish.audio 系だけだったため、本番中のナレーター声を含む全声が no-sample-audio-to-audition で不合格になった。当該ホストだけを厳密一致で追加（r2 ゾーン全体は不可）。**
  - 根拠digest: `8cc16ba8dfa3`
  - 種別: correction / 初回: 2026-09-24 / id: `02da2b58db25`
- **クライアントが提示した声IDでも、Fish 上の作者が本人でない公開モデルは契約の権利根拠（licensed / owner-authorized）に当たらない。提示＝許諾ではない。作者IDを model API で必ず確認し、当たらない声は blocked で登録し有料生成へ落とさない。**
  - 根拠digest: `d78a8ebb1050`
  - 種別: fact / 初回: 2026-09-24 / id: `0351aae79fc7`
