...
- **自動で書き換えてよいのは機械区画の内側だけ**。`LEARNED:BEGIN` /
  `LEARNED:END` に挟まれた範囲を `sync` が書き直す。人が書いた本文には
  触らない。hermes の curator が「agent作成スキルだけ触り、
...
書き換わるのは各正本の `LEARNED:BEGIN` 〜 `LEARNED:END` の内側だけ。
人が書いた本文は1文字も変わらない。マーカーの無い文書は対象外として
保留され、その旨が出る（人の文書の構造を機械が勝手に変える方が、
反映漏れより高くつく）。

機械区画は**次のセッションが読む**ので、これだけで学習は成立する。
...
- 提案を1件ずつ別々の節として追記する
- 機械区画（`LEARNED` マーカーの内側）を手で編集する（次の sync で消える）
