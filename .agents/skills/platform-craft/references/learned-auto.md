...
- **最終化は現在のハーネス版の実MP4・plan・自動監査・contact sheet・外部review notesをSHA拘束し、旧版auditを代替証拠として受理しない。自動監査は独立目視signoffを生成せず awaiting-independent-signoff で停止する**
  - 根拠digest: `220852389de1`
  - 種別: correction / 初回: 2026-08-29 / id: `85c4ff144509`
- **複数セッションを統合監査するときは、各セッションの未解決主張を原子単位でID化し、現在実装・レビュー台帳・最終回答への三者クロスウォークを作る。Fish Audioの直接接続・課金外・Receipt未接続のような個別リスクを『<引用 217eb836a147>』へ丸めて消さず、解決・未解決・枝差分・検証不能をそれぞれ明記する。**
...
- **台帳がSHAで拘束している成果物は、必ずその成果物そのものを開いて確認する。レンダラが落ちる等で開けないときは、代用物を作って『<引用 cb7ca3b6c54a>』と記録してはいけない。巨大なdata URIを含むSVGは一般的なCLIラスタライザ(rsvg-convert等)がXMLパース段階で落ちることがあるが、ファイル自体は正常なことが多い。その場合はSVGの埋め込みbase64画像とgroupのtransformを自前で展開して描画すれば原寸確認できる。代用物で確認した場合は、何を見て何を見ていないかを検品記録に明記する。**
...
- **帯先行（安全域の絵を先に確定→外側を延長）で作ったバナーは、TV の全体表示で人物が腰から下なしで浮く。是正は『<引用 6410be51bf1d>』の編集。『<引用 972dd1a18617>』と書くとモデルが全身を縦に詰め込み、頭やキャラが安全域から出る・体格が破綻する（5案中4案）。帯差（安全域の画素差）で構図移動を検知する**
...
- **長時間の有料生成（人物設定画の一括生成、本編レンダー）の前に空き容量を確かめ、目安10GB未満なら掃除してから始める。書き込み失敗で止まると払った分が戻らないため。安全な掃除先は、OS の一時ディレクトリにある終わったセッションの作業領域、~/.codex/generated_images の2日より古いキャッシュ、プロジェクトの .render-work と .chrome-*。生成の checkpoint があれば、同じ入力と同じ文脈で再開できる。**
  - 根拠digest: `4c63a2ac6975`
  - 種別: fact / 初回: 2026-09-24 / id: `fad576c47b2c`
- **npm pack は package-lock.json を tgz に入れない。配布の tgz から npm ci するには prepack で lockfile を別名で同梱し、展開時に戻す**
  - 根拠digest: `0f2eb24f0914`
  - 種別: fact / 初回: 2026-09-25 / id: `26868aefc869`
- **macOSで隔離したホームと使い捨てプロファイルを使うChrome描画プローブでも、実キーチェーンへ接続すると警告ダイアログが繰り返し出る。ローカルSVG専用の自動描画では --use-mock-keychain を付け、ユーザーのキーチェーン初期化で対処しない。**
  - 根拠digest: `6d1ddeaf0764`
  - 種別: fact / 初回: 2026-09-25 / id: `9987684f8011`
