...
- **帯先行（安全域の絵を先に確定→外側を延長）で作ったバナーは、TV の全体表示で人物が腰から下なしで浮く。是正は『<引用 3fe8bfed9267>』の編集。『<引用 8e012a3fb732>』と書くとモデルが全身を縦に詰め込み、頭やキャラが安全域から出る・体格が破綻する（5案中4案）。帯差（安全域の画素差）で構図移動を検知する**
