...
const evidencePath = join(episodeDir, "v39-natural-audio-final-evidence.json");
...
if (!cameraAudit.pass || cameraAudit.gates?.some((gate) => !gate.pass)) {
  throw new Error("V39 rendered camera audit has not passed; refusing to finalize.");
}
...
) throw new Error(`Unexpected V39 streams: ${JSON.stringify(probe.streams)}`);
...
if (bubbleStats.faceOverlaps > 0 || bubbleStats.typographyFailures > 0) {
  throw new Error(`V39 bubble gates failed: ${JSON.stringify(bubbleStats)}`);
}

const evidence = {
  version: "v39-natural-audio-r1",
  ...
  approvedAudio: { previousBaselinePcmMd5: approvedAudioPcmMd5, newBaselinePcmMd5: finalAudioPcmMd5, baselineReset: "R57 all utterances regenerated through the v25 eleven_v3 dialogue pipeline (natural performance restored)" },
  fullDecodePassed: true,
  createdAt,
};

manifest.status = "final-v39-natural-audio-r1";
manifest.video = { ...(manifest.video || {}), fileName: finalVideo.fileName, statusAfterRender: "final-v39-natural-audio-r1" };
