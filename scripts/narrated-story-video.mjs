...
function usage() {
  return [
    ...
    "  --script-quality-work-dir DIR is the script quality loop's work folder (scripts/script-quality-loop.mjs --work-dir). It must be the folder the outer Job declares in options.scriptQualityWorkDir (run-video-harness.mjs start sets it to the folder of --script-path unless --script-quality-work-dir is given); otherwise the runner stops with script-quality-work-dir-not-bound-to-job. From audit contract v8, before any paid work the runner asks that loop whether the exact script bytes were accepted (the loop passed that SHA, or a person recorded accept-human --human-verified for it); if not, it stops in awaiting-human-review with script-quality-required:<reason code> and the next commands, and records the answer as the scriptQualityAccepted audit.",
    ...
    "  --operator-image-manifest FILE (buzzassist-operator-image-manifest-v1) imports the operator's own scene images (ChatGPT web / Codex / local model / Grok / other) instead of image Media Jobs, when the Channel Pack declares image.source \"operator-file\". It must be the path the outer Job declares in options.operatorImageManifestPath (run-video-harness.mjs start --operator-image-manifest FILE); otherwise the runner stops with operator-image-manifest-not-bound-to-job. Every scene of the script must be covered (no mixing with paid image generation); the image and prompt files are re-hashed against the manifest, references must be approved character sheets, sizes must be within the Pack tolerance, and any failure stops before paid work with an operator-image-* reason code. Replacing an image changes the input fingerprint, so resume re-imports it and re-renders without re-billing voices or music. Conversation URLs stay in the private Job folder (.media/.../operator-images); only their SHA-256 reaches the generation manifest, audit, RunReceipt and Canvas. A scene row may also carry cameraFocus { x, y } (0-1 of the image, top-left 0; e.g. the face position): the scene focus is taken from the manifest first, then the script package, then the Channel Pack's per-move focus, and a manifest value that differs from the script package's stops before paid work with camera-focus-conflict:<scene>:image-vs-package. Write focus decided after the images in the manifest, not the script package (editing the package changes its bytes and drops its script-quality acceptance).",
    "  --operator-video-manifest FILE (buzzassist-operator-video-manifest-v1) imports the operator's own short videos for the slots the Channel Pack declares (episode-opening when bookends.opening.kind is episode-video; review-presenter when bookends.review.presenter.episodeVideo is true — the review TV layout keeps the presenter frame empty when that optional clip is absent). It must be the path the outer Job declares in options.operatorVideoManifestPath (pass it at start through --options-json or the MCP options); otherwise the runner stops with operator-video-manifest-not-bound-to-job. Each clip is re-hashed, probed (duration, size, frame rate, audio) against the slot rules and copied into the Job folder before any paid work; failures stop with an operator-video-* reason code. Conversation URLs and route notes stay in the private Job folder; only SHA-256 values and numbers reach the generation manifest, audit, RunReceipt and Canvas. From audit contract v7 each clip also needs assetLoop { statePath, passedSha256 } of a video-clip asset quality loop that passed in the manifest folder (measure the clip with node scripts/asset-quality-loop.mjs measure-video first); otherwise the run stops awaiting-human-review before paid work with video-clip-asset-loop-not-passed:<slot>:<reason> and the suggested measurement declaration.",
