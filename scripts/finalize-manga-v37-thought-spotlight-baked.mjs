...
const evidencePath = join(episodeDir, "v37-thought-spotlight-baked-final-evidence.json");
...
const finalVideoPath = join(
  projectDir,
  "canvas/assets/videos/manga-photo-homecoming-001-v37-thought-spotlight-baked-r1.mp4",
);
...
if (!cameraAudit.pass || cameraAudit.gates?.some((gate) => !gate.pass)) {
  throw new Error("V37 rendered camera audit has not passed; refusing to finalize.");
}
...
if (approvedAudioPcmMd5 !== finalAudioPcmMd5) {
  throw new Error(`V37 changed approved audio PCM: ${approvedAudioPcmMd5} != ${finalAudioPcmMd5}`);
}
...
) throw new Error(`Unexpected V37 streams: ${JSON.stringify(probe.streams)}`);
...
if (bubbleStats.faceOverlaps > 0 || bubbleStats.typographyFailures > 0) {
  throw new Error(`V37 bubble gates failed: ${JSON.stringify(bubbleStats)}`);
}

const evidence = {
  version: "v37-thought-spotlight-baked-r1",
  continuedFromTask: "019fe044-aa46-7a83-992d-d5c095a20201", p0Fix: "inner-voice spotlight baked into source image before camera (screen-space compositing removed)",
...
manifest.status = "final-v37-thought-spotlight-baked-r1";
manifest.video = { ...(manifest.video || {}), fileName: finalVideo.fileName, statusAfterRender: "final-v37-thought-spotlight-baked-r1" };
