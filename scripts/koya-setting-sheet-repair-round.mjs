...
function repairEntries() {
  ...
  return [
    {
      findingId: "cast-8c15-turnaround-duplicate-34-direction",
      cellId: "<private-term>-character-7:turnaround:row2-col3",
    ...
    {
      findingId: "cast-8c15-outfit-office-wrong-garment-and-background",
      cellId: "<private-term>-character-7:outfit:office:all",
      issue: "Regenerate Ema's office outfit sheet from the exact approved garment authority on pure white.",
      repairRegion: [0, 0, 1, 1],
      protectRegions: [],
      acceptCriteria: [
        "The exact garment authority is canvas/assets/appare-revisions/cast-8c15-B-officelady.png; no generic garment description is used as authority.",
    ...
    {
      findingId: "cast-8c15-outfit-private-background-only",
      cellId: "<private-term>-character-7:outfit:private-dressy:all",
      issue: "Remove the living-room background from Ema's approved private-dressy outfit sheet.",
      repairRegion: [0, 0, 1, 1],
      protectRegions: [],
      acceptCriteria: ["The clothing matches cast-8c15-B-kireime.png.", "All four cells use a pure white background."],
      rejectCriteria: ["Living-room or other scenery remains.", "The approved outfit or identity changes."],
    },
    {
      findingId: "cast-0fc4-turnaround-towel-side-swap-profiles",
      cellId: "<private-term>-2-character-1:turnaround:row1-col2-col3",
    ...
    {
      findingId: "cast-be83-turnaround-glasses-missing-profiles",
      cellId: "<private-term>-character-2:turnaround:row1-col2-col3",
    ...
    {
      findingId: "<private-term>-expressions-background-contamination",
      cellId: "<private-term>-character-3:expression:row3-all",
      issue: "Replace only <private-term> expression-sheet row3 so all four coverage cells have pure white backgrounds.",
      repairRegion: [0, row3Y, 1, 1 - row3Y],
      protectRegions: [[0, 0, 1, row3Y]],
      acceptCriteria: ["All four row3 cells have pure white backgrounds.", "<private-term> remains the same person in all four row3 cells."],
...
async function composeRepairs() {
  ...
  await compositeRgbRoi(
    resolve(PROJECT_DIR, "canvas/assets/characters/<private-term>/approved-identity-packs/<private-term>-character-7-turnaround.png"),
    join(raw, "cast-8c15-turnaround.png"), join(composed, "cast-8c15-turnaround.png"), [836, 470, 418, 471],
  );
  await copyFile(join(raw, "cast-8c15-outfit-office.png"), join(composed, "cast-8c15-outfit-office.png"));
  await copyFile(join(raw, "cast-8c15-outfit-private-dressy.png"), join(composed, "cast-8c15-outfit-private-dressy.png"));
  await compositeRgbRoi(
    resolve(PROJECT_DIR, "canvas/assets/characters/<private-term>-2/approved-identity-packs/<private-term>-2-character-1-turnaround-refresh-new-standard-20260830-r2-tool-import.png"),
    join(raw, "cast-0fc4-turnaround-take1.png"), join(composed, "cast-0fc4-turnaround.png"), [418, 0, 836, 470],
  );
  await compositeRgbRoi(
    resolve(PROJECT_DIR, "canvas/assets/characters/<private-term>/approved-identity-packs/<private-term>-character-2-turnaround-refresh-new-standard-20260830-r2-tool-import.png"),
    join(raw, "cast-be83-turnaround.png"), join(composed, "cast-be83-turnaround.png"), [418, 0, 836, 470],
  );
  await compositeRgbRoi(
    resolve(PROJECT_DIR, "canvas/assets/characters/<private-term>/approved-identity-packs/<private-term>-character-3-expressions-refresh-new-standard-20260830-r2-tool-import.png"),
    join(raw, "<private-term>-expressions.png"), join(composed, "<private-term>-expressions.png"), [0, 627, 1672, 314],
  );
  const outputs = await Promise.all(["cast-8c15-turnaround.png", "cast-8c15-outfit-office.png", "cast-8c15-outfit-private-dressy.png", "cast-0fc4-turnaround.png", "cast-be83-turnaround.png", "<private-term>-expressions.png"]
...
async function createImportMaps() {
  ...
  const sourceByCastRole = {
    "<private-term>-character-7": {
      turnaround: join(OUTPUT_DIR, "composed/cast-8c15-turnaround.png"),
      "outfit:office": join(OUTPUT_DIR, "composed/cast-8c15-outfit-office.png"),
      "outfit:private-dressy": join(OUTPUT_DIR, "composed/cast-8c15-outfit-private-dressy.png"),
    },
    "<private-term>-character-2": { turnaround: join(OUTPUT_DIR, "composed/cast-be83-turnaround.png") },
    "<private-term>-character-3": { expression: join(OUTPUT_DIR, "composed/<private-term>-expressions.png") },
    "<private-term>-2-character-1": { turnaround: join(OUTPUT_DIR, "composed/cast-0fc4-turnaround.png") },
  };
  const targets = [
    { episodeId: "<private-term>", workflowId: "workflow-mt5hucll-8d4ec683", castId: "<private-term>-character-7", repairId: "human-qa-20260830-r4b-cast-8c15" },
    { episodeId: "<private-term>", workflowId: "workflow-mt5hucll-8d4ec683", castId: "<private-term>-character-2", repairId: "human-qa-20260830-r4b-cast-be83" },
    { episodeId: "<private-term>", workflowId: "workflow-mt5hucll-8d4ec683", castId: "<private-term>-character-3", repairId: "human-qa-20260830-r4b-<private-term>" },
    { episodeId: "<private-term>-2", workflowId: "workflow-mt5o5z4j-d592f6d6", castId: "<private-term>-2-character-1", repairId: "human-qa-20260830-r4b-cast-0fc4" },
  ];
  const results = [];
  for (const target of targets) {
    ...
    const jobs = buildApprovedIdentityPackRepairJobs(workflow, cast, identityCandidate, failedRoles, {
      ...
      strictOutfitAuthorityStages: relevantFindings
        .filter((entry) => entry.findingId === "cast-8c15-outfit-office-wrong-garment-and-background")
        .map(() => "office"),
    });
  ...
  await writeJsonAtomic(join(OUTPUT_DIR, "generation-selection.json"), {
    ...
    cast-0fc4: {
      ...
      selectedTake: "take1",
      selectedPath: join(OUTPUT_DIR, "raw/cast-0fc4-turnaround-take1.png"),
      selectedSha256: sha256(await readFile(join(OUTPUT_DIR, "raw/cast-0fc4-turnaround-take1.png"))),
      rejectedTake: "take2",
      rejectedPath: join(OUTPUT_DIR, "raw/cast-0fc4-turnaround-take2.png"),
      rejectedSha256: sha256(await readFile(join(OUTPUT_DIR, "raw/cast-0fc4-turnaround-take2.png"))),
...
function protectedCellKeys(castId) {
  ...
  if (castId === "<private-term>-character-7") {
    return [...turn.filter((id) => id !== "right-three-quarter-head").map((id) => `turnaround:${id}`), ...expression.map((id) => `expression:${id}`)];
  }
  if (["<private-term>-character-2", "<private-term>-2-character-1"].includes(castId)) {
    return [
  ...
  if (castId === "<private-term>-character-3") {
...
async function writeCheckpoint() {
  ...
  const recheckByFinding = {
    "cast-8c15-turnaround-duplicate-34-direction": ["row2 col3: 右3/4（本人右頬側）で、row2 col2の左向き複製でないこと", "本人左側頭部の髪留めが分け目越しに僅かに見えるか、解剖学的に隠れること"],
    "cast-8c15-outfit-office-wrong-garment-and-background": ["4セル全て: cast-8c15-B-officelady.png と同じ承認衣装で、ブレザー化していないこと", "全セル純白背景、同一人物、髪留めが本人左側頭部であること"],
    "cast-8c15-outfit-private-background-only": ["4セル全て: cast-8c15-B-kireime.png の衣装内容を維持していること", "リビング等の背景がなく全セル純白であること"],
    "cast-0fc4-turnaround-towel-side-swap-profiles": ["row1 col2: 鼻が左、タオルは首後ろへ僅かに回り込むだけであること", "row1 col3: 鼻が右、本人右肩のタオルが近位肩から胸前へ明確に垂れること"],
    "cast-be83-turnaround-glasses-missing-profiles": ["row1 col2・col3: 両方の厳密横顔に細いレンズ縁があること", "両セルでつるがこめかみから耳まで連続して見えること"],
    "<private-term>-expressions-background-contamination": ["row3 col1〜col4: ホテル内装等がなく全て純白背景であること", "row3の4人が同一のノドカで、髪・アホ毛・たれ目・衣装が連続すること"],
  };
  ...
  const checkpoint = {
    ...
    generation: {
      ...
      taishoTakeCount: generationSelection.cast-0fc4.generatedTakeCount,
      taishoSelectedTake: generationSelection.cast-0fc4.selectedTake,
