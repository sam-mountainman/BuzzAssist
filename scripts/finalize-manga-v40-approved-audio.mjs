...
const evidencePath = join(episodeDir, "v40-approved-audio-final-evidence.json");
...
const approvedVideoPath = join(
  projectDir,
  "canvas/assets/videos/manga-photo-homecoming-001-v39-natural-audio-r1.mp4",
);
...
if (!cameraAudit.pass || cameraAudit.gates?.some((gate) => !gate.pass)) {
  throw new Error("V40 rendered camera audit has not passed; refusing to finalize.");
}
...
) throw new Error(`Unexpected V40 streams: ${JSON.stringify(probe.streams)}`);
...
if (bubbleStats.faceOverlaps > 0 || bubbleStats.typographyFailures > 0) {
  throw new Error(`V40 bubble gates failed: ${JSON.stringify(bubbleStats)}`);
}

const evidence = {
  version: "v40-approved-audio-restored-r1",
  continuedFromTask: "019fe044-aa46-7a83-992d-d5c095a20201", fixes: "R60 approved dialogue audio restored (corr 1.000 x22) + narration semantic-intent direction with prosody gate, R61 onset integrity gate (29/29) + STT verification gate (29/29)",
  ...
  approvedAudio: { previousBaselinePcmMd5: approvedAudioPcmMd5, newBaselinePcmMd5: finalAudioPcmMd5, baselineReset: "R60/R61: all 22 dialogue lines restored byte-equivalent from the approved master (corr 1.000); 7 narration lines re-directed with wired semantic intents; onset margin 100ms" },
  fullDecodePassed: true,
  createdAt,
};

manifest.status = "final-v40-approved-audio-restored-r1";
manifest.video = { ...(manifest.video || {}), fileName: finalVideo.fileName, statusAfterRender: "final-v40-approved-audio-restored-r1" };
