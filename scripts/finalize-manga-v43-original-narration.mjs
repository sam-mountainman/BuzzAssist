...
const evidencePath = join(episodeDir, "v43-original-narration-final-evidence.json");
...
const finalVideoPath = join(
  projectDir,
  "canvas/assets/videos/manga-photo-homecoming-001-v43-original-narration-r1.mp4",
);
...
if (!cameraAudit.pass || cameraAudit.gates?.some((gate) => !gate.pass)) {
  throw new Error("V43 rendered camera audit has not passed; refusing to finalize.");
}
...
) throw new Error(`Unexpected V43 streams: ${JSON.stringify(probe.streams)}`);
...
if (bubbleStats.faceOverlaps > 0 || bubbleStats.typographyFailures > 0) {
  throw new Error(`V43 bubble gates failed: ${JSON.stringify(bubbleStats)}`);
}

const evidence = {
  version: "v43-original-narration-r1",
...
manifest.status = "final-v43-original-narration-r1";
manifest.video = { ...(manifest.video || {}), fileName: finalVideo.fileName, statusAfterRender: "final-v43-original-narration-r1" };
