...
const evidencePath = join(episodeDir, "v38-viewing-feedback-final-evidence.json");
...
if (!cameraAudit.pass || cameraAudit.gates?.some((gate) => !gate.pass)) {
  throw new Error("V38 rendered camera audit has not passed; refusing to finalize.");
}
...
) throw new Error(`Unexpected V38 streams: ${JSON.stringify(probe.streams)}`);
...
if (bubbleStats.faceOverlaps > 0 || bubbleStats.typographyFailures > 0) {
  throw new Error(`V38 bubble gates failed: ${JSON.stringify(bubbleStats)}`);
}

const evidence = {
  version: "v38-viewing-feedback-r1",
  continuedFromTask: "019fe044-aa46-7a83-992d-d5c095a20201", fixes: "R51 split-panel readability (face-centred pre-crops), R52 viewpoint rules, R53 cut-09 viewpoint swaps, R54 stale lead-in removed, R55 reference image pacing (consolidated multi-bubble illustrations), R56 narrator voice abolished",
  ...
  approvedAudio: { previousBaselinePcmMd5: approvedAudioPcmMd5, newBaselinePcmMd5: finalAudioPcmMd5, baselineReset: "R56 narration re-voiced with protagonist voice" },
  fullDecodePassed: true,
  createdAt,
};

manifest.status = "final-v38-viewing-feedback-r1";
manifest.video = { ...(manifest.video || {}), fileName: finalVideo.fileName, statusAfterRender: "final-v38-viewing-feedback-r1" };
