...
function usage() {
  return [
    ...
    "asset-quality-references: writes canvas/quality/approved-references.json (buzzassist-approved-references-v1: SHA-256 of approved registry references, human-selected faces, approved location anchors, and scene images whose scene-image loop passed — the source stills of substituted cuts) for asset-quality-loop record --approved-references",
...
const common = {
  ...
  // 画と台詞の音声を重ねる（full の既定）。--no-speech-overlap で従来の直列に戻す。
  ...(args.noSpeechOverlap === true ? { speechOverlap: false } : {}),
