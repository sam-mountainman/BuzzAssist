...
- 記録を残す決まりが無かった頃に作った背景ボードは、プロンプトと会話idがどこにも残っていない。書き起こせば作り話の記録になり、放置すればその場所は永久に未登録になる。そこでボード1枚単位で`provenanceGap`（`promptRecorded: false`、`generatorContextRecorded: false`、理由、その絵が満たすべき文書のpathとSHA-256）を宣言できる。1枚のボードは「完全な出所」か「欠落の宣言」のどちらかで、両方は持てない。`generator`の`host`・`id`・`generatedAt`は欠落を宣言しても要る。宣言したボードには確認項目`provenanceGapAcknowledged`が増え、理由と文書を読んだ独立reviewerがtrueにするまで、アンカー審査も本審査も登録も通らない（無記入・falseは不合格）。欠落は生成manifest・審査・台帳の承認・引き渡しのattestationに残り、verifyは台帳が記録した欠落を落としたattestationを拒否する。形は`docs/examples/koya-location-import-map-provenance-gap.example.json`を参照する
